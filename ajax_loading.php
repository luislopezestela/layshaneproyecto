...
if ($wo['loggedin'] == true) {
    $update_last_seen = lui_LastSeen($wo['user']['user_id']);
}
if (!empty($_GET)) {
...
$data['title']           = stripslashes(lui_Secure($wo['title']));
$data['page']            = $wo['page'];
$data['welcome_page']    = 0;
$data['is_css_file']     = 0;
...
if ($wo['page'] == 'timeline' && $wo['loggedin'] == true && $wo['config']['css_upload'] == 1 && !empty($wo['user_profile'])) {
    if (!empty($wo['user_profile']['css_file']) && file_exists($wo['user_profile']['css_file'])) {
        $data['is_css_file']     = 1;
        $data['css_file']        = '<link rel="stylesheet" class="styled-profile" href="' . lui_GetMedia($wo['user_profile']['css_file']) . '">';
...
//    $data['redirect'] = 1;
// }
$data['url'] = lui_SeoLink('index.php' . $url);
?>
