...
if ($wo['config']['shout_box_system'] == 1 && $wo['story']['postPrivacy'] == 4 && $wo['comment']['publisher']['id'] == $wo['story']['user_id']) {
  $anonymous = true;
   ...
   $wo['comment']['publisher']['avatar'] = lui_GetMedia('upload/photos/incognito.png');
...
<div class="comment comment-container" id="comment_<?php echo $wo['comment']['id'];?>" data-comment-id ="<?php echo $wo['comment']['id'];?>">
  <?php if (!isset($anonymous)) { ?>
   <a onclick="InjectAPI('{&quot;profile_id&quot; : &quot;<?php echo $wo['comment']['publisher']['id']?>&quot;, &quot;type&quot;:&quot;<?php echo $wo['comment']['publisher']['type']?>&quot;}', event);" class="<?php echo lui_RightToLeft('pull-left'); ?>" href="<?php echo $wo['comment']['publisher']['url']?>">
      ...
      <div class="comment-heading">
        ...
        <?php } ?>
         ...
         <div class="<?php echo lui_RightToLeft('pull-right');?> comment-options comment_edele_options">
            ...
            <?php } ?>
            <?php if ($wo['comment']['post_onwer'] === true || $wo['comment']['onwer'] === true || lui_IsAdmin()) {?>
        ...
        <?php  }else{ ?>
         ...
         <div class="comment-image">
            <?php if (!empty($wo['comment']['c_file'])): ?>
               <span data-href="<?php echo lui_GetMedia($wo['comment']['c_file']);?>" onclick="Wo_OpenLighteBox(this,event);" class="pointer">
               <img src="<?php echo lui_GetMedia($wo['comment']['c_file']);?>" alt="Comment image">
               </span>
            <?php endif ?>
         </div>
         <?php if (!empty($wo['comment']['record'])): ?>
            <div class="comment-record" id="audio_c_<?php echo $wo['comment']['id']?>">
				<audio crossorigin preload="none">
					<source src="<?php echo lui_GetMedia($wo['comment']['record']);?>" type="audio/wav">
				</audio>
			</div>
            <script>
				$(function() {
        ...
        <?php if ($wo['config']['second_post_button'] == 'reaction') { ?>
			<div class="wo-reaction wo-reaction-comment stat-item" id="comment_reactions" data-id="<?php echo $wo['comment']['id']; ?>">
                <span class="like-btn like-btn-comment" data-id="<?php echo $wo['comment']['id']; ?>">
                    <?php
                    if ($wo['loggedin'] && lui_IsReacted($wo['comment']['id'], $wo['user']['user_id'], "comment") === true) {    
                  ...
                  <?php 
                     ...
                     if( count( $stickers_system ) > 0 ){
                        foreach ($stickers_system as $wo['stickerlist']) {
                           echo '<img alt="gif" src="'. lui_GetMedia( $wo['stickerlist']['media_file'] ).'" data-gif="'.lui_GetMedia( $wo['stickerlist']['media_file'] ).'" onclick="Wo_PostCommentSticker_'.$wo['comment']['id'].'(this);" autoplay loop>';
                        }
                     } else {
...
<div class="comment-reply-image-<?php echo $wo['comment']['id']; ?> comment-image-con"></div>
   ...
   <?php echo lui_LoadPage('modals/delete-comment');?>
   <script>
