...
<div class="page-margin">
	<div class="wow_main_float_head new_market searchs new_searchh">
		<div class="container">
			...
			<h1><?php echo $wo['lang']['search'];?></h1>
			<p><?php echo TextForMode('search_header_label'); ?></p>
		</div>
	</div>
	
	<div class="wow_main_blogs_bg"></div>
	
	<form action="<?php echo lui_SeoLink('index.php?link1=search') ?>" method="get" class="search-filter-form" id="search_form">
		<div class="wow_content wo_job_head_filter wo_market_head_filter wow_search_head">
			<div class="search-blog">
				<input type="text" name="query" id="query" placeholder="<?php echo $wo['lang']['keyword'];?>" value="<?php echo (isset($_GET['query'])) ? lui_Secure($_GET['query']): '';?>" autocomplete="off">
			</div>
			...
			<div class="dropdown wo_job_main_widget">
				...
				<ul class="dropdown-menu wow_srch_drp_mnu" aria-labelledby="srch_fltrs">
					<li>
						<?php
							$fields = lui_GetUserCustomFields();
							if (count($fields) > 0) {
						...
						<script>
							<?php
								...
								if (!empty($_GET['age_from']) && is_numeric($_GET['age_from'])) {
									$age_from = lui_Secure( $_GET['age_from'] );
								}
								$age_to = 50;
								if (!empty($_GET['age_to']) && is_numeric($_GET['age_to'])) {
									$age_to = lui_Secure( $_GET['age_to'] );
								}
							?>
							$( function() {
								$( "#slider-range" ).slider({
					...
					<li>
						...
						<div class="wow_srch_innr_filtr">
							...
							<div class="pp_mat_input">
								...
								<?php 
									$gender_images = lui_GetGendersImages();
			...
			<div id="users" class="tab-pane fade in active">
				<div id="users-cont" class="row wow_frnd_usr">
					...
					$search_query = '';
						...
						if (isset($_GET['query'])) {
							$search_query = lui_GetSearchFilter($_GET, 30);
							if (count($search_query) != 0) {
								foreach ($search_query as $wo['result']) {
									if (!empty($wo['result']['avatar_full'])) {
										$wo['result']['avatar'] = lui_GetMedia($wo['result']['avatar_full']);
									}
									echo lui_LoadPage('search/user-result');
								}
							} else {
								echo $not_found;
							}
				...
				<div id="pages" class="tab-pane fade">
					<div id="pages-cont" class="row wo_my_pages">
						...
						$search_query = '';
							...
							if (isset($_GET['query'])) {
								$search_query = lui_GetSearchAdv($_GET['query'], 'pages');
								if (count($search_query) != 0) {
									foreach ($search_query as $wo['result']) {
										echo lui_LoadPage('search/page-result');
									}
								} else {
									echo $not_found;
								}
				...
				<div id="groups" class="tab-pane fade">
					<div id="groups-cont" class="row">
						...
						$search_query = '';
							...
							if (isset($_GET['query'])) {
								$search_query = lui_GetSearchAdv($_GET['query'], 'groups');
								if (count($search_query) != 0) {
									foreach ($search_query as $wo['result']) {
										echo lui_LoadPage('search/group-result');
									}
								} else {
									echo $not_found;
								}
				...
				<div id="games" class="tab-pane fade">
					<div class="row wow_srch_game_tabss">
						<?php
							...
							if (isset($_GET['query'])) {
								$search_query = lui_GetSearchAdv($_GET['query'], 'games');
								if (count($search_query) != 0) {
									foreach ($search_query as $wo['game']) {
										echo lui_LoadPage('games/game-style');
									}
								} else {
									echo $not_found;
								}
