...
function lui_ReportPost($post_data = array()) {
    ...
    if (isset($post_data["post_id"])) {
        ...
        if (lui_PostExists($post_data["post_id"]) === false) {
            return false;
        }
    }
    $comment_id = 0;
    if (isset($post_data["comment_id"])) {
        $comment_id = lui_Secure($post_data["comment_id"]);
    }
    if ($post_id !== 0) {
        if (lui_IsPostRepotred($post_id, $user_id)) {
    ...
    if ($comment_id !== 0) {
        if (lui_IsCommentRepotred($comment_id, $user_id)) {
...
function lui_CountUnseenReports() {
...
function lui_UpdateSeenReports() {
    global $wo, $sqlConnect;
...
function lui_DeleteReport($report_id = "") {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    $report_id = lui_Secure($report_id);
    $query     = mysqli_query($sqlConnect, "DELETE FROM " . T_REPORTS . " WHERE `id` = {$report_id}");
    if ($query) {
        return true;
    }
}
function lui_GetReports() {
    global $wo, $sqlConnect;
    $data      = array();
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            if ($fetched_data["post_id"] != 0) {
                $fetched_data["reporter"] = lui_UserData($fetched_data["user_id"]);
                $fetched_data["story"]    = lui_PostData($fetched_data["post_id"]);
                $fetched_data["type"]     = "post";
                $data[]                   = $fetched_data;
            } elseif ($fetched_data["profile_id"] != 0) {
                $fetched_data["reporter"] = lui_UserData($fetched_data["user_id"]);
                $fetched_data["user"]     = lui_UserData($fetched_data["profile_id"]);
                $fetched_data["type"]     = "profile";
                $data[]                   = $fetched_data;
            } elseif ($fetched_data["page_id"] != 0) {
                $fetched_data["reporter"] = lui_UserData($fetched_data["user_id"]);
                $fetched_data["page"]     = lui_PageData($fetched_data["page_id"]);
                $fetched_data["type"]     = "page";
                $data[]                   = $fetched_data;
            } elseif ($fetched_data["group_id"] != 0) {
                $fetched_data["reporter"] = lui_UserData($fetched_data["user_id"]);
                $fetched_data["group"]    = lui_GroupData($fetched_data["group_id"]);
                $fetched_data["type"]     = "group";
                $data[]                   = $fetched_data;
            } elseif ($fetched_data["comment_id"] != 0) {
                $fetched_data["reporter"] = lui_UserData($fetched_data["user_id"]);
                $fetched_data["comment"]  = lui_GetPostComment($fetched_data["comment_id"]);
...
function lui_IsPostRepotred($post_id = "", $user_id = "") {
    global $sqlConnect;
...
function lui_IsCommentRepotred($comment_id = "", $user_id = "") {
    global $sqlConnect;
...
function lui_CountUnseenVerifications() {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_UpdateSeenVerifications() {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_DeleteVerificationRequest($id = "") {
    ...
    if (mysqli_num_rows($sql)) {
        ...
        @lui_DeleteFromToS3($fetched_data['passport']);
        @lui_DeleteFromToS3($fetched_data['photo']);
    }
...
function lui_DeleteVerification($id = 0, $type = "") {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_RemoveVerificationRequest($id = 0, $type = "") {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_VerifyUser($id = 0, $verification_id = 0, $type = "") {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $update      = false;
    if ($type == "Page") {
        $update = lui_UpdatePageData($id, $update_data);
    } elseif ($type == "User") {
        $update = lui_UpdateUserData($id, $update_data);
    }
    if ($update) {
        if (lui_DeleteVerificationRequest($verification_id) === true) {
            return true;
        }
    }
}
function lui_RequestVerification($id = 0, $type = "") {
    global $sqlConnect, $db;
    if (empty($id) or !is_numeric($id) or $id < 1 or empty($type) or $type != "Page") {
        return false;
    }
    if (lui_IsVerificationRequests($id, $type) === true) {
        return false;
    }
    $values = "";
    if ($type == "Page") {
...
function lui_IsVerificationRequests($id = "", $type = "") {
    global $sqlConnect;
...
function lui_GetVerificationButton($id, $type) {
    global $sqlConnect, $wo;
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
    }
    ...
    $verified_type = 0;
    if ($type == "Page") {
        $wo["verification"]       = lui_PageData($id);
        $wo["verification"]["id"] = $wo["verification"]["page_id"];
    } elseif ($type == "User") {
        $wo["verification"]       = lui_UserData($id);
    ...
    if (lui_IsVerificationRequests($id, $type)) {
        return lui_LoadPage($pending);
    } elseif ($verified == 1) {
        return lui_LoadPage($remove);
    } else {
        return lui_LoadPage($request);
    }
}
function lui_GetVerifications() {
    global $wo, $sqlConnect;
    $data      = array();
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            if (!empty($fetched_data["user_id"])) {
                $fetched_data["request_from"]       = lui_UserData($fetched_data["user_id"]);
                $fetched_data["request_from"]["id"] = $fetched_data["user_id"];
            } elseif (!empty($fetched_data["page_id"])) {
                $fetched_data["request_from"]       = lui_PageData($fetched_data["page_id"]);
...
function lui_GetAllPosts($posts = array("limit" => 10, "after_user_id" => 0)) {
    global $wo, $sqlConnect;
    $data     = array();
    $subquery = "";
    $limit    = lui_Secure($posts["limit"]);
    if (isset($posts["after_post_id"]) && !empty($posts["after_post_id"]) && $posts["after_post_id"] > 0) {
        $after_post_id = lui_Secure($posts["after_post_id"]);
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $data[] = lui_PostData($fetched_data["id"], "admin");
        }
    }
    return $data;
}
function lui_IsPostSaved($post_id, $user_id) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_GetSavedPosts($user_id, $after_post_id = 0, $limit = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (isset($after_post_id) && !empty($after_post_id) && is_numeric($after_post_id)) {
        $after_post_id = lui_Secure($after_post_id);
...
function lui_GetPostIdFromCommentId($comment_id = 0) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_GetUserIdFromCommentId($comment_id = 0) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_AddCommentLikes($comment_id, $text = "") {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $comment_id          = lui_Secure($comment_id);
    $user_id             = lui_Secure($wo["user"]["user_id"]);
    $comment_timeline_id = lui_GetUserIdFromCommentId($comment_id);
    ...
    if (lui_IsPageOnwer($post_data["page_id"]) === false) {
        $page_id = 0;
    }
    if (empty($comment_timeline_id)) {
        return false;
    }
    $comment_data = lui_GetPostComment($comment_id);
    $text         = lui_Secure($comment_data["text"]);
    if (isset($text) && !empty($text)) {
        $text = substr($text, 0, 10) . "..";
    }
    if (lui_IsCommentLiked($comment_id, $user_id) === true) {
        ...
        if ($wo["config"]["second_post_button"] == "dislike" && lui_IsCommentWondered($comment_id, $user_id)) {
            lui_AddCommentWonders($comment_id);
        }
...
function lui_CountCommentLikes($comment_id) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_IsCommentLiked($comment_id, $user_id) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_AddCommentWonders($comment_id, $text = "") {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $comment_id      = lui_Secure($comment_id);
    $user_id         = lui_Secure($wo["user"]["user_id"]);
    $comment_user_id = lui_GetUserIdFromCommentId($comment_id);
    $post_id         = lui_GetPostIdFromCommentId($comment_id);
    $page_id         = "";
    $post_data       = lui_PostData($post_id);
    if (!empty($post_data["page_id"])) {
    ...
    if (lui_IsCommentWondered($comment_id, $wo["user"]["user_id"]) === true) {
        ...
        if ($wo["config"]["second_post_button"] == "dislike" && lui_IsCommentLiked($comment_id, $user_id)) {
            lui_AddCommentLikes($comment_id);
        }
...
function lui_CountCommentWonders($comment_id) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_IsCommentWondered($comment_id, $user_id) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_GetCommentLikes($comment_id) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_GetCommentWonders($comment_id) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
    ...
    if (mysqli_num_rows($sql_query)) {
        ...
        if ($sql_numrows > 0) {
            while ($sql_fetch = mysqli_fetch_assoc($sql_query)) {
                $sql_fetch["url"] = lui_SeoLink("index.php?link1=hashtag&hash=" . $sql_fetch["tag"]);
                $data[]           = $sql_fetch;
            }
        }
    }
...
function lui_GetHashtagPosts($s_query, $after_post_id = 0, $limit = 5, $before_post_id = 0) {
    global $sqlConnect;
    $data         = array();
    $search_query = str_replace("#", "", lui_Secure($s_query));
    $hashdata     = lui_GetHashtag($search_query, false);
    if (is_array($hashdata) && count($hashdata) > 0) {
        ...
        if (isset($before_post_id) && !empty($before_post_id) && is_numeric($before_post_id)) {
            $before_post_id = lui_Secure($before_post_id);
        ...
        if (mysqli_num_rows($sql_query_one)) {
            while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
                $posts = lui_PostData($sql_fetch_one["id"]);
                if (is_array($posts)) {
                    $data[] = $posts;
                }
            }
...
function lui_SearchForPosts($id = 0, $s_query = "", $limit = 5, $type = "") {
    global $wo, $sqlConnect;
    $data = array();
    if ($wo["loggedin"] == false) {
        return false;
    ...
    $search_query = lui_Secure($s_query);
...
function lui_GetSerachHash($s_query) {
    global $sqlConnect;
    $search_query = str_replace("#", "", lui_Secure($s_query));
    $data         = array();
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data["url"] = lui_SeoLink("index.php?link1=hashtag&hash=" . $fetched_data["tag"]);
            $data[]              = $fetched_data;
        }
    }
    return $data;
}
function lui_CountOnlineUsers() {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetChatUsers($type) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_ChatSearchUsers($search_query = "") {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_UpdateStatus($status = "online") {
...
function lui_IsOnline($user_id) {
    global $wo;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    }
    $user_id  = lui_Secure($user_id);
    $lastseen = lui_UserData($user_id);
    if ($wo["config"]["node_socket_flow"] == "1") {
        $time = time() - 03;
    } else {
        $time = time() - 60;
...
function lui_GetOfflineTyping() {
    ...
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            lui_DeleteAllTyping($fetched_data["user_id"]);
        }
    }
}
function lui_IsTyping($recipient_id) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $query_one    = mysqli_query($sqlConnect, $query);
    return lui_Sql_Result($query_one, 0) == 1 ? true : false;
}
function lui_RegisterTyping($recipient_id, $isTyping = 1) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (lui_IsFollowing($user_id, $recipient_id) === false) {
        return false;
    }
...
function lui_DeleteAllTyping($user_id) {
    global $sqlConnect;
...
function lui_UpdateAdsCode($update_data = array()) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetAd($type, $admin = true) {
    global $sqlConnect;
    $type      = lui_Secure($type);
...
function lui_UpdateAdActivation($type) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_AddNewAnnouncement($text) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetAnnouncement($id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetAnnouncementViews($id) {
    global $sqlConnect, $wo;
    $id        = lui_Secure($id);
...
function lui_GetActiveAnnouncements() {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($row = mysqli_fetch_assoc($query)) {
            $data[] = lui_GetAnnouncement($row["id"]);
        }
    }
    return $data;
}
function lui_GetHomeAnnouncements() {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        $data         = lui_GetAnnouncement($fetched_data["id"]);
        return $data;
    }
    return false;
}
function lui_GetInactiveAnnouncements() {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_DeleteAnnouncement($id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_IsActiveAnnouncement($id) {
    global $sqlConnect;
    $id    = lui_Secure($id);
    $query = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_ANNOUNCEMENT . " WHERE `id` = '{$id}' AND `active` = '1'");
    return luiSql_Result($query, 0) == 1 ? true : false;
}
function lui_IsViewedAnnouncement($id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    return luiSql_Result($query, 0) > 0 ? true : false;
}
function lui_IsThereAnnouncement() {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_DisableAnnouncement($id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (lui_IsActiveAnnouncement($id) === false) {
        return false;
    }
...
function lui_ActivateAnnouncement($id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (lui_IsActiveAnnouncement($id) === true) {
        return false;
    }
...
function lui_UpdateAnnouncementViews($id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (lui_IsViewedAnnouncement($id) === true) {
        return false;
    }
...
function lui_RegisterApp($registration_data) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $registration_data["app_id"]     = lui_Secure(substr($id_str, 0, 20));
    $secret_str                      = sha1($registration_data["app_user_id"] . lui_GenerateKey(55, 55) . microtime());
    $registration_data["app_secret"] = lui_Secure(substr($secret_str, 0, 39));
...
function lui_IsAppOnwer($app_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    $user_id   = lui_Secure($wo["user"]["user_id"]);
    $app_id    = lui_Secure($app_id);
...
function lui_GetAppsData($placement = "") {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            if (is_array($fetched_data)) {
                $data[] = lui_GetApp($fetched_data["id"]);
            }
        }
    }
    return $data;
}
function lui_GetApp($app_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_one) == 1) {
        $sql_query_one               = mysqli_fetch_assoc($query_one);
        $sql_query_one["app_onwer"]  = lui_UserData($sql_query_one["app_user_id"]);
        $sql_query_one["app_avatar"] = lui_GetMedia($sql_query_one["app_avatar"]);
        return $sql_query_one;
    }
    //}
    return false;
}
function lui_GetCode($code = "") {
    global $sqlConnect, $wo;
    if (empty($code)) {
        return false;
    }
...
function lui_UpdateAppImage($app_id, $image) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_UpdateAppData($app_id, $update_data) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $app_id = lui_Secure($app_id);
    if (!lui_IsAppOnwer($app_id) && !lui_IsAdmin()) {
        return false;
    }
    $update = array();
    foreach ($update_data as $field => $data) {
        $update[] = "`" . $field . '` = \'' . lui_Secure($data) . '\'';
    }
...
function lui_GetIdFromAppID($app_id) {
    global $sqlConnect;
    if (empty($app_id)) {
        return false;
    }
...
function lui_AccessToken($app_id, $app_secret) {
    global $sqlConnect;
    if (empty($app_id)) {
        return false;
    }
    ...
    $app_id        = lui_Secure($app_id);
    $app_secret    = lui_Secure($app_secret);
...
function lui_IsValidApp($app_id) {
    global $sqlConnect;
    if (empty($app_id)) {
        return false;
    }
...
function lui_VerifyAPIApii($app_id = "", $secret_id = "") {
    global $sqlConnect;
    if (empty($app_id) || empty($secret_id)) {
        return false;
    }
    $app_id        = lui_Secure($app_id);
    $secret_id     = lui_Secure($secret_id);
...
function lui_AppHasPermission($user_id, $app_id) {
    global $sqlConnect, $wo;
    if (empty($app_id)) {
        return false;
    }
...
function lui_AcceptPermissions($app_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GenrateToken($user_id, $app_id) {
    global $sqlConnect, $wo;
    $app_id  = lui_Secure($app_id);
    $user_id = lui_Secure($user_id);
    if (empty($app_id) || empty($user_id)) {
...
function lui_GenrateCode($user_id, $app_id) {
    global $sqlConnect, $wo;
    $app_id  = lui_Secure($app_id);
    $user_id = lui_Secure($user_id);
    if (empty($app_id) || empty($user_id)) {
...
function lui_UserIdFromToken($token = "") {
    global $sqlConnect, $wo;
    if (empty($token)) {
        return false;
    }
...
function lui_GetIdFromToken($token) {
    global $sqlConnect, $wo;
    if (empty($token)) {
        return false;
    }
...
function lui_RegisterPage($registration_data = array()) {
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    }
...
function lui_GetMyPages($user_id = false, $limit = 0, $offset = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetMyPagesAPI($limit = 0, $offset = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_IsPageOnwer($page_id,$with_admin = true) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    return lui_Sql_Result($query, "0") == 1 || lui_IsPageAdminExists($user_id, $page_id) ? true : false;
}
function lui_IsCanGroupUpdate($group_id, $page) {
    global $sqlConnect, $wo;
    $array = array(
        "general",
        "privacy",
...
function lui_GetAllowedGroupPages($group_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetGroupAdminInfo($user_id, $group_id) {
    global $sqlConnect, $wo;
...
function lui_IsCanPageUpdate($page_id, $page) {
    global $sqlConnect, $wo;
    $array = array(
        "general",
        "info",
...
function lui_GetAllowedPages($page_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_PageExists($page_name = "") {
    global $sqlConnect;
    if (empty($page_name)) {
        return false;
    }
...
function lui_PageExistsByID($id = 0) {
    global $sqlConnect;
    if (empty($id)) {
        return false;
    }
...
function lui_PageIdFromPagename($page_name = "") {
    global $sqlConnect;
    if (empty($page_name)) {
        return false;
    }
    ...
    return lui_Sql_Result($query, 0, "page_id");
}
function lui_PageData($page_id = 0) {
    ...
    $fetched_data["type"]              = "page";
    $fetched_data["url"]               = lui_SeoLink("index.php?link1=timeline&u=" . $fetched_data["page_name"]);
    $fetched_data["name"]              = $fetched_data["page_title"];
    $fetched_data["rating"]            = lui_PageRating($fetched_data["page_id"]);
    $fetched_data["category"]          = "";
    $fetched_data["page_sub_category"] = "";
    $fetched_data["is_reported"]       = lui_IsReportExists($fetched_data["page_id"], "page");
    ...
    if ($wo["loggedin"] == true) {
        $fetched_data["is_page_onwer"] = lui_IsPageOnwer($fetched_data["page_id"]) ? true : false;
    }
...
function lui_PageActive($page_name) {
    global $sqlConnect;
    if (empty($page_name)) {
        return false;
    }
...
function lui_GetPagePostPublisherBox($page_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (!lui_IsPageOnwer($page_id) && lui_UserCanPostPage($page_id)) {
        $wo["page_profile"]["avatar"] = $wo["user"]["avatar"];
        $wo["page_profile"]["name"]   = $wo["user"]["name"];
        return lui_LoadPage("story/publisher-box");
    }
    if (lui_IsPageOnwer($page_id)) {
        return lui_LoadPage("story/publisher-box");
    }
}
function lui_UserCanPostPage($page_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetLikeButton($page_id = 0) {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $page = $wo["like"] = lui_PageData($page_id);
    if (lui_IsPageOnwer($page_id)) {
        if ($page["user_id"] != $wo["user"]["id"] && !lui_IsAdmin() && !lui_IsModerator() && lui_IsPageAdminExists($wo["user"]["id"], $page_id)) {
            return false;
        } elseif ($page["user_id"] == $wo["user"]["id"] || lui_IsPageAdminExists($wo["user"]["id"], $page_id)) {
            return false;
        }
    }
    //$page = $wo['like'] = lui_PageData($page_id);
    ...
    if (lui_IsPageLiked($page_id, $logged_user_id) === true) {
        return lui_LoadPage($unlike_button);
    } else {
        return lui_LoadPage($like_button);
    }
}
function lui_GetPageMessageButton($page_id = 0) {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_IsPoked($received_user_id = 0, $send_user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    return lui_Sql_Result($query_one, 0) > 0 ? true : false;
}
function lui_IsPageLiked($page_id = 0, $user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_RegisterPageLike($page_id = 0, $user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $page_id    = lui_Secure($page_id);
    $user_id    = lui_Secure($user_id);
    $page_onwer = lui_GetUserIdFromPageId($page_id);
    $active     = 1;
    if (lui_IsPageLiked($page_id, $user_id) === true) {
        return false;
    }
    ...
    if ($query) {
        if (lui_IsPageInvited($user_id, $page_id) > 0) {
            foreach (lui_GetPageInviters($user_id, $page_id) as $user) {
            ...
            $delete_invite = lui_DeleteInvites($user_id, $page_id);
        }
        $notification_data = array(
...
function lui_DeletePageLike($page_id = 0, $user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $user_id = lui_Secure($user_id);
    $active  = 1;
    if (lui_IsPageLiked($page_id, $user_id) === false) {
        return false;
    }
...
function lui_UpdatePageData($page_id = 0, $update_data = array()) {
    global $wo, $sqlConnect, $cache;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $update = array();
    foreach ($update_data as $field => $data) {
        $update[] = "`" . $field . '` = \'' . lui_Secure($data, 1) . '\'';
    }
    $impload   = implode(", ", $update);
...
function lui_UpdatePageAdminData($page_id, $update_data, $user_id) {
    global $wo, $sqlConnect, $cache;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_UpdateGroupAdminData($group_id, $update_data, $user_id) {
    global $wo, $sqlConnect, $cache;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_UpdatePostData($post_id = 0, $update_data = array()) {
    global $wo, $sqlConnect, $cache;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetPageIdFromPostId($post_id = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_GetUserIdFromPageId($page_id = 0) {
    global $sqlConnect;
...
function lui_DeletePage($page_id = 0) {
    ...
    if (mysqli_num_rows($query_four_delete_media) > 0) {
        while ($fetched_data = mysqli_fetch_assoc($query_four_delete_media)) {
            ...
            $delete_posts = lui_DeletePost($fetched_data["post_id"]);
        }
    }
    if ($wo["config"]["cacheSystem"] == 1) {
...
function lui_CountPageLikes($page_id = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($page_id) or !is_numeric($page_id) or $page_id < 1) {
        return false;
...
function lui_CountPagePosts($page_id = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($page_id) or !is_numeric($page_id) or $page_id < 1) {
        return false;
...
function lui_CountLikesThisWeek($page_id = 0) {
    global $wo, $sqlConnect;
    $data = array();
...
function lui_PageSug($limit = 1, $page_id = 0, $type = "next") {
    global $wo, $sqlConnect;
    if (!is_numeric($limit)) {
        return false;
    }
...
function lui_GetLikes($user_id = 0, $type = "", $limit = "", $after_user_id = "", $placement = array()) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
    ...
    if (!empty($placement)) {
        if ($placement["in"] == "profile_sidebar" && is_array($placement["likes_data"])) {
            foreach ($placement["likes_data"] as $key => $id) {
                $page_data = lui_PageData($id, false);
...
function lui_CountUserLikes($user_id) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
...
function lui_GetAllPages($limit = "", $after = "") {
    global $wo, $sqlConnect;
    $data      = array();
    $query_one = " SELECT `page_id` FROM " . T_PAGES;
    if (!empty($after) && is_numeric($after) && $after > 0) {
        $query_one .= " WHERE `page_id` < " . lui_Secure($after);
    }
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $page_data          = lui_PageData($fetched_data["page_id"]);
            $page_data["owner"] = lui_UserData($page_data["user_id"]);
            $data[]             = $page_data;
        }
    }
    return $data;
}
function lui_RegisterGroup($registration_data = array()) {
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    }
    ...
    if ($query) {
        ...
        lui_RegisterGroupJoin($query_id, $wo["user"]["user_id"]);
        return true;
    } else {
        return false;
    }
}
function lui_GetMyGroups() {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetMyGroupsAPI($limit = 0, $offset = 0, $sort = "") {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_IsGroupOnwer($group_id = 0, $user_id = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    return lui_Sql_Result($query, "0") == 1 || lui_IsGroupUserExists($user_id, $group_id) ? true : false;
}
function lui_GroupExists($group_name = "") {
    global $sqlConnect;
    if (empty($group_name)) {
        return false;
    }
...
function lui_GroupIdFromGroupname($group_name = "") {
    global $sqlConnect;
    if (empty($group_name)) {
        return false;
    }
    ...
    return lui_Sql_Result($query, 0, "id");
}
function lui_GroupData($group_id = 0) {
    global $wo, $sqlConnect, $cache;
    ...
    $fetched_data["avatar"]             = lui_GetMedia($fetched_data["avatar"]);
    $fetched_data["cover"]              = lui_GetMedia($fetched_data["cover"]);
    $fetched_data["url"]                = lui_SeoLink("index.php?link1=timeline&u=" . $fetched_data["group_name"]);
    ...
    $fetched_data["is_reported"]        = lui_IsReportExists($fetched_data["id"], "group");
    ...
    $fetched_data["fields"] = array();
    $fields                 = lui_GetCustomFields("group");
    if (!empty($fields)) {
        foreach ($fields as $key => $field) {
            if (in_array($field["fid"], array_keys($fetched_data))) {
                $fetched_data["fields"][$field["fid"]] = $fetched_data[$field["fid"]];
    ...
    if (lui_IsJoinRequested($fetched_data["group_id"])) {
        $fetched_data["is_group_joined"] = 2;
    } elseif (lui_IsGroupJoined($fetched_data["group_id"])) {
        $fetched_data["is_group_joined"] = 1;
    } else {
        $fetched_data["is_group_joined"] = 0;
    }
    $fetched_data["members_count"] = lui_CountGroupMembers($fetched_data["group_id"]);
    return $fetched_data;
}
function lui_GroupActive($group_name) {
    global $sqlConnect;
    if (empty($group_name)) {
        return false;
    }
...
function lui_CanBeOnGroup($group_id) {
    global $sqlConnect;
    if (empty($group_id)) {
        return false;
    }
    $group_id = lui_Secure($group_id);
    if (lui_IsGroupOnwer($group_id)) {
        return true;
    }
    ...
    if ($group["privacy"] == 2) {
        if (lui_IsGroupJoined($group_id) === true) {
            return true;
        }
...
function lui_GetGroupPostPublisherBox($group_id = 0) {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $group_id = lui_Secure($group_id);
    $continue = false;
    if (lui_CanBeOnGroup($group_id) === true) {
        $group = lui_GroupData($group_id);
        if ($group["privacy"] == 2) {
            if (lui_IsGroupJoined($group_id) === true) {
                $continue = true;
...
function lui_GetJoinButton($group_id = 0) {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $group_id = lui_Secure($group_id);
    $group    = $wo["join"] = lui_GroupData($group_id);
    if (!isset($wo["join"]["id"])) {
        return false;
    }
    $logged_user_id        = lui_Secure($wo["user"]["user_id"]);
    ...
    if (lui_IsGroupJoined($group_id, $logged_user_id) === true) {
        return lui_LoadPage($leave_button);
    } else {
        if (lui_IsJoinRequested($group_id) === true) {
            return lui_LoadPage($accept_request_button);
        } else {
            return lui_LoadPage($join_button);
        }
    }
}
function lui_GetGoingButton($event_id = 0) {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $event    = $wo["going"] = lui_EventData($event_id);
    ...
    if (lui_EventGoingExists($event_id) === true) {
        return lui_LoadPage($no_going);
    } else {
        return lui_LoadPage($going);
    }
}
function lui_GetInterestedButton($event_id = 0) {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $event_id = lui_Secure($event_id);
    $event    = $wo["interested"] = lui_EventData($event_id);
    ...
    if (lui_EventInterestedExists($event_id) === true) {
        return lui_LoadPage($no_interested);
    } else {
        return lui_LoadPage($interested);
    }
}
function lui_IsGroupJoined($group_id = 0, $user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_IsJoinRequested($group_id = 0, $user_id = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_RegisterGroupJoin($group_id = 0, $user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $group_id    = lui_Secure($group_id);
    $user_id     = lui_Secure($user_id);
    $group_onwer = lui_GetUserIdFromGroupId($group_id);
    $active      = 1;
    if (lui_IsGroupJoined($group_id, $user_id) === true) {
        return false;
    }
    $group_data = lui_GroupData($group_id);
...
function lui_LeaveGroup($group_id = 0, $user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $user_id  = lui_Secure($user_id);
    $active   = 1;
    if (lui_IsGroupJoined($group_id, $user_id) === false && lui_IsJoinRequested($group_id, $user_id) === false) {
        return false;
    }
...
function lui_UpdateGroupData($group_id = 0, $update_data = array()) {
    global $wo, $sqlConnect, $cache;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetGroupIdFromPostId($post_id = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_GetUserIdFromGroupId($group_id = 0) {
    global $sqlConnect;
...
function lui_DeleteGroup($group_id = 0) {
    global $wo, $sqlConnect, $cache;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CountGroupMembers($group_id = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
        return false;
...
function lui_CountGroupPosts($group_id = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
        return false;
...
function lui_CountJoinedThisWeek($group_id = 0) {
    global $wo, $sqlConnect;
    $data = array();
    $time = strtotime("-1 week");
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
...
function lui_GroupSug($limit = 20) {
    global $wo, $sqlConnect;
    if (!is_numeric($limit)) {
        return false;
    }
...
function lui_GetGroupMembers($group_id = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
        return false;
...
function lui_GetUsersGroups($user_id = 0, $limit = 12, $placement = array(), $offset = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
    ...
    if (mysqli_num_rows($sql_query)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
            $data[] = lui_GroupData($fetched_data["group_id"]);
        }
    }
    return $data;
}
function lui_GetUsersGroupsAPI($user_id, $limit = 0, $offset = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
...
function lui_GetGroupSettingMembers($group_id = 0, $limit = 0, $offset = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
        return false;
...
function lui_CountUserGroups($user_id) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
...
function lui_GetAllGroups($limit = "", $after = "") {
    global $wo, $sqlConnect;
    $data      = array();
    $query_one = " SELECT `id` FROM " . T_GROUPS;
    if (!empty($after) && is_numeric($after) && $after > 0) {
        $query_one .= " WHERE `id` < " . lui_Secure($after);
    }
    $query_one .= " ORDER BY `id` DESC";
    if (isset($limit) and !empty($limit)) {
        $query_one .= " LIMIT {$limit}";
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $group_data            = lui_GroupData($fetched_data["id"]);
            $group_data["members"] = lui_CountGroupMembers($fetched_data["id"]);
            $group_data["owner"]   = lui_UserData($group_data["user_id"]);
            $data[]                = $group_data;
        }
    }
    return $data;
}
function lui_GetRegisteredDataStatics($month, $type = "user") {
    global $wo, $sqlConnect;
    $year       = date("Y");
    $type_table = T_USERS;
    $type_id    = "user_id";
...
function lui_CountAllData($type) {
    global $wo, $sqlConnect;
    $type_table = T_USERS;
    $type_id    = "user_id";
    if ($type == "user") {
...
function lui_CountOnlineData($type = "") {
    global $wo, $sqlConnect;
    $data       = array();
    $type_table = T_USERS;
    $type_id    = lui_Secure("user_id");
    $time       = time() - 60;
...
function lui_BanNewIp($ip, $reason = "") {
    global $sqlConnect;
    $ip        = lui_Secure($ip);
...
function lui_DeleteBanned($id) {
    global $sqlConnect;
    $id = lui_Secure($id);
    if (lui_IsIpBanned($id) === false) {
        return false;
    }
...
function lui_GameData($game_id) {
    ...
    $fetched_data["game_avatar"] = lui_GetMedia($fetched_data["game_avatar"]);
    $fetched_data["url"]         = lui_SeoLink("index.php?link1=game&id=" . $fetched_data["id"]);
    $fetched_data["name"]        = $fetched_data["game_name"];
    $fetched_data["last_play"]   = lui_LastPlay($fetched_data["id"]);
    return $fetched_data;
}
function lui_LastPlay($id) {
    global $wo, $sqlConnect;
    $data = array();
    if ($wo["loggedin"] == false) {
        return false;
...
function lui_GetAllGames($limit = 5, $after = 0) {
    global $wo, $sqlConnect;
    $data      = array();
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $fetched_data            = lui_GameData($fetched_data["id"]);
            $fetched_data["players"] = lui_CountGamePlayers($fetched_data["id"]);
            $data[]                  = $fetched_data;
        }
    }
    return $data;
}
function lui_AddGame($data = array()) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetAllGifts($limit = 5, $after = 0) {
    global $wo, $sqlConnect;
    $data      = array();
...
function lui_GetAllStickers($limit = 5, $after = 0) {
    global $wo, $sqlConnect;
    $data      = array();
...
function lui_CountGamePlayers($id) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
...
function lui_GetMyGames($limit = 0, $offset = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $offset_ = "";
    if (!empty($offset) && is_numeric($offset) && $offset > 0) {
        $offset_ .= " AND `id` < " . lui_Secure($offset);
    }
    $limit_ = "";
    if (!empty($limit)) {
        $limit  = lui_Secure($limit);
    ...
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            if (is_array($fetched_data)) {
                $game_data              = lui_GameData($fetched_data["game_id"]);
...
function lui_IsPhoneExist($phone) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($phone)) {
        return false;
...
function lui_GetGroupRequests($group_id) {
    global $wo, $sqlConnect;
    $data      = array();
    $group_id  = lui_Secure($group_id);
    $user_id   = $wo["user"]["user_id"];
...
function lui_GetGroupRequestsWithOffset($request = array()) {
    global $wo, $sqlConnect;
    if (empty($request) || !is_array($request) || empty($request["group_id"])) {
        return false;
    }
    $data         = array();
    $group_id     = lui_Secure($request["group_id"]);
    $limit_query  = "";
    $offset_query = "";
    if (!empty($request["limit"])) {
        $limit_query = " LIMIT " . lui_Secure($request["limit"]);
    }
    if (!empty($request["offset"])) {
        $offset_query = " AND `id` < '" . lui_Secure($request["offset"]) . "' ";
    }
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            ...
            $new_data["user_data"] = lui_UserData($fetched_data["user_id"]);
            $data[]                = $new_data;
        }
    }
    return $data;
}
function lui_AcceptJoinRequest($user_id, $group_id) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (lui_IsJoinRequested($group_id, $user_id) === false) {
        return false;
    }
    if (lui_IsGroupJoined($group_id, $user_id) === true) {
        return false;
...
function lui_DeleteJoinRequest($user_id, $group_id) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CountGroupRequests($group_id) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($group_id) or !is_numeric($group_id) or $group_id < 1) {
        return false;
...
function lui_RegisterAlbumMedia($id, $media, $parent_id = 0) {
    global $wo, $sqlConnect;
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
    }
    ...
    if (!empty($parent_id) && is_numeric($parent_id) && $parent_id > 0) {
        $parent_id = lui_Secure($parent_id);
    }
...
function lui_CountAlbumImages($post_id) {
    global $wo, $sqlConnect;
...
function lui_CountUserAlbums($user_id) {
    global $wo, $sqlConnect;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    }
...
function lui_DeleteImageFromAlbum($post_id, $id) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_2)) {
        ...
        if (mysqli_num_rows($query)) {
            ...
            $delete_post    = lui_DeletePost($fetched_data_2["post_id"]);
    ...
    if (mysqli_num_rows($delete_query_2) == 1) {
        $delete_post = lui_DeletePost($post_id);
    }
...
function lui_AlbumImageData($data = array()) {
    global $wo, $sqlConnect;
    if (!empty($data["id"])) {
    ...
    if (mysqli_num_rows($sql)) {
        $fetched_data = mysqli_fetch_assoc($sql);
        if (!empty($fetched_data)) {
            $fetched_data["image_org"] = lui_GetMedia($fetched_data["image"]);
        }
        return $fetched_data;
    }
    return false;
}
function lui_GetCommentReplies($comment_id = 0, $limit = 5, $order_by = "ASC") {
    global $sqlConnect, $wo;
...
function lui_GetCommentReply($reply_id = 0) {
    global $wo, $sqlConnect;
    ...
    if (mysqli_num_rows($query_one)) {
        ...
        $fetched_data["Orginaltext"]         = lui_EditMarkup($fetched_data["text"], true, true, true, 0, 0, $reply_id);
        $fetched_data["Orginaltext"]         = str_replace("<br>", "\n", $fetched_data["Orginaltext"]);
        $fetched_data["text"]                = lui_Markup($fetched_data["text"], true, true, true, 0, 0, $reply_id);
        $fetched_data["text"]                = lui_Emo($fetched_data["text"]);
        $fetched_data["onwer"]               = false;
        $fetched_data["post_onwer"]          = false;
        $fetched_data["comment_likes"]       = lui_CountCommentReplyLikes($fetched_data["id"]);
        $fetched_data["comment_wonders"]     = lui_CountCommentReplyWonders($fetched_data["id"]);
        ...
        if ($wo["loggedin"] == true) {
            ...
            $fetched_data["is_comment_wondered"] = lui_IsCommentReplyWondered($fetched_data["id"], $wo["user"]["user_id"]) ? true : false;
            $fetched_data["is_comment_liked"]    = lui_IsCommentReplyLiked($fetched_data["id"], $wo["user"]["user_id"]) ? true : false;
        }
        if ($wo["config"]["second_post_button"] == "reaction") {
            $fetched_data["reaction"] = lui_GetPostReactionsTypes($fetched_data["id"], "replay");
        }
        return $fetched_data;
    }
    return false;
}
function lui_CountCommentReplies($comment_id = "") {
    global $sqlConnect;
    if (empty($comment_id) || !is_numeric($comment_id) || $comment_id < 0) {
        return false;
    }
...
function lui_DeleteCommentReply($comment_id = "") {
    global $wo, $sqlConnect;
...
function lui_RegisterCommentReply($data = array()) {
    ...
    $comment = lui_GetPostComment($data["comment_id"]);
    $text    = "";
    $type2   = "";
    $page_id = "";
    ...
    $user_id = lui_GetUserIdFromCommentId($data["comment_id"]);
    if (empty($user_id)) {
        $user_id = lui_GetUserIdFromPageId($comment["page_id"]);
        if (empty($user_id)) {
            return false;
        }
    }
    ...
    if ($query) {
        ...
        $post_data         = lui_PostData($comment["post_id"]);
        ...
        $also = array();
        if (!empty($user_id)) {
            $also = lui_GetRepliedUsers($data["comment_id"]);
...
function lui_IsCommentOnwer($user_id, $comment_id) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_GetRepliedUsers($comment_id) {
    global $sqlConnect, $wo;
    $data = array();
    if ($wo["loggedin"] == false) {
        return false;
...
function lui_GetUserProfilePicture($image = "", $type = "") {
    global $sqlConnect, $wo;
    if (empty($image)) {
        return false;
    }
...
function lui_RegsiterRecent($id = 0, $type = "") {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetSearchAdv($search_qeury, $type, $offset = 0, $limit = 0) {
    global $sqlConnect;
    ...
    if ($type == "groups") {
        ...
        if (mysqli_num_rows($query)) {
            while ($fetched_data = mysqli_fetch_assoc($query)) {
                $data[] = lui_GameData($fetched_data["id"]);
            }
        }
    } elseif ($type == "posts") {
        ...
        if (mysqli_num_rows($query)) {
            while ($fetched_data = mysqli_fetch_assoc($query)) {
                $data[] = lui_PostData($fetched_data["id"]);
            }
        }
    }
    return $data;
}
function lui_GetUserAlbums($user_id, $placement = "", $limit = 5000, $offset = 0) {
    global $sqlConnect, $wo;
    $data         = array();
    $user_id      = lui_Secure($user_id);
    $offset_query = "";
...
function lui_AddCommentReplyWonders($reply_id, $text = "") {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $reply_id        = lui_Secure($reply_id);
    $user_id         = lui_Secure($wo["user"]["user_id"]);
    $comment_user_id = lui_GetUserIdFromReplyId($reply_id);
    ...
    $reply_data = lui_GetCommentReply($reply_id);
    $text       = lui_Secure($reply_data["text"]);
    if (isset($text) && !empty($text)) {
        $text = mb_substr($text, 0, 10, "UTF-8") . "..";
    }
    if (lui_IsCommentReplyWondered($reply_id, $wo["user"]["user_id"]) === true) {
        ...
        if ($wo["config"]["second_post_button"] == "dislike" && lui_IsCommentReplyLiked($reply_id, $wo["user"]["user_id"])) {
            lui_AddCommentReplyLikes($reply_id);
        }
...
function lui_CountCommentReplyWonders($reply_id) {
    global $sqlConnect;
...
function lui_IsCommentReplyWondered($reply_id, $user_id) {
    global $sqlConnect;
    if (empty($reply_id) or !is_numeric($reply_id) or $reply_id < 1) {
        return false;
    }
...
function lui_GetCommentIdFromReplyId($reply_id = 0) {
    global $sqlConnect;
    if (empty($reply_id) or !is_numeric($reply_id) or $reply_id < 1) {
        return false;
    }
...
function lui_GetUserIdFromReplyId($reply_id = 0) {
    global $sqlConnect;
    if (empty($reply_id) or !is_numeric($reply_id) or $reply_id < 1) {
        return false;
    }
...
function lui_AddCommentReplyLikes($reply_id, $text = "") {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (lui_IsCommentReplyLiked($reply_id, $user_id) === true) {
        ...
        if ($wo["config"]["second_post_button"] == "dislike" && lui_IsCommentReplyWondered($reply_id, $wo["user"]["user_id"])) {
            lui_AddCommentReplyWonders($reply_id);
        }
...
function lui_CountCommentReplyLikes($reply_id) {
    global $sqlConnect;
    if (empty($reply_id) or !is_numeric($reply_id) or $reply_id < 1) {
        return false;
    }
...
function lui_IsCommentReplyLiked($reply_id, $user_id) {
    global $sqlConnect;
    if (empty($reply_id) or !is_numeric($reply_id) or $reply_id < 1) {
        return false;
    }
...
function lui_CanSeeBirthday($user_id, $privacy) {
    global $sqlConnect, $wo;
    ...
    if ($privacy == 0) {
        ...
        if ($wo["loggedin"] !== false) {
            if (lui_IsFollowing($wo["user"]["user_id"], $user_id) === true) {
                return true;
            }
        } else {
            return false;
...
function lui_CountPageInvites($page_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetPageInvites($page_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            $data[] = lui_UserData($fetched_data["following_id"]);
        }
    }
    return $data;
}
function lui_RegsiterInvite($user_id, $page_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (lui_IsPageInvited($user_id, $page_id) === true) {
        return false;
    }
    if (lui_PageExistsByID($page_id) === false) {
        return false;
    }
    $page_id        = lui_Secure($page_id);
    $logged_user_id = lui_Secure($wo["user"]["user_id"]);
...
function lui_IsPageInvited($user_id, $page_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetPageInviters($user_id, $page_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            $data[] = lui_UserData($fetched_data["inviter_id"]);
        }
    }
    return $data;
}
function lui_GetUserInviters($user_id, $limit = 20, $offset = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $data        = array();
    $page_id     = lui_Secure($page_id);
    $limit       = !empty($limit) && is_numeric($limit) && $limit > 0 ? lui_Secure($limit) : 20;
    $offset_text = "";
    if (!empty($offset) && is_numeric($offset) && $offset > 0) {
        $offset      = lui_Secure($offset);
...
function lui_DeleteInvites($user_id, $page_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetCallInAction($id, $url) {
    global $sqlConnect, $wo;
    if (empty($id)) {
        return false;
    }
    ...
    return lui_LoadPage("buttons/call-action");
}
function lui_CountUserData($type) {
    global $wo, $sqlConnect;
    $type_table = T_USERS;
    $type_id    = "user_id";
    $where      = "";
...
function lui_CountPageData($type) {
    global $wo, $sqlConnect;
    $type_table = T_PAGES;
    $type_id    = "id";
    $where      = "";
...
function lui_CountGroupData($type) {
    global $wo, $sqlConnect;
    $type_table = T_PAGES;
    $type_id    = "id";
    $where      = "";
...
function lui_CountPostData($type) {
    global $wo, $sqlConnect;
    $type_table = T_PAGES;
    $type_id    = "id";
    $where      = "";
...
function lui_CountGroupsNotMember($group_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetGroupsNotMember($group_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GroupExistsByID($id) {
    global $sqlConnect;
    if (empty($id)) {
        return false;
    }
...
function lui_UserExistsById($id) {
    global $sqlConnect;
    if (empty($id)) {
        return false;
    }
...
function lui_RegsiterGroupAdd($user_id, $group_id) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (lui_GroupExistsByID($group_id) === false) {
        return false;
    }
    if (lui_UserExistsById($user_id) === false) {
        return false;
    }
    if (lui_IsGroupOnwer($group_id, $user_id)) {
        return false;
    }
    $logged_user_id = lui_Secure($wo["user"]["user_id"]);
    $group_data     = lui_GroupData($group_id);
...
function lui_SendMessageFromDB() {
    global $wo, $sqlConnect;
...
function lui_AddPostVideoView($post_id = false) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_SendMessage($data = array()) {
    global $wo, $sqlConnect;
    include_once "luisincludes/librerias/PHPMailer-Master/vendor/autoload.php";
    $mail = new PHPMailer\PHPMailer\PHPMailer();
    ...
    $email_from      = $data["from_email"] = lui_Secure($data["from_email"]);
    $to_email        = $data["to_email"] = lui_Secure($data["to_email"]);
    $subject         = $data["subject"];
    $message_body    = mysqli_real_escape_string($sqlConnect, $data["message_body"]);
    $data["charSet"] = lui_Secure($data["charSet"]);
...
function lui_CheckBirthdays($user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_SendSMSMessage($to, $message) {
    global $wo, $sqlConnect;
    if (empty($to)) {
        return false;
    }
...
function lui_ConfirmUserSMS($user_id, $code) {
    global $sqlConnect;
    $user_id = lui_Secure($user_id);
    $code    = lui_Secure($code);
...
function lui_ConfirmUser($user_id, $code) {
    global $sqlConnect;
    $user_id = lui_Secure($user_id);
    $code    = lui_Secure($code);
    if (!is_numeric($code) || $code <= 0) {
...
function lui_ConfirmSMSUser($user_id, $code, $email_code = "") {
    global $sqlConnect;
    $user_id = lui_Secure($user_id);
    $code    = lui_Secure($code);
    if (!is_numeric($code) || $code <= 0) {
...
function lui_CheckSession($hash = "") {
...
function lui_CreateMainSession() {
...
function lui_CheckMainSession($hash = "") {
...
function lui_ReplenishingUserBalance($sum) {
    global $wo, $sqlConnect;
    ...
    $update      = lui_UpdateUserData($wo["user"]["user_id"], $update_data);
    return $update;
}
function lui_ReplenishWallet($sum) {
    global $wo;
...
function lui_IsUserPro($user_pro = 0) {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetUserProType($user_type = 0, $type = "post") {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $data["type_url"]   = lui_SeoLink("index.php?link1=upgrade-to");
    $data["icon"]       = "";
...
function lui_GetAvUpgrades($user_id) {
    global $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    foreach ($wo["pro_packages"] as $key => $value) {
        ...
        if ($user['pro_type'] < $key) {
            $can_see[$key] = $value["name"] . " " . $wo["lang"]["member"] . " " . $value["price"] . lui_GetCurrency($wo["config"]["currency"]) . " " . (!empty($value['time_count']) ? $value['time_count'] : '').' '.$wo['lang'][$value['time']];
        }
    }
    return $can_see;
    ...
    //     $hot_member = $wo["lang"]["hot"] . " " . $wo["lang"]["member"] . " " . $wo["pro_packages"]["hot"]["price"] . lui_GetCurrency($wo["config"]["currency"]) . " " . $wo["lang"]["per_month"];
    // }
    // if ($wo["pro_packages"]["ultima"]["status"] == 1) {
    //     $ultima_member = $wo["lang"]["ultima"] . " " . $wo["lang"]["member"] . " " . $wo["pro_packages"]["ultima"]["price"] . lui_GetCurrency($wo["config"]["currency"]) . " " . $wo["lang"]["per_year"];
    // }
    // if ($wo["pro_packages"]["vip"]["status"] == 1) {
    //     $vip_member = $wo["lang"]["vip"] . " " . $wo["lang"]["member"] . " " . $wo["pro_packages"]["vip"]["price"] . lui_GetCurrency($wo["config"]["currency"]) . " " . $wo["lang"]["life_time"];
    // }
    // if ($user["pro_type"] == 1) {
    //     $can_see = array();
    //     if ($wo["pro_packages"]["hot"]["status"] == 1) {
...
function lui_CreatePayment($payment_type = 1) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CountAllPaymentData($type) {
    global $wo, $sqlConnect;
...
function lui_CountThisMonthPayment() {
    global $wo, $sqlConnect;
    $type_table = T_PAYMENTS;
    $date       = date("n") . "/" . date("Y");
...
function lui_GetRegisteredPaymentsStatics($month, $type = "") {
    global $wo, $sqlConnect;
    $year       = date("Y");
    $type_table = T_PAYMENTS;
...
function lui_GetBoostedPosts($user_id) {
    global $wo, $sqlConnect;
    $data  = array();
    $logged_user_id = lui_Secure($wo["user"]["user_id"]);
...
function lui_GetBoostedPages($user_id) {
    global $wo, $sqlConnect;
    $data  = array();
...
function lui_RedirectSmooth($url) {
    global $wo;
...
function lui_DownUpgradeUser($user_id) {
    global $wo, $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
...
function lui_GetCurrency($currency) {
    global $wo, $sqlConnect;
    if (empty($currency)) {
        return false;
    }
...
function lui_CreateNewVideoCall($re_data) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $user_data  = lui_UserData($re_data["from_id"]);
    $user_data2 = lui_UserData($re_data["to_id"]);
...
function lui_CreateNewAgoraCall($re_data = array()) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CreateNewAudioCall($re_data) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CheckCallAnswer($id = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CheckAudioCallAnswer($id = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CheckAudioCallAnswerDeclined($id = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CheckCallAnswerDeclined($id = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CheckFroInCallsAgora() {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_CheckFroInCalls($type = "video") {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_UpdateCallsActiveToZero($user_id1 = 0, $user_id2 = 0) {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    $user_id1 = lui_Secure($user_id1);
    $user_id2 = lui_Secure($user_id2);
...
function lui_GetAllDataFromCallID($id = 0) {
    global $sqlConnect, $wo;
    $user_id = $wo["user"]["user_id"];
    if ($wo["loggedin"] == false) {
        return false;
...
function lui_GetScriptWarnings() {
    global $sqlConnect, $wo;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_RegisterNewField($registration_data) {
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    }
...
function lui_GetProfileFields($type = "all") {
    global $wo, $sqlConnect;
    $data       = array();
    $where      = "";
    $placements = array(
...
function lui_GetUserCustomFields() {
    global $wo, $sqlConnect;
    $data      = array();
...
function lui_UserFieldsData($user_id) {
    global $wo, $sqlConnect;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
    }
...
function lui_UpdateUserCustomData($user_id, $update_data, $loggedin = true) {
    global $wo, $sqlConnect, $cache;
    if ($loggedin == true) {
        if ($wo["loggedin"] == false) {
            return false;
...
function lui_GetFieldData($id = 0) {
    global $wo, $sqlConnect;
...
function lui_UpdateField($id, $update_data) {
    global $wo, $sqlConnect, $cache;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    foreach ($update_data as $field => $data) {
        $update[] = "`" . $field . '` = \'' . lui_Secure($data, 0) . '\'';
        if ($field == "length") {
            $mysqli = mysqli_query($sqlConnect, "ALTER TABLE " . T_USERS_FIELDS . " CHANGE `fid_{$id}` `fid_{$id}` VARCHAR(" . lui_Secure($data) . ") CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT '';");
        }
    }
    $impload   = implode(", ", $update);
...
function lui_DeleteProMemebership() {
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            ...
            if ($update_data == true) {
                $update      = lui_UpdateUserData($fetched_data["user_id"], array(
                    "is_pro" => 0,
                    'verified' => 0,
                    'pro_' => 1
                ));
...
function lui_GetPopularGames($limit = 10, $after = 0) {
    global $wo, $sqlConnect;
    $data = array();
    $q    = "";
    if (!empty($after) && is_numeric($after)) {
        $after = lui_Secure($after);
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $fetched_data            = lui_GameData($fetched_data["game_id"]);
            $fetched_data["players"] = lui_CountGamePlayers($fetched_data["id"]);
            $data[]                  = $fetched_data;
        }
    }
...
function lui_GetGenders($lang = "english", $langs = array()) {
    global $wo, $db;
...
function lui_SearchFor($search_qeury, $type, $offset = 0) {
    global $sqlConnect, $wo;
    $search_qeury = lui_Secure($search_qeury);
    $data         = array();
    $offset_to    = "";
    ...
    if (mysqli_num_rows($pro)) {
        while ($fetched_data = mysqli_fetch_assoc($pro)) {
            ...
            if (!empty($fetched_data["night_image"])) {
                $fetched_data["night_image"] = lui_GetMedia($fetched_data["night_image"]);
            }
...
function lui_GetSucursalesTypes($type = "page") {
    global $sqlConnect, $wo;
    $data      = array();
    $reactions = mysqli_query($sqlConnect, "SELECT * FROM lui_sucursales");
    if (!empty($reactions)) {
        while ($fetched_data = mysqli_fetch_assoc($reactions)) {
            $fetched_data["foto"] = lui_GetMedia($fetched_data["foto"]);
            $data[$fetched_data["id"]] = $fetched_data;
        }
        return $data;
    }
...
function lui_GetProveedoresTypes($type = "page") {
    global $sqlConnect, $wo;
    $data      = array();
    $reactions = mysqli_query($sqlConnect, "SELECT * FROM lui_proveedores");
    if (!empty($reactions)) {
        while ($fetched_data = mysqli_fetch_assoc($reactions)) {
...
function lui_GetProveedoresSucursal($id) {
    global $sqlConnect, $wo;
    $data      = array();
    $sucursalproveedor = mysqli_query($sqlConnect, "SELECT * FROM sucursal_proveedor WHERE `id_proveedor` = {$id}");
    if (!empty($sucursalproveedor)) {
        while ($fetched_data = mysqli_fetch_assoc($sucursalproveedor)) {
            $data[$fetched_data["id"]] = $fetched_data;
        }
        return $data;
    }
...
function lui_GetUnidadmedidaTypes($type = "page") {
    global $sqlConnect, $wo;
    $data      = array();
    $unidadmedida = mysqli_query($sqlConnect, "SELECT * FROM lui_unidad_medida");
    if (!empty($unidadmedida)) {
        while ($fetched_data = mysqli_fetch_assoc($unidadmedida)) {
            $data[$fetched_data["id"]] = $fetched_data;
        }
        return $data;
    }
...
// manage packages
function lui_GetReactionsTypes($type = "page") {
    global $sqlConnect, $wo;
    $data      = array();
    ...
    if (!empty($reactions)) {
        while ($fetched_data = mysqli_fetch_assoc($reactions)) {
            ...
            if ($type == "page") {
                if (!empty($fetched_data["layshane_star"])) {
                    $fetched_data["layshane_star"] = $fetched_data["layshane_star_small_icon"] = lui_GetMedia($fetched_data["layshane_star"]);
                    // $explode2  = @end(explode('.', $fetched_data['layshane_star']));
                    // $explode3  = @explode('.', $fetched_data['layshane_star']);
                    // $fetched_data['layshane_star_small_icon'] = $explode3[0] . '_small.' . $explode2;
                    $fetched_data["is_html"]       = 0;
                } elseif (!file_exists("./datos/modulos/" . $wo["config"]["theme"] . "/reaction/like-sm.png")) {
                    if ($fetched_data["id"] == 1) {
                        $fetched_data['layshane_star_small_icon'] = $wo['config']['theme_url']."/reaction/reactions_like.png";
                        $fetched_data["layshane_star"] = '<div class="emoji emoji--like"><div class="emoji__hand"><div class="emoji__thumb"></div></div></div>';
                    }
                    if ($fetched_data["id"] == 2) {
                        $fetched_data['layshane_star_small_icon'] = $wo['config']['theme_url']."/reaction/reactions_love.png";
                        $fetched_data["layshane_star"] = '<div class="emoji emoji--love"><div class="emoji__heart"></div></div>';
                    }
                    if ($fetched_data["id"] == 3) {
                        $fetched_data['layshane_star_small_icon'] = $wo['config']['theme_url']."/reaction/reactions_haha.png";
                        $fetched_data["layshane_star"] = '<div class="emoji emoji--haha"><div class="emoji__face"><div class="emoji__eyes"></div><div class="emoji__mouth"><div class="emoji__tongue"></div></div></div></div>';
                    }
                    if ($fetched_data["id"] == 4) {
                        $fetched_data['layshane_star_small_icon'] = $wo['config']['theme_url']."/reaction/reactions_wow.png";
                        $fetched_data["layshane_star"] = '<div class="emoji emoji--wow"><div class="emoji__face"><div class="emoji__eyebrows"></div><div class="emoji__eyes"></div><div class="emoji__mouth"></div></div></div>';
                    }
                    if ($fetched_data["id"] == 5) {
                        $fetched_data['layshane_star_small_icon'] = $wo['config']['theme_url']."/reaction/reactions_sad.png";
                        $fetched_data["layshane_star"] = '<div class="emoji emoji--sad"><div class="emoji__face"><div class="emoji__eyebrows"></div><div class="emoji__eyes"></div><div class="emoji__mouth"></div></div></div>';
                    }
                    if ($fetched_data["id"] == 6) {
                        $fetched_data['layshane_star_small_icon'] = $wo['config']['theme_url']."/reaction/reactions_angry.png";
                        $fetched_data["layshane_star"] = '<div class="emoji emoji--angry"><div class="emoji__face"><div class="emoji__eyebrows"></div><div class="emoji__eyes"></div><div class="emoji__mouth"></div></div></div>';
                    }
                ...
                if (!empty($fetched_data["layshane_star"])) {
                    ...
                    $fetched_data["layshane_star_small_icon"] = "";
                    $fetched_data["is_html"]             = 1;
                }

                // if (!empty($fetched_data['layshane_star']) && !empty($fetched_data['sunshine_icon'])) {
                //     $fetched_data['sunshine_icon'] = lui_GetMedia($fetched_data['sunshine_icon']);
                //     $fetched_data['layshane_star'] = lui_GetMedia($fetched_data['layshane_star']);
                // }
                // else{
...
function lui_GetPokeById($id) {
    ...
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            $fetched_data["user_data"] = lui_UserData($fetched_data["received_user_id"]);
            $data                      = $fetched_data;
        }
    }
    return $data;
}
function lui_GetAllColors() {
    global $sqlConnect, $wo, $db;
    $data      = array();
...
function lui_GetMemoriesPosts($user_id) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_GetMemoriesFreinds($user_id) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_AddNotifyMemories() {
    global $wo, $sqlConnect, $db;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        ...
        if ($fetched_data["count"] < 1) {
            $friends = lui_GetMemoriesFreinds($user_id);
            if (count($friends) < 1) {
                $posts = lui_GetMemoriesPosts($user_id);
                if (count($posts) < 1) {
                    $notify = false;
                } else {
                    $notify = true;
...
function lui_GetCustomFields($type = "all") {
    global $wo, $sqlConnect;
    $data       = array();
    $where      = "";
    $placements = array(
...
function lui_RegisterNewCustomField($registration_data) {
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    }
...
function lui_DeleteCustomField($id, $type) {
    global $wo, $sqlConnect;
    if ($wo["loggedin"] == false) {
        return false;
    }
...
function lui_UpdateCustomField($id, $update_data) {
    global $wo, $sqlConnect, $cache;
    if ($wo["loggedin"] == false) {
        return false;
    }
    ...
    foreach ($update_data as $field => $data) {
        $update[] = "`" . $field . '` = \'' . lui_Secure($data, 0) . '\'';
        if ($field == "length") {
            $mysqli = mysqli_query($sqlConnect, "ALTER TABLE " . $types_array[$update_data["placement"]] . " CHANGE `fid_{$id}` `fid_{$id}` VARCHAR(" . lui_Secure($data) . ") CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT '';");
        }
    }
    $impload   = implode(", ", $update);
...
function lui_CheckPaystackPayment($ref) {
    global $wo, $db;
