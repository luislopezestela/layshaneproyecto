...
function lui_LoadPage($page_url = '') {
    global $wo, $db;
    $create_file = false;
...
function lui_CleanCache($user_id = '', $where = 'sidebar') {
    global $wo;
...
function lui_CustomCode($a = false, $code = array()) {
    global $wo;
    $theme       = $wo['config']['theme'];
    $data        = array();
    $result      = false;
    $custom_code = array(
        "datos/modulos/$theme/custom/js/head.js",
        "datos/modulos/$theme/custom/js/footer.js",
        "datos/modulos/$theme/custom/css/style.css"
    );
    if ($a == 'g') {
...
function lui_MultipleArrayFiles($file_post) {
...
function lui_IsValidMimeType($mimeTypes = array()) {
...
function lui_SeoLink($query = '') {
    ...
    if ($wo['config']['seoLink'] == 1) {
        $query = preg_replace(array(
            ...
            '/^index\.php\?link1=item&items=([A-Za-z0-9_-]+)&opcion=([A-Za-z0-9_-]+)$/i',
            '/^index\.php\?link1=item&items=(.*)$/i',
            ...
            '/^index\.php\?link1=carta&c_id=([A-Za-z0-9_]+)&sub_id=([A-Za-z0-9_]+)$/i',
            ...
            $config['site_url'] . '/item/$1/$2',
            $config['site_url'] . '/item/$1',
...
function lui_IsLogged() {
    if (isset($_SESSION['user_id']) && !empty($_SESSION['user_id'])) {
        $id = lui_GetUserFromSessionID($_SESSION['user_id']);
        if (is_numeric($id) && !empty($id)) {
            return true;
        }
    } else if (!empty($_COOKIE['user_id']) && !empty($_COOKIE['user_id'])) {
        $id = lui_GetUserFromSessionID($_COOKIE['user_id']);
        if (is_numeric($id) && !empty($id)) {
            return true;
        }
    } else {
...
function lui_Redirect($url) {
    return header("Location: {$url}");
}
function lui_Link($string) {
    global $site_url;
    return $site_url . '/' . $string;
}
function lui_Sql_Result($res, $row = 0, $col = 0) {
...
function lui_BbcodeSecure($string) {
    global $sqlConnect;
    $string = trim($string);
    $string = mysqli_real_escape_string($sqlConnect, $string);
    $string = htmlspecialchars($string, ENT_QUOTES);
...
function lui_Decode($string) {
    return htmlspecialchars_decode($string);
}
function lui_GenerateKey($minlength = 20, $maxlength = 20, $uselower = true, $useupper = true, $usenumbers = true, $usespecial = false) {
    $charset = '';
    if ($uselower) {
...
$can = 0;
function lui_CropAvatarImage($file = '', $data = array()) {
    global $wo;
    if (empty($file)) {
        return false;
    }
...
function lui_Time_Elapsed_String($ptime) {
    global $wo;
    $etime = (time()) - $ptime;
    if ($etime < 1) {
        //return '0 seconds';
...
function lui_GetThemes() {
    global $wo;
    $themes = glob('datos/modulos/*', GLOB_ONLYDIR);
    return $themes;
}
function lui_ReturnBytes($val) {
    $val  = trim($val);
...
function lui_CompressImage($source_url, $destination_url, $quality) {
...
function lui_Backup($sql_db_host, $sql_db_user, $sql_db_pass, $sql_db_name, $tables = false, $backup_name = false) {
...
function lui_CheckUserSessionID($user_id = 0, $session_id = '', $platform = 'web') {
...
function lui_ValidateAccessToken($access_token = '') {
    global $wo, $sqlConnect;
    if (empty($access_token)) {
        return false;
    }
    $access_token = lui_Secure($access_token);
...
function lui_UploadToS3($filename, $config = array()) {
    global $wo;
...
function lui_DeleteFromToS3($filename, $config = array()) {
    global $wo;
...
function lui_GetIcon($icon) {
    global $wo;
    return $wo['config']['theme_url'] . '/icons/png/' . $icon . '.png';
}
function lui_IsFileAllowed($file_name, $fileType = '') {
    global $wo;
...
function lui_IsVideoNotAllowedMime($file_type) {
    global $wo;
...
function lui_IsFfmpegFileAllowed($file_name) {
    global $wo;
    $new_string        = pathinfo($file_name, PATHINFO_FILENAME) . '.' . strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
...
function lui_ShortText($text = "", $len = 100) {
...
function lui_DelexpiredEnvents() {
    global $wo, $sqlConnect;
    $t_events     = T_EVENTS;
...
function lui_RunInBackground($data = array()) {
...
function watermark_image($target) {
    global $wo;
    include_once('luisincludes/librerias/SimpleImage-master/vendor/autoload.php');
    if ($wo['config']['watermark'] != 1) {
        return false;
    }
    try {
        $image = new \claviska\SimpleImage();
        $image->fromFile($target)->autoOrient()->overlay("./datos/modulos/{$wo['config']['theme']}/img/icon.png", 'top left', 1, 30, 30)->toFile($target, 'image/jpeg');
        return true;
    }
    catch (Exception $err) {
