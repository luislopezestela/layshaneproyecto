...
function lui_GetCompra($id = 0) {
    ...
    $query_one = " SELECT * FROM " . T_COMPRAS . " WHERE `id` = '{$id}' AND `completado` = '1' ORDER BY `id` DESC";
...
function lui_RegisterPoint($post_id, $type, $action = '+', $user_id = 0) {
    global $wo, $sqlConnect, $db;
    ...
    switch ($type) {
        case "comments":
            ...
            break;
        case "likes":
            if (!lui_IsWondered($post_id, $user_id)) {
                $points = $wo['config']['likes_point'];
            }
            break;
        case "dislikes":
            if (!lui_IsLiked($post_id, $user_id)) {
...
function lui_RegisterProduct($registration_data) {
    global $wo, $sqlConnect;
...
function lui_GetProduct($id = 0) {
    ...
    $fetched_data['images']           = lui_GetProductImages($fetched_data['id']);
...
function lui_DeleteProductImage($id) {
    global $wo, $sqlConnect;
...
function lui_ProductImageData($data = array()) {
    ...
    if (!empty($data['id'])) {
        if (is_numeric($data['id'])) {
            $id = lui_Secure($data['id']);
        }
    }
    $order_by = '';
    if (!empty($data['after_image_id']) && is_numeric($data['after_image_id'])) {
        $data['after_image_id'] = lui_Secure($data['after_image_id']);
        ...
        $data['before_image_id'] = lui_Secure($data['before_image_id']);
    ...
    if (!empty($data['product_id']) && is_numeric($data['product_id'])) {
        $data['product_id'] = lui_Secure($data['product_id']);
...
function lui_MarkPostAsSold($post_id = 0, $product_id = 0) {
    ...
    if (lui_IsProductSold($product_id)) {
        return false;
    }
...
function lui_GetPostIDFromProdcutID($id) {
    global $sqlConnect;
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
    }
...
function lui_UpdateProductData($product_id, $update_data) {
    ...
    $post_id    = lui_GetPostIDFromProdcutID($product_id);
    if (empty($post_id)) {
        return false;
    }
    ...
    foreach ($update_data as $field => $data) {
        $update[] = '`' . $field . '` = \'' . lui_Secure($data, 1,true,1) . '\'';
    }
...
function LinkedinSearch($filter_data = array()) {
    ...
    if (!empty($filter_data['experience']) && is_numeric($filter_data['experience']) && $filter_data['experience'] > 0) {
        $experience = lui_Secure($filter_data['experience']);
    ...
    if ($filter_data['search_type'] == 'all' || $filter_data['search_type'] == 'users') {
        ...
        if (mysqli_num_rows($sql)) {
            while ($fetched_data = mysqli_fetch_assoc($sql)) {
                $user              = lui_UserData($fetched_data['user_id']);
    ...
    if ($filter_data['search_type'] == 'all' || $filter_data['search_type'] == 'groups') {
        ...
        if (mysqli_num_rows($sql)) {
            while ($fetched_data = mysqli_fetch_assoc($sql)) {
                $group              = lui_GroupData($fetched_data['id']);
...
function lui_GetProducts($filter_data = array()) {
    global $wo, $sqlConnect;
    $data      = array();
...
function lui_AddOption($post_id, $text) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetPostOptions($post_id = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_GetPostIDFromOptionID($id) {
    global $sqlConnect;
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
    }
...
function lui_VoteUp($option_id = 0, $user_id = 0) {
    global $sqlConnect;
    ...
    $user_id   = lui_Secure($user_id);
    $option_id = lui_Secure($option_id);
    $post_id   = lui_GetPostIDFromOptionID($option_id);
    if (empty($post_id)) {
        return false;
    }
    if (lui_IsPostVoted($post_id, $user_id)) {
        return false;
    }
    if (lui_IsOptionVoted($option_id, $user_id)) {
        return false;
    }
...
function lui_IsOptionVoted($option_id, $user_id) {
    global $wo, $sqlConnect;
...
function lui_IsPostVoted($post_id, $user_id) {
    global $wo, $sqlConnect;
...
function Ju_GetPercentageOfOptionPost($post_id) {
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $fetched_data['option_votes'] = lui_GetVotes($fetched_data['id']);
            $data[]                       = $fetched_data;
        }
    }
    foreach ($data as $key => $value) {
...
function lui_GetVotes($option_id) {
    global $wo, $sqlConnect;
...
function lui_GetCustomPages() {
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $data[] = lui_GetCustomPage($fetched_data['page_name']);
        }
    }
    return $data;
}
function lui_GetCustomPage($page_name) {
    global $sqlConnect;
    if (empty($page_name)) {
        return false;
    }
    $data          = array();
    $page_name     = lui_Secure($page_name);
...
function lui_RegisterNewPage($registration_data) {
    global $wo, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    }
...
function lui_DeleteCustomPage($id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    if (lui_IsAdmin() === false) {
        return false;
    }
    $id    = lui_Secure($id);
...
function lui_UpdateCustomPageData($id, $update_data) {
    ...
    $update = array();
    foreach ($update_data as $field => $data) {
        $update[] = '`' . $field . '` = \'' . lui_Secure($data, 0) . '\'';
    }
    $impload   = implode(', ', $update);
...
function lui_GetReferrers($user_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_UpdateBalance($user_id = 0, $balance = 0, $type = '+',$withdrawal = '') {
    global $wo, $sqlConnect;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
    }
    ...
    $user_id   = lui_Secure($user_id);
    $balance   = lui_Secure($balance);
    $c_balance   = lui_Secure($balance);
    $user_data = lui_UserData($user_id);
...
function lui_RequestNewPayment($user_id = 0, $amount = 0, $insert_array = array()) {
    ...
    $user_id = lui_Secure($user_id);
    $amount  = lui_Secure($amount);
    if (lui_IsUserPaymentRequested($user_id) === true) {
        return false;
    }
    $user_data   = lui_UserData($user_id);
    $full_amount = lui_Secure($user_data['balance']);
    $time        = time();
    $options     = array(
        "iban" => "",
        "country" => "",
...
function lui_IsUserPaymentRequested($user_id = 0) {
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
...
function lui_GetPaymentsHistory($user_id = 0) {
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $data[] = lui_GetPaymentHistory($fetched_data['id']);
        }
    }
    return $data;
}
function lui_GetAllPaymentsHistory($type = 0) {
    global $sqlConnect;
...
function lui_CountPaymentHistory($id) {
    global $sqlConnect;
    $data          = array();
    $id            = lui_Secure($id);
...
function lui_GetPaymentHistory($id) {
    global $sqlConnect, $wo;
    if (empty($id)) {
        return false;
    }
    ...
    if (mysqli_num_rows($sql_query_one)) {
        $fetched_data                 = mysqli_fetch_assoc($sql_query_one);
        $fetched_data['user']         = lui_UserData($fetched_data['user_id']);
        $fetched_data['total_refs']   = lui_CountRefs($fetched_data['user_id']);
        $fetched_data['time_text']    = lui_Time_Elapsed_String($fetched_data['time']);
...
function lui_CountRefs($user_id = 0) {
    global $sqlConnect;
    $data          = array();
    $user_id       = lui_Secure($user_id);
...
function lui_InsertBlog($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_RegisterBlogComment($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($query) {
        ...
        lui_RegisterPoint($comment_id, "blog_comment");
        return $comment_id;
    }
    return false;
}
function lui_RegisterBlogCommentReply($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetBlogCommentsCount($id) {
    global $sqlConnect, $wo;
    $is_owner = false;
...
function lui_GetBlogComments($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "id" => false,
        "offset" => 0,
    ...
    $blog_id   = lui_Secure($args['blog_id']);
    $query_one = '';
    $data      = array();
    if ($offset > 0) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $comment = lui_GetBlogCommentData($fetched_data['id']);
...
function lui_GetBlogCommLikes($id) {
    global $sqlConnect, $wo;
    $id    = lui_Secure($id);
    $likes = 0;
...
function lui_GetBlogCommReplyLikes($id) {
    global $sqlConnect, $wo;
    $id    = lui_Secure($id);
    $likes = 0;
...
function lui_GetBlogCommReplyDisLikes($id) {
    global $sqlConnect, $wo;
    $id    = lui_Secure($id);
    $likes = 0;
...
function lui_GetBlogCommDisLikes($id) {
    global $sqlConnect, $wo;
    $id    = lui_Secure($id);
    $likes = 0;
...
function lui_GetBlogCommentReplies($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "id" => false,
        "comm_id" => false
    ...
    $args      = array_merge($options, $args);
    $id        = lui_Secure($args['id']);
    $comm_id   = lui_Secure($args['comm_id']);
    $limit     = "";
    $query_one = '';
    $data      = array();
    if ($id && $id > 0 && is_numeric($id)) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $comment = lui_GetBlogCommReplyData($fetched_data['id']);
            if ($comment && !empty($comment)) {
                $data[] = $comment;
            }
        }
...
function lui_IsBlogCommentOwner($id) {
    global $sqlConnect, $wo;
    $is_owner = false;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function lui_IsBlogCommReplyOwner($id) {
    global $sqlConnect, $wo;
    $is_owner = false;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function lui_GetBlogCommentData($id) {
    global $sqlConnect, $wo;
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data'] = lui_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']  = lui_IsBlogCommentOwner($fetched_data['id']);
            $fetched_data['likes']     = lui_GetBlogCommLikes($fetched_data['id']);
            $fetched_data['dislikes']  = lui_GetBlogCommDisLikes($fetched_data['id']);
            $fetched_data['replies']   = lui_GetBlogCommentReplies(array(
...
function lui_GetBlogCommReplyData($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data'] = lui_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']  = lui_IsBlogCommReplyOwner($fetched_data['id']);
            $fetched_data['likes']     = lui_GetBlogCommReplyLikes($fetched_data['id']);
            $fetched_data['dislikes']  = lui_GetBlogCommReplyDisLikes($fetched_data['id']);
            $data                      = $fetched_data;
        }
        return $data;
    }
...
function lui_IsBlogCommentLikeExists($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_IsBlogCommentDisLikeExists($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_IsBlogCommentReplyDisLikeExists($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_IsBlogCommentReplyLikeExists($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_RemoveBlogCommentLikes($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_RemoveBlogCommentReplyLikes($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_RemoveBlogCommentReplyDisLikes($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_RemoveBlogCommentDisLikes($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_AddBlogCommentLikes($id, $blog) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$blog || !is_numeric($blog)) {
        return false;
    }
    $id      = lui_Secure($id);
    $blog    = lui_Secure($blog);
    $comment = lui_GetBlogCommentData($id);
    $result  = false;
    $user    = $wo['user']['id'];
    @lui_RemoveBlogCommentDisLikes($id);
    if ($comment && !empty($comment) && !lui_IsBlogCommentLikeExists($id)) {
    ...
    } else if ($comment && !empty($comment) && lui_RemoveBlogCommentLikes($id)) {
        $result = true;
    }
    return $result;
}
function lui_AddBlogCommReplyLikes($id, $blog) {
    global $sqlConnect, $wo;
    ...
    $id      = lui_Secure($id);
    $blog    = lui_Secure($blog);
    $comment = lui_GetBlogCommReplyData($id);
    $result  = false;
    $user    = $wo['user']['id'];
    if ($comment && !empty($comment) && !lui_IsBlogCommentReplyLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_LIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`, `movie_commreply_id`, `user_id`, `blog_id`)
        ...
        if ($query) {
            $result = true;
            @lui_RemoveBlogCommentReplyDisLikes($id);
        }
    } else if ($comment && !empty($comment) && lui_RemoveBlogCommentReplyLikes($id)) {
        $result = true;
    }
    return $result;
}
function lui_DeleteBlogComment($id, $blog = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !lui_IsBlogCommentOwner($id)) {
        return false;
    }
    $blog_id = lui_Secure($blog);
    ...
    lui_RegisterPoint($id, "blog_comment", "-");
    return mysqli_query($sqlConnect, "DELETE FROM " . T_BLOG_COMM . " WHERE `id` = '$id'");
}
function lui_DeleteBlogCommReply($id, $blog = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !lui_IsBlogCommReplyOwner($id) || !$blog || $blog < 1) {
        return false;
    }
    $blog_id = lui_Secure($blog);
...
function lui_IsBlogOwner($blog_id = 0, $user_id = 0) {
    global $sqlConnect, $wo;
    if (empty($blog_id)) {
        return false;
    }
...
function lui_UpdateBlog($id = 0, $update_data = array()) {
    global $sqlConnect, $wo;
    $update = array();
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $id = lui_Secure($id);
    if (lui_IsBlogOwner($id) === false) {
        return false;
    }
    foreach ($update_data as $field => $data) {
        $update[] = '`' . $field . '` = \'' . lui_Secure($data, 0, false) . '\'';
    }
    $impload   = implode(', ', $update);
...
function lui_GetMyBlogs($user = 0, $offset = 0) {
    global $sqlConnect, $wo;
    $data = array();
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user   = lui_Secure($user);
    $offset = lui_Secure($offset);
    $t_blog = T_BLOG;
...
function lui_GetBlogs($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "category" => false,
        "offset" => 0,
    ...
    $offset    = lui_Secure($args['offset']);
    $limit     = lui_Secure($args['limit']);
    $category  = lui_Secure($args['category']);
    $order_by  = lui_Secure($args['order_by']);
    $id        = lui_Secure($args['id']);
    $user_id   = lui_Secure($args['user_id']);
    $keyword   = lui_Secure($args['keyword']);
...
function lui_DeleteMyBlog($id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($thumbnail) {
        ...
        if ($path['thumbnail'] != 'upload/photos/d-blog.jpg') {
            if (file_exists($path['thumbnail'])) {
                ...
                @lui_DeleteFromToS3($path['thumbnail']);
            }
        }
    }
    ...
    if ($sql_query_one) {
        ...
        $delete_post   = lui_DeletePost($mysqli['id']);
    }
    return $sql_query_one;
}
function lui_GetArticle($id = 0) {
    ...
    if (mysqli_num_rows($sql_query_one)) {
        ...
        if (!empty($fetched_data)) {
            ...
            if ($wo['config']['useSeoFrindly'] == 1) {
                ...
                $fetched_data['url'] = lui_SeoLink('index.php?link1=read-blog&id=' . $fetched_data['id']);
            }
            $fetched_data['author']        = lui_UserData($fetched_data['user']);
            $fetched_data['category_link'] = lui_SeoLink('index.php?link1=blog-category&id=' . $fetched_data['category']);
...
// *** luis  Forum ***  //
function lui_GetForumSec($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "id" => false,
        "offset" => 0,
    ...
    $id        = lui_Secure($args['id']);
    $limit     = lui_Secure($args['limit']);
    $search    = lui_Secure($args['search']);
    $keyword   = lui_Secure($args['keyword']);
    $forums    = lui_Secure($args['forums']);
    $order_by  = lui_Secure($args['order_by']);
    $query_one = "";
    if ($offset > 0) {
        $query_one .= " AND `id` < {$offset} AND `id` <> {$offset} ";
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $fetched_data['section_name_lang'] = lui_ReplaceText($fetched_data['section_name']);
            $fetched_data['description_lang'] = lui_ReplaceText($fetched_data['description']);
            if ($forums) {
                $fetched_data['forums'] = lui_GetForums(array(
...
function lui_GetForums($args = array()) {
    global $sqlConnect;
    $options   = array(
        "section" => false,
        "offset" => 0,
    ...
    $args      = array_merge($options, $args);
    $offset    = lui_Secure($args['offset']);
    $section   = lui_Secure($args['section']);
    $limit     = lui_Secure($args['limit']);
    $search    = lui_Secure($args['search']);
    $keyword   = lui_Secure($args['keyword']);
    $order_by  = lui_Secure($args['order_by']);
    ...
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['name_lang'] = lui_ReplaceText($fetched_data['name']);
        $fetched_data['description_lang'] = lui_ReplaceText($fetched_data['description']);
        $fetched_data['posts'] = lui_GetTotalThreads(array(
...
function lui_GetShortTitle($text = false, $preview = false, $len = 40) {
    if (!$text) {
        return false;
    }
...
function lui_GetForumInfo($fid) {
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $fetched_data['name_lang'] = lui_ReplaceText($fetched_data['name']);
            $fetched_data['description_lang'] = lui_ReplaceText($fetched_data['description']);
            $data['threads'] = lui_GetForumThreads(array(
                "forum" => $fetched_data['id'],
                "limit" => 10
            ));
            $data['forum']   = $fetched_data;
            $data['forums']  = lui_GetForums();
        }
    }
    return $data;
}
function lui_GetForum($fid) {
    global $sqlConnect;
    if (!$fid || !is_numeric($fid)) {
        return array();
    }
    ...
    if (mysqli_num_rows($sql_query_one)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['name_lang'] = lui_ReplaceText($fetched_data['name']);
            $fetched_data['description_lang'] = lui_ReplaceText($fetched_data['description']);
            $fetched_data['name'] = lui_GetShortTitle($fetched_data['name'], true);
        }
        return $fetched_data;
    }
    return false;
}
function lui_GetForumThreads($args = array()) {
    global $sqlConnect;
    $options   = array(
        "forum" => false,
        "offset" => 0,
    ...
    $offset    = lui_Secure($args['offset']);
    $search    = lui_Secure($args['search']);
    $subject   = lui_Secure($args['subject']);
    $post      = lui_Secure($args['post']);
    $limit     = lui_Secure($args['limit']);
    $forum     = lui_Secure($args['forum']);
    $order_by  = lui_Secure($args['order_by']);
    $id        = lui_Secure($args['id']);
    $user      = lui_Secure($args['user']);
    $preview   = lui_Secure($args['preview']);
    $query_one = "";
    $ordering  = "";
    if ($offset > 0) {
        $query_one .= " AND `id` < {$offset} AND `id` <> {$offset} ";
    ...
    $data         = array();
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['user_data']        = lui_UserData($fetched_data['user']);
        $fetched_data['url']              = lui_SeoLink("index.php?link1=showthread&tid=" . $fetched_data['id']);
        $fetched_data['author_url']       = lui_SeoLink("index.php?link1=timeline&u=" . $fetched_data['user_data']['username']);
        $fetched_data['orginal_headline'] = $fetched_data['headline'];
        $fetched_data['headline']         = lui_GetShortTitle($fetched_data['headline'], $preview);
        $fetched_data['edit_url']         = lui_SeoLink('index.php?link1=edithread&tid=' . $fetched_data['id']);
        if (empty($args['threadreplies'])) {
            $fetched_data['threadreplies'] = lui_GetThreadReplies(array(
                "thread_id" => $fetched_data['id']
            ));
        }
        $fetched_data['replies'] = lui_GetTotalReplies(array(
...
function lui_GetMyReplies($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "offset" => 0,
        "limit" => 10
    ...
    $data         = array();
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['user_data']    = lui_UserData($fetched_data['poster_id']);
        $fetched_data['forum']        = lui_GetForum($fetched_data['forum_id']);
        $fetched_data['edit_url']     = lui_SeoLink("index.php?link1=editreply&tid=" . $fetched_data['id']);
        $fetched_data['url']          = lui_SeoLink("index.php?link1=showthread&tid=" . $fetched_data['thread_id']);
        $fetched_data['post_subject'] = lui_GetShortTitle($fetched_data['post_subject']);
        $data[]                       = $fetched_data;
    }
    return $data;
}
function lui_GetThreadReplies($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "thread_id" => false,
        "offset" => 0,
    ...
    $forum     = lui_Secure($args['forum']);
    $limit     = lui_Secure($args['limit']);
    $thread_id = lui_Secure($args['thread_id']);
    $limit     = lui_Secure($args['limit']);
    $order_by  = lui_Secure($args['order_by']);
    $id        = lui_Secure($args['id']);
    $poster_id = lui_Secure($args['user']);
    $query_one = "";
    if ($thread_id) {
    ...
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        ...
        $fetched_data['reply-url'] = lui_SeoLink("index.php?link1=threadquote&tid=" . $fetched_data['id']);
        $fetched_data['edit-url']  = lui_SeoLink("index.php?link1=editreply&tid=" . $fetched_data['id']);
        if ($forum) {
            $fetched_data['forum'] = lui_GetForum($fetched_data['forum_id']);
        }
        $data[] = $fetched_data;
...
function lui_SearchThreadReplies($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "thread_id" => false,
        "offset" => 0,
...
function lui_IsReplyOwner($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetForumUsers($args = array()) {
    global $wo, $sqlConnect;
    $data      = array();
    $options   = array(
        "key" => false,
    ...
    $offset    = lui_Secure($args['offset']);
    $limit     = lui_Secure($args['limit']);
    $key       = lui_Secure($args['key']);
    $name      = lui_Secure($args['name']);
    $id        = lui_Secure($args['id']);
    $query_one = "";
    if ($offset > 0) {
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $data_user                = lui_UserData($fetched_data['user_id']);
            $data_user['forum_posts'] = lui_GetTotalThreads(array(
...
function lui_IsThreadOwner($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_AddTopic($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_AddForumSection($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || lui_IsAdmin() == false) {
        return false;
    }
    $fields = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data   = '\'' . implode('\', \'', $registration_data) . '\'';
...
function lui_AddForum($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || lui_IsAdmin() == false) {
        return false;
    }
...
function lui_ThreadReply($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_BbcodeToHtml($bbtext) {
    $bbtags     = array(
        '[paragraph]' => '<p>',
        '[/paragraph]' => '</p>',
...
function lui_ThreadUpdate($id = 0, $update_data = array()) {
    global $sqlConnect, $wo;
    $update = array();
    if ($wo['loggedin'] == false) {
        return false;
...
function lui_UpdateTopic($id = 0, $update_data = array()) {
    global $sqlConnect, $wo;
    $update = array();
    if ($wo['loggedin'] == false) {
        return false;
...
function lui_DeleteThreadReply($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteForumThread($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteForum($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false && lui_IsAdmin() == false) {
        return false;
    }
...
function lui_DeleteForumSection($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false && lui_IsAdmin() == false) {
        return false;
    }
...
function lui_AddThreadView($id = false) {
    global $sqlConnect;
    if (!$id || !is_numeric($id)) {
        return false;
    }
...
function lui_UpdateThreadLastPostTime($id = false) {
    global $sqlConnect;
    if (!$id || !is_numeric($id)) {
        return false;
    }
...
function lui_GetTotalReplies($args = array()) {
    global $sqlConnect;
    $options = array(
        "thread" => false,
        "user" => false
...
function lui_NotificationWebPushNotifier() {
    ...
    if (mysqli_num_rows($query_get) > 0) {
        while ($sql_get_notification_for_push = mysqli_fetch_assoc($query_get)) {
            $get_session_data = lui_GetSessionDataFromUserID($sql_get_notification_for_push['recipient_id']);
            if (empty($get_session_data) || !empty($get_session_data)) {
                ...
                $to_data         = lui_UserData($sql_get_notification_for_push['recipient_id']);
                $ids             = array();
                if (!empty($to_data['android_n_device_id']) || !empty($to_data['ios_n_device_id']) || !empty($to_data['web_device_id'])) {
                    ...
                    $send_array                                 = array();
                    $lang                                       = $wo['lang'] = lui_LangsFromDB($to_data['language']);
                    ...
                    if (!empty($sql_get_notification_for_push['type'])) {
                        ...
                        if ($sql_get_notification_for_push['type'] == 'liked_page') {
                            $page                                       = lui_PageData($sql_get_notification_for_push['page_id']);
                        ...
                        if ($sql_get_notification_for_push['type'] == 'joined_group') {
                            $group                                      = lui_GroupData($sql_get_notification_for_push['group_id']);
                        ...
                        if ($sql_get_notification_for_push['type'] == 'interested_event') {
                            $event_data                                 = lui_EventData($sql_get_notification_for_push['event_id']);
                    ...
                    if ($wo['config']['android_push_native'] == 1 && !empty($to_data['android_n_device_id'])) {
                        ...
                        $send                                                       = lui_SendPushNotification($send_array, 'android_native');
                    }
                    if ($wo['config']['ios_push_native'] == 1 && !empty($to_data['ios_n_device_id'])) {
                        ...
                        $send_array['notification']['notification_image']           = $wo['user']['avatar'];
                        $send_array['notification']['notification_data']['user_id'] = $user_id;
                        $send                                                       = lui_SendPushNotification($send_array, 'ios_native');
                    }
                    if ($wo['config']['web_push'] == 1 && !empty($to_data['web_device_id'])) {
                        ...
                        $send_array['notification']['notification_image']           = $wo['user']['avatar'];
                        $send_array['notification']['notification_data']['user_id'] = $user_id;
                        $send                                                       = lui_SendPushNotification($send_array, 'web');
                    }
                }
...
function lui_MessagesPushNotifier() {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_get) > 0) {
        while ($sql_get_messages_for_push = mysqli_fetch_assoc($query_get)) {
            if (!in_array($sql_get_messages_for_push['to_id'], $to_ids)) {
                $get_session_data = lui_GetSessionDataFromUserID($sql_get_messages_for_push['to_id']);
                if (empty($get_session_data)) {
                    ...
                    if ($send_notify) {
                        ...
                        $to_data           = lui_UserData($sql_get_messages_for_push['to_id']);
                        ...
                        if (!empty($to_data['android_m_device_id']) && $wo['config']['android_push_messages'] != 0) {
                            ...
                            $send       = lui_SendPushNotification($send_array, 'android_messenger');
                            if ($send) {
                        ...
                        if (!empty($to_data['ios_m_device_id']) && $wo['config']['ios_push_messages'] != 0) {
                            ...
                            $send       = lui_SendPushNotification($send_array, 'ios_messenger');
                            if ($send) {
                                $query_get_messages_for_push = mysqli_query($sqlConnect, "UPDATE " . T_MESSAGES . " SET `notification_id` = '$send' WHERE `id` = '$message_id'");
                            }
                        }
...
function lui_InsertEvent($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($query) {
        ...
        $register_post = lui_RegisterPost(array(
            'user_id' => lui_Secure($wo['user']['user_id']),
            'time' => time(),
            'postPrivacy' => '0',
            'page_event_id' => $id
...
function lui_UpdateEvent($id = 0, $update_data = array()) {
    global $sqlConnect, $wo;
    $update = array();
    if ($wo['loggedin'] == false) {
        return false;
...
function lui_EventGoingExists($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_EventInterestedExists($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_EventInvitedExists($event_id, $user_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_TotalInvitedUsers($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_TotalGoingUsers($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_TotalInterestedUsers($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_AddEventGoingUsers($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (lui_EventGoingExists($event_id)) {
        return false;
    }
    $user_id    = $wo['user']['id'];
    $event_id   = lui_Secure($event_id);
    $event_data = lui_EventData($event_id);
...
function lui_AddEventInvitedUsers($event_id, $user_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (lui_EventInvitedExists($event_id, $user_id)) {
        return false;
    }
    $invited_id = lui_Secure($user_id);
...
function lui_AddEventInterestedUsers($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (lui_EventInterestedExists($event_id)) {
        return false;
    }
    $user_id    = $wo['user']['id'];
    $event_id   = lui_Secure($event_id);
...
function lui_UnsetEventInterestedUsers($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_UnsetEventGoingUsers($event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetEvents($args = array()) {
    global $sqlConnect, $wo;
    // if ($wo['loggedin'] == false) {
    //     return false;
    // }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['cover']      = lui_GetMedia($fetched_data['cover']);
            $fetched_data['url']        = lui_SeoLink("index.php?link1=show-event&eid=" . $fetched_data['id']);
            $data[]                     = $fetched_data;
        }
    }
    return $data;
}
function lui_GetSuggestedEvents($args = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetGoingEvents($offset = 0, $limit = 10) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        if ($query && !empty($query)) {
            while ($fetched_data = mysqli_fetch_assoc($query)) {
                $data[] = lui_EventData($fetched_data['event_id']);
            }
        }
    }
    return $data;
}
function lui_GetInvitedEvents($offset = 0, $limit = 10) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetInterestedEvents($offset = 0, $limit = 10) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $event = lui_EventData($fetched_data['event_id']);
...
function lui_GetInterestedEventsUsers($event_id, $offset = 0, $limit = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $event          = lui_UserData($fetched_data['user_id']);
...
function lui_GetGoingEventsUsers($event_id, $offset = 0, $limit = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetPastEvents($offset = 0, $limit = 10) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetMyEvents($offset = 0, $limit = 10) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteApp($id = false) {
    global $sqlConnect, $wo;
    $result = false;
    if ($wo['loggedin'] == false) {
        return false;
...
function lui_DeleteEvent($id = false) {
    global $sqlConnect, $wo;
    $result = false;
    if ($wo['loggedin'] == false) {
        return false;
...
function lui_EventData($id = false) {
    global $sqlConnect, $wo;
...
function lui_RegsiterEventInvite($user_id, $event_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (!Is_EventOwner($event_id, $user_id) && lui_AddEventInvitedUsers($event_id, $user_id)) {
        $notification_data_array = array(
...
function DetermineUserLang() {
    ...
    if (array_key_exists($lang, $lui_langs)) {
        $language = $lui_langs[$lang];
    }
    return $language;
}
// *** Movies ***
function lui_InsertFilm($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_UpdateFilm($id = 0, $update_data = array()) {
    global $sqlConnect, $wo;
    $update = array();
    if ($wo['loggedin'] == false) {
        return false;
...
function lui_AddBlogCommReplyDisLikes($id, $blog) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$blog || $blog < 1) {
        return false;
    }
    ...
    $result  = false;
    $user    = $wo['user']['id'];
    if ($comment && !empty($comment) && !lui_IsBlogCommentReplyDisLikeExists($id)) {
        ...
        if ($query) {
            $result = true;
            @lui_RemoveBlogCommentReplyLikes($id);
        }
    } else if ($comment && !empty($comment) && lui_RemoveBlogCommentReplyDisLikes($id)) {
        $result = true;
    }
    return $result;
}
function lui_AddBlogCommentDisLikes($id, $blog) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$blog || $blog < 1) {
        return false;
    }
    ...
    $result  = false;
    $user    = $wo['user']['id'];
    @lui_RemoveBlogCommentLikes($id);
    if ($comment && !empty($comment) && !lui_IsBlogCommentDisLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_DISLIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`,`movie_commreply_id`, `user_id`,`blog_id`)
                        VALUES (NULL, '$id', '0','0','0', '$user','$blog')";
        $query = mysqli_query($sqlConnect, $sql);
    ...
    } else if ($comment && !empty($comment) && lui_RemoveBlogCommentDisLikes($id)) {
        $result = true;
    }
    return $result;
}
function lui_GetMovies($args = array()) {
    global $sqlConnect, $wo;
    // if ($wo['loggedin'] == false) {
    //     return false;
    // }
    ...
    $offset  = $args['offset'];
    $limit   = $args['limit'];
    $genre   = lui_Secure($args['genre']);
    $id      = lui_Secure($args['id']);
    $country = lui_Secure($args['country']);
    $sub_sql = "";
    $total   = "";
    if ($offset && $offset > 0) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['cover']  = lui_GetMedia($fetched_data['cover']);
            $fetched_data['source'] = lui_GetMedia($fetched_data['source']);
            $fetched_data['url']    = lui_SeoLink("index.php?link1=watch-film&film-id=" . $fetched_data['id']) . '_' . lui_SlugPost($fetched_data['name']);
            $data[]                 = $fetched_data;
        }
    }
    return $data;
}
function lui_GetRecommendedFilms() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetMtwFilms($limit = 26) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_SearchFilms($key) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['url']   = lui_SeoLink("index.php?link1=watch-film&film-id=" . $fetched_data['id']) . '_' . lui_SlugPost($fetched_data['name']);
            $fetched_data['name']  = lui_GetShortTitle($fetched_data['name']);
            $fetched_data['cover'] = lui_GetMedia($fetched_data['cover']);
            $data[]                = $fetched_data;
        }
    }
...
function lui_DeleteFilm($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($id) || ($wo['user']['admin'] != 1 && lui_IsModerator() == false)) {
        return false;
    }
    $id  = lui_Secure($id);
...
function lui_RegisterMovieComment($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_RegisterMovieCommentReply($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetMovieCommentsCount($id) {
    global $sqlConnect, $wo;
    $is_owner = false;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function lui_GetMovieComments($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "id" => false,
        "offset" => 0,
    ...
    $offset    = lui_Secure($args['offset']);
    $id        = lui_Secure($args['id']);
    $movie_id  = lui_Secure($args['movie_id']);
    $query_one = '';
    $data      = array();
    if ($offset > 0) {
        $query_one .= " AND `id` < {$offset} AND `id` <> {$offset} ";
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $comment = lui_GetMovieCommentData($fetched_data['id']);
            if ($comment && !empty($comment)) {
                $data[] = $comment;
            }
        }
...
function lui_GetMovieCommentReplies($args = array()) {
    global $sqlConnect, $wo;
    $options   = array(
        "id" => false,
        "movie_id" => false,
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $comment = lui_GetMovieCommReplyData($fetched_data['id']);
            if ($comment && !empty($comment)) {
                $data[] = $comment;
            }
        }
...
function lui_IsMovieCommentOwner($id) {
    global $sqlConnect, $wo;
    $is_owner = false;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function lui_IsMovieCommReplyOwner($id) {
    global $sqlConnect, $wo;
    $is_owner = false;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function lui_GetMovieCommentData($id) {
    global $sqlConnect, $wo;
    if (!$id || !is_numeric($id) || $id < 1) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data'] = lui_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']  = lui_IsMovieCommentOwner($fetched_data['id']);
            $fetched_data['likes']     = lui_GetMovieCommLikes($fetched_data['id']);
            $fetched_data['dislikes']  = lui_GetMovieCommDisLikes($fetched_data['id']);
            $fetched_data['replies']   = lui_GetMovieCommentReplies(array(
                'comm_id' => $fetched_data['id']
            ));
            $data                      = $fetched_data;
        }
...
function lui_GetMovieCommReplyData($id) {
    global $sqlConnect, $wo;
    if (!$id || !is_numeric($id) || $id < 1) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data'] = lui_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']  = lui_IsMovieCommReplyOwner($fetched_data['id']);
            $fetched_data['likes']     = lui_GetMovieCommReplyLikes($fetched_data['id']);
            $fetched_data['dislikes']  = lui_GetMovieCommReplyDisLikes($fetched_data['id']);
            $data                      = $fetched_data;
        }
        return $data;
    }
...
function lui_GetMovieCommLikes($id) {
    global $sqlConnect, $wo;
    $id    = lui_Secure($id);
    $likes = 0;
...
function lui_GetMovieCommDisLikes($id) {
    global $sqlConnect, $wo;
    $id    = lui_Secure($id);
    $likes = 0;
...
function lui_GetMovieCommReplyDisLikes($id) {
    global $sqlConnect, $wo;
    $id    = lui_Secure($id);
    $likes = 0;
...
function lui_GetMovieCommReplyLikes($id) {
    global $sqlConnect, $wo;
    $id    = lui_Secure($id);
    $likes = 0;
...
function lui_IsMovieCommentLikeExists($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_IsMovieCommentDisLikeExists($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_IsMovieCommentReplyLikeExists($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_IsMovieCommentReplyDisLikeExists($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_RemoveMovieCommentLikes($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_RemoveMovieCommentDisLikes($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_RemoveMovieCommentReplyDisLikes($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_RemoveMovieCommentReplyLikes($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_AddMovieCommentLikes($id, $movie) {
    global $sqlConnect, $wo;
    ...
    $user    = $wo['user']['id'];
    $id      = lui_Secure($id);
    $movie   = lui_Secure($movie);
    $comment = lui_GetMovieCommentData($id);
    $result  = false;
    @lui_RemoveMovieCommentDisLikes($id);
    if ($comment && !empty($comment) && !lui_IsMovieCommentLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_LIKES . "
    ...
    } else if ($comment && !empty($comment) && lui_RemoveMovieCommentLikes($id)) {
        $result = true;
    }
    return $result;
}
function lui_AddMovieCommentDisLikes($id, $movie) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$movie || $movie < 1) {
        return false;
    }
    ...
    $result  = false;
    $user    = $wo['user']['id'];
    @lui_RemoveMovieCommentLikes($id);
    if ($comment && !empty($comment) && !lui_IsMovieCommentDisLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_DISLIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`,`movie_commreply_id`, `user_id`,`movie_id`)
                        VALUES (NULL, '0', '0','$id','0', '$user','$movie')";
        $query = mysqli_query($sqlConnect, $sql);
    ...
    } else if ($comment && !empty($comment) && lui_RemoveMovieCommentDisLikes($id)) {
        $result = true;
    }
    return $result;
}
function lui_AddMovieCommReplyLikes($id, $movie) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$movie || $movie < 1) {
        return false;
    }
    $id      = lui_Secure($id);
    $movie   = lui_Secure($movie);
    $comment = lui_GetMovieCommReplyData($id);
    $result  = false;
    $user    = $wo['user']['id'];
    if ($comment && !empty($comment) && !lui_IsMovieCommentReplyLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_LIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`, `movie_commreply_id`, `user_id`,`movie_id`)
        ...
        if ($query) {
            $result = true;
            @lui_RemoveMovieCommentReplyDisLikes($id);
        }
    } else if ($comment && !empty($comment) && lui_RemoveMovieCommentReplyLikes($id)) {
        $result = true;
    }
    return $result;
}
function lui_AddMovieCommReplyDisLikes($id, $movie) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$movie || $movie < 1) {
        return false;
    }
    ...
    $result  = false;
    $user    = $wo['user']['id'];
    if ($comment && !empty($comment) && !lui_IsMovieCommentReplyDisLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_DISLIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`,`movie_commreply_id`, `user_id`,`movie_id`)
                        VALUES (NULL, '0', '0','0','$id', '$user','$movie')";
        $query = mysqli_query($sqlConnect, $sql);
        if ($query) {
            $result = true;
            @lui_RemoveMovieCommentReplyLikes($id);
        }
    } else if ($comment && !empty($comment) && lui_RemoveMovieCommentReplyDisLikes($id)) {
        $result = true;
    }
    return $result;
}
function lui_GetSideBarAds() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['ad_media']    = lui_GetMedia($fetched_data['ad_media']);
            $fetched_data['headline']    = lui_GetShortTitle($fetched_data['headline'], false, 30);
            $fetched_data['description'] = lui_GetShortTitle($fetched_data['description'], false, 60);
            if ($fetched_data['bidding'] == 'views') {
                @lui_RegisterAdConversionView($fetched_data['id']);
            }
            $data[] = $fetched_data;
        }
    }
...
function lui_DeleteMovieComment($id, $movie) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !lui_IsMovieCommentOwner($id) || !$movie || $movie < 1) {
        return false;
    }
    $movie = lui_Secure($movie);
...
function lui_DeleteMovieCommReply($id, $movie) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !lui_IsMovieCommReplyOwner($id) || !$movie || $movie < 1) {
        return false;
    }
...
function lui_UpdateUserAds($id, $update_data = array()) {
    global $sqlConnect, $wo;
...
function lui_IsAdsOwnerNotAdmin($ads_id = 0, $user_id = 0) {
    global $sqlConnect, $wo;
    if (empty($user_id)) {
        $user_id = $wo['user']['user_id'];
    }
    $user_id = lui_Secure($user_id);
    $ads_id  = lui_Secure($ads_id);
    $result  = false;
...
function lui_IsAdsOwner($ads_id = 0, $user_id = 0) {
    global $sqlConnect, $wo;
    if (empty($user_id)) {
        $user_id = $wo['user']['user_id'];
    }
    ...
    if (mysqli_num_rows($query)) {
        if (!empty($query)) {
            $fetched_data = mysqli_fetch_assoc($query);
            if ($fetched_data['user_id'] == $wo['user']['user_id'] || $wo['user']['admin'] == 1 || lui_IsModerator() == true) {
                $result = true;
            }
        }
    }
...
function lui_DeleteUserAd($id = false) {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false || !$id || !lui_IsAdsOwner($id)) {
        return false;
    }
    $ad = $db->where('id', $id)->getOne(T_USER_ADS);
    if (!empty($ad) && !empty($ad->ad_media)) {
        @unlink($ad->ad_media);
        lui_DeleteFromToS3($ad->ad_media);
    }
    $query_one = "DELETE FROM " . T_USER_ADS . "  WHERE `id` = {$id} ";
    $query     = mysqli_query($sqlConnect, $query_one);
    return $query;
}
function lui_GetMyAds($args = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $ad = lui_GetUserAdData($fetched_data['id']);
            if ($ad && !empty($ad)) {
                $ad['name']      = lui_GetShortTitle($ad['name']);
                $ad['edit-url']  = lui_SeoLink('index.php?link1=edit-ads&id=' . $ad['id']);
                $ad['chart-url'] = lui_SeoLink('index.php?link1=chart-ads&id=' . $ad['id']);
                $data[]          = $ad;
            }
        }
    }
...
function lui_GetMytransactions($args = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetUserAdData($id) {
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data']   = lui_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']    = lui_IsAdsOwner($fetched_data['id']);
...
function lui_GetPostAds($last_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetAdsByType($type = 'post', $last_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetAds($last_id = 0) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['is_owner']  = lui_IsAdsOwner($fetched_data['id']);
            $fetched_data['ad_media']  = lui_GetMedia($fetched_data['ad_media']);
            $fetched_data['edit-url']  = lui_SeoLink('index.php?link1=manage-ads&id=' . $fetched_data['id']);
            $data[]                    = $fetched_data;
        }
    }
    return $data;
}
function lui_GetAllStories($last_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_IsConversionExists($id) {
    global $wo;
    $adv_ids  = $wo['ad-con'];
    $result   = false;
    $is_admin = lui_IsAdsOwnerNotAdmin($id);
...
function lui_RegisterAdConversionClick($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id) {
        return false;
    }
    $ad       = lui_GetUserAdData($id);
    $user     = $wo['user']['user_id'];
    $result   = false;
    $is_admin = lui_IsAdsOwnerNotAdmin($id);
    if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data']) && !lui_IsConversionExists($id) && $is_admin == false) {
        ...
        lui_RegisterAdClick($id);
    } else if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data']) && $is_admin == false) {
        $result = lui_RegisterAdClick($id);
    } else {
        return true;
    }
    return $result;
}
function lui_RegisterAdConversionView($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id) {
        return false;
    }
    ...
    if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data']) && !lui_IsConversionExists($id) && $is_admin == false) {
        ...
        $wallet = $ad['user_data']['wallet'] -= $price;
        $result = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `wallet` = '$wallet' WHERE `user_id` = '$ad_user_id'");
        lui_RegisterAdView($id);
    } else if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data']) && $is_admin == false) {
        $result = lui_RegisterAdView($id);
    } else {
        return true;
    }
    return $result;
}
function lui_RegisterAdClick($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id) {
        return false;
    }
    ...
    if ($ad && is_array($ad) && !empty($ad)) {
        ...
        $result  = lui_UpdateUserAds($id, array(
            'clicks' => ($ad['clicks'] + 1)
        ));
    }
    return $result;
}
function lui_RegisterAdView($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id) {
        return false;
    }
...
function lui_IsStoryOwner($id) {
    ...
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        if ($fetched_data['user_id'] == $user || lui_IsAdmin() || lui_IsModerator()) {
            $result = true;
        }
    }
    return $result;
}
function lui_InsertUserStory($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_InsertUserStoryMedia($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetStroies($args = array()) {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $story_images = lui_GetStoryMedia($fetched_data['id'], 'image');
            if (count($story_images) > 0) {
                $fetched_data['thumb']  = array_shift($story_images);
                $fetched_data['images'] = $story_images;
            }
            $fetched_data['user_data'] = lui_UserData($fetched_data['user_id']);
            $fetched_data['videos']    = lui_GetStoryMedia($fetched_data['id'], 'video');
            $fetched_data['is_owner']  = ($fetched_data['user_id'] == $wo['user']['id'] || lui_IsAdmin() || lui_IsModerator()) ? true : false;
            $fetched_data['reaction']  = lui_GetPostReactionsTypes($fetched_data['id'], 'story');
...
function lui_GetStoryMedia($id = false, $type = 'image') {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['filename'] = lui_GetMedia($fetched_data['filename']);
            $data[]                   = $fetched_data;
        }
    }
    return $data;
}
function lui_CountUserStatus($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteStatus($id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !lui_IsStoryOwner($id)) {
        return false;
    }
    $id         = lui_Secure($id);
    $data       = array();
...
function lui_UpdateCommentReply($id, $update_data = array()) {
    global $sqlConnect, $wo;
...
function lui_GetUsersByName($name = '', $friends = false, $limit = 25) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$name) {
        return false;
    }
    $user        = $wo['user']['id'];
    $name        = lui_Secure($name);
    $data        = array();
    $sub_sql     = "";
    $t_users     = T_USERS;
...
function lui_RegisterAdminNotification($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !lui_IsAdmin() || empty($registration_data) || empty($registration_data['text'])) {
        return false;
    }
...
function lui_HidePost($id = false) {
    global $sqlConnect, $wo;
...
function lui_SendVerificationRequest($registration_data = array()) {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false || !is_array($registration_data) || empty($registration_data) || lui_IsAdmin()) {
        return false;
    }
    $notification_data_array = array(
        'recipient_id' => 0,
...
function lui_UpdateVerificationRequest($id = false, $update_data = array()) {
    global $sqlConnect, $wo;
...
function lui_IsVerificationRequestExists() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_IsThisPostShared($id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_IsPostShared($id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_IsSharedPostExists($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function lui_SharePost($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || $id < 1) {
        return false;
    }
    ...
    $time = time();
    $sql  = '';
    if (lui_IsPostShared($id)) {
        $shared_post = lui_PostData($id);
    ...
    if ($post) {
        ...
        $post_data['post_url']    = lui_SeoLink('index.php?link1=post&id=' . $id);
...
function lui_GenirateSiteMap($updating = 'daily') {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !lui_IsAdmin()) {
        return false;
    }
...
function lui_GetAdminInvitation() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !lui_IsAdmin()) {
        return false;
    }
...
function lui_InsertAdminInvitation() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !lui_IsAdmin()) {
        return false;
    }
...
function lui_DeleteAdminInvitation($col = '', $val = false) {
    global $sqlConnect, $wo;
    if (!$val && !$col) {
        return false;
    }
...
function lui_IsAdminInvitationExists($code = false) {
    global $sqlConnect, $wo;
    if (!$code) {
        return false;
    }
...
function lui_FriendPrivacy($user_id = false, $friend_privacy = false) {
    global $sqlConnect, $wo;
    if (!$user_id) {
        return false;
    }
    if ($wo['loggedin'] == true) {
        ...
        $user_id        = lui_Secure($user_id);
        $friend_privacy = lui_Secure($friend_privacy);
        $privacy        = false;
        if ($friend_privacy == 0) {
            return true;
...
function lui_IsGroupUserExists($user_id = false, $group_id = false) {
    global $sqlConnect, $wo;
...
function lui_IsPageAdminExists($user_id = false, $page_id = false) {
    global $sqlConnect, $wo;
...
function lui_AddGroupAdmin($user_id = false, $group_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($user_id) || !is_numeric($group_id)) {
        return false;
    }
    $user_id  = lui_Secure($user_id);
    $group_id = lui_Secure($group_id);
    $code     = false;
    $group    = lui_GroupData($group_id);
    if ($wo['user']['id'] != $group['user_id'] && !lui_IsGroupUserExists($wo['user']['id'], $group_id)) {
        return false;
    }
    if (lui_IsGroupUserExists($user_id, $group_id)) {
...
function lui_CheckGroupAdminPassword($password = false, $group_id = false) {
    global $sqlConnect, $wo;
    ...
    if (lui_IsGroupUserExists($user_id, $group_id)) {
        ...
        if (mysqli_num_rows($data) == 1) {
            $fetched_data = mysqli_fetch_assoc($data);
            if (lui_HashPassword($password, $fetched_data['password'])) {
                $match = true;
            }
        }
    }
...
function lui_CheckPageAdminPassword($password = false, $page_id = false) {
    global $sqlConnect, $wo;
    ...
    $match   = false;
    if (lui_IsPageAdminExists($user_id, $page_id)) {
        $sql  = "SELECT `password` FROM " . T_USERS . " WHERE `user_id` = {$user_id}";
        $data = mysqli_query($sqlConnect, $sql);
        if (mysqli_num_rows($data) == 1) {
            $fetched_data = mysqli_fetch_assoc($data);
...
function lui_AddPageAdmin($user_id = false, $page_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($user_id) || !is_numeric($page_id)) {
        return false;
    }
    ...
    if ($wo['user']['id'] != $page['user_id'] && !lui_IsPageAdminExists($wo['user']['id'], $page_id) && !lui_IsAdmin() && !lui_IsModerator()) {
        return false;
    }
    if (lui_IsPageAdminExists($user_id, $page_id)) {
...
function lui_GetPageAdmins($page_id = false, $return = 'all') {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($page_id)) {
        return false;
    }
...
function lui_GetPageAdminInfo($user_id, $page_id) {
    global $sqlConnect, $wo;
...
function lui_CreateGChat($name = false, $parts = array()) {
    global $sqlConnect, $wo;
...
function lui_UpdateGChat($id = false, $update_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_array($update_data) || empty($update_data) || !$id) {
        return false;
    }
    $id = lui_Secure($id);
    if (!lui_IsGChatOwner($id)) {
       return false;
    }
    foreach ($update_data as $field => $data) {
        $update[] = '`' . $field . '` = \'' . $data . '\'';
...
function lui_GroupTabData($id = false, $update_seen = true) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id)) {
        return false;
    }
    ...
    $query = mysqli_query($sqlConnect, $sql);
    if ($update_seen == true) {
        @lui_UpdateGChatLastSeen($id);
    }
    if ($query && mysqli_num_rows($query) > 0) {
        $data             = mysqli_fetch_assoc($query);
        $data['avatar']   = lui_GetMedia($data['avatar']);
        $data['messages'] = lui_GetGroupMessages(array(
            'group_id' => $data['group_id']
        ));
    }
    return $data;
}
function lui_UpdateGChatLastSeen($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id)) {
        return false;
    }
...
function lui_IsGChatOwner($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id)) {
        return false;
    }
    $group = lui_GroupTabData($id);
    $owner = false;
    if (is_array($group) && isset($group['user_id'])) {
        $group_admin = $group['user_id'];
        $user_id     = $wo['user']['id'];
        $owner       = ($group_admin == $user_id || lui_IsAdmin()) ? true : false;
    }
    return $owner;
}
function lui_GetChatGroupLastMessage($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !is_numeric($id)) {
        return false;
    }
...
function lui_GetChatGroups($after_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $sql   = "SELECT * FROM " . T_GROUP_CHAT . "
                WHERE (`user_id` = {$user} OR `group_id` IN
                   (SELECT `group_id` FROM lui_groupchatusers  WHERE `user_id` = {$user} AND active = 1)) {$sub_sql} ORDER BY `time` DESC";
    $query = mysqli_query($sqlConnect, $sql);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data']    = lui_UserData($fetched_data['user_id']);
...
// group data
function lui_GetChatGroupData($id = false) {
    global $sqlConnect, $wo;
    ...
    $sql   = "SELECT * FROM " . T_GROUP_CHAT . "
                WHERE (`user_id` = {$user} OR `group_id` IN
                   (SELECT `group_id` FROM lui_groupchatusers  WHERE `user_id` = {$user})) AND `group_id` = {$id}";
    $query = mysqli_query($sqlConnect, $sql);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data']    = lui_UserData($fetched_data['user_id']);
...
// group data
function lui_CheckLastGroupAction() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $last_message = lui_GetChatGroupLastMessage($fetched_data['group_id']);
...
function lui_CheckLastGroupUnread() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetGChatMemebers($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_IsGChatMemebers($group_id) {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false || empty($group_id)) {
        return false;
    }
    $id      = lui_Secure($group_id);
    $user_id = $wo['user']['id'];
...
function lui_CountGroupChatRequests() {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_ClearGChat($group_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !lui_IsGChatOwner($group_id)) {
        return false;
    }
    return mysqli_query($sqlConnect, "DELETE FROM " . T_MESSAGES . " WHERE `group_id` = {$group_id}");
}
function lui_DeleteGChat($group_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !lui_IsGChatOwner($group_id)) {
        return false;
    }
...
function lui_ExitGChat($group_id = false) {
    global $sqlConnect, $wo;
...
function lui_IsGChatMemeberExists($group_id = false, $user_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$group_id || $group_id < 1) {
        return false;
    }
    $user  = lui_Secure($user_id);
    $group = lui_Secure($group_id);
    if ($user_id === false) {
        $user = $wo['user']['user_id'];
    }
...
function lui_AddGChatPart($group_id = false, $user_id = false) {
    global $sqlConnect, $wo;
    ...
    $user  = lui_Secure($user_id);
    $code  = 0;
    if (!lui_IsGChatOwner($group)) {
        return false;
    }
    if (lui_IsGChatMemeberExists($group, $user)) {
...
function lui_ReportUser($user = false, $text = '', $reason = '') {
    global $sqlConnect, $wo, $db;
    ...
    $time    = time();
    $code    = null;
    $reason = lui_Secure($reason);
    if (!lui_IsReportExists($user, 'user')) {
...
function lui_ReportPage($page = false, $text = '') {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false || !$page) {
        return false;
    }
    $user      = $wo['user']['id'];
    $page_data = lui_PageData($page);
    ...
    $time    = time();
    $code    = null;
    if (!lui_IsReportExists($page, 'page')) {
...
function lui_ReportGroup($group = false, $text = '') {
    global $sqlConnect, $wo, $db;
    if ($wo['loggedin'] == false || !$group) {
        return false;
    }
    $user       = $wo['user']['id'];
    $group_data = lui_GroupData($group);
    ...
    $time    = time();
    $code    = null;
    if (!lui_IsReportExists($group, 'group')) {
...
function lui_IsPageRatingExists($page_id = false, $user_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($user_id) || !is_numeric($page_id)) {
        return false;
    }
...
function lui_RatePage($page_id = false, $value = false, $text = '') {
    global $sqlConnect, $wo;
...
function lui_GetPageReviews($page_id = false, $after_id = false, $limit = 10) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$page_id || !is_numeric($page_id)) {
        return false;
    }
...
function lui_RegisterFamilyMember($registration_data = array()) {
    global $sqlConnect, $wo;
...
function lui_DeleteFamilyMember($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id) {
        return false;
    }
...
function lui_AcceptFamilyMember($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id) {
        return false;
    }
...
function lui_AcceptRelationRequest($id = false, $member = fasle, $type = false) {
    global $sqlConnect, $wo;
...
function lui_DeleteMyRelationShip() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteRelationRequest($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id) {
        return false;
    }
...
function lui_IsRelationRequestExists($from_id = false, $to_id = false, $type = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$from_id || !$to_id || !$type) {
        return false;
    }
    $to_id   = lui_Secure($to_id);
    $from_id = lui_Secure($from_id);
...
function lui_IsFamilyMemberExists($user_id = false, $member_id = false, $active = false) {
    global $sqlConnect, $wo;
...
function lui_GetFamalyMember($member_id = false, $user_id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_numeric($user_id) || !is_numeric($member_id)) {
        return false;
    }
...
function lui_GetFamaly($user_id = false, $after_id = false, $active = 1, $requests = false) {
    global $sqlConnect, $wo;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['user_data'] = lui_UserData(($user_data));
...
function lui_RegisterRelationship($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !is_array($registration_data) || empty($registration_data)) {
        return false;
    }
...
function lui_UserRelationship($user = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        $with_data    = lui_UserData($with_id);
...
function lui_GetUserCountryName($user_data = array()) {
    global $wo;
...
function lui_GetNearbyUsers($args = array()) {
    global $wo, $sqlConnect;
    ...
    $args         = array_merge($options, $args);
    $offset       = lui_Secure($args['offset']);
    $gender       = lui_Secure($args['gender']);
    $name         = lui_Secure($args['name']);
    $loc_distance = lui_Secure($args['distance']);
    $status       = lui_Secure($args['status']);
    $relship      = lui_Secure($args['relship']);
    $limit        = lui_Secure($args['limit']);
    $unit         = 6371;
    $user_lat     = $wo['user']['lat'];
    $user_lng     = $wo['user']['lng'];
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data']        = lui_UserData($fetched_data['user_id']);
            $fetched_data['user_data']['age'] = lui_GetUserCountryName($fetched_data['user_data']);
...
function lui_GetNearbyUsersCount($args = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($args)) {
        return false;
    }
...
function lui_CountStories($user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetFriendsStatus($data_array = array('limit' => 8, 'user_id' => 0, 'offset' => 0)) {
    global $wo, $sqlConnect,$db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $data     = array();
    $user_id  = lui_Secure($data_array['user_id']);
    ...
    if (!empty($data_array['offset'])) {
        $offset       = lui_Secure($data_array['offset']);
    ...
    while ($fetched_data = mysqli_fetch_assoc($query_run)) {
        ...
        $fetched_data['thumb']['filename'] = lui_GetMedia($fetched_data['thumb']['filename']);
        $fetched_data['videos']            = lui_GetStoryMedia($fetched_data['id'], 'video');
        $fetched_data['is_owner']          = ($fetched_data['user_id'] == $wo['user']['id'] || lui_IsAdmin() || lui_IsModerator()) ? true : false;
        $data[]                            = $fetched_data;
    }
...
function lui_GetFriendsStatusAPI($data_array = array('limit' => 8, 'user_id' => 0, 'offset' => 0)) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_HashPassword($password = '', $hashed_password = '') {
    global $wo, $sqlConnect;
    if (empty($password)) {
        return '';
    }
...
function lui_UpdateUserDetails($user_id = 0, $me = false, $time = true, $get_data = false, $counts = false) {
    global $wo, $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
    ...
    if ($cache == true) {
        $final_data = array(
            'post_count' => lui_CountUserPosts($wo['user_profile']['user_id']),
            'album_count' => lui_CountUserAlbums($wo['user_profile']['user_id']),
            'following_count' => lui_CountFollowing($wo['user_profile']['user_id']),
            'followers_count' => lui_CountFollowers($wo['user_profile']['user_id']),
            'groups_count' => lui_CountUserGroups($wo['user_profile']['user_id']),
            'likes_count' => lui_CountUserLikes($wo['user_profile']['user_id']),
            'mutual_friends_count' => lui_CountMutualFriends($wo['user_profile']['user_id'])
        );
        if ($counts == false) {
            $get_following_ids = lui_GetFollowing($wo['user_profile']['user_id'], 'profile', 9);
            $following_ids     = array();
            foreach ($get_following_ids as $key => $user) {
                $following_ids[] = $user['user_id'];
            }
            $get_followers_ids = lui_GetFollowers($wo['user_profile']['user_id'], 'profile', 9);
            $followers_ids     = array();
            foreach ($get_followers_ids as $key => $user) {
                $followers_ids[] = $user['user_id'];
            }
            $get_mutual_ids = lui_GetMutualFriends($wo['user_profile']['user_id'], 'profile', 9);
            $mutual_ids     = array();
            ...
            $get_likes_ids = lui_GetLikes($wo['user_profile']['user_id'], 'profile', 9);
            $likes_ids     = array();
            foreach ($get_likes_ids as $key => $page) {
                $likes_ids[] = $page['page_id'];
            }
            $get_groups_ids = lui_GetUsersGroups($wo['user_profile']['user_id'], 9);
            $groups_ids     = array();
        ...
        if ($query && $get_data == true) {
            return lui_UserData($wo['user_profile']['user_id']);
        }
    }
}
function lui_DeleteAllData($type = 0) {
    global $wo, $sqlConnect;
    if (empty($type) || lui_IsAdmin() == false) {
        return false;
    }
    $me = $wo['user']['user_id'];
    if ($type == 1) {
        ...
        if (mysqli_num_rows($query)) {
            while ($row = mysqli_fetch_assoc($query)) {
                $delete_user = lui_DeleteUser($row['user_id']);
            }
        }
    }
    if ($type == 2) {
    ...
    if ($type == 5) {
        ...
        if (mysqli_num_rows($query)) {
            while ($row = mysqli_fetch_assoc($query)) {
                $delete_post = lui_DeletePost($row['id']);
            }
        }
    }
    if ($type == 6) {
...
function lui_AutoFollow($user_id = 0) {
    global $wo, $db;
    if (empty($user_id)) {
        return false;
    }
    ...
    if (!empty($get_users)) {
        foreach ($get_users as $key => $user) {
            $user      = trim($user);
            $user      = lui_Secure($user);
            $getUserID = lui_UserIdFromUsername($user);
            if (!empty($getUserID)) {
                if ($wo['config']['connectivitySystem'] == 1) {
                    $registerFollow = lui_RegisterFollow($wo['user']['user_id'], $getUserID);
                    $registerFollow = lui_AcceptFollowRequest($getUserID, $wo['user']['user_id']);
                } else {
                    $registerFollow = lui_RegisterFollow($getUserID, $wo['user']['user_id']);
                }
            }
        }
        return true;
...
function lui_AutoPageLike($user_id = 0) {
    global $wo, $db;
    if (empty($user_id)) {
        return false;
    }
    ...
    if (!empty($pages_name)) {
        foreach ($pages_name as $key => $page_name) {
            ...
            $page_name = lui_Secure($page_name);
            $page_id   = lui_PageIdFromPagename($page_name);
            lui_RegisterPageLike($page_id, $wo['user']['user_id']);
        }
        return true;
    } else {
        return false;
...
function lui_AutoGroupJoin($user_id = 0) {
    global $wo, $db;
    if (empty($user_id)) {
        return false;
    }
    ...
    if (!empty($groups_name)) {
        foreach ($groups_name as $key => $group_name) {
            ...
            $group_name = lui_Secure($group_name);
            $group_id   = lui_GroupIdFromGroupname($group_name);
            lui_RegisterGroupJoin($group_id, $wo['user']['user_id']);
        }
        return true;
    } else {
        return false;
...
function lui_MarkAllChatsAsRead($user_id = 0) {
    global $wo, $db;
    if (lui_IsAdmin() === false) {
...
function lui_TwoFactor($username = '', $id_or_u = 'user') {
    global $wo, $db;
    if (empty($username)) {
        return true;
    }
    ...
    if ($id_or_u == 'id') {
        $getuser = lui_UserData($username);
    } else {
        $getuser = lui_UserData(lui_UserIdForLogin($username));
    }
    ...
    if (!empty($getuser['phone_number']) && ($wo['config']['two_factor_type'] == 'both' || $wo['config']['two_factor_type'] == 'phone')) {
        $send_message = lui_SendSMSMessage($getuser['phone_number'], $message);
    }
...
function lui_VerfiyIP($username = '') {
    global $wo, $db;
    if (empty($username)) {
        return false;
    }
    ...
    if ($getIpInfo['status'] == 'success' && !empty($getIpInfo['regionName']) && !empty($getIpInfo['countryCode']) && !empty($getIpInfo['timezone']) && !empty($getIpInfo['city'])) {
        ...
        if (empty($getuser['last_login_data'])) {
            ...
            if (($getIpInfo['regionName'] != $lastLoginData['regionName']) || ($getIpInfo['countryCode'] != $lastLoginData['countryCode']) || ($getIpInfo['timezone'] != $lastLoginData['timezone']) || ($getIpInfo['city'] != $lastLoginData['city'])) {
                ...
                $email_body                 = lui_LoadPage("emails/unusual-login");
                $send_message_data          = array(
                    'from_email' => $wo['config']['siteEmail'],
                    'from_name' => $wo['config']['siteName'],
                    'to_email' => $getuser['email'],
...
function lui_GetAllStatus() {
    global $wo, $db;
    $user_id = $wo['user']['user_id'];
    return $db->rawQuery("SELECT DISTINCT user_id,title,description,posted,expire,thumbnail,(SELECT MAX(us.id) FROM " . T_USER_STORY . " us WHERE us.user_id = " . T_USER_STORY . ".user_id) AS id  FROM " . T_USER_STORY . " WHERE (user_id IN (SELECT following_id FROM " . T_FOLLOWERS . " WHERE follower_id = '$user_id') OR user_id = $user_id) AND user_id IN (SELECT user_id FROM " . T_USERS . " WHERE active = '1') GROUP BY user_id ORDER BY id DESC");
}
function lui_SharePostOn($id = false, $type_id = 0, $type = '') {
    global $sqlConnect, $wo;
...
function lui_updateProInfo($update_data) {
    global $sqlConnect, $wo;
    ...
    $type     = lui_Secure($update_data['type']);
    $update   = array();
    foreach ($update_data as $field => $data) {
...
// manage packages
function lui_DeleteAllUserPosts($user_id) {
    global $sqlConnect, $wo;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 1 || !lui_IsAdmin()) {
        return false;
    }
    $posts = mysqli_query($sqlConnect, "SELECT `id` FROM " . T_POSTS . " WHERE `user_id` = {$user_id}");
    if ($posts) {
        while ($fetched_data = mysqli_fetch_assoc($posts)) {
            lui_DeletePost($fetched_data['id']);
        }
        return true;
    }
    return false;
}
function lui_InsertBankTrnsfer($inserted_data) {
    global $wo, $sqlConnect, $db;
...
function lui_GetPageJobs($page_id) {
    global $wo, $sqlConnect, $db;
    ...
    if (!empty($jobs)) {
        foreach ($jobs as $key => $value) {
            ...
            // if (!empty($post)) {
            //     $data[$key]['story'] = lui_PostData($post->id);
            // }
            $data[$key]['page']  = $page;
        }
    }
...
function lui_GetJobById($job_id) {
    global $wo, $sqlConnect, $db;
    if (empty($job_id)) {
        return false;
    }
    $data      = array();
    $job_id    = lui_Secure($job_id);
    $query_one = " SELECT * FROM " . T_JOB . " WHERE id = '{$job_id}'";
    $sql       = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql)) {
        $jobs = mysqli_fetch_assoc($sql);
        if (!empty($jobs)) {
            $page = lui_PageData($jobs['page_id']);
            ...
            if (!empty($jobs['page_id'])) {
                ...
                $jobs['user'] = lui_UserData($jobs['user_id']);
            }
            $jobs['apply'] = false;
            if ($wo['loggedin']) {
                $user_id       = $wo['user']['id'];
            ...
            $sql                 = mysqli_query($sqlConnect, $query_one);
            $fetched_data        = mysqli_fetch_assoc($sql);
            $jobs['url']         = lui_SeoLink('index.php?link1=post&id=' . $fetched_data['id']);
...
function lui_GetAllJobs($filter_data = array()) {
    global $wo, $sqlConnect;
    $data      = array();
    ...
    if (!empty($filter_data['type'])) {
        $type = lui_Secure($filter_data['type']);
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $job    = lui_GetJobById($fetched_data['id']);
            //$products['seller'] = lui_UserData($fetched_data['user_id']);
            $data[] = $job;
        }
    }
...
function lui_GetApplyJob($info = array()) {
    global $wo, $sqlConnect, $db;
    ...
    if (!empty($info['user_id']) && is_numeric($info['user_id']) && $info['user_id'] > 0) {
        $user_id = lui_Secure($info['user_id']);
        if (!empty($info['offset']) && is_numeric($info['offset']) && $info['offset'] > 0) {
            $offset = lui_Secure($info['offset']);
            $db->where('id', $offset, '>');
        }
    ...
    if (!empty($jobs)) {
        foreach ($jobs as $key => $value) {
            $data[$key]              = (array) $value;
            $data[$key]['job_info']  = lui_GetJobById($value->job_id);
            $data[$key]['user_data'] = lui_UserData($value->user_id);
        }
        // $jobs['page'] = $page;
        ...
        // $jobs['url'] = lui_SeoLink('index.php?link1=post&id=' . $post->id);
        // $jobs['apply_count'] = $apply;
    }
    return $data;
}
function lui_GetCommonUsers($args = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $args        = array_merge($options, $args);
    $before      = lui_Secure($args['before']);
    $after       = lui_Secure($args['after']);
    $limit       = lui_Secure($args['limit']);
    $user        = $wo['user']['id'];
    $t_users     = T_USERS;
    $t_followers = T_FOLLOWERS;
...
function GetFundingByUserId($user_id, $limit = 6, $offset = 0) {
    ...
    if (!empty($funding)) {
        foreach ($funding as $key => $fund) {
            $new_data         = $fund;
            $new_data->image  = lui_GetMedia($new_data->image);
            ...
            $new_data->user_data = lui_UserData($fund->user_id);
...
function GetFundingById($id, $type = 'id') {
    ...
    if (!empty($funding)) {
        $funding->image        = lui_GetMedia($funding->image);
        ...
        $funding->user_data = lui_UserData($funding->user_id);
        $funding->is_donate = 0;
        if ($wo['loggedin']) {
...
function lui_GetOfferById($offer_id) {
    global $wo, $sqlConnect, $db;
    if (empty($offer_id)) {
        return false;
    }
    $data     = array();
    $offer_id = lui_Secure($offer_id);
    $offer    = $db->where('id', $offer_id)->getOne(T_OFFER);
    if (!empty($offer)) {
        $page               = lui_PageData($offer->page_id);
        $offer->image       = lui_GetMedia($offer->image);
        ...
        $offer['url']     = lui_SeoLink('index.php?link1=post&id=' . $post->id);
    }
    return $offer;
}
function lui_GetAllOffers($filter_data = array()) {
    global $wo, $sqlConnect;
    $data      = array();
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $offer  = lui_GetOfferById($fetched_data['id']);
            $data[] = $offer;
        }
    }
    return $data;
}
function lui_GetNearbyShops($args = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($args)) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['page_data'] = lui_PageData($fetched_data['page_id']);
            $fetched_data['product']   = lui_GetProduct($fetched_data['product_id']);
            $data[]                    = $fetched_data;
        }
    }
    return $data;
}
function lui_GetNearbyShopsCount($args = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($args)) {
        return false;
    }
...
function lui_GetNearbyBusiness($args = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($args)) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['page_data']         = lui_PageData($fetched_data['page_id']);
            $fetched_data['job']               = lui_GetJobById($fetched_data['job_id']);
            $fetched_data['job']['full_image'] = lui_GetMedia($fetched_data['job']['image']);
            $data[]                            = $fetched_data;
        }
    }
    return $data;
...
function lui_GetNearbyBusinessCount($args = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($args)) {
        return false;
    }
...
function lui_IfCanGenerateLink($user_id) {
    global $wo, $sqlConnect;
...
function lui_GetAvailableLinks($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    }
...
function lui_GetGeneratedLinks($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    }
...
function lui_GetUsedLinks($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    }
...
function lui_GetMyInvitaionCodes($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            if (!empty($fetched_data['invited_id'])) {
                $user_data                 = lui_UserData($fetched_data['invited_id']);
...
function lui_AddInvitedUser($user_id, $code) {
    global $wo, $sqlConnect, $db;
...
function lui_IsUserInvitationExists($code = false) {
    global $sqlConnect, $wo;
    if (!$code) {
        return false;
    }
...
function lui_GetAllInvitaionCodes() {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteUserInvitation($col = '', $val = false) {
    global $sqlConnect, $wo;
    if (!$val && !$col) {
        return false;
    }
...
function lui_notifyUsersLive($post_id) {
    global $sqlConnect, $wo;
...
function lui_CheckRazorpayPayment($payment_id, $data) {
    global $wo;
...
function lui_CheckAnonymous($id, $type) {
    global $sqlConnect, $wo;
...
function GetBroadcastChatById($id, $user_id = 0) {
    ...
    $broadcast = $db->where('id', lui_Secure($id))->where('user_id', $user_id)->getOne(T_CAST);
    if (!empty($broadcast)) {
        $broadcast->org_image = $broadcast->image;
        $broadcast->image     = lui_GetMedia($broadcast->image);
        $broadcast->users     = array();
        $users                = $db->where('broadcast_id', $broadcast->id)->get(T_CAST_USERS);
        foreach ($users as $key => $value) {
            $broadcast->users[] = lui_UserData($value->user_id);
        }
        return $broadcast;
    }
    return false;
...
function GetBroadcastChatByUserId($user_id = 0, $limit = 10, $offset = 0) {
    ...
    if (!empty($offset) && is_numeric($offset) && $offset > 0) {
        $db->where('time', lui_Secure($offset), '<');
    }
    $limit     = lui_Secure($limit);
...
function FFMPEGUpload($data) {
    ...
    $data['id']                    = lui_RegisterPost($data['post_data']);
    if (file_exists($video_output_full_path_240)) {
        if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
            $upload_s3 = lui_UploadToS3($video_path_240);
        }
        $processing = 0;
    ...
    if (empty($data['video_thumb'])) {
        ...
        $hash       = sha1(time() + time() - rand(9999, 9999)) . lui_GenerateKey();
        ...
        if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
            $upload_s3 = lui_UploadToS3($file_thumb);
        }
        $db->where('id', $data['id'])->update(T_POSTS, array(
    ...
    if ($resolution >= 640 || $resolution == 0) {
        ...
        if (file_exists($video_output_full_path_360)) {
            if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                $upload_s3 = lui_UploadToS3($video_path_360);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '360p' => 1
            ));
    ...
    if ($resolution >= 854 || $resolution == 0) {
        ...
        if (file_exists($video_output_full_path_480)) {
            if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                $upload_s3 = lui_UploadToS3($video_path_480);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '480p' => 1
            ));
    ...
    if ($resolution >= 1280 || $resolution == 0) {
        ...
        if (file_exists($video_output_full_path_720)) {
            if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                $upload_s3 = lui_UploadToS3($video_path_720);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '720p' => 1
            ));
    ...
    if ($resolution >= 1920 || $resolution == 0) {
        ...
        if (file_exists($video_output_full_path_1080)) {
            if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                $upload_s3 = lui_UploadToS3($video_path_1080);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '1080p' => 1
            ));
    ...
    if ($resolution >= 2048 || $resolution == 0) {
        ...
        if (file_exists($video_output_full_path_2048)) {
            if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                $upload_s3 = lui_UploadToS3($video_path_2048);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '2048p' => 1
            ));
    ...
    if ($resolution >= 3840 || $resolution == 0) {
        ...
        if (file_exists($video_output_full_path_4096)) {
            if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                $upload_s3 = lui_UploadToS3($video_path_4096);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '4096p' => 1
            ));
...
function GetReview($id) {
    ...
    $review      = $db->where('id', lui_Secure($id))->getOne(T_PRODUCT_REVIEW);
    $images      = $db->where('review_id', $review->id)->get(T_ALBUMS_MEDIA);
    $images_data = array();
    foreach ($images as $key => $value) {
        $new              = array();
        $new['org_image'] = $value->image;
        $new['image']     = lui_GetMedia($value->image);
        $review->images[] = $new;
    }
    $review->user_data = lui_UserData($review->user_id);
    return $review;
}
...
function PatreonPostBlur() {
    ...
    if ($wo['story']['user_id'] == $wo['user']['id'] || lui_IsAdmin()) {
        return '';
    }
...
function PatreonSubscribed($user_id) {
    ...
    if ($user_id == $wo['user']['id'] || lui_IsAdmin()) {
        return true;
    }
...
function PatreonPostSubscribed($user_id, $post_id) {
    ...
    $post_patron = $db->where('id', lui_Secure($post_id))->where('user_id', lui_Secure($user_id))->where('postPrivacy', '5')->getValue(T_POSTS, 'COUNT(*)');
    if ($post_patron > 0) {
        $is_subscribed = $db->where('user_id', $user_id)->where('subscriber_id', $wo['user']['id'])->getValue(T_PATREON_SUBSCRIBERS, 'COUNT(*)');
        if ($is_subscribed > 0) {
            return true;
...
function lui_GetSearchServices($query = '') {
    global $wo, $sqlConnect, $db;
    $data = array();
    if (empty($query)) {
        return array();
    }
    $query    = lui_Secure($query);
    ...
    if (!empty($services)) {
        foreach ($services as $key => $value) {
            $data[$value->user_id] = lui_UserData($value->user_id);
        }
        return $data;
    }
    return array();
}
function lui_UserSugServices($limit = 20) {
    global $wo, $sqlConnect, $db;
    if (!is_numeric($limit)) {
        return false;
    }
...
function lui_GetOpenToWorkPosts($limit = 10, $offset = 0) {
    global $wo, $sqlConnect, $db;
