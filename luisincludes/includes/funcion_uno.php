...
function lui_GetUserFromSessionID($session_id, $platform = 'web') {
    global $sqlConnect, $db;
    if (empty($session_id)) {
        return false;
    }
    $session_id = lui_Secure($session_id);
...
function lui_GetDataFromSessionID($session_id, $platform = 'web') {
    global $sqlConnect;
    if (empty($session_id)) {
        return false;
    }
    $platform   = lui_Secure($platform);
    $session_id = lui_Secure($session_id);
    $data       = array();
...
function lui_GetSessionDataFromUserID($user_id = 0) {
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
    $user_id = lui_Secure($user_id);
    $time    = time() - 30;
...
function lui_GetAllSessionsFromUserID($user_id = 0,$limit = 10,$offset = array()) {
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($row = mysqli_fetch_assoc($query)) {
            $row['browser']    = 'Unknown';
            $row['time']       = lui_Time_Elapsed_String($row['time']);
...
function lui_GetPlatformFromUser_ID($user_id = 0) {
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
...
function lui_SaveTerm($update_name, $value) {
    global $wo, $config, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $update_name = lui_Secure($update_name);
...
function lui_SaveHTMLEmails($update_name, $value) {
    global $wo, $config, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_LangsFromDB($lang = 'english') {
    global $sqlConnect, $wo;
    $data = array();
    if (empty($lang)) {
        $lang = 'english';
...
function lui_LangsNamesFromDB($lang = 'english') {
    global $sqlConnect, $wo;
    $data  = array();
...
function lui_SaveConfig($update_name, $value) {
    global $wo, $config, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_Login($username, $password) {
    ...
    if (mysqli_num_rows($query_hash)) {
        ...
        if ($hash == 'password_hash') {
            ...
            $login_password = lui_Secure($hash($password));
        }
        ...
        if (lui_Sql_Result($query, 0) == 1) {
            if ($hash == 'sha1' || $hash == 'md5') {
                $new_password = lui_Secure(password_hash($password, PASSWORD_DEFAULT));
...
function lui_CreateLoginSession($user_id = 0) {
    global $sqlConnect, $db;
    if (empty($user_id)) {
        return false;
    }
...
function lui_IsUserCookie($user_id, $password) {
    global $sqlConnect;
    if (empty($user_id) || empty($password)) {
        return false;
    }
    $user_id  = lui_Secure($user_id);
    $password = lui_Secure($password);
    $query    = mysqli_query($sqlConnect, "SELECT COUNT(`user_id`) FROM " . T_USERS . " WHERE `user_id` = '{$user_id}' AND `password` = '{$password}'");
    return (lui_Sql_Result($query, 0) == 1) ? true : false;
}
function lui_SetLoginWithSession($user_email) {
    if (empty($user_email)) {
        return false;
    }
    $user_email          = lui_Secure($user_email);
    $_SESSION['user_id'] = lui_CreateLoginSession(lui_UserIdFromEmail($user_email));
...
function lui_UserActive($username) {
    global $sqlConnect;
    if (empty($username)) {
        return false;
    }
...
function lui_UserInactive($username) {
    global $sqlConnect;
    if (empty($username)) {
        return false;
    }
...
function lui_UserExists($username) {
    global $sqlConnect;
    if (empty($username)) {
        return false;
    }
...
function lui_IsUserComplete($user_id) {
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
...
function lui_UserIdFromUsername($username) {
    global $sqlConnect;
    if (empty($username)) {
        return false;
    }
    ...
    return lui_Sql_Result($query, 0, 'user_id');
}
function lui_UserIdFromPhoneNumber($phone_number) {
    global $sqlConnect;
    if (empty($phone_number)) {
        return false;
    }
    $phone_number = lui_Secure($phone_number);
    $query        = mysqli_query($sqlConnect, "SELECT `user_id` FROM " . T_USERS . " WHERE `phone_number` = '{$phone_number}'");
    return lui_Sql_Result($query, 0, 'user_id');
}
function lui_UserNameFromPhoneNumber($phone_number) {
    global $sqlConnect;
    if (empty($phone_number)) {
        return false;
    }
    ...
    return lui_Sql_Result($query, 0, 'username');
}
function lui_UserIdForLogin($username) {
    global $sqlConnect;
    if (empty($username)) {
        return false;
    }
...
function lui_UserIdFromEmail($email) {
    global $sqlConnect;
    if (empty($email)) {
        return false;
    }
    $email = lui_Secure($email);
    $query = mysqli_query($sqlConnect, "SELECT `user_id` FROM " . T_USERS . " WHERE `email` = '{$email}'");
    return lui_Sql_Result($query, 0, 'user_id');
}
function lui_UserIDFromEmailCode($email_code) {
    global $sqlConnect;
    if (empty($email_code)) {
        return false;
    }
    $email_code = lui_Secure($email_code);
...
function lui_UserIDFromSMSCode($email_code) {
    global $sqlConnect;
    if (empty($email_code)) {
        return false;
    }
...
function lui_IsBlocked($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    $user_id        = lui_Secure($user_id);
...
function lui_RegisterBlock($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_RemoveBlock($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetBlockedMembers($user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = lui_UserData($fetched_data['blocked']);
        }
    }
    return $data;
}
function lui_EmailExists($email) {
    global $sqlConnect;
    if (empty($email)) {
        return false;
    }
...
function lui_IsOnwer($user_id) {
    ...
    $user_id        = lui_Secure($user_id);
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    if (lui_IsAdmin($logged_user_id) === false) {
        if ($user_id == $logged_user_id) {
            return true;
        } else {
            return false;
...
function lui_IsReportExists($id = false, $type = 'user') {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || !$id || !$type) {
        return false;
    }
    $id    = lui_Secure($id);
    $type  = lui_Secure($type);
...
function lui_UserData($user_id, $password = true) {
    ...
    $fetched_data['avatar']        = lui_GetMedia($fetched_data['avatar']) . '?cache=' . $fetched_data['last_avatar_mod'];
    $fetched_data['cover']         = lui_GetMedia($fetched_data['cover']) . '?cache=' . $fetched_data['last_cover_mod'];
    $fetched_data['id']            = $fetched_data['user_id'];
    $fetched_data['user_platform'] = lui_GetPlatformFromUser_ID($fetched_data['user_id']);
    $fetched_data['type']          = 'user';
    $fetched_data['url']           = lui_SeoLink('index.php?link1=timeline&u=' . $fetched_data['username']);
    $fetched_data['name']          = '';
    ...
    if (lui_IsReportExists($user_id, 'user')) {
    ...
    if (!empty($wo['user']['id'])) {
        ...
        $fetched_data['is_following_me'] = (lui_IsFollowing($wo['user']['user_id'], $user_id)) ? 1 : 0;
    }
...
function lui_UserStatus($user_id, $lastseen, $type = '') {
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($lastseen < $time) {
        if ($type == 'profile') {
            $status = '<span class="small-last-seen"><span style="font-size:12px; color:#777;">' . lui_Time_Elapsed_String($lastseen) . '</span></span>';
        } else {
            $status = '<span class="small-last-seen">' . lui_Time_Elapsed_String($lastseen) . '</span>';
        }
    } else {
...
function lui_LastSeen($user_id, $type = '') {
...
function lui_RegisterUser($registration_data, $invited = false) {
    global $wo, $sqlConnect;
    ...
    $registration_data['password']   = lui_Secure(password_hash($registration_data['password'], PASSWORD_DEFAULT));
    $registration_data['ip_address'] = lui_Secure($ip);
    ...
    if (!empty($_SESSION['lang'])) {
        ...
        if (in_array($lang_name, $langs)) {
            $registration_data['language'] = lui_Secure($lang_name);
        }
    }
    ...
    if ($query) {
        if ($invited) {
            @lui_DeleteAdminInvitation('code', $invited);
            lui_AddInvitedUser($user_id, $invited);
        }
        return true;
    } else {
        return false;
...
function lui_ResetPassword($user_id, $password) {
    global $sqlConnect;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
    }
    ...
    $user_id  = lui_Secure($user_id);
    $password = lui_Secure(password_hash($password, PASSWORD_DEFAULT));
...
function lui_GetLanguages() {
    $data           = array();
    $dir            = scandir('luisincludes/languages');
...
function lui_GetPostIdFromUrl($string) {
    $slug_string = '';
    $string      = lui_Secure($string);
    ...
    return lui_Secure($slug_string);
}
function lui_GetBlogIdFromUrl($string) {
    $slug_string = '';
    $string      = lui_Secure($string);
    if (preg_match('/[^a-z\s-]/i', $string)) {
        $string_exp  = @explode('_', $string);
...
function lui_isValidPasswordResetToken($string) {
    global $sqlConnect;
    $string_exp = explode('_', $string);
    $user_id    = lui_Secure($string_exp[0]);
    $password   = lui_Secure($string_exp[1]);
...
function lui_isValidPasswordResetToken2($string) {
    global $sqlConnect;
    $string_exp = explode('_', $string);
    $user_id    = lui_Secure($string_exp[0]);
    $password   = lui_Secure($string_exp[1]);
...
function lui_DeleteUser($user_id) {
    global $wo, $sqlConnect, $cache, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (lui_IsModerator() === true) {
        if (lui_IsAdmin($user_id)) {
            return false;
        }
    }
    ...
    $user_data               = lui_UserData($user_id);
    ...
    if (mysqli_num_rows($query_one_delete_photos)) {
        ...
        if (isset($fetched_data['avatar']) && !empty($fetched_data['avatar']) && $fetched_data['avatar'] != $wo['userDefaultAvatar'] && $fetched_data['avatar'] != $wo['userDefaultFAvatar']) {
            $xpror = explode('.', $fetched_data['avatar'] ?? '');
            $explode2 = @end($xpror);
            $explode3 = @explode('.', $fetched_data['avatar']);
            $media_2  = $explode3[0] . '_avatar_full.' . $explode2;
            @unlink(trim($media_2));
            @unlink($fetched_data['avatar']);
            $delete_from_s3 = lui_DeleteFromToS3($fetched_data['avatar']);
        ...
        if (isset($fetched_data['cover']) && !empty($fetched_data['cover']) && $fetched_data['cover'] != $wo['userDefaultCover']) {
            $xpror = explode('.', $fetched_data['cover'] ?? '');
            $explode2 = @end($xpror);
            $explode3 = @explode('.', $fetched_data['cover']);
            $media_2  = $explode3[0] . '_cover_full.' . $explode2;
            @unlink(trim($media_2));
            @unlink($fetched_data['cover']);
            $delete_from_s3 = lui_DeleteFromToS3($fetched_data['cover']);
            $delete_from_s3 = lui_DeleteFromToS3($media_2);
        }
    }
    ...
    if ($query_four_delete_media) {
        if (mysqli_num_rows($query_four_delete_media) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($query_four_delete_media)) {
                $delete_posts = lui_DeletePage($fetched_data['page_id']);
            }
        }
    }
    ...
    if ($query_five_delete_media) {
        if (mysqli_num_rows($query_five_delete_media) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($query_five_delete_media)) {
                $delete_groups = lui_DeleteGroup($fetched_data['id']);
            }
        }
    }
    ...
    if ($query_6_delete_media) {
        if (mysqli_num_rows($query_6_delete_media) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($query_6_delete_media)) {
                $delete_posts = lui_DeletePost($fetched_data['id']);
            }
        }
    }
    ...
    if ($query_7_delete_media) {
        if (mysqli_num_rows($query_7_delete_media) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($query_7_delete_media)) {
                $delete_posts = lui_DeleteForumThread($fetched_data['id']);
            }
        }
    }
    ...
    if ($query_8_delete_media) {
        if (mysqli_num_rows($query_8_delete_media) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($query_8_delete_media)) {
                $delete_posts = lui_DeleteThreadReply($fetched_data['id']);
            }
        }
    }
    ...
    if ($query_9_delete_media) {
        if (mysqli_num_rows($query_9_delete_media) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($query_9_delete_media)) {
                $delete_posts = lui_DeleteEvent($fetched_data['id']);
            }
        }
    }
    ...
    if ($querydelete) {
        if (mysqli_num_rows($querydelete) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($querydelete)) {
                @lui_DeleteFromToS3($fetched_data['image']);
    ...
    if ($querydelete) {
        if (mysqli_num_rows($querydelete) > 0) {
            while ($fetched_data = mysqli_fetch_assoc($querydelete)) {
                @lui_DeleteFromToS3($fetched_data['pdf']);
    ...
    if ($query_one) {
        ...
        $send_message_data = array(
            ...
            'charSet' => 'utf-8',
            'message_body' => lui_LoadPage('emails/account-deleted'),
            'is_html' => true
        );
...
function lui_UpdateUserData($user_id, $update_data, $unverify = false) {
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($is_mod) {
        $user_data_ = lui_UserData($user_id);
    ...
    foreach ($update_data as $field => $data) {
        ...
        if (in_array($field, $filter)) {
            $finalData = lui_Secure($data, 1);
        } else {
            $finalData = lui_Secure($data, 0);
        }
        if ($field != 'pro_') {
    ...
    if ($query1) {
        if (!empty($update_data['username'])) {
            lui_UpdateUsernameInNotifications($user_id, $update_data['username']);
        }
        return true;
    } else {
        return false;
...
function lui_UpdateUsernameInNotifications($user_id = 0, $username = '') {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_UploadImage($file, $name, $type, $type_file, $user_id = 0, $placement = '') {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($placement == 'page') {
        $image_data['page_id'] = lui_Secure($user_id);
    } else if ($placement == 'group') {
        $image_data['id'] = lui_Secure($user_id);
    } else if ($placement == 'event') {
        $image_data['event_id'] = lui_Secure($user_id);
    } else {
        $image_data['user_id'] = lui_Secure($user_id);
    }
    if ($type == 'cover') {
        if ($placement == 'page') {
        ...
        $filename            = $dir . '/' . lui_GenerateKey() . '_' . date('d') . '_' . md5(time()) . '_cover.' . $ext;
        ...
        if (move_uploaded_file($file, $filename)) {
            ...
            $update_data = false;
            if ($placement == 'page') {
                $update_data = lui_UpdatePageData($image_data['page_id'], $image_data);
            } else if ($placement == 'group') {
                $update_data = lui_UpdateGroupData($image_data['id'], $image_data);
            } else if ($placement == 'event') {
                $update_data = lui_UpdateEvent($image_data['event_id'], array(
                    "cover" => $image_data['cover']
                ));
            } else {
                $image_file = lui_GetMedia($image_data['cover']);
                $blur       = 0;
                $upload_p   = true;
                ...
                if ($upload_p == true) {
                    $update_data = lui_UpdateUserData($image_data['user_id'], $image_data);
                    if ($update_data) {
                        $last_file = $filename;
                        $explode2  = @end(explode('.', $filename));
                        $explode3  = @explode('.', $filename);
                        $last_file = $explode3[0] . '_full.' . $explode2;
                        @lui_CompressImage($filename, $last_file, 50);
                        $upload_s3 = lui_UploadToS3($last_file);
                        if ($wo['config']['website_mode'] != 'askfm') {
                            $regsiter_cover_image = lui_RegisterPost(array(
                                'user_id' => lui_Secure($image_data['user_id']),
                                'postFile' => lui_Secure($last_file, 0),
                                'time' => time(),
                                'postType' => lui_Secure('profile_cover_picture'),
                                'postPrivacy' => '0',
                                'blur' => $blur
                            ));
                        }
            ...
            if ($update_data == true) {
                lui_Resize_Crop_Image(918, 332, $filename, $filename, 80);
        ...
        $filename             = $dir . '/' . lui_GenerateKey() . '_' . date('d') . '_' . md5(time()) . '_avatar.' . $ext;
        $image_data['avatar'] = $filename;
        if ($placement == 'page') {
            $user_data = lui_PageData($image_data['page_id']);
        } elseif ($placement == 'group') {
            $user_data = lui_GroupData($image_data['id']);
        } else {
            $user_data = lui_UserData($image_data['user_id']);
        }
        $image_data_d = array();
        ...
        if (move_uploaded_file($file, $filename)) {
            ...
            if ($placement == 'page') {
                $update_data = lui_UpdatePageData($image_data['page_id'], $image_data);
                lui_Resize_Crop_Image($wo['profile_picture_width_crop'], $wo['profile_picture_height_crop'], $filename, $filename, $wo['profile_picture_image_quality']);
                $upload_s3 = lui_UploadToS3($filename);
                return true;
            } else if ($placement == 'group') {
                $update_data = lui_UpdateGroupData($image_data['id'], $image_data);
                ...
                $update_data = lui_UpdateAppImage($user_id, $filename);
                lui_Resize_Crop_Image($wo['profile_picture_width_crop'], $wo['profile_picture_height_crop'], $filename, $filename, $wo['profile_picture_image_quality']);
                $upload_s3 = lui_UploadToS3($filename);
                return true;
            } else {
                $image_file = lui_GetMedia($image_data['avatar']);
                $blur       = 0;
                $upload_p   = true;
                if ($wo['config']['adult_images'] == 1 && detect_safe_search($image_file) == false && $wo['config']['adult_images_action'] == 1) {
                    $blur = 1;
                ...
                if ($upload_p == true) {
                    ...
                    if (lui_UpdateUserData($image_data['user_id'], $image_data)) {
                        $explode2  = @end(explode('.', $filename));
                        $explode3  = @explode('.', $filename);
                        $last_file = $explode3[0] . '_full.' . $explode2;
                        $compress  = lui_CompressImage($filename, $last_file, 50);
                        if ($compress) {
                            $upload_s3 = lui_UploadToS3($last_file);
                            if ($wo['config']['website_mode'] != 'askfm') {
                                $regsiter_image = lui_RegisterPost(array(
                                    'user_id' => lui_Secure($image_data['user_id']),
                                    'postFile' => lui_Secure($last_file, 0),
                                    'time' => time(),
                                    'postType' => lui_Secure('profile_picture'),
                                    'postPrivacy' => '0',
                                    'blur' => $blur
                                ));
                            }
                            ...
                            lui_UpdateUserData($image_data['user_id'], $image_data_d);
                        }
                        return true;
                    }
                }
        ...
        if (mysqli_num_rows($query_one_delete_background_image)) {
            ...
            $filename                       = $dir . '/' . lui_GenerateKey() . '_' . date('d') . '_' . md5(time()) . '_background_image.' . $ext;
        ...
        if (mysqli_num_rows($query_one_delete_background_image)) {
            ...
            if (move_uploaded_file($file, $filename)) {
                ...
                if (lui_UpdatePageData($image_data['page_id'], $image_data)) {
                    return true;
                }
            }
        }
...
function lui_UserBirthday($birthday) {
    global $wo;
    if (empty($birthday)) {
        return false;
    }
    $birthday = lui_Secure($birthday);
    if ($wo['config']['age'] == 0) {
...
function lui_GetAllUsers($limit = '', $type = '', $filter = array(), $after = '') {
    global $wo, $sqlConnect;
    $data      = array();
    $query_one = " SELECT `user_id` FROM " . T_USERS . " WHERE `type` = 'user'";
    if (isset($filter) and !empty($filter)) {
        if (!empty($filter['query'])) {
            $query_one .= " AND ((`email` LIKE '%" . lui_Secure($filter['query']) . "%') OR (`username` LIKE '%" . lui_Secure($filter['query']) . "%') OR CONCAT( `first_name`,  ' ', `last_name` ) LIKE  '%" . lui_Secure($filter['query']) . "%')";
        }
        if (isset($filter['source']) && $filter['source'] != 'all') {
            $query_one .= " AND `src` = '" . lui_Secure($filter['source']) . "'";
        }
        if (isset($filter['status']) && $filter['status'] != 'all') {
            $query_one .= " AND `active` = '" . lui_Secure($filter['status']) . "'";
        }
    }
    if (!empty($after) && is_numeric($after) && $after > 0) {
        $query_one .= " AND `user_id` < " . lui_Secure($after);
    }
    if ($type == 'sidebar') {
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $user_data        = lui_UserData($fetched_data['user_id']);
...
function lui_GetAllUsersByType($type = 'all') {
    global $sqlConnect;
    $data      = array();
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $data[] = lui_UserData($fetched_data['user_id']);
        }
    }
    return $data;
}
function lui_GetUsersByTime($type = 'week') {
    global $sqlConnect;
    $types = array(
        'week',
        'month',
...
function lui_GetFollowingSug($limit, $query) {
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $query_one_search = " WHERE ((`username` LIKE '%" . lui_Secure($query) . "%') OR CONCAT( `first_name`,  ' ', `last_name` ) LIKE  '%" . lui_Secure($query) . "%')";
...
function lui_GetHashtagSug($limit, $query) {
    global $wo, $sqlConnect;
    $data      = array();
    $html_fi   = array();
...
function lui_WelcomeUsers($limit = '', $type = '') {
    global $wo, $sqlConnect;
    if (empty($limit)) {
        $limit = 12;
    }
    $data      = array();
    $query_one = " SELECT `user_id` FROM " . T_USERS . " WHERE `active` = '1' AND `avatar` <> '" . lui_Secure($wo['userDefaultAvatar']) . "' ORDER BY RAND() LIMIT {$limit}";
    $sql       = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $data[] = lui_UserData($fetched_data['user_id']);
...
function lui_UsersTotal() {
    global $wo, $sqlConnect;
    $data      = array();
    $query_one = ' SELECT count(*) as c FROM ' . T_USERS;
    $sql       = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $data[] = $fetched_data['c'];
...
function lui_FeaturedUsersAPI($limit = '', $offset = '') {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_FeaturedUsers($limit = '', $type = '') {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_ImportImageFromLogin($media, $amazon = 0) {
    global $wo;
    if (!file_exists('upload/photos/' . date('Y'))) {
        mkdir('upload/photos/' . date('Y'), 0777, true);
    }
    ...
    $dir      = 'upload/photos/' . date('Y') . '/' . date('m');
    $file_dir = $dir . '/' . lui_GenerateKey() . '_avatar.jpg';
    $getImage = fetchDataFromURL($media);
    if (!empty($getImage)) {
        $importImage = file_put_contents($file_dir, $getImage);
        if ($importImage) {
            lui_Resize_Crop_Image($wo['profile_picture_width_crop'], $wo['profile_picture_height_crop'], $file_dir, $file_dir, 100);
        }
    }
    if (file_exists($file_dir)) {
        $upload_s3 = lui_UploadToS3($file_dir, array(
            'amazon' => $amazon
        ));
        return $file_dir;
    } else {
...
// function lui_ImportImageFromFile($media, $custom_name = '_url_image') {
//     global $wo;
//     if (empty($media)) {
//         return false;
//     }
...
//     }
//     $dir               = 'upload/photos/' . date('Y') . '/' . date('m');
//     $file_dir          = $dir . '/' . lui_GenerateKey() . $custom_name . $extension;
//     $fileget           = file_get_contents($media);
//     if (!empty($fileget)) {
//         $importImage = @file_put_contents($file_dir, $fileget);
//     }
//     if (file_exists($file_dir)) {
//         $upload_s3 = lui_UploadToS3($file_dir);
...
//     }
// }
function lui_ImportImageFromFile($media, $custom_name = '_url_image', $type = '') {
    global $wo;
    if (empty($media)) {
        return false;
    }
...
function lui_ImportImageFromUrl($media, $custom_name = '_url_image') {
    global $wo;
    if (empty($media)) {
        return false;
    }
...
function lui_IsFollowingNotify($following_id, $user_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $following_id = lui_Secure($following_id);
    $user_id      = lui_Secure($user_id);
...
function lui_IsFollowing($following_id, $user_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_RegisterFollow($following_id = 0, $followers_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    foreach ($followers_id as $follower_id) {
        ...
        if (lui_IsBlocked($following_id)) {
            continue;
        }
        $following_id = lui_Secure($following_id);
        $follower_id  = lui_Secure($follower_id);
        $active       = 1;
        if (lui_IsFollowing($following_id, $follower_id) === true) {
            continue;
        }
        $follower_data  = lui_UserData($follower_id);
        $following_data = lui_UserData($following_id);
        ...
        if ($query) {
            if ($active == 1) {
                ...
                lui_RegisterNotification($notification_data);
                $activity_data = array(
                    'user_id' => $follower_id,
                    'follow_id' => $following_id,
                    'activity_type' => 'following'
                );
                $add_activity  = lui_RegisterActivity($activity_data);
            }
        }
    }
    return true;
}
function lui_CountFollowRequests($data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($data['account_id'] != $user_id) {
        $data['account_id'] = lui_Secure($data['account_id']);
        $account            = lui_UserData($data['account_id']);
    }
...
function lui_IsFollowRequested($following_id = 0, $follower_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteFollow($following_id = 0, $follower_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $following_id = lui_Secure($following_id);
    $follower_id  = lui_Secure($follower_id);
    if (lui_IsFollowing($following_id, $follower_id) === false && lui_IsFollowRequested($following_id, $follower_id) === false) {
        return false;
    } else {
        ...
        if ($wo['config']['connectivitySystem'] == 1) {
            ...
            lui_DeleteSelectedActivity($follower_id, 'friend', $following_id);
            lui_DeleteSelectedActivity($following_id, 'friend', $follower_id);
        } else {
            lui_DeleteSelectedActivity($follower_id, 'following', $following_id);
        }
        if ($query) {
            return true;
        }
...
function lui_CountMutualFriends($user_id, $active = true) {
    global $wo, $sqlConnect;
    $data = array();
    if ($wo['loggedin'] == false) {
        return false;
...
function lui_CountFollowing($user_id, $active = true) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
...
function lui_AcceptFollowRequest($following_id = 0, $follower_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $following_id = lui_Secure($following_id);
    $follower_id  = lui_Secure($follower_id);
    if (lui_IsFollowRequested($following_id, $follower_id) === false) {
        return false;
    }
    $follower_data = lui_UserData($follower_id);
    ...
    if ($query) {
        ...
        $add_activity      = lui_RegisterActivity($activity_data);
        if (lui_RegisterNotification($notification_data) === true) {
            return true;
        } else {
            return false;
        }
...
function lui_DeleteFollowRequest($following_id, $follower_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetFollowRequests($user_id = 0, $search_query = '') {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (!empty($search_query)) {
        $search_query = lui_Secure($search_query);
    ...
    if (mysqli_num_rows($sql_query)) {
        while ($sql_fetch = mysqli_fetch_assoc($sql_query)) {
            $data[] = lui_UserData($sql_fetch['user_id']);
        }
    }
    return $data;
}
...
function lui_CountFollowers($user_id) {
    global $wo, $sqlConnect;
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
    }
...
function lui_SearchFollowers($user_id, $filter = '', $limit = 10, $event_id = 0) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    ...
    $user_id  = lui_Secure($user_id);
    $filter   = lui_Secure($filter);
    $event_id = lui_Secure($event_id);
...
function lui_GetFollowing($user_id, $type = '', $limit = '', $after_user_id = '', $placement = array()) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
    ...
    $user_id       = lui_Secure($user_id);
    $after_user_id = lui_Secure($after_user_id);
    ...
    if (!empty($placement)) {
        if ($placement['in'] == 'profile_sidebar' && is_array($placement['following_data'])) {
            foreach ($placement['following_data'] as $key => $id) {
                $user_data = lui_UserData($id, false);
                if (!empty($user_data)) {
                    $data[] = $user_data;
                }
            }
    ...
    if (mysqli_num_rows($sql_query)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
            $user_data = lui_UserData($fetched_data['user_id'], false);
            if ($wo['loggedin']) {
                $user_data['family_member'] = lui_GetFamalyMember($fetched_data['user_id'], $wo['user']['id']);
            }
            $data[] = $user_data;
        }
    }
...
function lui_GetMutualFriends($user_id, $type = '', $limit = '', $after_user_id = '', $placement = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
WHERE f1.follower_id = {$user_id}
  AND f2.follower_id = {$logged_user_id} AND f1.`following_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$logged_user_id}') AND f1.`following_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$logged_user_id}') AND f1.active = 1 GROUP BY following_id ";
    ...
    if (mysqli_num_rows($sql_query)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
            $user_data = lui_UserData($fetched_data['following_id'], false);
            $data[]    = $user_data;
        }
    }
    return $data;
}
function lui_GetFollowers($user_id, $type = '', $limit = '', $after_user_id = '', $placement = array()) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
...
function lui_GetFollowButton($user_id = 0) {
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $account = $wo['follow'] = lui_UserData($user_id);
    ...
    if (lui_IsFollowing($user_id, $logged_user_id)) {
        if ($wo['config']['connectivitySystem'] == 1) {
            return lui_LoadPage($unfrined_button);
        } else {
            return lui_LoadPage($unfollow_button);
        }
    } else {
        if (lui_IsFollowRequested($user_id, $logged_user_id)) {
            return lui_LoadPage($request_button);
        } else if (lui_IsFollowRequested($logged_user_id, $user_id)) {
            return lui_LoadPage($accept_button);
        } else {
            if ($account['follow_privacy'] == 1) {
                if (lui_IsFollowing($logged_user_id, $user_id)) {
                    if ($wo['config']['connectivitySystem'] == 1) {
                        return lui_LoadPage($add_frined_button);
                    } else {
                        return lui_LoadPage($follow_button);
                    }
                }
...
function lui_GetNotifyButton($user_id = 0) {
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (lui_IsFollowing($user_id, $logged_user_id)) {
        if (lui_IsFollowingNotify($user_id, $logged_user_id)) {
            if ($wo['config']['connectivitySystem'] == 1) {
                return lui_LoadPage($unnotify_button);
            } else {
                return lui_LoadPage($unnotify_button);
            }
        } else {
            return lui_LoadPage($notify_button);
        }
    }
    return '';
}
function lui_GetFollowNotifyUsers($user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_RegisterNotification($data = array()) {
    global $wo, $sqlConnect;
    ...
    if (lui_IsBlocked($data['recipient_id'])) {
        return false;
    }
    ...
    if (empty($data['notifier_id']) or $data['notifier_id'] == 0) {
        $data['notifier_id'] = lui_Secure($wo['user']['user_id']);
    }
    ...
    if ($data['notifier_id'] == $wo['user']['user_id']) {
        ...
        $data['notifier_id'] = lui_Secure($data['notifier_id']);
        $notifier            = lui_UserData($data['notifier_id']);
    ...
    if (!isset($data['comment_id']) or empty($data['comment_id'])) {
        ...
        $data['comment_id'] = lui_Secure($data['comment_id']);
    }
    if (!isset($data['reply_id']) or empty($data['reply_id'])) {
        $data['reply_id'] = 0;
    } else {
        $data['reply_id'] = lui_Secure($data['reply_id']);
    }
    ...
    $recipient = lui_UserData($data['recipient_id']);
    if (!isset($recipient['user_id'])) {
        return false;
    }
    $url                  = $data['url'];
    $recipient['user_id'] = lui_Secure($recipient['user_id']);
    $data['post_id']      = lui_Secure($data['post_id']);
    $data['type']         = lui_Secure($data['type']);
    if (!empty($data['type2'])) {
        $data['type2'] = lui_Secure($data['type2']);
    } else {
        $data['type2'] = '';
    }
    ...
    $data['text']            = lui_Secure($data['text']);
    $notifier['user_id']     = lui_Secure($notifier['user_id']);
    ...
    if (!empty($recipient['notification_settings'])) {
        ...
        //     //$query1    = mysqli_query($sqlConnect, $query_one);
        //     // lui_UpdateUserData($recipient['user_id'], array(
    ...
    if (!empty($data['page_id']) && $data['page_id'] > 0) {
        $page = lui_PageData($data['page_id']);
        if (!isset($page['page_id'])) {
            return false;
        }
        $page_id = lui_Secure($page['page_id']);
    ...
    if (!empty($data['group_id']) && $data['group_id'] > 0) {
        $group = lui_GroupData($data['group_id']);
        if (!isset($group['id'])) {
        }
        $group_id                 = lui_Secure($group['id']);
    ...
    if (!empty($data['event_id']) && $data['event_id'] > 0) {
        $event                    = lui_EventData($data['event_id']);
        $event_id                 = lui_Secure($event['id']);
    ...
    if (!empty($data['thread_id']) && $data['thread_id'] > 0) {
        $thread_id                 = lui_Secure($data['thread_id']);
    ...
    if (!empty($data['story_id']) && $data['story_id'] > 0) {
        $story_id                 = lui_Secure($data['story_id']);
    ...
    if (!empty($data['blog_id']) && $data['blog_id'] > 0) {
        $blog_id                 = lui_Secure($data['blog_id']);
    ...
    if (!empty($data['group_chat_id']) && $data['group_chat_id'] > 0) {
        $group_chat_id                 = lui_Secure($data['group_chat_id']);
    ...
    if (!isset($data['undo']) or $data['undo'] != true) {
        ...
        if (!empty($data['post_id'])) {
            $post_data = lui_PostData($data['post_id']);
        }
        $my_id = $wo['user']['user_id'];
        if (!empty($post_data['page_id'])) {
            $admin_post_id = $post_data['id'];
            $admins        = lui_GetPageAdmins($post_data['page_id'], 'user_id');
            // $PageData = lui_PageData($post_data['page_id']);
            // if (!empty($PageData)) {
            //     $admin_notify = array();
        ...
        if ($sql_query_three) {
            if ($wo['config']['emailNotification'] == 1 && $recipient['emailNotification'] == 1) {
                ...
                if ($send_mail == true) {
                    ...
                    $send_message_data       = array(
                        ...
                        'charSet' => 'utf-8',
                        'message_body' => lui_LoadPage('emails/notifiction-email'),
                        'is_html' => true,
                        'notifier' => $notifier
                    );
            ...
            if ($wo['config']['android_push_native'] == 1 || $wo['config']['ios_push_native'] == 1 || $wo['config']['web_push'] == 1) {
                lui_NotificationWebPushNotifier();
            }
            return true;
        }
    }
}
function lui_GetNotifications($data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $new_notif = lui_CountNotifications(array(
        'unread' => true
    ));
    if ($new_notif > 0) {
        $query_4 = '';
        ...
        if (mysqli_num_rows($sql_query_one) > 0) {
            while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
                if (!empty($sql_fetch_one['page_id']) && empty($sql_fetch_one['notifier_id'])) {
                    $sql_fetch_one['notifier']        = lui_PageData($sql_fetch_one['page_id']);
                    $sql_fetch_one['notifier']['url'] = lui_SeoLink('index.php?link1=timeline&u=' . $sql_fetch_one['notifier']['page_name']);
                } else {
                    if (!empty($sql_fetch_one['notifier_id'])) {
                        $sql_fetch_one['notifier']        = lui_UserData($sql_fetch_one['notifier_id']);
                        $sql_fetch_one['notifier']['url'] = lui_SeoLink('index.php?link1=timeline&u=' . $sql_fetch_one['notifier']['username']);
                    }
                }
                // if (preg_match_all('/^index\.php\?link1=post&id=(.*)$/i', $sql_fetch_one['url'],$matches)) {
                //     if (!empty($matches[1][0]) && is_numeric($matches[1][0])) {
                //         $post = lui_PostData($matches[1][0]);
                ...
                //     $sql_fetch_one['url']      = lui_SeoLink($sql_fetch_one['url']);
...
function lui_CountNotifications($data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (empty($data['account_id']) or $data['account_id'] == 0) {
        $data['account_id'] = lui_Secure($wo['user']['user_id']);
        $account            = $wo['user'];
    }
    if (!is_numeric($data['account_id']) or $data['account_id'] < 1) {
        return false;
...
function lui_GetSearch($search_qeury) {
    global $sqlConnect, $wo;
    $search_qeury = lui_Secure($search_qeury);
    $data         = array();
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = lui_PageData($fetched_data['page_id']);
        }
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = lui_GroupData($fetched_data['id']);
        }
    }
    return $data;
}
function lui_GetRecentSerachs() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            if ($fetched_data['search_type'] == 'user') {
                $fetched_data_2 = lui_UserData($fetched_data['search_id']);
            } else if ($fetched_data['search_type'] == 'page') {
                $fetched_data_2 = lui_PageData($fetched_data['search_id']);
            } else if ($fetched_data['search_type'] == 'group') {
                $fetched_data_2 = lui_GroupData($fetched_data['search_id']);
            } else {
                return false;
            }
            $data[] = $fetched_data_2;
...
function lui_GetSearchFilter($result, $limit = 30, $offset = 0) {
    global $wo, $sqlConnect, $db;
    $data        = array();
    $profiledata = array();
    $time        = time() - 60;
    ...
    foreach ($_GET as $key => $val) {
        if (substr($key, 0, 4) == 'fid_' && !empty($val)) {
            ...
            if (!empty($custom_type)) {
                $profile_search[$key] = lui_Secure($val);
                ...
                if (!empty($custom_type) && ($custom_type->type == 'textbox' || $custom_type->type == 'textarea')) {
                    $profile_search_sql .= "`" . lui_Secure($key) . "` LIKE '%" . lui_Secure($val) . "%' AND";
                } else {
                    $profile_search_sql .= "`" . lui_Secure($key) . "` = '" . lui_Secure($val) . "' AND";
                }
            }
        }
    }
    ...
    if (!empty($result['query'])) {
        $query = lui_Secure($result['query']);
    }
    if (!empty($result['country'])) {
        $country = lui_Secure($result['country']);
    }
    if (!empty($result['status'])) {
        $result['status'] = lui_Secure($result['status']);
    }
    if (!empty($result['verified'])) {
        $result['verified'] = lui_Secure($result['verified']);
    }
    if (!empty($result['filterbyage']) && $result['filterbyage'] == 'yes') {
        if (!empty($result['age_from'])) {
            $result['age_from'] = lui_Secure($result['age_from']);
        }
        if (!empty($result['age_to'])) {
            $result['age_to'] = lui_Secure($result['age_to']);
        }
    }
    if (!empty($result['image'])) {
        $result['image'] = lui_Secure($result['image']);
    }
    $job_type_main = "";
        ...
        foreach ($result['job_type'] as $key => $value) {
            if (in_array($value, array(
                ...
                if (empty($job_type_query)) {
                    $job_type_query = "`job_type` LIKE '%" . lui_Secure($value) . "%' ";
                } else {
                    $job_type_query .= " OR `job_type` LIKE '%" . lui_Secure($value) . "%' ";
                }
            }
        }
    ...
    if (!empty($result['workplaces'])) {
        ...
        foreach ($result['workplaces'] as $key => $value) {
            if (in_array($value, array(
                ...
                if (empty($job_type_query)) {
                    $job_type_query = "`workplaces` LIKE '%" . lui_Secure($value) . "%' ";
                } else {
                    $job_type_query .= " OR `workplaces` LIKE '%" . lui_Secure($value) . "%' ";
                }
            }
        }
    ...
    if (!empty($result['gender']) && $result['gender'] != 'all') {
        $query .= " AND (`gender` = '" . lui_Secure($result['gender']) . "') ";
    }
    if (!empty($result['country'])) {
    ...
    if (isset($result['image'])) {
        $result['image'] = lui_Secure($result['image']);
        $d_image         = lui_Secure($wo['userDefaultAvatar']);
        if ($result['image'] == 'yes') {
    ...
    if ($wo['loggedin'] == true || !empty($result['user_id'])) {
        if (!empty($result['user_id'])) {
            $user_id = lui_Secure($result['user_id']);
        } else {
            $user_id = lui_Secure($wo['user']['user_id']);
        }
    ...
    //         $data[$fetched_data['user_id']] = lui_UserData($profile_fetched_data['user_id']);
    //     }
    // }
    return $data;
}
function lui_GetMessagesUsers($user_id, $searchQuery = '', $limit = 50, $new = false, $update = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($sql_query_one) > 0) {
        while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
            $user = lui_UserData($sql_fetch_one['conversation_user_id']);
            if (!empty($user)) {
...
function lui_GetMessagesUsersAPP2($fetch_array = array()) {
    global $wo, $sqlConnect;
    ...
    $user_id     = lui_Secure($fetch_array['user_id']);
    $searchQuery = '';
    if (!empty($fetch_array['searchQuery'])) {
        $searchQuery = lui_Secure($fetch_array['searchQuery']);
    }
    $data         = array();
    $excludes     = array();
    $offset_query = "";
    ...
    if (isset($searchQuery) and !empty($searchQuery)) {
        ...
        if (!empty($fetch_array['limit'])) {
            $limit = lui_Secure($fetch_array['limit']);
            $query_one .= "LIMIT {$limit}";
        }
    } else {
        $time        = time() - 60;
    ...
    if (mysqli_num_rows($sql_query_one) > 0) {
        while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
            $new_data = lui_UserData($sql_fetch_one['conversation_user_id']);
...
function lui_GetPageChatList($user_id, $limit = 50, $new = false, $update = 0) {
    global $wo, $sqlConnect, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
            ...
            while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_page)) {
                ...
                if (!in_array($to_id . ',' . $from_id . ',' . $sql_fetch_one['page_id'], $ids) && !in_array($from_id . ',' . $to_id . ',' . $sql_fetch_one['page_id'], $ids)) {
                    ...
                    if ($sql_fetch_one['from_id'] == $user_id) {
                        $user = lui_UserData($sql_fetch_one['to_id']);
                        if (!empty($user)) {
                            $user_data            = $user;
                        ...
                        $user = lui_UserData($sql_fetch_one['from_id']);
                        if (!empty($user)) {
                            $user_data            = $user;
                            $user_data['message'] = $sql_fetch_one;
                            if (!empty($sql_fetch_one['time'])) {
...
function lui_GetMessages($data = array(), $limit = 50) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $message_data   = array();
    $user_id        = lui_Secure($data['user_id']);
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
    }
    ...
    if (!empty($data['message_id'])) {
        $data['message_id'] = lui_Secure($data['message_id']);
        ...
        $data['before_message_id'] = lui_Secure($data['before_message_id']);
        ...
        $data['after_message_id'] = lui_Secure($data['after_message_id']);
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['messageUser'] = lui_UserData($fetched_data['from_id']);
            $fetched_data['or_text']     = $fetched_data['text'];
            $fetched_data['text']        = lui_Markup($fetched_data['text']);
            $fetched_data['text']        = lui_Emo($fetched_data['text']);
            $fetched_data['onwer']       = ($fetched_data['messageUser']['user_id'] == $wo['user']['user_id']) ? 1 : 0;
            if (!empty($fetched_data['stickers']) && !lui_IsUrl($fetched_data['stickers'])) {
                $fetched_data['stickers'] = lui_GetMedia($fetched_data['stickers']);
            }
            ...
            if (!empty($fetched_data['story_id'])) {
                $fetched_data['story'] = lui_GetStroies(array(
...
function GetMessageById($id) {
    ...
    if (mysqli_num_rows($query)) {
        ...
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['reaction'] = lui_GetPostReactionsTypes($fetched_data['id'], 'message');
            $fetched_data['pin']      = 'no';
...
function lui_GetGroupMessages($args = array()) {
    global $wo, $sqlConnect, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $offset         = lui_Secure($args['offset']);
    $id             = lui_Secure($args['id']);
    $group_id       = lui_Secure($args['group_id']);
    $limit          = lui_Secure($args['limit']);
    $new            = lui_Secure($args['new']);
    $old            = lui_Secure($args['old']);
    $query_one      = '';
    $data           = array();
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    $message_data   = array();
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data'] = lui_UserData($fetched_data['from_id']);
            $fetched_data['text']      = lui_Markup($fetched_data['text']);
            $fetched_data['text']      = lui_Emo($fetched_data['text']);
...
function lui_GetPageMessages($args = array()) {
    global $wo, $sqlConnect, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $offset         = lui_Secure($args['offset']);
    $id             = lui_Secure($args['id']);
    $page_id        = lui_Secure($args['page_id']);
    $limit          = lui_Secure($args['limit']);
    $new            = lui_Secure($args['new']);
    $old            = lui_Secure($args['old']);
    $query_one      = '';
    ...
    $page_data    = lui_PageData($page_id);
...
function lui_GetGroupMessagesAPP($args = array()) {
    global $wo, $sqlConnect,$db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data']    = lui_UserData($fetched_data['from_id']);
            $fetched_data['orginal_text'] = lui_EditMarkup($fetched_data['text']);
            $fetched_data['text']         = lui_Markup($fetched_data['text']);
            $fetched_data['text']         = lui_Emo($fetched_data['text']);
            $fetched_data['onwer']        = ($fetched_data['user_data']['user_id'] == $wo['user']['user_id']) ? 1 : 0;
            $fetched_data['reply']        = array();
...
function lui_GetMessagesHeader($data = array(), $type = '') {
    global $wo, $sqlConnect;
    if (empty($data['session_id'])) {
        if ($wo['loggedin'] == false) {
            return false;
    ...
    $user_id      = lui_Secure($data['user_id']);
    if (!empty($data['session_id'])) {
        $logged_user_id = lui_GetUserFromSessionID($data['session_id'], $data['platform']);
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data['text'])) {
            $fetched_data['text'] = lui_EditMarkup($fetched_data['text']);
        }
        $fetched_data['reaction'] = lui_GetPostReactionsTypes($fetched_data['id'], 'message');
        return $fetched_data;
    }
...
function lui_RegisterMessage($ms_data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    foreach ($matches[1] as $match) {
        $match         = lui_Secure($match);
        $match_user    = lui_UserData(lui_UserIdFromUsername($match));
        $match_search  = '@' . $match;
    ...
    if ($query) {
        ...
        $update_user_chats = lui_CreateUserChat($ms_data['to_id'], $from_id);
        return $message_id;
    } else {
        return false;
    }
}
function lui_RegisterMessageGroup($ms_data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_RegisterGroupMessage($ms_data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_RegisterPageMessage($ms_data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($query) {
        $message_id = mysqli_insert_id($sqlConnect);
        lui_CreateUserChat($ms_data['to_id'], $ms_data['from_id'], $ms_data['page_id']);
        return $message_id;
    } else {
        return false;
    }
}
function lui_CreateUserChat($user_id = 0, $from_id = 0, $page_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteConversation($user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeletePageConversation($user_id = 0, $page_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_DeleteGroupConversation($id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
            $deleteMessage = lui_DeleteMessage($sql_fetch_one['id'], '', $deleter_id);
        }
    }
    $query_one = mysqli_query($sqlConnect, "DELETE FROM " . T_U_CHATS . " WHERE `conversation_user_id` = '$user_id' AND `user_id` = '$my_id'");
    if ($query_one) {
...
function lui_DeleteMessage($message_id, $media = '', $deleter_id = 0) {
    global $wo, $sqlConnect;
    if (empty($deleter_id)) {
        if ($wo['loggedin'] == false) {
            return false;
    ...
    $message_id    = lui_Secure($message_id);
            ...
            if ($sql_fetch_one['deleted_one'] == 1 || $sql_fetch_one['deleted_two'] == 1) {
                ...
                if ($query) {
                    if (isset($sql_fetch_one['media']) and !empty($sql_fetch_one['media'])) {
                        ...
                        $delete_from_s3 = lui_DeleteFromToS3($sql_fetch_one['media']);
                    }
                    return true;
                } else {
                    return false;
...
function lui_CountMessages($data = array(), $type = '') {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $data['user_id'] = lui_Secure($data['user_id']);
    if ($type == 'interval') {
        $account = $wo['user'];
    } else {
        $account = lui_UserData($data['user_id']);
    }
    ...
    if (!empty($data['page_id']) && $data['page_id'] > 0) {
        $page_id = lui_Secure($data['page_id']);
...
function lui_SeenMessage($message_id) {
    global $sqlConnect;
    $message_id = lui_Secure($message_id);
    ...
    if (mysqli_num_rows($query)) {
        ...
        if ($fetched_data['seen'] > 0) {
            ...
            $data['seen'] = lui_Time_Elapsed_String($fetched_data['seen']);
            return $data;
        } else {
            return false;
        }
...
function lui_GetMessageButton($user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $account        = $wo['message'] = lui_UserData($user_id);
    if (!isset($account['user_id'])) {
        return false;
    }
    if ($account['message_privacy'] == 1) {
        if (lui_IsFollowing($logged_user_id, $user_id) === true) {
            return lui_LoadPage($message_button);
        }
...
function lui_MarkupAPI($text, $link = true, $hashtag = true, $mention = true, $post_id = 0) {
    global $sqlConnect;
    if ($mention == true) {
        $Orginaltext   = $text;
        $mention_regex = '/@\[([0-9]+)\]/i';
    ...
    if ($hashtag == true) {
        ...
        foreach ($matches[1] as $match) {
            ...
            $hashdata = lui_GetHashtag($hashkey);
            if (is_array($hashdata)) {
...
function lui_Markup($text, $link = true, $hashtag = true, $mention = true, $post_id = 0, $comment_id = 0, $reply_id = 0) {
    global $sqlConnect;
    if ($mention == true) {
        $Orginaltext   = $text;
        $mention_regex = '/@\[([0-9]+)\]/i';
        if (preg_match_all($mention_regex, $text ?? '', $matches)) {
            foreach ($matches[1] as $match) {
                ...
                $match_search = '@[' . $match . ']';
                if (isset($match_user['user_id'])) {
                    $match_replace = '<span class="user-popover" data-id="' . $match_user['id'] . '" data-type="' . $match_user['type'] . '"><a href="' . lui_SeoLink('index.php?link1=timeline&u=' . $match_user['username']) . '" class="hash" data-ajax="?link1=timeline&u=' . $match_user['username'] . '">' . $match_user['name'] . '</a></span>';
                    $text          = str_replace($match_search, $match_replace, $text);
                } else {
                    $match_replace = '';
                    $Orginaltext   = str_replace($match_search, $match_replace, $Orginaltext);
    ...
    if ($hashtag == true) {
        ...
        foreach ($matches[1] as $match) {
            ...
            $hashdata = lui_GetHashtag($hashkey);
            if (is_array($hashdata)) {
                $hashlink = '<a href="' . lui_SeoLink('index.php?link1=hashtag&hash=' . $hashdata['tag']) . '" class="hash">#' . $hashdata['tag'] . '</a>';
                $text     = str_replace($hashtag, $hashlink, $text);
            }
            $match_i++;
        }
...
function lui_EditMarkup($text, $link = true, $hashtag = true, $mention = true, $post_id = 0, $comment_id = 0, $reply_id = 0) {
    global $sqlConnect;
    if ($mention == true) {
        $Orginaltext   = $text;
        $mention_regex = '/@\[([0-9]+)\]/i';
...
function lui_UploadLogo($data = array()) {
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        ...
        if (lui_SaveConfig('logo_extension', $file_extension.'?cache='.rand(100,999))) {
            return true;
        }
    }
}
...
function lui_UploadBackground($data = array()) {
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        ...
        if (lui_SaveConfig('background_extension', $file_extension)) {
            return true;
        }
    }
}
...
function lui_UploadFavicon($data = array()) {
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        ...
        if (lui_SaveConfig('favicon_extension', $file_extension)) {
            return true;
        }
    }
}
...
function lui_DisplaySharedFile($media, $placement = '', $cache = false, $is_video = false) {
    ...
    if (!$is_video) {
        $wo['media']['filename'] = lui_GetMedia($media['filename']);
    }
    $wo['media']['video_thumb'] = ((!empty($media['postFileThumb'])) ? lui_GetMedia($media['postFileThumb']) : '');
    $wo['media']['name']        = lui_Secure($media['name']);
    ...
    if (!empty($wo['media']['filename'])) {
        ...
        if ($file_extension == 'mp3' || $file_extension == 'wav') {
            if ($placement == 'chat') {
                ...
                $media_file .= lui_LoadPage('players/chat-audio');
            } else if ($placement == 'record') {
                $media_file .= lui_LoadPage('players/audio');
            } else {
                $media_file .= lui_LoadPage('players/audio');
            }
        }
        ...
        if ($file_extension == 'mp4' || $file_extension == 'mkv' || $file_extension == 'avi' || $file_extension == 'webm' || $file_extension == 'mov' || $file_extension == 'm3u8' || $is_video) {
            if ($placement == 'message' || $placement == 'chat') {
                $media_file .= lui_LoadPage('players/chat-video');
            } else {
                $t_users    = T_USERS;
                ...
                if (!empty($video_ad)) {
                    ...
                    if ($video_ad->bidding == 'views') {
                        lui_RegisterAdConversionView($video_ad->id);
                    } else {
                        lui_RegisterAdView($video_ad->id);
                    }
                }
                $wo['story']['240p_video']  = '';
                $wo['story']['360p_video']  = '';
                ...
                if ($file_extension == 'm3u8') {
                    ...
                    $media_file .= lui_LoadPage('players/videojs');
                } else {
                    ...
                    $media_file .= lui_LoadPage('players/video');
                }
            }
        }
        $last_file_view = '';
...
function lui_IsAdmin($user_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_IsModerator($user_id = '') {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_CheckIfUserCanPost($num = 10) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_CheckIfUserCanRegister($num = 10) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == true) {
        return false;
    }
...
function lui_RegisterPost($re_data = array('recipient_id' => 0)) {
    global $wo, $sqlConnect;
    ...
    if ($re_data['user_id'] == $wo['user']['user_id']) {
        ...
        $re_data['user_id'] = lui_Secure($re_data['user_id']);
        $timeline           = lui_UserData($re_data['user_id']);
    }
    if ($timeline['user_id'] != $wo['user']['user_id'] && !lui_IsAdmin()) {
        return false;
    }
    if (!empty($re_data['page_id'])) {
        if (lui_IsPageOnwer($re_data['page_id']) === false && lui_UserCanPostPage($re_data['page_id']) === false) {
            return false;
        }
    }
    if (!empty($re_data['group_id'])) {
        if (lui_CanBeOnGroup($re_data['group_id']) === false) {
            return false;
        }
    }
    if (!lui_CheckIfUserCanPost($wo['config']['post_limit'])) {
        return false;
    }
    if (!empty($re_data['postText'])) {
        ...
        if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $re_data['postText'], $match)) {
            $re_data['postYoutube'] = lui_Secure($match[1]);
        ...
        if (lui_IsUrl($wo['config']['playtube_url']) && preg_match('#' . $playtube_root . '\/(?:watch|embed)\/(.*)#i', $re_data['postText'], $match)) {
            $re_data['postPlaytube'] = ((!empty($match[1])) ? lui_Secure($match[1]) : '');
            $is_there_video          = true;
        }
        if (lui_IsUrl($wo['config']['deepsound_url']) && preg_match('#' . $deepsound_root . '\/(?:track|embed)\/(.*)#i', $re_data['postText'], $match)) {
            $re_data['postDeepsound'] = ((!empty($match[1])) ? lui_Secure($match[1]) : '');
        }
        if (preg_match("#(?<=vine.co/v/)[0-9A-Za-z]+#", $re_data['postText'], $match)) {
            $re_data['postVine'] = lui_Secure($match[0]);
            $is_there_video      = true;
        }
        if (preg_match("#https?://vimeo.com/([0-9]+)#i", $re_data['postText'], $match)) {
            $re_data['postVimeo'] = lui_Secure($match[1]);
            $is_there_video       = true;
        }
        if (preg_match('#(http|https)://www.dailymotion.com/video/([A-Za-z0-9]+)#s', $re_data['postText'], $match)) {
            $re_data['postDailymotion'] = lui_Secure($match[2]);
            $is_there_video             = true;
        }
        if (preg_match('~([A-Za-z0-9]+)/videos/(?:t\.\d+/)?(\d+)~i', $re_data['postText'], $match)) {
            $re_data['postFacebook'] = lui_Secure($match[0]);
            $is_there_video          = true;
        }
        if (preg_match('~fb.watch\/(.*)~', $re_data['postText'], $match)) {
            $re_data['postFacebook'] = lui_Secure($match[1]);
            $is_there_video          = true;
        }
        ...
        if (preg_match('~https://www.facebook.com\/(.*)\/(.*)\/(?:t\.\d+/)?(\d+)~i', $re_data['postText'], $match) || preg_match('~https://fb.watch\/(.*)~', $re_data['postText'], $match)) {
            ...
            if (!empty($matches) && !empty($matches[0]) && !empty($matches[0][0])) {
                $re_data['postFacebook'] = lui_Secure($matches[0][0]);
                $is_there_video          = true;
            }
        }
    ...
    if (!empty($re_data['recipient_id']) && is_numeric($re_data['recipient_id']) && $re_data['recipient_id'] > 0) {
        ...
        $recipient = lui_UserData($re_data['recipient_id']);
        if (empty($recipient['user_id'])) {
            return false;
        }
        if (!empty($recipient['user_id'])) {
            if ($recipient['post_privacy'] == 'ifollow') {
                if (lui_IsFollowing($recipient['user_id'], $wo['user']['user_id']) === false) {
                    return false;
                }
    ...
    if (!empty($re_data['page_id'])) {
        if (lui_IsPageOnwer($re_data['page_id'])) {
            $re_data['user_id'] = 0;
        }
    }
    ...
    if ($query) {
        ...
        if (!empty($re_data['blog_id']) && $re_data['active'] == 1) {
            lui_RegisterPoint($post_id, "createblog");
        } else {
            if (isset($re_data['multi_image_post']) && $re_data['multi_image_post'] != 1 && empty($re_data['blog_id'])) {
                lui_RegisterPoint($post_id, "createpost");
            }
        }
        return $post_id;
    }
}
function lui_GetHashtag($tag = '', $type = true) {
    global $sqlConnect;
    $create = false;
    if (empty($tag)) {
        return false;
...
function lui_PublicacionData($post_id,$placement = '', $limited = '', $comments_limit = 0) {
    ...
    $data      = array();
    $post_id   = lui_Secure($post_id);
    $query_one = "SELECT * FROM " . T_POSTS . " WHERE `id_web` = '{$post_id}'";
    if ($wo['config']['post_approval'] == 1 && !lui_IsAdmin()) {
    ...
    if (!empty($fetched_data['page_id'])) {
        if (empty($fetched_data['user_id'])) {
            $fetched_data['publisher'] = lui_PageData($fetched_data['page_id']);
            $fetched_data['publisher']['banned'] = 0;
            $fetched_data['page_info'] = array();
        } else {
            $fetched_data['publisher'] = lui_UserData($fetched_data['user_id']);
            $fetched_data['page_info'] = lui_PageData($fetched_data['page_id']);
        }
    } else {
        $fetched_data['publisher'] = lui_UserData($fetched_data['user_id']);
    }
    if ($fetched_data['id'] == $fetched_data['post_id']) {
        ...
        if ($sql_query_two) {
            ...
            if (!empty($story['page_id'])) {
                $story['publisher'] = lui_PageData($story['page_id']);
            } else {
                $story['publisher'] = lui_UserData($story['user_id']);
            }
        } else {
            return false;
        }
    ...
    if (!empty($limited) && is_numeric($limited) && $limited > 0) {
        $story['limit_comments']   = lui_Secure($limited);
        $story['limited_comments'] = false;
    }
    ...
    if ($placement != 'admin') {
        if (!empty($story['group_id'])) {
            ...
            $story['group_recipient']        = lui_GroupData($story['group_id']);
            if ($story['group_recipient']['privacy'] == 2) {
                if ($wo['loggedin'] == true) {
                    if ($story['publisher']['user_id'] != $wo['user']['user_id']) {
                        if (lui_IsGroupOnwer($story['group_id']) === false) {
                            if (lui_IsGroupJoined($story['group_id']) === false && (!lui_IsAdmin() || lui_IsModerator())) {
                                return false;
                            }
                        }
                    }
        ...
        if ($story['postPrivacy'] == 1) {
            if ($wo['loggedin'] == true) {
                if (!empty($story['publisher']['page_id'])) {
                    ...
                    if ($story['publisher']['user_id'] != $wo['user']['user_id']) {
                        if (lui_IsFollowing($wo['user']['user_id'], $story['publisher']['user_id']) === false) {
                            return false;
                        }
                    }
                }
        ...
        if ($story['postPrivacy'] == 2) {
            if ($wo['loggedin'] == true) {
                if (!empty($story['publisher']['page_id'])) {
                    if ($story['publisher']['user_id'] != $wo['user']['user_id']) {
                        if (lui_IsPageLiked($story['publisher']['page_id'], $wo['user']['user_id']) === false) {
                            return false;
                        }
                    }
                } else {
                    if ($story['publisher']['user_id'] != $wo['user']['user_id']) {
                        if (lui_IsFollowing($story['publisher']['user_id'], $wo['user']['user_id']) === false && empty($story['group_id'])) {
                            return false;
                        }
                    }
                }
    ...
    $story['postText_API']     = lui_MarkupAPI($story['postText'], true, true, true, $story['post_id']);
    $story['postText_API']     = lui_Emo($story['postText_API']);
    $story['Orginaltext']      = lui_EditMarkup($story['postText'], true, true, true, $story['post_id']);
    $story['Orginaltext']      = str_replace('<br>', "\n", $story['Orginaltext'] ?? '');
    $story['postText']         = lui_Emo($story['postText']);
    $story['postText']         = lui_Markup($story['postText'], true, true, true, $story['post_id']);
    $story['post_time']        = lui_Time_Elapsed_String($story['time']);
    $story['page']             = 0;
    ...
    if ($story['recipient_id'] > 0) {
        ...
        $story['recipient']        = lui_UserData($story['recipient_id']);
    }
    $story['admin'] = false;
    if ($wo['loggedin'] == true) {
        if (!empty($story['page_id'])) {
            if (lui_IsPageOnwer($story['page_id'])) {
                $story['admin'] = true;
            }
        } else {
    ...
    $story['postLinkImage']    = lui_GetMedia($story['postLinkImage']);
    $story['is_post_pinned']   = (lui_IsPostPinned($story['id']) === true) ? true : false;
    if (!empty($comments_limit) && $comments_limit > 0) {
        $story['get_post_comments'] = lui_GetPostCommentsLimited($story['id'], $comments_limit);
    } else {
        $story['get_post_comments'] = ($story['comments_status'] == 1) ? lui_GetPostComments($story['id'], $story['limit_comments']) : array();
    }
    $story['photo_album'] = array();
    if (!empty($story['album_name'])) {
        $parent_id            = ($story['parent_id'] > 0) ? $story['parent_id'] : $story['id'];
        $story['photo_album'] = lui_GetAlbumPhotos($parent_id);
    }
    if ($story['boosted'] == 1) {
    ...
    if ($story['multi_image'] == 1) {
        $parent_id            = ($story['parent_id'] > 0) ? $story['parent_id'] : $story['id'];
        $story['photo_multi'] = lui_GetAlbumPhotos($parent_id);
    }
    if ($story['product_id'] > 0) {
        $story['product'] = lui_GetProduct($story['product_id']);
    }
    if ($story['page_event_id'] > 0) {
        $story['event'] = lui_EventData($story['page_event_id']);
    }
    if ($story['event_id'] > 0) {
        $story['event'] = lui_EventData($story['event_id']);
    }
    $story['options']  = array();
    $story['voted_id'] = 0;
    if ($story['poll_id'] == 1) {
    ...
    if ($wo['loggedin'] == true) {
        $story['post_share']       = lui_CountPostShare($story['id']);
        $story['post_comments']    = lui_CountPostComment($story['id']);
        $story['post_shares']      = lui_CountShares($story['id']);
        $story['post_likes']       = lui_CountLikes($story['id']);
        $story['post_wonders']     = lui_CountWonders($story['id']);
        $story['is_liked']         = (lui_IsLiked($story['id'], $wo['user']['user_id']) === true) ? true : false;
        $story['is_wondered']      = (lui_IsWondered($story['id'], $wo['user']['user_id']) === true) ? true : false;
        $story['is_post_saved']    = (lui_IsPostSaved($story['id'], $wo['user']['user_id']) === true) ? true : false;
        $story['is_post_reported'] = (lui_IsPostRepotred($story['id'], $wo['user']['user_id']) === true) ? true : false;
        if (lui_IsBlocked($story['user_id']) || lui_IsBlocked($story['recipient_id'])) {
            if (empty($story['group_id'])) {
                return false;
            }
        }
    ...
    $story['postFile_full'] = '';
    $story['shared_from']   = ($story['shared_from'] > 0) ? lui_UserData($story['shared_from']) : false;
    if (!empty($story['postFile'])) {
        $story['postFile_full'] = lui_GetMedia($story['postFile']);
    }
    if (!empty($story['postPhoto'])) {
        $story['postPhoto'] = lui_GetMedia($story['postPhoto']);
    }
    if (!empty($story['blog_id'])) {
        $story['blog'] = lui_GetArticle($story['blog_id']);
    }
    if ($wo['config']['second_post_button'] == 'reaction') {
        $story['reaction'] = lui_GetPostReactionsTypes($story['id']);
    }
    $story['job'] = array();
    if (!empty($story['job_id'])) {
        $story['job'] = lui_GetJobById($story['job_id']);
    }
    $story['offer'] = array();
    if (!empty($story['offer_id'])) {
        $story['offer'] = lui_GetOfferById($story['offer_id']);
    }
    $story['fund'] = array();
    ...
    if (!empty($story['forum_id'])) {
        $forum = lui_GetForumInfo($story['forum_id']);
    ...
    if (!empty($story['thread_id'])) {
        $thread = lui_GetForumThreads(array(
            "id" => $story['thread_id'],
            "preview" => true
        ));
    ...
    $after_post_id = lui_Secure($story['id']);
...
/////////// END PUBLICACION DATA
function lui_PostData($post_id, $placement = '', $limited = '', $comments_limit = 0) {
    global $wo, $sqlConnect, $cache, $db;
    ...
    if ($wo['config']['useSeoFrindly'] == 1) {
        $story['url']    = lui_SeoLink('index.php?link1=post&id=' . $story['id']) . '_' . lui_SlugPost($story['Orginaltext']);
        $story['seo_id'] = $story['id'] . '_' . lui_SlugPost($story['Orginaltext']);
    } else {
        $story['url']    = lui_SeoLink('index.php?link1=post&id=' . $story['id']);
        $story['seo_id'] = $story['id'];
    }
    $story['via_type'] = '';
    if ($story['id'] != $fetched_data['id'] && $story['user_id'] != $fetched_data['user_id']) {
...
function lui_CountPostShare($post_id) {
    global $wo, $sqlConnect;
    $data = array();
...
function lui_CountUserPosts($user_id) {
    global $wo, $sqlConnect;
    $data = array();
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
        return false;
...
function lui_IsPostOnwer($post_id, $user_id) {
    global $sqlConnect;
    if (empty($post_id) || !is_numeric($post_id) || $post_id < 0) {
        return false;
    }
...
function lui_GetPostPublisherBox($user_id = 0, $recipient_id = 0) {
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if ($user_id == $wo['user']['user_id']) {
        ...
        $user_id      = lui_Secure($user_id);
        $user_timline = lui_UserData($user_id);
    }
    ...
    $recipient_id = lui_Secure($recipient_id);
    if ($user_id == $recipient_id) {
        $recipient_id = 0;
    }
    if ($recipient_id > 0) {
        $recipient = lui_UserData($recipient_id);
        if (!isset($recipient['user_id'])) {
            return false;
        }
        if ($recipient['post_privacy'] == "ifollow") {
            if (lui_IsFollowing($wo['user']['user_id'], $recipient_id) === false) {
                $continue = false;
            }
    ...
    if ($continue == true) {
        ...
        return lui_LoadPage('story/publisher-box');
    }
}
function lui_GetPosts($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0)) {
    global $wo, $sqlConnect;
    ...
    if (!empty($data['after_post_id']) && is_numeric($data['after_post_id']) && $data['after_post_id'] > 0) {
        $data['after_post_id'] = lui_Secure($data['after_post_id']);
        ...
        $data['before_post_id'] = lui_Secure($data['before_post_id']);
    ...
    if (!empty($data['publisher_id']) && is_numeric($data['publisher_id']) && $data['publisher_id'] > 0) {
        $data['publisher_id'] = lui_Secure($data['publisher_id']);
        $Wo_publisher         = lui_UserData($data['publisher_id']);
    }
    if (!empty($data['page_id']) && is_numeric($data['page_id']) && $data['page_id'] > 0) {
        $data['page_id']   = lui_Secure($data['page_id']);
        $Wo_page_publisher = lui_PageData($data['page_id']);
    }
    if (!empty($data['group_id']) && is_numeric($data['group_id']) && $data['group_id'] > 0) {
        $data['group_id']   = lui_Secure($data['group_id']);
        $Wo_group_publisher = lui_GroupData($data['group_id']);
    }
    if (!empty($data['event_id']) && is_numeric($data['event_id']) && $data['event_id'] > 0) {
        $data['event_id']   = lui_Secure($data['event_id']);
        $Wo_event_publisher = lui_EventData($data['event_id']);
    }
    $multi_image_post = '';
    ...
    if (isset($Wo_publisher['user_id'])) {
        $user_id = lui_Secure($Wo_publisher['user_id']);
        ...
        $page_id = lui_Secure($Wo_page_publisher['page_id']);
        ...
        $group_id = lui_Secure($Wo_group_publisher['id']);
        ...
        $event_id = lui_Secure($Wo_event_publisher['id']);
    ...
    $limit   = lui_Secure($data['limit']);
    $last_ad = 0;
    if (!empty($data['ad-id'])) {
        $last_ad = $data['ad-id'];
    }
    if (isset($data['order'])) {
        $query_text .= " ORDER BY `id` " . lui_Secure($data['order']) . " LIMIT {$limit}";
    } else {
    ...
    if ($data['filter_by'] == 'most_liked') {
        ...
        if ((isset($data['after_post_id']) && $data['after_post_id'] > 0) && $data['lasttotal'] > 0 && $data['dt'] > 0) {
            $id    = lui_Secure($data['after_post_id']);
            $total = lui_Secure($data['lasttotal']);
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            if ($filter !== 'most_liked') {
                $post = lui_PostData($fetched_data['id']);
                if (is_array($post)) {
                    $data[] = $post;
                }
            } else {
    ...
    if ($filter !== 'most_liked' && $filter !== 'job') {
        if (is_numeric($last_ad) && count($data) > 1) {
            $ad = lui_GetPostAds(lui_Secure($last_ad));
            if (is_array($ad) && !empty($ad)) {
                if ($ad['bidding'] == 'views') {
                    lui_RegisterAdConversionView($ad['id']);
                }
                $data[] = $ad;
            }
        }
...
function lui_DeletePost($post_id = 0, $type = '') {
    global $wo, $sqlConnect, $cache;
    ...
    if ($is_me > 0 || (lui_IsAdmin() || lui_IsModerator()) || $type == 'shared') {
        ...
        //     $delete_from_s3 = lui_DeleteFromToS3($media_2);
        //     $delete_from_s3 = lui_DeleteFromToS3($post_image->image);
        // }
        // delete shared posts
        ...
        //}
        // delete shared posts
        $is_this_post_shared = lui_IsThisPostShared($post_id);
        $is_post_shared      = lui_IsPostShared($post_id);
        //$fetched_data = mysqli_fetch_assoc($query);
            ...
            if (!empty($user_pic)) {
                if ($fetched_data['postType'] == 'profile_picture' || $fetched_data['postType'] == 'profile_picture_deleted') {
                    ...
                    if ($user_pic['avatar'] != $wo['userDefaultAvatar'] && $user_pic['avatar'] != $wo['userDefaultFAvatar']) {
                        ...
                        lui_DeleteFromToS3($explode3[0] . '_full.' . $explode2);
                        if (file_exists($user_pic['avatar'])) {
                            @unlink($user_pic['avatar']);
                        }
                        lui_DeleteFromToS3($user_pic['avatar']);
                    }
                }
                if ($fetched_data['postType'] == 'profile_cover_picture' || $fetched_data['postType'] == 'profile_picture_deleted') {
                    ...
                    if ($user_pic['cover'] != $wo['userDefaultCover']) {
                        ...
                        lui_DeleteFromToS3($user_pic['cover']);
                    }
                }
            }
        } */
        ...
        if (!empty($fetched_data['postText'])) {
            ...
            foreach ($matches[1] as $match) {
                ...
                $hashdata = lui_GetHashtag($hashkey);
                if (is_array($hashdata)) {
                    $hash_id          = lui_Secure($hashdata['id']);
        ...
        if (!empty($fetched_data['blog_id']) && $fetched_data['blog_id'] > 0) {
            //lui_DeleteMyBlog($fetched_data['blog_id']);
        }
        if (isset($fetched_data['postFile']) && !empty($fetched_data['postFile'])) {
            if ($fetched_data['postType'] != 'profile_picture' && $fetched_data['postType'] != 'profile_cover_picture' && !$is_post_shared && !$is_this_post_shared) {
                ...
                $delete_from_s3 = lui_DeleteFromToS3($fetched_data['postFile']);
        ...
        if (!empty($fetched_data['postFileThumb']) && !$is_post_shared && !$is_this_post_shared) {
            if (file_exists($fetched_data['postFileThumb'])) {
                ...
                @lui_DeleteFromToS3($fetched_data['postFileThumb']);
            }
        }
        if (!empty($fetched_data['postRecord']) && !$is_post_shared && !$is_this_post_shared) {
            if (file_exists($fetched_data['postRecord'])) {
                ...
                @lui_DeleteFromToS3($fetched_data['postRecord']);
            }
        }
        if (isset($fetched_data['postLinkImage']) && !empty($fetched_data['postLinkImage']) && !$is_post_shared && !$is_this_post_shared) {
            ...
            $delete_from_s3 = lui_DeleteFromToS3($fetched_data['postLinkImage']);
        }
        ...
        if (mysqli_num_rows($query_two_2)) {
            while ($fetched_data = mysqli_fetch_assoc($query_two_2)) {
                lui_DeletePostComment($fetched_data['id']);
            }
        }
        $product    = lui_PostData($post_id);
        ...
        if ($is_me > 0 || (lui_IsAdmin() || lui_IsModerator())) {  
            @lui_RegisterPoint($post_id, "createpost", "-", $fetched_data['user_id']);
        }
...
function lui_DeleteGame($game_id) {
    global $wo, $sqlConnect, $cache;
    ...
    $user_id = lui_Secure($wo['user']['user_id']);
    if (lui_IsAdmin($user_id) === false) {
        return false;
    }
    $game_id      = lui_Secure($game_id);
...
function lui_DeleteGift($gift_id) {
    global $wo, $sqlConnect, $cache;
...
function lui_DeleteSticker($sticker_id) {
    global $wo, $sqlConnect, $cache;
    ...
    $sticker_id   = lui_Secure($sticker_id);
...
function lui_GetUserIdFromPostId($post_id = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_GetPinnedPost($user_id, $type = '') {
    global $sqlConnect, $wo;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            $post = lui_PostData($fetched_data['post_id']);
            if (is_array($post)) {
                $data[] = $post;
            }
        }
...
function lui_IsPostPinned($post_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_PinPost($post_id = 0, $type = '', $id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (lui_PostExists($post_id) === false) {
        return false;
    }
    if (lui_IsPostOnwer($post_id, $user_id) === false) {
        return false;
    }
    if ($type == 'page') {
        if (lui_IsPageOnwer($id) === false) {
            return false;
        }
        $where_delete_query = " WHERE `page_id` = {$id} AND `active` = '1'";
        $where_insert_query = " (`page_id`, `post_id`, `active`) VALUES ({$id}, {$post_id}, '1')";
    } else if ($type == 'group') {
        if (lui_IsGroupOnwer($id) === false) {
            return false;
        }
    ...
    if (lui_IsPostPinned($post_id)) {
        $query_two = mysqli_query($sqlConnect, $query_text);
        return 'unpin';
    } else {
        if (lui_IsUserPinned($id, $type)) {
            $query_two = mysqli_query($sqlConnect, $query_text);
            $continue  = true;
        } else {
            $continue = true;
...
function lui_BoostPost($post_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_IsPostBoosted($post_id) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_RegisterActivity($data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (empty($data['comment_id']) || !is_numeric($data['comment_id']) || $data['comment_id'] < 1) {
        ...
        $comment_id = lui_Secure($data['comment_id']);
    }
    $replay_id = 0;
    if (empty($data['reply_id']) || !is_numeric($data['reply_id']) || $data['reply_id'] < 1) {
        $replay_id = 0;
    } else {
        $replay_id = lui_Secure($data['reply_id']);
    }
    @$post_id = lui_Secure($data['post_id']);
    @$user_id = lui_Secure($data['user_id']);
    @$post_user_id = lui_Secure($data['post_user_id']);
    @$activity_type = lui_Secure($data['activity_type']);
    $time = time();
    ...
    $query_insert = "INSERT INTO " . T_ACTIVITIES . " (`user_id`, `post_id`,`comment_id`,`reply_id`, `activity_type`, `time`) VALUES ('{$user_id}', ".
        (($post_id=='')?"0":("'".$post_id."'")) . ", '{$comment_id}','{$replay_id}','{$activity_type}', '{$time}')";
    if (lui_IsActivity($post_id, $comment_id, $replay_id, $user_id, $activity_type) === true) {
...
function lui_IsActivity($post_id, $comment_id, $replay_id, $user_id, $activity_type) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $query = mysqli_query($sqlConnect, "SELECT `id` FROM " . T_ACTIVITIES . " WHERE `user_id` = '{$user_id}' AND ( `post_id` = '{$post_id}' OR `comment_id` = '{$comment_id}' OR `reply_id` = '{$replay_id}') AND `activity_type` = '{$activity_type}'");
    return (mysqli_num_rows($query) > 0) ? true : false;
}
function lui_DeleteSelectedActivity($user_id, $activity_type) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $query = mysqli_query($sqlConnect, "DELETE FROM " . T_ACTIVITIES . " WHERE `user_id` = '{$user_id}' AND `activity_type` = '{$activity_type}'");
    return ($query) ? true : false;
}
function lui_DeleteActivity($post_id, $user_id, $activity_type) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetActivity($id) {
    ...
    if (mysqli_num_rows($query) == 1) {
        ...
        $finel_fetched_data['postData']  = lui_PostData($finel_fetched_data['post_id']);
        $finel_fetched_data['activator'] = lui_UserData($finel_fetched_data['user_id']);
        return $finel_fetched_data;
    }
    return false;
}
function lui_GetActivities($data = array('after_activity_id' => 0, 'before_activity_id' => 0, 'limit' => 5, 'me' => false)) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (!empty($data['after_activity_id']) && is_numeric($data['after_activity_id']) && $data['after_activity_id'] > 0) {
        $data['after_activity_id'] = lui_Secure($data['after_activity_id']);
        ...
        $data['before_activity_id'] = lui_Secure($data['before_activity_id']);
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            if (is_array($fetched_data)) {
                $get[] = lui_GetActivity($fetched_data['id']);
            }
        }
    }
    return $get;
}
function lui_DeleteReactions($post_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $post_id        = lui_Secure($post_id);
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    if (lui_IsReacted($post_id, $wo['user']['user_id']) == true) {
        lui_RegisterPoint($post_id, "reaction", '-');
...
function lui_DeleteCommentReactions($comment_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $comment_id     = lui_Secure($comment_id);
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    if (lui_IsReacted($comment_id, $wo['user']['user_id'], "comment") == true) {
...
function lui_DeleteReplayReactions($replay_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $replay_id      = lui_Secure($replay_id);
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    if (lui_IsReacted($replay_id, $wo['user']['user_id'], "replay") == true) {
...
function lui_AddReactions($post_id, $reaction) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $post_id        = lui_Secure($post_id);
    $user_id        = lui_GetUserIdFromPostId($post_id);
    $page_id        = 0;
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    $post           = lui_PostData($post_id);
    $text           = 'post';
    $type2          = lui_Secure($reaction);
    if (empty($user_id)) {
        $user_id = lui_GetUserIdFromPageId($post['page_id']);
        if (empty($user_id)) {
            return false;
        }
    }
    if (lui_IsReacted($post_id, $wo['user']['user_id']) == true) {
        ...
        $delete_activity  = lui_DeleteActivity($post_id, $logged_user_id, 'reaction');
        $sql_query_one    = mysqli_query($sqlConnect, $query_one);
...
function lui_AddReplayReactions($user_id, $reply_id, $reaction) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $reply_id       = lui_Secure($reply_id);
    $page_id        = 0;
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    $comment        = lui_GetCommentIdFromReplyId($reply_id);
    $post_id        = lui_GetPostIdFromCommentId($comment);
    $text           = 'replay';
    $type2          = $reaction;
    if (empty($user_id)) {
        return false;
    }
    if (lui_IsReacted($reply_id, $wo['user']['user_id'], "replay") == true) {
        ...
        $delete_activity  = lui_DeleteActivity($reply_id, $logged_user_id, 'reaction');
        $sql_query_one    = mysqli_query($sqlConnect, $query_one);
        //Register point level system for reaction
        //lui_RegisterPoint($post_id, "reaction" , "-");
    }
    ...
    $sql_query_two = mysqli_query($sqlConnect, $query_two);
    if ($sql_query_two) {
        $post_data = lui_PostData($post_id);
...
function lui_AddCommentReactions($comment_id, $reaction) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $comment_id     = lui_Secure($comment_id);
    $user_id        = lui_GetUserIdFromCommentId($comment_id);
    $page_id        = 0;
    $logged_user_id = lui_Secure($wo['user']['user_id']);
    $post_id        = lui_GetPostIdFromCommentId($comment_id);
    $text           = 'comment';
    $type2          = $reaction;
    if (empty($user_id)) {
        return false;
    }
    if (lui_IsReacted($comment_id, $logged_user_id, "comment") == true) {
        $query_one        = "DELETE FROM " . T_REACTIONS . " WHERE `comment_id` = {$comment_id} AND `user_id` = {$logged_user_id}";
        ...
        $delete_activity  = lui_DeleteActivity($comment_id, $logged_user_id, 'reaction');
        $sql_query_one    = mysqli_query($sqlConnect, $query_one);
        //Register point level system for reaction
        //lui_RegisterPoint($post_id, "reaction" , "-");
    }
...
function lui_IsReacted($object_id, $user_id, $col = "post", $type = '') {
    global $sqlConnect;
    ...
    $object_id = lui_Secure($object_id);
    if ($type == 'blog') {
...
function lui_GetReactedTextIcon($object_id, $user_id, $col = "post") {
    global $sqlConnect, $wo;
    if (empty($object_id) or !is_numeric($object_id) or $object_id < 1) {
        return false;
    }
...
function lui_CountReactions($object_id, $reaction, $col = "post") {
    global $sqlConnect;
    if (empty($object_id) or !is_numeric($object_id) or $object_id < 1) {
        return false;
    }
...
function lui_GetPostReactions($object_id, $col = "post", $type = '') {
    global $sqlConnect, $wo;
    if (empty($object_id) or !is_numeric($object_id) or $object_id < 1) {
        return false;
    }
    ...
    if (!empty($reactions)) {
        foreach ($reactions as $key => $val) {
            ...
            if (!file_exists('./datos/modulos/' . $wo['config']['theme'] . '/reaction/like-sm.png')) {
                if ($wo['reactions_types'][$key]['is_html'] == 1) {
                    ...
                    if (!empty($wo['reactions_types'][$key]['layshane_star_small_icon'])) {
                        $reactions_html .= $first . "<div class='inline_post_count_emoji reaction'><img src='{$wo['reactions_types'][$key]['layshane_star_small_icon']}' alt=\"" . $wo['reactions_types'][$key]['name'] . "hola mundo\"></div></span>";
                    }
                }
            } else {
                if (!empty($wo['reactions_types'][$key]['layshane_star_small_icon'])) {
                    $reactions_html .= $first . "<div class='inline_post_count_emoji'><img src='{$wo['reactions_types'][$key]['layshane_star_small_icon']}' alt=\"" . $wo['reactions_types'][$key]['name'] . "\"></div></span>";
                }
                // switch (strtolower($key)) {
                //     case 1:
...
function lui_GetPostReactionsTypes($object_id, $col = "post", $type = "post") {
    global $sqlConnect, $wo;
    if (empty($object_id) or !is_numeric($object_id) or $object_id < 1) {
        return false;
    }
...
function lui_AddLikes($post_id) {
    ...
    if (lui_IsLiked($post_id, $wo['user']['user_id']) === true) {
        ...
        $delete_activity  = lui_DeleteActivity($post_id, $logged_user_id, 'liked_post');
        $sql_query_one    = mysqli_query($sqlConnect, $query_one);
        if ($sql_query_one) {
            //Register point level system for unlikes
            lui_RegisterPoint($post_id, "likes", "-");
            return 'unliked';
        }
    } else {
        if ($wo['config']['second_post_button'] == 'dislike' && lui_IsWondered($post_id, $wo['user']['user_id'])) {
            lui_AddWonders($post_id);
        }
...
function lui_CountLikes($post_id) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_IsLiked($post_id, $user_id) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_IsUserPostReacted($post_id, $user_id) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_IsCommented($post_id, $user_id) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_AddWonders($post_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (lui_IsWondered($post_id, $logged_user_id) === true) {
        ...
        $delete_activity  = lui_DeleteActivity($post_id, $logged_user_id, 'wondered_post');
        $sql_query_one    = mysqli_query($sqlConnect, $query_one);
        if ($sql_query_one) {
            if ($wo['config']['second_post_button'] == 'dislike') {
                //Register point level system for dislikes -
                lui_RegisterPoint($post_id, "dislikes", "-");
            } else if ($wo['config']['second_post_button'] == 'wonder') {
                //Register point level system for wonders -
                lui_RegisterPoint($post_id, "wonders", "-");
            }
            return 'unwonder';
        }
    } else {
        if ($wo['config']['second_post_button'] == 'dislike' && lui_IsLiked($post_id, $wo['user']['user_id'])) {
            lui_AddLikes($post_id);
        }
...
function lui_CountWonders($post_id) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_IsWondered($post_id, $user_id) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_GetPostLikes($post_id = 0, $limit = 20, $offset = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_GetPostCommentLikes($comment_id = 0, $limit = 20, $offset = 0) {
    global $sqlConnect;
...
function lui_GetPostCommentReplyLikes($reply_id = 0, $limit = 20, $offset = 0) {
    global $sqlConnect;
...
function lui_GetPostCommentWonders($comment_id = 0, $limit = 20, $offset = 0) {
    global $sqlConnect;
    if (empty($comment_id) or !is_numeric($comment_id) or $comment_id < 1) {
        return false;
    }
...
function lui_GetPostCommentReplyWonders($reply_id = 0, $limit = 20, $offset = 0) {
    global $sqlConnect;
    if (empty($reply_id) or !is_numeric($reply_id) or $reply_id < 1) {
        return false;
    }
...
function lui_GetPostShared($post_id = 0, $limit = 20, $offset = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            if (!empty($fetched_data['page_id'])) {
                $page                = lui_PageData($fetched_data['page_id']);
                $user_data           = lui_UserData($fetched_data['user_id']);
                $user_data['row_id'] = $fetched_data['id'];
                $data[]              = $user_data;
            } else {
...
function lui_GetPostReactionUsers($post_id = 0, $type = "1", $limit = 20, $offset = 0, $col = 'post') {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            ...
            $ud             = lui_UserData($fetched_data['user_id']);
...
function lui_GetPostWonders($post_id = 0, $limit = 20, $offset = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_AddShare($post_id = 0) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] !== true) {
        return false;
    }
    ...
    if (lui_IsShared($post_id, $logged_user_id)) {
        ...
        $delete_activity  = lui_DeleteActivity($post_id, $logged_user_id, 'shared_post');
        $sql_query_one    = mysqli_query($sqlConnect, $query_one);
        if ($sql_query_one) {
            return 'unshare';
        }
...
function lui_CountShares($post_id = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_IsShared($post_id, $user_id) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
...
function lui_RegisterPostComment($data = array()) {
    ...
    if (!empty($data['page_id'])) {
        if (lui_IsPageOnwer($data['page_id']) === false) {
            $data['page_id'] = 0;
        }
    }
    $getPost = lui_PostData($data['post_id']);
    ...
    $post    = lui_PostData($data['post_id']);
    $text    = '';
    $type2   = '';
    $page_id = 0;
    ...
    $user_id = lui_GetUserIdFromPostId($data['post_id']);
    if (empty($user_id)) {
        $user_id = lui_GetUserIdFromPageId($post['page_id']);
        if (empty($user_id)) {
            return false;
    ...
    if ($query) {
        ...
        if ($getPost['user_id'] != $wo['user']['id']) {
            lui_RegisterPoint(lui_Secure($data['post_id']), "comments");
        }
        return $inserted_comment_id;
    }
}
function lui_GetGroupsListAPP($fetch_array = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $sql   = "SELECT * FROM " . T_GROUP_CHAT . "
                WHERE (`user_id` = {$user} OR `group_id` IN
                   (SELECT `group_id` FROM lui_groupchatusers  WHERE `user_id` = {$user} AND `active` = 1)) {$offset_query}  ORDER BY `time` DESC LIMIT {$limit}";
    $query = mysqli_query($sqlConnect, $sql);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data']    = lui_UserData($fetched_data['user_id']);
            $fetched_data['owner']        = ($fetched_data['user_id'] == $user) ? true : false;
            $fetched_data['last_message'] = lui_GetChatGroupLastMessage($fetched_data['group_id']);
            $fetched_data['parts']        = lui_GetGChatMemebers($fetched_data['group_id']);
            $fetched_data['avatar']       = lui_GetMedia($fetched_data['avatar']);
            $fetched_data['last_seen']    = lui_CheckLastGroupAction();
...
function lui_GetPostCommentsSort($post_id = 0, $limit = 5, $type = 'latest') {
    global $sqlConnect, $wo;
    if (empty($post_id) || !is_numeric($post_id) || $post_id < 0) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            $data[] = lui_GetPostComment($fetched_data['id']);
        }
    }
    return $data;
}
function lui_GetPostCommentsLimited($post_id = 0, $comment_id = 0) {
    global $sqlConnect, $wo;
    if (empty($post_id) || !is_numeric($post_id) || $post_id < 0) {
        return false;
    }
...
function lui_GetPostComments($post_id = 0, $limit = 5, $offset = 0) {
    global $sqlConnect, $wo;
    if (empty($post_id) || !is_numeric($post_id) || $post_id < 0) {
        return false;
    }
    ...
    if (($comments_num = lui_CountPostComment($post_id)) > $limit) {
...
// API
function lui_GetPostCommentsAPI($post_id = 0, $limit = 5, $offset = 0) {
    global $sqlConnect, $wo;
    if (empty($post_id) || !is_numeric($post_id) || $post_id < 0) {
        return false;
    }
...
function lui_GetCommentRepliesAPI($comment_id = 0, $limit = 5, $order_by = 'ASC', $offset = 0) {
    global $sqlConnect, $wo;
    ...
    if (($comments_num = lui_CountCommentReplies($comment_id)) > $limit) {
        $query .= " LIMIT {$limit} ";
    }
    $query_one = mysqli_query($sqlConnect, $query);
    if (mysqli_num_rows($query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
            $data[] = lui_GetCommentReply($fetched_data['id']);
        }
    }
    return $data;
}
// API
function lui_GetPostComment($comment_id = 0) {
    global $wo, $sqlConnect;
    if (empty($comment_id) || !is_numeric($comment_id) || $comment_id < 0) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_one)) {
        ...
        if (!empty($fetched_data['page_id'])) {
            $fetched_data['publisher'] = lui_PageData($fetched_data['page_id']);
            $fetched_data['url']       = lui_SeoLink('index.php?link1=timeline&u=' . $fetched_data['publisher']['page_name']);
            if ($fetched_data['publisher']['user_id'] != $fetched_data['user_id'] && !lui_IsPageAdminExists($fetched_data['user_id'], $fetched_data['page_id'])) {
                $fetched_data['publisher'] = lui_UserData($fetched_data['user_id']);
                $fetched_data['url']       = lui_SeoLink('index.php?link1=timeline&u=' . $fetched_data['publisher']['username']);
            }
        } else {
            $fetched_data['publisher'] = lui_UserData($fetched_data['user_id']);
            $fetched_data['url']       = lui_SeoLink('index.php?link1=timeline&u=' . $fetched_data['publisher']['username']);
        }
        $fetched_data['fullurl']             = lui_SeoLink("index.php?link1=post&id=" . $fetched_data['post_id'] . "&ref=" . $comment_id);
        $fetched_data['Orginaltext']         = lui_EditMarkup($fetched_data['text'], true, true, true, 0, $comment_id);
        $fetched_data['Orginaltext']         = str_replace('<br>', "\n", $fetched_data['Orginaltext']);
        $fetched_data['text']                = lui_Markup($fetched_data['text'], true, true, true, 0, $comment_id);
        $fetched_data['text']                = lui_Emo($fetched_data['text']);
        $fetched_data['onwer']               = false;
        $fetched_data['post_onwer']          = false;
        $fetched_data['comment_likes']       = lui_CountCommentLikes($fetched_data['id']);
        $fetched_data['comment_wonders']     = lui_CountCommentWonders($fetched_data['id']);
        ...
        if ($wo['loggedin'] == true) {
            ...
            $fetched_data['post_onwer']          = (lui_IsPostOnwer($fetched_data['post_id'], $wo['user']['user_id'])) ? true : false;
            $fetched_data['is_comment_wondered'] = (lui_IsCommentWondered($fetched_data['id'], $wo['user']['user_id'])) ? true : false;
            $fetched_data['is_comment_liked']    = (lui_IsCommentLiked($fetched_data['id'], $wo['user']['user_id'])) ? true : false;
        }
        if ($wo['config']['second_post_button'] == 'reaction') {
            $fetched_data['reaction'] = lui_GetPostReactionsTypes($fetched_data['id'], 'comment');
        }
        $fetched_data['replies_count'] = lui_CountCommentReplies($fetched_data['id']);
        return $fetched_data;
    }
    return false;
}
function lui_CountPostComment($post_id = '') {
    global $sqlConnect;
    if (empty($post_id) || !is_numeric($post_id) || $post_id < 0) {
        return false;
    }
...
function lui_CountUserPostComment($post_id = '', $user_id = '') {
    global $sqlConnect;
    if (empty($post_id) || !is_numeric($post_id) || $post_id < 0) {
        return false;
    }
...
function lui_DeletePostComment($comment_id = '') {
    global $wo, $sqlConnect;
    ...
    if (mysqli_num_rows($query_one) > 0 || lui_IsPostOnwer($post_id, $logged_user_id) === true || lui_IsAdmin()) {
        if ($query_one) {
        ...
        if (mysqli_num_rows($query_one) > 0) {
            lui_RegisterPoint($post_id, "comments", "-");
        }
        ...
        if ($query_delete) {
            ...
            if ($query_two) {
                while ($fetched_data = mysqli_fetch_assoc($query_two)) {
                    lui_DeleteCommentReply($fetched_data['id']);
                }
            }
            $delete_activity = lui_DeleteActivity($post_id, $logged_user_id, 'commented_post');
...
function lui_DeletePostReplyComment($comment_id = '') {
    global $wo, $sqlConnect;
    if ($comment_id < 0 || empty($comment_id) || !is_numeric($comment_id)) {
        return false;
    }
    ...
    if (mysqli_num_rows($query_one) > 0 || lui_IsAdmin()) {
        if ($query_one) {
            $query_img = mysqli_fetch_assoc($query_one);
            if (!empty($query_img['c_file'])) {
                @unlink($query_img['c_file']);
...
function lui_UpdateComment($data = array()) {
    global $wo, $sqlConnect;
    ...
    $user_id      = lui_Secure($wo['user']['user_id']);
    $comment_id   = lui_Secure($data['comment_id']);
    $comment_text = lui_Secure($data['text'],1);
            ...
            if (isset($mentions) && is_array($mentions)) {
                foreach ($mentions as $mention) {
                    $notification_data_array = array(
                        ...
                        'type' => 'comment_mention',
                        'page_id' => $page_id,
                        'post_id' => lui_GetPostIdFromCommentId($data['comment_id']),
                        'url' => 'index.php?link1=post&id=' . lui_GetPostIdFromCommentId($data['comment_id'])
                    );
                    lui_RegisterNotification($notification_data_array);
                }
            }
...
function lui_UpdatePostPrivacy($data = array()) {
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $privacy_type = lui_Secure($data['privacy_type']);
    $user_id      = lui_Secure($wo['user']['user_id']);
    $post_id      = lui_Secure($data['post_id']);
    if (lui_IsPostOnwer($post_id, $user_id) === false) {
        return false;
...
function lui_UpdatePost($data = array()) {
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $post_text = lui_Secure($data['text'],1);
    $user_id   = lui_Secure($wo['user']['user_id']);
    $post_id   = lui_Secure($data['post_id']);
    if (lui_IsPostOnwer($post_id, $user_id) === false) {
        return false;
    ...
    if ($query_one) {
        ...
        if (mysqli_num_rows($query)) {
            $fetched_data             = mysqli_fetch_assoc($query);
            $fetched_data['postText'] = lui_Markup($fetched_data['postText']);
            $fetched_data['postText'] = lui_Emo($fetched_data['postText']);
            return $fetched_data['postText'];
        }
        return false;
    }
}
function lui_SavePosts($post_data = array()) {
    global $wo, $sqlConnect;
    if (empty($post_data)) {
        return false;
    }
    $user_id = lui_Secure($wo['user']['user_id']);
    $post_id = lui_Secure($post_data['post_id']);
    if (lui_IsPostSaved($post_id, $user_id)) {
...
function lui_GetChatColor($user_id = 0, $conversation_user_id = 0, $page_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $user_id              = lui_Secure($user_id);
    $conversation_user_id = lui_Secure($conversation_user_id);
    ...
    if (mysqli_num_rows($sql_queryset)) {
        ...
        if (file_exists('./datos/modulos/' . $wo['config']['theme'] . '/reaction/like-sm.png') && empty($fetched_data['color'])) {
            $color = '';
        }
        return $color;
    }
...
function lui_UpdateChatColor($user_id = 0, $conversation_user_id = 0, $color = '', $page_id = 0) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $user_id              = lui_Secure($user_id);
    $conversation_user_id = lui_Secure($conversation_user_id);
    $color                = lui_Secure($color);
    $set_color_query      = "";
    if (!empty($page_id)) {
        $page_id = lui_Secure($page_id);
        $page    = lui_PageData($page_id);
    ...
    if (mysqli_num_rows($query_one)) {
        ...
        if ($query_one_fetch['count'] == 0) {
            if (!empty($page_id)) {
                $update_ = lui_CreateUserChat($conversation_user_id, $user_id, $page_id);
            } else {
                $update_ = lui_CreateUserChat($conversation_user_id, $user_id);
            }
        }
    }
...
function lui_ProfileCompletion() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetLastAttachments($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (mysqli_num_rows($sql_query)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
            $data[] = lui_GetMedia($fetched_data['media']);
        }
    }
    return $data;
}
function lui_GetMessagesPagesAPP($fetch_array = array()) {
    global $wo, $sqlConnect;
    if (empty($fetch_array['session_id'])) {
        if ($wo['loggedin'] == false) {
            return false;
...
function lui_AddCommentBlogReactions($comment_id, $reaction) {
    global $wo, $sqlConnect, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_AddBlogReplyReactions($user_id, $reply_id, $reaction) {
    global $wo, $sqlConnect, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function lui_GetMessagesAPPN($data = array(), $limit = 50) {
    global $wo, $sqlConnect, $db;
    $message_data   = array();
    $user_id        = lui_Secure($data['recipient_id']);
    $logged_user_id = lui_Secure($data['user_id']);
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
    }
    $query_one = " SELECT * FROM " . T_MESSAGES;
...
function lui_ReplaceText($html = '', $replaces = array()) {
    global $wo;
    $lang = $wo['lang'];
...
function FilterStripTags(string $string):string{
...
function filterFiles($results, $storage) {
    ...
    foreach ($results as $key => $fileName) {
        $checkIfFileExistsInUpload = $db->where('filename', lui_Secure($fileName))->where('storage', $storage)->getOne(T_UPLOADED_MEDIA);
...
function getStatus($config = array()) {
    ...
    if(!function_exists('mime_content_type')) {
        $errors[] = ["type" => "error", "message" => "PHP <strong>FileInfo</strong> extensión está deshabilitada en su servidor, se requiere que esté habilitada."];
    }
    if (!class_exists('DOMDocument')) {
        $errors[] = ["type" => "error", "message" => "PHP <strong>dom & xml</strong> las extensiones están deshabilitadas en su servidor, es necesario que estén habilitadas."];
    }
    if (!is_writable('./upload')) {
        $errors[] = ["type" => "error", "message" => "La carpeta: <strong>/upload</strong> no se puede escribir, la carpeta de carga y el permiso de todas las subcarpetas deben establecerse en <strong>777</strong>."];
    }
    if (!is_writable('./xml')) {
        $errors[] = ["type" => "error", "message" => "La carpeta: <strong>/xml</strong> no se puede escribir, el permiso de la carpeta xml debe establecerse en <strong>777</strong>."];
    }
    if ($wo['config']['amazone_s3'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['backblaze_storage'] == 1) {
        if (!is_writable('./upload/photos/d-avatar.jpg')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/d-avatar.jpg</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>.<br> Also make sure the file exists."];
        }
        if (!is_writable('./upload/photos/app-default-icon.png')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/app-default-icon.png</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>. <br> Also make sure the file exists."];
        }
        if (!is_writable('./upload/photos/d-blog.jpg')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/d-blog.jpg</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>.<br> Also make sure the file exists."];
        }
        if (!is_writable('./upload/photos/d-cover.jpg')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/d-cover.jpg</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>.<br> Also make sure the file exists."];
        }
        if (!is_writable('./upload/photos/d-film.jpg')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/d-film.jpg</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>.<br> Also make sure the file exists."];
        }
        if (!is_writable('./upload/photos/d-group.jpg')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/d-group.jpg</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>.<br> Also make sure the file exists."];
        }
        if (!is_writable('./upload/photos/d-page.jpg')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/d-page.jpg</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>.<br> Also make sure the file exists."];
        }
        if (!is_writable('./upload/photos/game-icon.png')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/game-icon.png</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>.<br> Also make sure the file exists."];
        }
        if (!is_writable('./upload/photos/incognito.png')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>./upload/photos/incognito.png</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>.<br> Also make sure the file exists."];
        }
    }

    if ($wo['config']['ffmpeg_system'] == 'on') {
        if (!isfuncEnabled("shell_exec")) {
            $errors[] = ["type" => "error", "message" => "La funcion: <strong>shell_exec</strong> no está habilitado, comuniquese con su proveedor de alojamiento para habilitarlo, es necesario para <strong>FFMPEG</strong>."];
        }
        if (!is_writable('./ffmpeg/ffmpeg')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>/ffmpeg/ffmpeg</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>."];
        }
    }
    

    if (!is_writable('./sitemap.xml')) {
        $errors[] = ["type" => "error", "message" => "El archivo: <strong>./sitemap.xml</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>."];
    }
    if (!is_writable('./sitemap-index.xml')) {
        $errors[] = ["type" => "error", "message" => "El archivo: <strong>./sitemap-index.xml</strong> no se puede escribir, el permiso del archivo debe establecerse en <strong>777</strong>."];
    }


    if (session_status() == PHP_SESSION_NONE) {
        $errors[] = ["type" => "error", "message" => "La sesion de PHP no puede iniciarse, verifique la configuracion de la sesion en su servidor, la ruta de la sesión debe poder escribirse, comuníquese con su servidor para obtener mas información."];
    }
    ...
    if (!empty($config['curl'])) {
        ...
        if (curl_errno ( $ch )) {
            $errors[] = ["type" => "error", "message" => "<strong>cURL</strong> no funciona, no se puede conectar con el mundo exterior, error encontrado: <strong>" . curl_error ( $ch ) . "</strong>, por favor, póngase en contacto con su proveedor de alojamiento para solucionarlo."];
        }
        curl_close ( $ch );
    }
    ...
    if (!empty($config['htaccess'])) {
        if (!file_exists('./.htaccess')) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>.htaccess</strong> no esta cargado en su servidor, asegurese de que el archivo <strong>.htaccess</strong> se carga en su servidor."];
        } else {
            $file_gethtaccess = file_get_contents("./.htaccess");
            if (strpos($file_gethtaccess, "index.php?link1") === false) {
                $errors[] = ["type" => "error", "message" => "El archivo: <strong>.htaccess</strong> no esta actualizado, vuelva a cargar el archivo .htaccess original."];
            }
        }
    }
    ...
    if (!empty($config['nodejsport']) && $wo['config']['node_socket_flow'] == "1") {
        ...
        foreach ($ports as $port)
        {
            ...
            {
                $errors[] = ["type" => "error", "message" => "<strong>NodeJS</strong> esta habilitado, pero el sistema no puede conectarse al servidor NodeJS, <strong> " . $host . ':' . $port . " </strong>esta abajo o puerto <strong>$port</strong> esta bloqueado."];
            } 
        }
    }
    ...
    foreach ($list_ofFiles as $key => $file) {
        if(!file_exists($file)) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>{$file}</strong> es obligatorio y no se ha cargado, vuelva a cargar la carpeta 'upload/files/09'."];
            
        }
        if ($wo['config']['amazone_s3'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['spaces'] == 1 || $wo['config']['cloud_upload'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['backblaze_storage'] == 1) {
            if(!is_readable($file)) {
                $errors[] = ["type" => "error", "message" => "El archivo: <strong>{$file}</strong> no se puede leer, asegurese de que el permiso de este archivo este establecido en 777."];
            }
        }
    }
    ...
    foreach ($dirs as $key => $value) {
        if (!is_writable($value)) {
            $errors[] = ["type" => "error", "message" => "El archivo: <strong>{$value}</strong> no se puede escribir, el permiso de la carpeta debe establecerse en <strong>777</strong>."];
        }
    }

    if (empty($wo['config']['smtp_host']) && empty($wo['config']['smtp_username'])) {
        $errors[] = ["type" => "error", "message" => "<strong>SMTP</strong> no esta configurado, se recomienda configurar <strong>SMTP</strong>, para que el sistema pueda enviar correos desde el servidor. <br> <a href=" . lui_LoadAdminLinkSettings('configurar-web') . ">Haga clic aqui para configurar SMTP</a>"];
    }



    if (!is_writable('./datos/modulos/' . $wo['config']['theme'] . '/img')) {
        $errors[] = ["type" => "error", "message" => "The folder: <strong>/themes/{$wo['config']['theme']}/img</strong> no se puede escribir, la ruta y el permiso de todas las subcarpetas deben establecerse en <strong>777</strong>, incluido <strong>logo.png</strong>"];
    }
    

    if (file_exists('./install')) {
        $errors[] = ["type" => "error", "message" => "La carpeta: <strong>./install</strong> no se elimina ni se le cambia el nombre, asegurese de que la carpeta <strong>./install</strong> esta borrado."];
    }
    

    if (!empty($wo['config']['filesVersion'])) {
        if ($wo['config']['filesVersion'] > $wo['config']['version']) {
            $errors[] = ["type" => "error", "message" => "Hay un conflicto en la version de la base de datos y la version de los archivos, la versión de su base de datos es:<strong>v{$wo['config']['version']}</strong>, pero la versión del script es: <strong>v{$wo['config']['filesVersion']}</strong>. <br> Please run <strong><a href='{$wo['config']['site_url']}/update.php'>{$wo['config']['site_url']}/update.php</a></strong> of <strong>v{$wo['config']['filesVersion']}</strong>. <br><br><a href='#updates'>Haga clic aqui para mas informacion.</a>"];
        } else if ($wo['config']['filesVersion'] < $wo['config']['version']) {
            $errors[] = ["type" => "error", "message" => "Hay un conflicto en la version de la base de datos y la version de los archivos, la version de su base de datos es: <strong>v{$wo['config']['version']}</strong>, pero la version del script es: <strong>v{$wo['config']['filesVersion']}</strong>. <br>Por favor suba los archivos de <strong>v{$wo['config']['filesVersion']}</strong> utilizando FTP o SFTP, no se recomiendan los administradores de archivos."];
        }
    } else {
        $errors[] = ["type" => "error", "message" => "Hay un conflicto en la versión de la base de datos y la versión de los archivos, la version de su base de datos es: <strong>v{$wo['config']['version']}</strong>, pero la version del script es: <strong>v{$wo['config']['filesVersion']}</strong>, <br>Por favor suba los archivos de <strong>v{$wo['config']['filesVersion']}</strong> utilizando FTP o SFTP, no se recomiendan los administradores de archivos."];
    }

    if (!empty($wo['config']['cronjob_last_run'])) {
        $now = strtotime("-15 minutes");
        if ($wo['config']['cronjob_last_run'] < $now) {
            $errors[] = ["type" => "error", "message" => "Archivo <strong>cron-job.php</strong> la última ejecución excedió los 15 minutos, asegúrese de que se agregue a la lista de cronjob. <br> <a href=" . lui_LoadAdminLinkSettings('cronjob_settings') . ">Configuracion de trabajo cron</a>"];
        }
    }
    ...
    $getSqlModes = $db->rawQuery("SELECT @@sql_mode as modes;");
      if (!empty($getSqlModes[0]->modes)) {
         ...
         if (in_array('strict_trans_tables', $results)) {
           $errors[] = ["type" => "error", "message" => "El sql-mode <b>strict_trans_tables</b> esta habilitado en su servidor mysql, comuniquese con su proveedor de alojamiento para deshabilitarlo."];
         }
         if (in_array('only_full_group_by', $results)) {
           $errors[] = ["type" => "error", "message" => "El sql-mode <b>only_full_group_by</b> esta habilitado en su servidor mysql, esto puede causar algunos problemas en su sitio web, comuniquese con su proveedor de alojamiento para deshabilitarlo."];
         }
      }
    ...
    if ($getUploadSize < 1000000000) {
        $errors[] = ["type" => "warning", "message" => "El tamaño máximo de carga de su servidor es inferior a 100 MB, actual: <strong>" . formatBytes($getUploadSize). "</strong> Recomendado es <strong>1024MB</strong>. Debe actualizar ambos: upload_max_filesize, post_max_size."];
    }

    if (ini_get('max_execution_time') < 100 && ini_get('max_execution_time') > 0) {
        $errors[] = ["type" => "warning", "message" => "Su servidor max_execution_time es inferior a 100 segundos, actual: <strong>" . ini_get('max_execution_time'). "</strong> Recomendado es <strong>3000</strong>."];
    }

    if ($wo['config']['developer_mode'] == "1") {
        $errors[] = ["type" => "warning", "message" => "<strong>Modo desarrollador</strong> esta habilitado en <strong>Ajustes -> Configuracion General</strong>, no se recomienda habilitar <strong>Modo desarrollador</strong> si su sitio web esta activo, es posible que se muestren algunos errores."];
    }

    if(!function_exists('exif_read_data')) {
        $errors[] = ["type" => "warning", "message" => "PHP <strong>exif</strong> extensión esta deshabilitada en su servidor, se recomienda habilitarla."];
    }
    ...
    try {
        ...
        if (!empty($getSqlWait[0]->Value)) {
            if ($getSqlWait[0]->Value < 1000) {
              $errors[] = ["type" => "warning", "message" => "La variable MySQL <b>wait_timeout</b> es {$getSqlWait[0]->Value}, minimo requerido es <strong>1000</strong>, pongase en contacto con su proveedor de alojamiento para actualizarlo."];
            }
        }
    } catch (Exception $e) {
