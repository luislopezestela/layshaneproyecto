...
module.exports.DefineTemplates = async (ctx) => {
    chatList = fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/chat-list.php'));
    groupList =  fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/group-list.php'));
    offlineUser =  fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/offline-user.php'));
    onlineUser =  fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/online-user.php'));
    messageList =  fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/message-text-list.php'));
    messageGroupRecipientsList =  fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/messages-group-list.php'));
    messageRecipientsList =  fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/messages-recipients-list.php'));
...
module.exports.chatListOwnerFalse = async (ctx, data, fromUser, nextId, hasHTML, sendable_message) => {
    ...
    ctx.reactions_types.forEach(element => {
        if (element.status == 1) {
            ...
            if (ctx.globalconfig['theme'] === "layshane-star") {
                r_img = '<img src="'+element.wowonder_icon+'">';
                var matches = element.wowonder_icon.match(/<[^<]+>/);
                if (matches) {
                    r_img = element.wowonder_icon;
                }
                reactions_html += '<li style="'+first_text+'" class="reaction reaction-'+element.id+'" data-reaction="'+element.name+'" data-reaction-id="'+element.id+'" data-reaction-lang="'+element.name+'" data-post-id="'+nextId+'" onclick="Wo_RegisterMessageReaction(this,\''+element.layshane_star_small_icon+'\','+element.is_html+');">'+r_img+'</li>';
