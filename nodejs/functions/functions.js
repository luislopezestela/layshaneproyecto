...
const lui_hashtags = require("../models/lui_hashtags");
const md5 = require('md5');
const moment = require('moment');
const striptags = require('striptags');
const lui_userschat = require("../models/lui_userschat");
...
class FunctionsUtils {
    async DefineAudioTemplates(ctx) {
        audio = fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/audio.php'));
        chat_audio = fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/chat-audio.php'));
        chat_video = fs.readFileSync(path.resolve(__dirname, '../../datos/modulos/'+ctx.globalconfig['theme']+'/paginas/nodejs/chat-video.php'));
        ...
        if (mention) {
            ...
            if (mention_matches) {
                for (let match of mention_matches) {
                    let mentionUser = await ctx.lui_users.findOne({
                        where: {
    ...
    async Wo_GetHashtag(ctx, tag) {
        ...
        if (typeof (+tag) === "number" && !isNaN((+tag))) {
            result = await ctx.lui_hashtags.findOne({
                where: {
                    id: tag
                }
            })
        ...
        if (result) {
            ...
            if (create) {
                let hash = md5(tag);
                let res = await ctx.lui_hashtags.create({
                    hash: hash,
                    tag: tag,
    ...
    async Wo_DisplaySharedFile(ctx, message_id, placement, isSticker) {
        ...
        let message = await ctx.lui_messages.findOne({
            where: {
                id: message_id
            }
        })
    ...
    async Wo_UserData(ctx, user_id) {
        ...
        let user = await ctx.lui_users.findOne({
            where: {
                user_id: user_id
            }
        })
    ...
    async Wo_IsFollowing(ctx, following_id, user_id) {
        let result = await ctx.lui_followers.count({
            where: {
    ...
    async Wo_IsTyping(ctx, user_id, recipient_id) {
        let follower_is_typing = await ctx.lui_followers.count({
            where: {
                follower_id: user_id,
    ...
    async Wo_GetChatUsers(ctx, user_id, type) {
        ...
        let blocker = await ctx.lui_blocks.findAll({
            attributes: [
                "blocker"
            ],
            where: {
        ...
        let followers = await ctx.lui_followers.findAll({
        ...
        let users;
        if (type == 'online') {
            users = await ctx.lui_users.findAll({
                attributes: [
    ...
    async getAllGroupsForUser(ctx, userId) {
        let groupIds = await ctx.lui_groupchatusers.findAll({
            attributes: ["group_id"],
            where: {
                user_id: userId
            }
    ...
    async getGroupUsers(ctx, group_id) {
        let userIds = await ctx.lui_groupchatusers.findAll({
    ...
    async Wo_GetMessagesUsers(ctx, user_id) {
        ...
        let usersChat = await ctx.lui_userschat.findAll({
            where: {
    ...
    async Wo_GetMessagesGroups(ctx, user_id) {
        let usersChat = await ctx.lui_groupchatusers.findAll({
            where: {
                user_id: user_id
            },
            limit: 50,
    ...
    async Wo_GetGroupChat(ctx, group_id) {
        let groupChat = await ctx.lui_groupchat.findOne({
            where: {
                group_id: group_id
            },
            raw: true
    ...
    async getLatestMessage(ctx, from_id, to_id) {
        let messageText = await ctx.lui_messages.findOne({
            where: {
                [Op.or]: [{
                        from_id: from_id,
                        to_id: to_id,
    ...
    async Wo_IsOnline(ctx, userId) {
        let user = await ctx.lui_users.count({
            attributes: ["lastseen"],
            where: {
                userid: userId,
                lastseen: {
    ...
    async Wo_CountNotifications(ctx, userId) {
        let result = await ctx.lui_notification.count({
            where: {
                recipient_id: userId,
                seen: 0
            }
    ...
    async Wo_GetNotifications(ctx, userId) {
        ...
        let result = await ctx.lui_notification.findAll({
            where: {
                recipient_id: userId,
                seen: 0,
                seen_pop: 0,
        ...
        await lui_notification.destroy({
            where: {
                time: {
    ...
    async Wo_CountUnseenMessages(ctx, user_id) {
        let result = await ctx.lui_messages.count({
            where: {
                to_id: user_id,
                seen: 0
            }
    ...
    async Wo_CheckLastGroupUnread(ctx, userId) {
        let result = await ctx.lui_groupchatusers.findAll({
            where: {
                id: userId,
                active: '1'
            },
    ...
    async Wo_GetChatGroupLastMessage(ctx, group_id) {
        let result = await ctx.lui_messages.findAll({
            where: {
                group_id: group_id
            },
            order: [
    ...
    async Wo_CheckFroInCalls(ctx, id, type) {
        let result;
        if (type === "video") {
            result = await ctx.lui_videocalls.findAll({
                where: {
                    to_id: id,
                    time: {
            ...
            result = await ctx.lui_audiocalls.findAll({
                where: {
                    to_id: id,
                    time: {
                        [Op.gt]: Math.floor(Date.now() / 1000) - 40
    ...
    async Wo_GetMessageByID(ctx,id,data) {
        var message = await ctx.lui_messages.findOne({
                        where: {
                            id: id
                        }
                    });
            ...
            if (message.text != '') {
                ...
                if (mentionSearch && mentionSearch.length > 0) {
                    ...
                    for (var mentionSearchOne of mentionSearch) {
                        var mention = await ctx.lui_users.findOne({
                            where: {
    ...
    async Wo_GetReactionsTypes(ctx,type = 'page') {
        let result = await ctx.lui_reactions_types.findAll({raw: true})
        let res = [];
        result.forEach(element => {
            ...
            if (type == 'page') {
                ...
                else if (!fs.existsSync(path.resolve('../datos/modulos/',ctx.globalconfig["theme"]+'/reaction/like-sm.png'))) {
                    if (element.id == 1) {
    ...
    async Wo_IsReacted(ctx,object_id, col = "post",type = '',user_id) {
        var name = col+'_id';
        if (type == 'blog') {
            var result = await ctx.lui_blog_reaction.count({
                where: {
                    [name]: object_id,
                    user_id: user_id
                },
            ...
            var result = await ctx.lui_reactions.count({
                where: {
                    [name]: object_id,
                    user_id: user_id
                },
    ...
    async Wo_GetPostReactionsTypes(ctx,object_id, col = "post",user_id,type = 'post') {
       ...
       if (type == 'blog') {
            var result = await ctx.lui_blog_reaction.findAll({
                where: {
                    [name]: object_id
                },
                raw: true
            ...
            var result = await ctx.lui_reactions.findAll({
                where: {
                    [name]: object_id
                },
                raw: true
