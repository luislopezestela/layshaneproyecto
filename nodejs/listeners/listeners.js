...
module.exports.registerListeners = async (socket, io, ctx) => {
    ...
    socket.on("join", async (data, callback) => {
        ...
        let user_id = await ctx.lui_appssessions.findOne({
            attributes: [
                "user_id",
            ],
            where: {
        ...
        let user_status = await ctx.lui_users.findOne({
            attributes: [
                "status"
            ],
            where: {
    ...
    socket.on("ping_for_lastseen", async (data) => {
        if (ctx.userHashUserId[data.user_id]) {
            let userlastseen_status = await ctx.lui_users.findOne({
                attributes: [
                    "status"
                ],
                where: {
    ...
    socket.on("is_chat_on", async (data) => {
        ...
        if (data.message_id) {
            last_message = await ctx.lui_messages.findOne({
                where: {
                    id: data.message_id
                }
            })
        }
        let toUser = await ctx.lui_users.findOne({
            where: {
                user_id: {
    ...
    socket.on("page_message", async (data, callback) => {
        ...
        let page_data = await ctx.lui_pages.findOne({
            where: {
                page_id: {
                    [Op.eq]: data.page_id
                }
        ...
        if (data.message_reply_id > 0) {
            let cansendreplyID = await ctx.lui_messages.findOne({
                where: {
        ...
        if ((ctx.userIdChatOpen[data.to_id] && ctx.userIdChatOpen[data.to_id].filter(d => d == ctx.userHashUserId[data.from_id]).length) ||
            ...
            var m_sent = await ctx.lui_messages.create({
        ...
        let new_message = await ctx.lui_messages.findOne({
            where: {
                id: {
                    [Op.eq]: nextId
                }
        ...
        if (!data.mediaId) {
            ...
            if (mentionSearch && mentionSearch.length > 0) {
                ...
                for (let mentionSearchOne of mentionSearch) {
                    let mention = await ctx.lui_users.findOne({
                        where: {
    ...
    socket.on("group_message", async (data, callback) => {
        ...
        let groupOwnerId = await ctx.lui_groupchat.findOne({
            attributes: ["user_id"],
            where: {
                group_id: {
                    [Op.eq]: data.group_id
        ...
        let nextId = m_sent.id;
        data.new_message = await ctx.lui_messages.findOne({
            where: {
                id: {
                    [Op.eq]: nextId
                }
        ...
        data.group_data = await ctx.lui_groupchat.findOne({
            where: {
                group_id: {
                    [Op.eq]: data.group_id
                }
    ...
    socket.on("private_message_page", async (data, callback) => {
        ...
        let nextId = (lastId && lastId.id) ? (+lastId.id + 1) : 1
        let fromUser = await ctx.lui_users.findOne({
            where: {
                user_id: {
                    [Op.eq]: ctx.userHashUserId[data.from_id]
                }
        ...
        if (data.record) {
            ...
            let ret = await ctx.lui_messages.create({
                from_id: ctx.userHashUserId[data.from_id],
                to_id: data.to_id,
                text: "",
        ...
        if (!data.mediaId) {
            ...
            await funcs.updateOrCreate(ctx.lui_userschat, {
    ...
    socket.on("mute", async (data, callback) => {
        ...
        if(info && info.id){
            await ctx.lui_mute.update(update_object,
            {
                where: {
                    id: info.id
                }
            ...
            await ctx.lui_mute.create(update_object)
    ...
    socket.on("loadmore", async (data, callback) => {
        ...
        let after_message_id = await ctx.lui_messages.findOne({
            where: {
                [Op.or]: [
                    {
                        from_id: {
        ...
        let messages = await ctx.lui_messages.findAll({
            limit: 15,
            where: {
                id: {
        ...
        for (let message_index = messages.length - 1; message_index >= 0; message_index--) {
            ...
            if (message.media && message.media != "") {
                ...
                if (message.story_id && message.story_id > 0) {
                    var story = await ctx.lui_userstory.findOne({
                                        where: {
                                            id: message.story_id
                                        }
                                    })
    ...
    socket.on("event_notification", async (data) => {
        ...
        let eventData = await ctx.lui_events.findOne({
    ...
    socket.on("group_notification", async (data) => {
        ...
        let to_id = data.to_id;
        let groupData = await ctx.lui_groups.findOne({
            attributes: ["user_id"],
            where: {
                id: parseInt(to_id)
            },
    ...
    socket.on("user_notification", async (data) => {
        ...
        let to_id = data.to_id;
        let userData = await ctx.lui_users.findOne({
            attributes: ["user_id"],
            where: {
            ...
            if (data.type === 'messages') {
                ...
                if (message && message !== undefined) {
                    if (await funcs.Wo_IsReacted(ctx,data.id,'message','',user_id) > 0) {
                        await ctx.lui_reactions.destroy({
                            where: {
                                message_id: data.id,
                                user_id: user_id
                            },
                    ...
                    await ctx.lui_reactions.create({
                        user_id: user_id,
                        message_id: data.id,
    ...
    socket.on("post_notification", async (data) => {
        ...
        let postData = await ctx.lui_posts.findOne({
            attributes: ["user_id"],
            where: {
                id: post_id
            },
    ...
    socket.on("comment_notification", async (data) => {
        ...
        let commentData = await ctx.lui_comments.findOne({
            attributes: ["user_id"],
            where: {
                id: comment_id
            },
        ...
        if (typeof data.for !== 'undefined') {
            let replyData = await ctx.lui_comment_replies.findAll({
                attributes: ["user_id"],
                where: {
    ...
    socket.on("reply_notification", async (data) => {
        ...
        let replyData = await ctx.lui_comment_replies.findOne({
            attributes: ["user_id"],
            where: {
                id: reply_id
            },
