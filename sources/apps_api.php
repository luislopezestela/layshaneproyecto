...
$user_id = lui_UserIdFromToken($_GET['access_token']);
if ($user_id === false) {
    $errors = array(
        'status' => 400,
        'errors' => array(
...
$user = $wo['user'] = lui_UserData($user_id);
...
if ($_GET['type'] == 'get_user_data') {
    ...
    foreach ($api_data as $post_data) {
        ...
        $json_data = array(
            ...
            'post_data' => array(
                ...
                'post_file' => lui_GetMedia($post_data['postFile']),
                ...
                'post_likes' => lui_CountLikes($post_data['post_id']),
                'post_wonders' => lui_CountWonders($post_data['post_id'])
            ),
    ...
    $pages = lui_GetMyPages(false,$limit);
    echo json_encode(array(
        'api_status' => 'success',
        'api_version' => '1.4.4',
        'status' => 200,
    ...
    $groups = lui_GetMyGroupsAPI($limit,0,'DESC');
    echo json_encode(array(
        'api_status' => 'success',
        'api_version' => '1.4.4',
        'status' => 200,
    ...
    $products = array();
    $get_products = lui_GetProducts(array('user_id' => $wo['user']['user_id'],
                                         'limit' => $limit));
    ...
    $following = lui_GetFollowers($wo['user']['user_id'], 'profile', $limit,false);
    foreach ($following as $key2 => $user_list) {
        ...
        $following[$key2]['lastseen_time_text'] = lui_Time_Elapsed_String($user_list['lastseen']);
        $following[$key2]['lastseen'] = $lastseen;
        $following[$key2]['user_platform'] = lui_GetPlatformFromUser_ID($user_list['user_id']);
        $following[$key2]['is_following'] = (lui_IsFollowing($user_list['user_id'],$wo['user']['user_id'])) ? 1 : 0;
    ...
    $following = lui_GetFollowing($wo['user']['user_id'], 'profile', $limit,false);
    foreach ($following as $key2 => $user_list) {
