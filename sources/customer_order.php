...
foreach ($wo['orders'] as $key => $wo['order']) {
    $wo['order']->product = lui_GetProduct($wo['order']->product_id);
    ...
    $wo['sucursal_id']  = $wo['order']->address_id;
    $wo['can_review']  = 0;
    $wo['is_reviewed'] = 0;
    if ($wo['loggedin']) {
    ...
    $wo['html'] .= lui_LoadPage('customer_order/list');
}
$wo['total']       = number_format($wo['total'], 2);
$wo['address']     = $db->where('id', $wo['address_id'])->getOne(T_USER_ADDRESS);
$wo['sucursales']     = $db->where('id', $wo['sucursal_id'])->getOne('lui_sucursales');
