...
if (isset($_GET['u'])) {
    ...
    if (in_array(true, $check_user)) {
        if ($check_user['type'] == 'user') {
            ...
            $wo['user_profile']['fields'] =lui_UserFieldsData($user_id);
...
if (!empty($_GET['ref'])) {
    if ($_GET['ref'] == 'se') {
        $regsiter_recent =lui_RegsiterRecent($id, $type);
    }
}
$con2 = 1;
if ($type == 'timeline') {
    $user_data =lui_UpdateUserDetails($wo['user_profile'], true, true, true);
...
if ($type == 'timeline' && $wo['loggedin'] == true) {
    $is_blocked = $wo['is_blocked'] =lui_IsBlocked($user_id);
    if (isset($_GET['block_user']) && !empty($_GET['block_user'])) {
        if ($_GET['block_user'] == 'block' && $is_blocked === false &&lui_IsAdmin($user_id) === false &&lui_IsModerator($user_id) === false) {
            $block =lui_RegisterBlock($user_id);
            if ($block) {
                if (!empty($_GET['redirect'])) {
                    header("Location: " .lui_SeoLink("index.php?link1={$_GET['redirect']}"));
                    exit();
                }
                header("Location: " .lui_SeoLink('index.php?link1=welcome'));
                exit();
            ...
            $unblock =lui_RemoveBlock($user_id);
            if ($unblock) {
    ...
    if ($is_blocked) {
        ...
        if (!isset($came_from)) {
           ...
           lui_RedirectSmooth(lui_SeoLink('index.php?link1=404'));
        }
    }
}
$can_                           = 0;
...
if ($type == 'timeline' && $wo['config']['website_mode'] == 'patreon') {
    $wo['content'] =lui_LoadPage("{$type}/tiers");
    ...
    foreach ($stories as $value) {
        $wo['story'] =lui_PostData($value->id);
        ...
        if (!empty($wo['story']['postFile']) && (strpos($wo['story']['postFile'], '.jpg') !== false || strpos($wo['story']['postFile'], '.jpeg') !== false || strpos($wo['story']['postFile'], '.png') !== false || strpos($wo['story']['postFile'], '.gif') !== false) ) {
            $wo['story']['main_thumb'] =lui_GetMedia($wo['story']['postFile']);
        }
        if (!empty($wo['story']['postFile']) && (strpos($wo['story']['postFile'], '.mp4') !== false || strpos($wo['story']['postFile'], '.mkv') !== false || strpos($wo['story']['postFile'], '.avi') !== false || strpos($wo['story']['postFile'], '.webm') || strpos($wo['story']['postFile'], '.mov') || strpos($wo['story']['postFile'], '.m3u8') !== false )) {
            ...
            if (!empty($wo['story']['postFileThumb'])) {
                $wo['story']['main_thumb'] =lui_GetMedia($wo['story']['postFileThumb']);
            }
            else{
                $wo['story']['main_thumb'] =lui_GetMedia('upload/photos/d-film.jpg');
            }
        }
        ...
        if (!empty($wo['story']['album_name'])) {
            ...
            $wo['story']['main_thumb'] =lui_GetMedia($wo['story']['photo_album'][0]['image_org']);
        }
        if (!empty($wo['story']['multi_image'])) {
            $wo['story']['model_type'] = 'multi_image';
            $wo['story']['model_photo'] = $wo['story']['photo_multi'][0]['parent_id'];
            $wo['story']['main_thumb'] =lui_GetMedia($wo['story']['photo_multi'][0]['image_org']);
        }
        $wo['stories'][] = $wo['story'];
    }


    $wo['content'] =lui_LoadPage("mode_instagram/profile/content");
} else {
    $wo['content'] =lui_LoadPage("{$type}/content");
}
