...
if ($f == 'ads') {
    if ($s == 'new' && $wo['loggedin'] === true) {
        ...
        if (in_array(true, $request)) {
            ...
            if (!in_array($_FILES["media"]["type"], $ad_media_types)) {
                ...
                $maxUpload = lui_SizeUnits($wo['config']['maxUpload']);
        ...
        if (empty($error)) {
            $page_id = 0;
            if (!empty($_POST['page'])) {
                $page_id = lui_PageIdFromPagename($_POST['page']);
                if (empty($page_id)) {
                    $page_id = 0;
                }
            }
            ...
            if (!empty($_POST['start'])) {
                $start = lui_Secure($_POST['start']);
            }
            $end = '';
            if (!empty($_POST['end'])) {
                $end = lui_Secure($_POST['end']);
            }
            $budget = 0;
            if (!empty($_POST['budget']) && is_numeric($_POST['budget']) && $_POST['budget'] > 0) {
                $budget = lui_Secure($_POST['budget']);
            }
            
            
            $registration_data             = array(
                'name' => lui_Secure($_POST['name']),
                'url' => lui_Secure($_POST['website']),
                'headline' => lui_Secure($_POST['headline']),
                'description' => lui_Secure($_POST['description']),
                'location' => lui_Secure($_POST['location']),
                'audience' => lui_Secure(implode(',', $_POST['audience-list'])),
                'gender' => lui_Secure($_POST['gender']),
                'bidding' => lui_Secure($_POST['bidding']),
                'posted' => time(),
                'appears' => lui_Secure($_POST['appears']),
            ...
            $data                          = array(
                ...
                'status' => 200,
                'url' => lui_SeoLink('index.php?link1=advertise')
            );
        } else {
            $data = array(
                'message' => $error,
    ...
    if ($s == 'update' && $wo['loggedin'] === true) {
        ...
        if (empty($error)) {
            ...
            if (isset($_GET['a']) && $_GET['a'] == 1) {
                $data['url'] = lui_SeoLink('index.php?link1=admincp&page=user_ads');
            }
        } else {
            $data = array(
                'message' => $error,
    ...
    if ($s == 'rads-c' && !empty($_GET['ad_id']) && is_numeric($_GET['ad_id'])) {
        ...
        $ad   = lui_Secure($_GET['ad_id']);
        if (lui_RegisterAdConversionClick($ad)) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'rads-v' && !empty($_GET['ad_id']) && is_numeric($_GET['ad_id'])) {
        ...
        $ad          = lui_Secure($_GET['ad_id']);
        $get_ad_data = lui_GetUserAdData($ad);
        if ($get_ad_data['bidding'] == 'clicks') {
            lui_RegisterAdConversionClick($ad);
        } else {
            lui_RegisterAdClick($ad);
        }
        $data['status'] = 200;
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'lm' && isset($_GET['ad_id']) && is_numeric($_GET['ad_id'])) {
        ...
        $last_id = lui_Secure($_GET['ad_id']);
        $ads     = lui_GetMyAds(array(
            'offset' => $last_id
        ));
        if ($ads && count($ads) > 0) {
            foreach ($ads as $wo['ad']) {
                $html .= lui_LoadPage('ads/includes/ads-list');
            }
            $data = array(
                'status' => 200,
                'html' => $html
    ...
    if ($s == 'alm' && isset($_GET['ad_id']) && is_numeric($_GET['ad_id'])) {
        ...
        $last_id = lui_Secure($_GET['ad_id']);
        $ads     = lui_GetAds($last_id);
        if ($ads && count($ads) > 0) {
            foreach ($ads as $wo['user_ad']) {
                $html .= lui_LoadPage('admin/user_ads/ads-list');
            }
            $data = array(
                'status' => 200,
                'html' => $html
    ...
    if ($s == 'get_estimated_users' && $wo['loggedin'] === true) {
        ...
        if (isset($_GET['estimated_audience']) && isset($_GET['estimated_gender'])) {
            ...
            elseif (in_array($_GET['estimated_gender'], array_keys($wo['genders']))){
                $gender = lui_Secure($_GET['estimated_gender']);
