...
if ($f == 'chat') {
    if ($s == 'get_group_info') {
        ...
        $group_tab = lui_GroupTabData($group_id);
    ...
    if ($s == 'edit_group') {
        $data['status'] = 400;
        $group_id       = $id = lui_Secure($_POST['group_id']);
        $group_tab      = lui_GroupTabData($group_id);
        $error          = '';
        if ($group_tab && is_array($group_tab)) {
            ...
            if (!$error) {
                ...
                if (!empty($_POST['group_name'])) {
                    $update_data['group_name'] = lui_Secure($_POST['group_name'],1);
                }
                ...
                if (!empty($update_data)) {
                    @lui_UpdateGChat($id, $update_data);
                    $data = array(
                        'status' => 200
                    );
                }
    ...
    if ($s == 'chat_side') {
        if (lui_CheckMainSession($hash_id) === true) {
            ...
            $count_chat    = lui_CountOnlineUsers();
            foreach ($OnlineUsers as $wo['chatList']) {
                $online_users .= lui_LoadPage('chat/online-user');
            }
            foreach ($OfflineUsers as $wo['chatList']) {
                $offline_users .= lui_LoadPage('chat/offline-user');
            }
            $data = array(
                'status' => 200,
            ...
            if (!empty($_GET['user_id']) && isset($_GET['message_id'])) {
                ...
                if (!empty($user_id)) {
                    $user_id  = $_GET['user_id'];
                    $messages = lui_GetMessages(array(
                    ...
                    if (count($messages) > 0) {
                        ...
                        foreach ($messages as $wo['chatMessage']) {
                            $messages_html .= lui_LoadPage('chat/chat-list');
                        }
                        $data['chat_user_tab'] = 200;
                        $data['messages']      = 200;
            ...
            if (!empty($_GET['user_id']) && $wo['config']['message_typing'] == 1) {
                $isTyping = lui_IsTyping($_GET['user_id']);
                if ($isTyping === true) {
                    $img               = lui_UserData($_GET['user_id']);
                    $data['is_typing'] = 200;
                    $data['img']       = $img['avatar'];
            ...
            if (isset($_GET['last_group']) && is_numeric($_GET['last_group'])) {
                $new_groups = lui_GetChatGroups();
                $groups     = '';
                if (is_array($new_groups) && count($new_groups) > 0) {
                    foreach ($new_groups as $wo['group']) {
                        $groups .= lui_LoadPage('chat/group-list');
                    }
                }
                $data['chat_groups']         = $groups;
                $data['update_group_status'] = lui_CheckLastGroupAction();
            }
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'recipient_is_typing') {
        if (!empty($_GET['recipient_id'])) {
            $isTyping = lui_RegisterTyping($_GET['recipient_id'], 1);
            if ($isTyping === true) {
                $data = array(
                    'status' => 200
                );
    ...
    if ($s == 'remove_typing') {
        if (!empty($_GET['recipient_id'])) {
            $isTyping = lui_RegisterTyping($_GET['recipient_id'], 0);
            if ($isTyping === true) {
                $data = array(
                    'status' => 200
                );
    ...
    if ($s == 'update_online_recipients') {
        ...
        $OnlineUsers = lui_GetChatUsers('online');
        foreach ($OnlineUsers as $wo['chatList']) {
            $html .= lui_LoadPage('chat/online-user');
        }
        $data = array(
            'status' => 200,
            'html' => $html
    ...
    if ($s == 'update_offline_recipients') {
        ...
        $OfflineUsers = lui_GetChatUsers('offline');
        foreach ($OfflineUsers as $wo['chatList']) {
            $html .= lui_LoadPage('chat/offline-user');
        }
        $data = array(
            'status' => 200,
            'html' => $html
    ...
    if ($s == 'set-chat-color') {
        ...
        if (isset($_GET['recipient_user']) && is_numeric($_GET['recipient_user']) && $_GET['recipient_user'] > 0) {
            $recipient_user = lui_Secure($_GET['recipient_user']);
        }
        ...
        if ($user_id && $color && $recipient_user) {
            if (lui_UpdateChatColor($user_id, $recipient_user, $color, $page_id)) {
                $data = array(
                    'status' => 200,
                    'message' => "color added",
                    'color' => $color
    ...
    if ($s == 'search_for_recipients') {
        if (!empty($_POST['search_query'])) {
            $html   = '';
            $search = lui_ChatSearchUsers($_POST['search_query']);
            foreach ($search as $wo['chatList']) {
                $html .= lui_LoadPage('chat/search-result');
            }
            $data = array(
                'status' => 200,
                'html' => $html
    ...
    if ($s == 'update_chat_status') {
        if (!empty($_POST['status'])) {
            $html   = '';
            $status = lui_UpdateStatus($_POST['status']);
            if ($status == 0) {
                $data = array(
                    'status' => $status
                );
    ...
    if ($s == 'load_chat_tab' && $wo['config']['can_use_chat']) {
        if ($wo['user']['message_privacy'] != 2) {
            if (!empty($_GET['recipient_id']) && is_numeric($_GET['recipient_id']) && $_GET['recipient_id'] > 0 && !empty($_GET['placement'])) {
                if (lui_IsBlocked($_GET['recipient_id'])) {
                    $data = array(
                        'status' => 400
                    );
                } else {
                    if ($_GET['recipient_id'] != 0) {
                        $recipient_id = lui_Secure($_GET['recipient_id']);
                        $recipient    = lui_UserData($recipient_id);
                        $el_recipiente = 0;
                        $el_recipiente = $recipient['message_privacy'];
                        if($el_recipiente != 2) {
                            if (isset($recipient['user_id'])) {
                                $wo['chat']['recipient'] = $recipient;
                                $wo['chat']['color']     = lui_GetChatColor($wo['user']['user_id'], $recipient['user_id']);
                                $wo['chat']['story_id']  = 0;
                                if (!empty($_GET['story_id']) && is_numeric($_GET['story_id']) && $_GET['story_id'] > 0) {
                                    $wo['chat']['story_id'] = lui_Secure($_GET['story_id']);
                                }
                                $data = array(
                                    'status' => 200,
                                    'html' => lui_LoadPage('chat/chat-tab')
                                );
                                    ...
                                    if (strpos($_SESSION['chat_id'], ',') !== false) {
                                        ...
                                        if (count($explode) > 2) {
                                            if (strpos($_SESSION['chat_id'], $recipient['user_id']) === false) {
                                                ...
                                                $_SESSION['chat_id'] .= ',' . lui_Secure($recipient['user_id']);
                                            }
                                        } else {
                                            $_SESSION['chat_id'] .= ',' . lui_Secure($recipient['user_id']);
                                        }
                ...
                $group_id = lui_Secure($_GET['group_id']);
                if (lui_IsGChatOwner($group_id) || lui_IsGChatMemebers($group_id)) {
                    $group_tab = lui_GroupTabData($group_id);
                    if ($group_tab && is_array($group_tab)) {
                        $wo['chat']['group']  = $group_tab;
                        $data                 = array(
                            'status' => 200,
                            'html' => lui_LoadPage('chat/group-tab')
                        );
                ...
                if (!empty($page_tab) && is_array($page_tab)) {
                    if ($wo['user']['user_id'] == $_GET['page_user_id'] || $wo['user']['user_id'] == $page_tab['user_id']) {
                        ...
                        $wo['chat']['page']['messages'] = lui_GetPageMessages(array(
                            'page_id' => $page_id,
                            'from_id' => $page_tab['user_id'],
                            'to_id' => !empty($_GET['page_user_id']) ? lui_Secure($_GET['page_user_id']) : 0
                        ));
                        $wo['chat']['from_id']          = !empty($_GET['from_id']) ? lui_Secure($_GET['from_id']) : 0;
                        $wo['chat']['to_id']            = !empty($_GET['page_user_id']) ? lui_Secure($_GET['page_user_id']) : 0;
                        $wo['chat']['user']             = lui_UserData($wo['chat']['to_id']);
                        $wo['chat']['color']            = lui_GetChatColor($wo['user']['user_id'], $wo['chat']['user']['user_id'], $page_id);
                        $data                           = array(
                            'status' => 200,
                            'html' => lui_LoadPage('chat/page-tab')
                        );
    ...
    if ($s == 'load_chat_messages') {
        if (!empty($_GET['recipient_id']) && is_numeric($_GET['recipient_id']) && $_GET['recipient_id'] > 0 && lui_CheckMainSession($hash_id) === true) {
            $recipient_id = lui_Secure($_GET['recipient_id']);
            if (!empty($_GET['product_id']) && is_numeric($_GET['product_id']) && $_GET['product_id'] > 0) {
                $messages = lui_RegisterMessage(array(
                    'from_id' => lui_Secure($wo['user']['user_id']),
                    'to_id' => $recipient_id,
                    'time' => time(),
                    'stickers' => '',
                    'product_id' => lui_Secure($_GET['product_id'])
                ));
            }
            $html                = '';
            $messages            = lui_GetMessages(array(
            ...
            $wo['chat']['color'] = lui_GetChatColor($wo['user']['user_id'], $recipient_id);
            foreach ($messages as $wo['chatMessage']) {
                $html .= lui_LoadPage('chat/chat-list');
            }
            $data = array(
                'status' => 200,
                'messages' => $html
    ...
    if ($s == 'send_message') {
        ...
        if (!empty($_POST['reply_id']) && is_numeric($_POST['reply_id']) && $_POST['reply_id'] > 0) {
            $me                    = lui_Secure($wo['user']['user_id']);
            $him                   = lui_Secure($_POST['user_id']);
            $reply_id              = lui_Secure($_POST['reply_id']);
        ...
        if ($wo['config']['who_upload'] == 'pro' && $wo['user']['is_pro'] == 0 && !lui_IsAdmin() && (!empty($_FILES['sendMessageFile']) || !empty($_POST['message-record']))) {
            $data['status']       = 500;
            $data['invalid_file'] = 3;
        } else {
            if ($wo['user']['message_privacy'] != 2) {
                if (!empty($_POST['user_id']) && lui_CheckMainSession($hash_id) === true) {
                    $html          = '';
                    $media         = '';
                    $mediaFilename = '';
                    $mediaName     = '';
                    ...
                    if (isset($_FILES['sendMessageFile']['name'])) {
                        if ($_FILES['sendMessageFile']['size'] > $wo['config']['maxUpload']) {
                            $invalid_file = 1;
                        } else if (lui_IsFileAllowed($_FILES['sendMessageFile']['name'], $_FILES["sendMessageFile"]["type"]) == false) {
                            $invalid_file = 2;
                        } else {
                            $fileInfo      = array(
                        ...
                        $mediaFilename = lui_Secure($_POST['message-record']);
                        $mediaName     = lui_Secure($_POST['media-name']);
                    }
                    $message_text = '';
                    if (!empty($_POST['textSendMessage'])) {
                        $message_text = $_POST['textSendMessage'];
                    }
                    $user_data = lui_UserData($_POST['user_id']);
                    if (!empty($user_data) && $user_data['message_privacy'] == 2) {
                        exit();
                    }
                    if (!empty($user_data) && $user_data['message_privacy'] == 1 && lui_IsFollowing($wo['user']['user_id'], $_POST['user_id']) === false) {
                        exit();
                    }
                    $is_sticker = ((isset($_POST['chatSticker']) && lui_IsUrl($_POST['chatSticker']) && strpos($_POST['chatSticker'], '.gif') !== false && !$mediaFilename && !$mediaName) ? true : false);
                    if (!empty($_POST['chatSticker']) && !strpos($_POST['chatSticker'], '.gif')) {
                        $fileend       = '_sticker_' . rand(111111, 999999);
                        $mediaFilename = lui_ImportImageFromUrl($_POST['chatSticker'], $fileend);
                        $is_sticker    = true;
                    ...
                    if (!empty($_POST['story_id']) && is_numeric($_POST['story_id']) && $_POST['story_id'] > 0) {
                        $story_id = lui_Secure($_POST['story_id']);
                        $story    = $db->where('user_id', lui_Secure($_POST['user_id']))->where('id', $story_id)->getValue(T_USER_STORY, 'COUNT(*)');
                        if ($story > 0) {
                            $story_id = lui_Secure($_POST['story_id']);
                        } else {
                            $story_id = 0;
                    ...
                    $messages = lui_RegisterMessage(array(
                        'from_id' => lui_Secure($wo['user']['user_id'],0),
                        'to_id' => lui_Secure($_POST['user_id'],0),
                        'text' => lui_Secure($message_text,1),
                        'media' => lui_Secure($mediaFilename,0),
                        'mediaFileName' => lui_Secure($mediaName,0),
                        'time' => time(),
                        'stickers' => (isset($_POST['chatSticker']) && lui_IsUrl($_POST['chatSticker']) && strpos($_POST['chatSticker'], '.gif') !== false && !$mediaFilename && !$mediaName) ? $_POST['chatSticker'] : '',
                        'reply_id' => $reply_id,
                        'story_id' => $story_id
                    ));
                    if ($messages > 0) {
                        ...
                        $wo['chat']['color'] = lui_GetChatColor($wo['user']['user_id'], $_POST['user_id']);
                        foreach ($messages as $wo['chatMessage']) {
                            $html .= lui_LoadPage('chat/chat-list');
                        }
                        $file = false;
                        ...
                        if ($wo['config']['emailNotification'] == 1) {
                            ...
                            if (!in_array(false, $send_notif)) {
                                ...
                                $wo['emailNotification']['msg_text'] = lui_Secure($message_text,1);
                                ...
                                lui_SendMessage($send_message_data);
                            }
                        }
                    }
                ...
                } else if (isset($_GET['group_id']) && is_numeric($_GET['group_id']) && lui_CheckMainSession($hash_id) === true) {
                    $html          = '';
                    $media         = '';
                    $mediaFilename = '';
                    $mediaName     = '';
                    ...
                    if (isset($_FILES['sendMessageFile']['name'])) {
                        if ($_FILES['sendMessageFile']['size'] > $wo['config']['maxUpload']) {
                            $invalid_file = 1;
                        } else if (lui_IsFileAllowed($_FILES['sendMessageFile']['name']) == false) {
                            $invalid_file = 2;
                        } else {
                            $fileInfo      = array(
                                'file' => $_FILES["sendMessageFile"]["tmp_name"],
                    ...
                    $last_id = lui_RegisterGroupMessage(array(
                        'from_id' => lui_Secure($wo['user']['user_id'],0),
                        'group_id' => lui_Secure($_GET['group_id'],0),
                        'text' => lui_Secure($_POST['textSendMessage'],1),
                        'media' => lui_Secure($mediaFilename,0),
                        'mediaFileName' => lui_Secure($mediaName,0),
                        'time' => time(),
                        'reply_id' => $reply_id
                    ...
                    if ($last_id && $last_id > 0) {
                        @lui_UpdateGChat(lui_Secure($_GET['group_id']), array(
                            "time" => time()
                        ));
                        $messages = lui_GetGroupMessages(array(
                            'id' => $last_id,
                            'group_id' => $_GET['group_id']
                        ));
                        foreach ($messages as $wo['chatMessage']) {
                            $html .= lui_LoadPage('chat/group-chat-list');
                        }
                        $file = false;
                        if (isset($_FILES['sendMessageFile']['name'])) {
                            $file = true;
                ...
                } else if (isset($_GET['page_id']) && is_numeric($_GET['page_id']) && lui_CheckMainSession($hash_id) === true) {
                    $page_data    = lui_PageData($_GET['page_id']);
                    $invalid_file = 1;
                    if (!empty($page_data)) {
                        $html          = '';
                        $media         = '';
                        ...
                        if ($page_data['user_id'] == $wo['user']['user_id']) {
                            if ($page_data['user_id'] == $_GET['to_id']) {
                                $to_id = lui_Secure($_GET['from_id']);
                            } else {
                                $to_id = lui_Secure($_GET['to_id']);
                            }
                        }
                        $last_id = lui_RegisterPageMessage(array(
                            'from_id' => lui_Secure($wo['user']['user_id'],0),
                            'page_id' => lui_Secure($_GET['page_id'],0),
                            'to_id' => $to_id,
                            'text' => lui_Secure($_POST['textSendMessage'],1),
                            'media' => lui_Secure($mediaFilename,0),
                            'mediaFileName' => lui_Secure($mediaName,0),
                            'time' => time(),
                            'stickers' => (isset($_POST['chatSticker']) && lui_IsUrl($_POST['chatSticker']) && !$mediaFilename && !$mediaName) ? $_POST['chatSticker'] : '',
                            'reply_id' => $reply_id
                        ));
                        if ($last_id && $last_id > 0) {
                            $messages = lui_GetPageMessages(array(
                                'id' => $last_id,
                                'page_id' => $_GET['page_id']
                            ));
                            foreach ($messages as $wo['chatMessage']) {
                                $wo['chat']['color'] = lui_GetChatColor($wo['user']['user_id'], $to_id, lui_Secure($_GET['page_id']));
                                $html .= lui_LoadPage('chat/page-chat-list');
                            }
                            $file = false;
                            if (isset($_FILES['sendMessageFile']['name'])) {
                                $file = true;
    ...
    if ($s == 'change_chat_color') {
        $recipient_id = (isset($_GET['recipient_id'])) ? lui_Secure($_GET['recipient_id']) : false;
        $user_id      = (isset($wo['user']['id'])) ? $wo['user']['id'] : false;
        $color        = (isset($_GET['color'])) ? lui_Secure($_GET['color']) : false;
        if ($recipient_id && $user_id && $color) {
            if (lui_UpdateChatColor($recipient_id, $user_id, $color)) {
                $data = array(
                    'status' => 200
                );
            }
    ...
    if ($s == 'get_new_messages') {
        if (!empty($_GET['user_id']) && lui_CheckMainSession($hash_id) === true) {
            $html    = '';
            $user_id = lui_Secure($_GET['user_id']);
            if (!empty($user_id)) {
                $user_id  = $_GET['user_id'];
    ...
    if ($s == 'close_chat') {
        ...
        if (!empty($_GET['recipient_id']) && is_numeric($_GET['recipient_id'])) {
            if (!empty($_GET['story_id']) && is_numeric($_GET['story_id']) && $_GET['story_id'] > 0) {
                $story = $db->where('id', lui_Secure($_GET['story_id']))->getOne(T_USER_STORY);
                if (!empty($story)) {
                    $data = array(
                        'url' => lui_SeoLink('index.php?link1=messages&user=' . $_GET['recipient_id']) . "?story_id=" . $story->id
                    );
                } else {
                    $data = array(
                        'url' => lui_SeoLink('index.php?link1=messages&user=' . $_GET['recipient_id'])
                    );
                }
            } else {
                $data = array(
    ...
    if ($s == 'is_chat_on') {
        $data = array(
            'url' => lui_SeoLink('index.php?link1=messages'),
    ...
    if ($s == 'get_parts' && isset($_GET['name'])) {
        ...
        $parts = lui_GetUsersByName($name, true);
        $html  = "";
        if (count($parts) > 0) {
            foreach ($parts as $wo['part']) {
                $html .= lui_LoadPage('chat/chat-part-list');
            }
            $data['status'] = 200;
            $data['html']   = $html;
        }
    ...
    if ($s == 'search_parts' && isset($_GET['name']) && isset($_GET['group_id']) && lui_IsGChatOwner($_GET['group_id'])) {
        $name  = lui_Secure($_GET['name']);
        $group = lui_Secure($_GET['group_id']);
        $data  = array(
            'status' => 404
        );
        $parts = lui_GetUsersByName($name, true);
        ...
        if (count($parts) > 0) {
            foreach ($parts as $wo['part']) {
                ...
                $html .= lui_LoadPage('chat/add-group-parts');
            }
            $data['status'] = 200;
            $data['html']   = $html;
        }
    ...
    if ($s == 'add_gchat_user' && isset($_GET['user_id']) && isset($_GET['group_id']) && lui_IsGChatOwner($_GET['group_id'])) {
        $data = array(
            'status' => 304
        );
        $code = lui_AddGChatPart($_GET['group_id'], $_GET['user_id']);
        if ($code === 0) {
            $data['status'] = 200;
            $data['code']   = 0;
        } else if ($code === 1) {
    ...
    if ($s == 'get_new_group_messages') {
        if (!empty($_GET['group_id']) && lui_CheckMainSession($hash_id) === true) {
            $html     = '';
            $group_id = lui_Secure($_GET['group_id']);
            if (!empty($group_id)) {
                $messages = lui_GetGroupMessages(array(
    ...
    if ($s == 'clear_group_chat' && isset($_GET['group_id']) && is_numeric($_GET['group_id'])) {
        $id     = lui_Secure($_GET['group_id']);
        $data   = array(
            'status' => 304
        );
        $result = lui_ClearGChat($id);
        if ($result === true) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
    ...
    if ($s == 'group_parts' && isset($_GET['group_id']) && is_numeric($_GET['group_id'])) {
        ...
        $parts = lui_GetGChatMemebers($id);
        $data  = array();
        if (is_array($parts)) {
            $wo['group']             = array();
            $wo['group']['owner']    = lui_IsGChatOwner($id);
            $wo['group']['parts']    = $parts;
            $wo['group']['group_id'] = $id;
            $data['status']          = 200;
            $data['count']           = count($parts);
            ;
            $data['parts'] = lui_LoadPage('chat/manage');
        }
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    ...
    if ($s == 'create_group' && isset($_POST['group_name']) && isset($_POST['parts'])) {
        ...
        if (!$error) {
            $users   = explode(',', lui_Secure($_POST['parts']));
            $users[] = $wo['user']['id'];
            $name    = lui_Secure($_POST['group_name'],1);
            $id      = lui_CreateGChat($name, $users);
            if ($id && is_numeric($id)) {
                $data = array(
                    'status' => 200,
                    'group_id' => $id
    ...
    if ($s == 'delete_group_request' && !empty($_GET['group_id']) && is_numeric($_GET['group_id']) && $_GET['group_id'] > 0) {
        $db->where('user_id', $wo['user']['id'])->where('group_id', lui_Secure($_GET['group_id']))->delete(T_GROUP_CHAT_USERS);
        $group_id          = lui_Secure($_GET['group_id']);
        $group_chat        = lui_GroupTabData($group_id);
    ...
    if ($s == 'seen' && !empty($_POST['recipient_id']) && is_numeric($_POST['recipient_id']) && $_POST['recipient_id'] > 0) {
        $db->where('from_id', lui_Secure($_POST['recipient_id']))->where('to_id', $wo['user']['id'])->update(T_MESSAGES, array(
            'seen' => time()
        ));
        $data['status'] = 200;
        header("Content-type: application/json");
