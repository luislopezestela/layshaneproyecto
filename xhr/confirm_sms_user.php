...
if ($f == 'confirm_sms_user') {
    if (!empty($_POST['confirm_code']) && !empty($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0) {
        ...
        $confirm_code = lui_ConfirmSMSUser($user_id, $confirm_code);
        ...
        if (empty($errors) && $confirm_code === true) {
            $data = array(
                'status' => 200,
                'location' => $wo['config']['site_url'] . '/index.php?link1=reset-password&code=' . $user_id . "_" . lui_UserData($user_id)['password']
            );
        }
    }
