...
if ($f == 'create_new_audio_call') {
    ...
    $user_1      = lui_UserData($_GET['user_id1']);
    $user_2      = lui_UserData($_GET['user_id2']);
    ...
    if ($wo['config']['agora_chat_video'] == 1) {
        ...
        if ($insertData > 0) {
            ...
            $data = array(
                ...
                'access_token' => '',
                'id' => $insertData,
                'html' => lui_LoadPage('modals/calling-audio'),
        ...
        $insertData = lui_CreateNewAudioCall(array(
            'access_token' => lui_Secure($token_),
            'from_id' => lui_Secure($_GET['user_id1']),
            'to_id' => lui_Secure($_GET['user_id2']),
            'access_token_2' => lui_Secure($token_2),
        ...
        if ($insertData > 0) {
            $wo['calling_user'] = lui_UserData($_GET['user_id2']);
            if (!empty($wo['calling_user']['ios_m_device_id']) && $wo['config']['ios_push_messages'] == 1) {
                $send_array = array(
                    'send_to' => array(
                        $wo['calling_user']['ios_m_device_id']
