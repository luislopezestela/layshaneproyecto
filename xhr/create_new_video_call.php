...
if ($f == 'create_new_video_call') {
    ...
    if (empty($_GET['user_id2']) || empty($_GET['user_id1']) || lui_CheckMainSession($hash_id) === false || $_GET['user_id1'] != $wo['user']['user_id']) {
        exit();
    }
    ...
    $user_1       = lui_UserData(lui_Secure($_GET['user_id1']));
    $user_2       = lui_UserData(lui_Secure($_GET['user_id2']));
    $room_script  = sha1(rand(1111111, 9999999999));

    if ($wo['config']['agora_chat_video'] == 1) {
        $wo['AgoraToken'] = null;
        if (!empty($wo['config']['agora_chat_app_certificate'])) {
            include_once 'luisincludes/librerias/AgoraDynamicKey/src/RtcTokenBuilder.php';
        ...
        $call_type = 'video';
        $insertData = lui_CreateNewAgoraCall(array(
            'from_id' => lui_Secure($_GET['user_id1']),
            'to_id' => lui_Secure($_GET['user_id2']),
        ...
        if ($insertData > 0) {
            ...
            if (!empty($wo['calling_user']['ios_m_device_id']) && $wo['config']['ios_push_messages'] == 1) {
                ...
                lui_SendPushNotification($send_array,'ios_messenger');
            }
            if (!empty($wo['calling_user']['android_m_device_id']) && $wo['config']['android_push_messages'] == 1) {
                ...
                lui_SendPushNotification($send_array,'android_messenger');
            }
            $data = array(
                'status' => 200,
                'access_token' => '',
                ...
                'html' => lui_LoadPage('modals/calling'),
        ...
        include_once('luisincludes/librerias/twilio/vendor/autoload.php');
        ...
        $insertData = lui_CreateNewVideoCall(array(
            'access_token' => lui_Secure($token_),
            'from_id' => lui_Secure($_GET['user_id1']),
            'to_id' => lui_Secure($_GET['user_id2']),
            'access_token_2' => lui_Secure($token_2),
            'room_name' => $room_script
        ));
        if ($insertData > 0) {
            $wo['calling_user'] = $user_2;
