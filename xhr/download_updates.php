...
if ($f == 'download_updates') {
    if (lui_CheckMainSession($hash_id) === true) {
        ...
        if (!empty($_GET['purchase_code'])) {
            ...
            $file          = file_get_contents("http://www.layshane.com/buscar_actualizaciones.php?code={$purchase_code}&version=$version&full=true&url=$siteurl", false, stream_context_create($arrContextOptions));
                ...
                if ($check['status'] == 'SUCCESS') {
                    if (!empty($check['versions'])) {
                        foreach ($check['versions'] as $key => $version) {
                            ...
                            //     @fclose($f);
                            // }
                            $updater = file_put_contents('updates/' . $version . '/script.zip', file_get_contents("https://www.layshane.com/get_update.php?code={$purchase_code}&version=$version&full=true", false, stream_context_create($arrContextOptions)));
                            if ($updater) {
                                ...
                                $data['ERROR_NAME'] = 'Se encontró un error durante la descarga, descargue y actualice su sitio manualmente.';
                                $data['status']     = 400;
                            }
                        }
                    }
