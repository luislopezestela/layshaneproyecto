...
if ($f == 'get_messages') {
    if (lui_CheckMainSession($hash_id) === true) {
        ...
        $messages = lui_GetMessagesUsers($wo['user']['user_id'], '', 5);
        //$page_messages = lui_GetPageChatList($wo['user']['user_id'], 5);
        $groups_messages = lui_GetGroupsListAPP(array('limit' => 5));
        $array = array();
        if (!empty($messages)) {
            ...
            foreach ($array as $key => $value) {
                if ($array_count < 5) {
                    if (!empty($value['group_id']) && !empty($value['last_message'])) {
                        $wo['group'] = $value;
                        $data['html'] .= lui_LoadPage('header/group_messages');
                    }
                    elseif (!empty($value['message']['page_id']) && $value['message']['page_id'] > 0) {
                        ...
                        $data['html'] .= lui_LoadPage('header/page_messages');
                    }
                    elseif(!empty($value['message']['user_id']) && $value['message']['user_id'] > 0){
                        $message = lui_GetMessagesHeader(array('user_id' => $value['user_id']), 'user');
                        if (!empty($message['messageUser'])) {
                            $wo['message'] = $value;
                            $data['html'] .= lui_LoadPage('header/messages');
                        }
                    }
        ...
        //             $message = lui_GetMessagesHeader(array('user_id' => $wo['message']['user_id']), 'user');
        ...
        //                 $message = lui_GetMessagesHeader(array('user_id' => $messages_value['user_id']), 1);
        ...
        //         foreach ($messages as $key => $wo['message']) {
        //             $message = lui_GetMessagesHeader(array('user_id' => $wo['message']['user_id']), 1);
        //             if (!empty($message['messageUser'])) {
        //                 $data['html'] .= lui_LoadPage('header/messages');
        //             }
        //         }
        ...
        //     }
        // } 
        $data['messages_url']  = lui_SeoLink('index.php?link1=messages');
