...
if ($f == 'google_login') {
    ...
    if ($wo['loggedin'] != true && $wo['config']['googleLogin'] != 0 && !empty($wo['config']['googleAppId']) && !empty($wo['config']['googleAppKey']) && !empty($_POST['id_token'])) {
        ...
        if (!empty($social_id)) {
            ...
            if (lui_EmailExists($social_email) === true) {
                $create_session = true;
            } else {
                $str          = md5(microtime());
                $id           = substr($str, 0, 9);
                ...
                $re_data      = array(
                    ...
                    'email' => lui_Secure($social_email, 0),
                    'password' => lui_Secure(md5($password), 0),
                    'email_code' => lui_Secure(md5(time()), 0),
                    'first_name' => lui_Secure($social_name),
                    'src' => 'google',
                    'lastseen' => time(),
                    'social_login' => 1,
                    'active' => '1'
            ...
            if ($create_session == true) {
                lui_SetLoginWithSession($social_email);
                $user_id = lui_UserIdFromEmail($social_email);
