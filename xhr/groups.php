...
if ($f == 'groups') {
    if ($s == 'create_group' && $wo['config']['can_use_groups']) {
        if (empty($_POST['group_name']) || empty($_POST['group_title']) || empty(lui_Secure($_POST['group_title'])) || lui_CheckSession($hash_id) === false) {
            $errors[] = $error_icon . $wo['lang']['please_check_details'];
        } else {
            $is_exist = lui_IsNameExist($_POST['group_name'], 0);
        ...
        if (empty($errors)) {
            ...
            $re_group_data = array(
                'group_name' => lui_Secure($_POST['group_name']),
                'user_id' => lui_Secure($wo['user']['user_id']),
                'group_title' => lui_Secure($_POST['group_title'],1),
                'about' => lui_Secure($_POST['about'],1),
                'category' => lui_Secure($_POST['category']),
                'sub_category' => $sub_category,
                'privacy' => lui_Secure($privacy),
                'active' => '1',
                'time' => time()
            );
            if ($privacy == 2) {
            ...
            if (!empty($fields)) {
                foreach ($fields as $key => $field) {
                    if ($field['required'] == 'on' && empty($_POST['fid_' . $field['id']])) {
                        ...
                        $re_group_data['fid_' . $field['id']] = lui_Secure($_POST['fid_' . $field['id']]);
                    }
                }
            }
            $register_group = lui_RegisterGroup($re_group_data);
            if ($register_group) {
                if ($privacy == 2) {
                    $group_id = lui_GroupIdFromGroupname(lui_Secure($_POST['group_name']));
                    $user_id  = $wo['user']['id'];
                    $active   = 1;
                ...
                $data = array(
                    'status' => 200,
                    'location' => lui_SeoLink('index.php?link1=timeline&u=' . lui_Secure($_POST['group_name']))
                );
            }
        }
        header("Content-type: application/json");
    ...
    if ($s == 'update_privacy_setting') {
        if (!empty($_POST['group_id']) && is_numeric($_POST['group_id']) && $_POST['group_id'] > 0 && lui_CheckSession($hash_id) === true) {
            $group_data   = lui_GroupData($_POST['group_id']);
            $privacy      = 1;
            $join_privacy = 1;
            $array        = array(
                1,
            ...
            if ($group_data['user_id'] == $wo['user']['id'] || lui_IsCanGroupUpdate($_POST['group_id'], 'privacy')) {
                if (empty($errors)) {
                    $Update_data = array(
                        'privacy' => $privacy,
                        'join_privacy' => $join_privacy
                    );
                    if (lui_UpdateGroupData($_POST['group_id'], $Update_data)) {
                        $data = array(
                            'status' => 200,
                            'message' => $success_icon . $wo['lang']['setting_updated']
                        );
    ...
    if ($s == 'update_images_setting') {
        if (isset($_POST['group_id']) && is_numeric($_POST['group_id']) && $_POST['group_id'] > 0 && lui_CheckSession($hash_id) === true) {
            $Userdata = lui_GroupData($_POST['group_id']);
            if (!empty($Userdata['id'])) {
                if (!empty($_FILES['avatar']['name'])) {
                    if (lui_UploadImage($_FILES["avatar"]["tmp_name"], $_FILES['avatar']['name'], 'avatar', $_FILES['avatar']['type'], $_POST['group_id'], 'group') === true) {
                        $page_data = lui_GroupData($_POST['group_id']);
                    }
                }
                if (!empty($_FILES['cover']['name'])) {
                    if (lui_UploadImage($_FILES["cover"]["tmp_name"], $_FILES['cover']['name'], 'cover', $_FILES['cover']['type'], $_POST['group_id'], 'group') === true) {
                        $page_data = lui_GroupData($_POST['group_id']);
                    }
                }
                if ($Userdata['user_id'] == $wo['user']['id'] || lui_IsCanGroupUpdate($_POST['group_id'], 'avatar')) {
                    if (empty($errors)) {
                        $Update_data = array(
                            'active' => '1'
                        );
                        if (lui_UpdateGroupData($_POST['group_id'], $Update_data)) {
                            $userdata2 = lui_GroupData($_POST['group_id']);
                            $data      = array(
                                'status' => 200,
                                'message' => $success_icon . $wo['lang']['setting_updated'],
    ...
    if ($s == 'update_general_settings') {
        if (!empty($_POST['group_id']) && is_numeric($_POST['group_id']) && $_POST['group_id'] > 0 && lui_CheckSession($hash_id) === true) {
            $group_data = lui_GroupData($_POST['group_id']);
            if (empty($_POST['group_name']) OR empty($_POST['group_category']) OR empty($_POST['group_title']) OR empty(lui_Secure($_POST['group_title']))) {
                $errors[] = $error_icon . $wo['lang']['please_check_details'];
            } else {
                ...
                if ($group_data['user_id'] == $wo['user']['id'] || lui_IsCanGroupUpdate($_POST['group_id'], 'general')) {
                    if (empty($errors)) {
                        $sub_category = '';
    ...
    if ($s == 'delete_group') {
        if (!empty($_POST['group_id']) && is_numeric($_POST['group_id']) && $_POST['group_id'] > 0 && lui_CheckSession($hash_id) === true) {
            if (!lui_HashPassword($_POST['password'], $wo['user']['password']) && !lui_CheckGroupAdminPassword($_POST['password'], $_POST['group_id'])) {
                $errors[] = $error_icon . $wo['lang']['current_password_mismatch'];
            }
            $group_data = lui_GroupData($_POST['group_id']);
            if ($group_data['user_id'] == $wo['user']['id'] || lui_IsCanGroupUpdate($_POST['group_id'], 'delete_group')) {
                if (empty($errors)) {
                    if (lui_DeleteGroup($_POST['group_id']) === true) {
                        $data = array(
                            'status' => 200,
                            'message' => $success_icon . $wo['lang']['group_deleted'],
                            'location' => lui_SeoLink('index.php?link1=groups')
                        );
                    }
                }
            } else {
    ...
    if ($s == 'accept_request') {
        if (isset($_GET['user_id']) && is_numeric($_GET['user_id']) && $_GET['user_id'] > 0 && !empty($_GET['group_id']) && is_numeric($_GET['group_id']) && $_GET['group_id'] > 0) {
            if (lui_AcceptJoinRequest($_GET['user_id'], $_GET['group_id']) === true) {
                $data = array(
                    'status' => 200
                );
            }
    ...
    if ($s == 'delete_request') {
        if (isset($_GET['user_id']) && is_numeric($_GET['user_id']) && $_GET['user_id'] > 0 && !empty($_GET['group_id']) && is_numeric($_GET['group_id']) && $_GET['group_id'] > 0) {
            if (lui_DeleteJoinRequest($_GET['user_id'], $_GET['group_id']) === true) {
                $data = array(
                    'status' => 200
                );
            }
    ...
    if ($s == 'delete_joined_user') {
        if (isset($_GET['user_id']) && is_numeric($_GET['user_id']) && $_GET['user_id'] > 0 && !empty($_GET['group_id']) && is_numeric($_GET['group_id']) && $_GET['group_id'] > 0) {
            $group_data = lui_GroupData($_GET['group_id']);
            if ($group_data['user_id'] == $wo['user']['id'] || lui_IsCanGroupUpdate($_GET['group_id'], 'members')) {
                if (lui_LeaveGroup($_GET['group_id'], $_GET['user_id']) === true) {
                    $data = array(
                        'status' => 200
                    );
                }
    ...
    if ($s == 'add_admin') {
        if (isset($_GET['user_id']) && is_numeric($_GET['user_id']) && $_GET['user_id'] > 0 && !empty($_GET['group_id']) && is_numeric($_GET['group_id']) && $_GET['group_id'] > 0) {
            $group_data = lui_GroupData($_GET['group_id']);
            if ($group_data['user_id'] == $wo['user']['id'] || lui_IsCanGroupUpdate($_GET['group_id'], 'members')) {
                $member = lui_Secure($_GET['user_id']);
                $group  = lui_Secure($_GET['group_id']);
                $data   = array(
                    'status' => 304
                );
                $code   = lui_AddGroupAdmin($member, $group);
                if ($code === 1) {
                    $data['status'] = 200;
                    $data['code']   = 1;
                } elseif ($code === 0) {
    ...
    if ($s == 'privileges') {
        if (!empty($_POST['group_id']) && is_numeric($_POST['group_id']) && $_POST['group_id'] > 0 && !empty($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0) {
            $group_data = lui_GroupData($_POST['group_id']);
            if ($group_data['user_id'] == $wo['user']['id'] || lui_IsCanGroupUpdate($_POST['group_id'], 'members')) {
                $update_array = array(
                    'general' => 0,
                    'privacy' => 0,
                    'avatar' => 0,
                ...
                if (lui_UpdateGroupAdminData($_POST['group_id'], $update_array, $_POST['user_id'])) {
                    $data = array(
                        'status' => 200,
                        'message' => $success_icon . $wo['lang']['setting_updated']
                    );
