...
if ($f == "iyzipay") {
    if ($s == 'create') {
        if (!empty($_GET['amount']) && is_numeric($_GET['amount']) && $_GET['amount'] > 0) {
            $price = lui_Secure($_GET['amount']);
            require_once 'luisincludes/librerias/iyzipay/samples/config.php';
        ...
        if (!empty($_GET['ConversationId']) && !empty($_POST['token'])) {
            ...
            require_once 'luis/librerias/iyzipay/samples/config.php';
            ...
            if ($checkoutForm->getPaymentStatus() == 'SUCCESS') {
                ...
                $_GET['user_id'] = lui_Secure($_GET['user_id']);
