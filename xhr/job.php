...
if ($f == 'job' && $wo['config']['job_system'] == 1) {
    ...
    if ($s == 'create_job' && $wo['config']['can_use_jobs']) {
        if (!empty($_POST['job_title']) && !empty($_POST['description']) && !empty($_POST['location']) && !empty($_POST['job_type']) && in_array($_POST['job_type'], $job_type) && !empty($_POST['category']) && in_array($_POST['category'], array_keys($wo['job_categories']))) {
            if (!empty($_POST['page_id'])) {
                $page_data = $db->where('page_id', lui_Secure($_POST['page_id']))->getOne(T_PAGES);
                if (empty($page_data) || !lui_IsPageOnwer($page_data->page_id)) {
            ...
            if (empty($data['error'])) {
                ...
                if (!empty($_POST['job_title'])) {
                    $insert_array['title'] = lui_Secure($_POST['job_title'],1);
                }
                if (!empty($_POST['location'])) {
                    $insert_array['location'] = lui_Secure($_POST['location']);
                }
                if (!empty($_POST['lat'])) {
                    $insert_array['lat'] = lui_Secure($_POST['lat']);
                }
                if (!empty($_POST['lng'])) {
                    $insert_array['lng'] = lui_Secure($_POST['lng']);
                }
                if (!empty($_POST['minimum']) && is_numeric($_POST['minimum']) && $_POST['minimum'] > 0) {
                    $insert_array['minimum'] = lui_Secure($_POST['minimum']);
                }
                if (!empty($_POST['maximum']) && is_numeric($_POST['maximum']) && $_POST['maximum'] > 0) {
                    $insert_array['maximum'] = lui_Secure($_POST['maximum']);
                }
                if (!empty($_POST['salary_date']) && in_array($_POST['salary_date'], $salary_date)) {
                    $insert_array['salary_date'] = lui_Secure($_POST['salary_date']);
                }
                if (!empty($_POST['job_type'])) {
                    $insert_array['job_type'] = lui_Secure($_POST['job_type']);
                }
                if (!empty($_POST['category'])) {
                    $insert_array['category'] = lui_Secure($_POST['category']);
                }
                if (isset($_POST['currency'])) {
                    if (in_array($_POST['currency'], array_keys($wo['currencies']))) {
                        $insert_array['currency'] = lui_Secure($_POST['currency']);
                    }
                }
                if (!empty($_POST['question_one'])) {
                    if (!empty($_POST['question_one_type']) && in_array($_POST['question_one_type'], $question_type)) {
                        if ($_POST['question_one_type'] != 'multiple_choice_question') {
                            $insert_array['question_one']      = lui_Secure($_POST['question_one'],1);
                            $insert_array['question_one_type'] = lui_Secure($_POST['question_one_type']);
                ...
                if (!empty($_POST['question_two'])) {
                    if (!empty($_POST['question_two_type']) && in_array($_POST['question_two_type'], $question_type)) {
                        if ($_POST['question_two_type'] != 'multiple_choice_question') {
                            $insert_array['question_two']      = lui_Secure($_POST['question_two'],1);
                            $insert_array['question_two_type'] = lui_Secure($_POST['question_two_type']);
                ...
                if (!empty($_POST['question_three'])) {
                    if (!empty($_POST['question_three_type']) && in_array($_POST['question_three_type'], $question_type)) {
                        if ($_POST['question_three_type'] != 'multiple_choice_question') {
                            $insert_array['question_three']      = lui_Secure($_POST['question_three'],1);
                            $insert_array['question_three_type'] = lui_Secure($_POST['question_three_type']);
                ...
                if (!empty($_POST['description'])) {
                    $insert_array['description'] = lui_Secure($_POST['description'],1);
                }
                $insert_array['image'] = '';
    ...
    if ($s == 'get_apply_modal') {
        if (!empty($_POST['job_id']) && is_numeric($_POST['job_id']) && $_POST['job_id'] > 0) {
            $wo['job']      = lui_GetJobById($_POST['job_id']);
            $data['html']   = lui_LoadPage("modals/apply_job");
            $data['status'] = 200;
        }
    }
    if ($s == 'apply_job' && !empty($_POST['job_id']) && is_numeric($_POST['job_id']) && $_POST['job_id'] > 0) {
        $job = lui_GetJobById($_POST['job_id']);
        if (!empty($job) && !empty($_POST['user_name']) && !empty($_POST['phone_number']) && !empty($_POST['location']) && !empty($_POST['email']) && $job['apply'] == false) {
            ...
            if (!empty($job['question_one'])) {
                if ($job['question_one_type'] == 'yes_no_question' && !empty($_POST['question_one_answer']) && in_array($_POST['question_one_answer'], array(
                    ...
                    $insert_data['question_one_answer'] = lui_Secure($_POST['question_one_answer']);
            ...
            if (!empty($job['question_two'])) {
                if ($job['question_two_type'] == 'yes_no_question' && in_array($_POST['question_two_answer'], array(
                    ...
                    $insert_data['question_two_answer'] = lui_Secure($_POST['question_two_answer']);
            ...
            if (!empty($job['question_three'])) {
                if ($job['question_three_type'] == 'yes_no_question' && in_array($_POST['question_three_answer'], array(
                    ...
                    $insert_data['question_three_answer'] = lui_Secure($_POST['question_three_answer']);
            ...
            if ($insert == true) {
                $insert_data['user_name']    = lui_Secure($_POST['user_name']);
                $insert_data['phone_number'] = lui_Secure($_POST['phone_number']);
                $insert_data['location']     = lui_Secure($_POST['location']);
                $insert_data['email']        = lui_Secure($_POST['email']);
                $insert_data['job_id']       = lui_Secure($_POST['job_id']);
                $insert_data['user_id']      = $wo['user']['id'];
                $insert_data['page_id']      = $job['page_id'];
                $insert_data['time']         = time();
                if (!empty($_POST['position'])) {
                    $insert_data['position'] = lui_Secure($_POST['position']);
                }
                if (!empty($_POST['where_did_you_work'])) {
                    $insert_data['where_did_you_work'] = lui_Secure($_POST['where_did_you_work']);
                }
                if (!empty($_POST['experience_description'])) {
                    $insert_data['experience_description'] = lui_Secure($_POST['experience_description']);
                }
                if (!empty($_POST['position']) && !empty($_POST['where_did_you_work']) && !empty($_POST['experience_start_date'])) {
                    $insert_data['experience_start_date'] = lui_Secure($_POST['experience_start_date']);
                } else {
                ...
                if (!empty($_POST['i_currently_work']) && $_POST['i_currently_work'] == 'on') {
                    ...
                    if (!empty($_POST['position']) && !empty($_POST['where_did_you_work']) && !empty($_POST['experience_end_date'])) {
                        $insert_data['experience_end_date'] = lui_Secure($_POST['experience_end_date']);
                    } else {
                        $insert_data['experience_end_date'] = '';
                    }
                }
    ...
    if ($s == 'search_jobs') {
        ...
        $result = lui_GetAllJobs($array);
        foreach ($result as $key => $wo['job']) {
            $html .= lui_LoadPage('jobs/jobs_list');
        }
        $data = array(
            'status' => 200,
            'html' => $html
    ...
    if ($s == 'edit_job' && !empty($_POST['job_id']) && is_numeric($_POST['job_id']) && $_POST['job_id'] > 0) {
        $job = lui_GetJobById($_POST['job_id']);
        if (!empty($job) && (((!empty($job['page']) && $job['page']['is_page_onwer']) || empty($job['page'])) || $job['user_id'] == $wo['user']['id'] || lui_IsAdmin() || lui_IsModerator())) {
            $insert_array = array();
            if (!empty($_POST['job_title'])) {
                $insert_array['title'] = lui_Secure($_POST['job_title'],1);
            }
    ...
    if ($s == 'load' && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0 && !empty($_GET['job_id']) && is_numeric($_GET['job_id']) && $_GET['job_id'] > 0) {
        ...
        $job_id    = lui_Secure($_GET['job_id']);
        $html      = '';
        $job_apply = lui_GetApplyJob(array(
            'job_id' => $job_id,
            'offset' => $offset
        ));
        if (!empty($job_apply)) {
            foreach ($job_apply as $key => $wo['job_apply']) {
                $html .= lui_LoadPage('page/job_apply');
            }
            $data = array(
                'status' => 200,
                'html' => $html
    ...
    if ($s == 'delete_job' && !empty($_GET['job_id']) && is_numeric($_GET['job_id']) && $_GET['job_id'] > 0) {
        ...
        if (!empty($job) && ($job->user_id == $wo['user']['id'] || lui_IsModerator() || lui_IsAdmin())) {
