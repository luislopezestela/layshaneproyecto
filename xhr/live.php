...
if ($f == 'live') {
    if ($s == 'create' && $wo['config']['can_use_live']) {
        if (empty($_POST['stream_name'])) {
            ...
            if (!empty($_COOKIE['post_privacy']) && in_array($_COOKIE['post_privacy'], $privacy_array)) {
                $postPrivacy = lui_Secure($_COOKIE['post_privacy']);
            }
            $token = null;
            if (!empty($_POST['token']) && !is_null($_POST['token'])) {
                $token = lui_Secure($_POST['token']);
            }
            $post_id = $db->insert(T_POSTS, array(
                ...
                'agora_token' => $token,
                'stream_name' => lui_Secure($_POST['stream_name']),
                'time' => time()
            ));
            ...
            if (!empty($post_data)) {
                if ($post_data->live_ended == 0) {
                    ...
                    foreach ($comments as $key => $value) {
                        if (!empty($value->text)) {
                            $wo['comment'] = lui_GetPostComment($value->id);
                            $html .= lui_LoadPage('story/includes/live_comment');
                            $count = $count + 1;
                            if ($count == 4) {
                                break;
                            }
                        ...
                        if ($wo['user']['id'] == $post_data->user_id) {
                            ...
                            if (!empty($joined_users)) {
                                foreach ($joined_users as $key => $value) {
                                    ...
                                    $user_data     = lui_UserData($value->user_id);
                                    if (!empty($user_data)) {
    ...
    if ($s == 'delete') {
        if (!empty($_POST['post_id']) && is_numeric($_POST['post_id']) && $_POST['post_id'] > 0) {
            $db->where('post_id', lui_Secure($_POST['post_id']))->where('user_id', $wo['user']['id'])->update(T_POSTS, array(
                'live_ended' => 1
            ));
            if ($wo['config']['live_video_save'] == 0) {
                lui_DeletePost(lui_Secure($_POST['post_id']));
            } else {
                if ($wo['config']['agora_live_video'] == 1 && !empty($wo['config']['agora_app_id']) && !empty($wo['config']['agora_customer_id']) && !empty($wo['config']['agora_customer_certificate']) && $wo['config']['live_video_save'] == 1) {
                    $post = $db->where('post_id', lui_Secure($_POST['post_id']))->getOne(T_POSTS);
                    if (!empty($post)) {
        ...
        if (!empty($posts)) {
            foreach ($posts as $key => $value) {
                if ((!empty($value->agora_resource_id) || !empty($value->agora_sid) || !empty($value->agora_token)) && empty($value->postFile)) {
                    lui_DeletePost($value->id,'shared');
                }
            }
        }
    }
    if ($s == 'create_thumb') {
        if (!empty($_POST['post_id']) && is_numeric($_POST['post_id']) && $_POST['post_id'] > 0 && !empty($_FILES['thumb'])) {
            $is_post = $db->where('post_id', lui_Secure($_POST['post_id']))->where('user_id', $wo['user']['id'])->getValue(T_POSTS, 'COUNT(*)');
            if ($is_post > 0) {
                $fileInfo = array(
