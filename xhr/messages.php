...
if ($f == 'messages') {
    if ($s == 'get_user_messages') {
        if (!empty($_GET['user_id']) AND is_numeric($_GET['user_id']) AND $_GET['user_id'] > 0 && lui_CheckMainSession($hash_id) === true) {
            $html       = '';
            $user_id    = $_GET['user_id'];
            $can_replay = true;
            $recipient  = lui_UserData($user_id);
            $messages   = lui_GetMessages(array(
                'user_id' => $user_id,
                'type' => 'user'
            ));
            if (!empty($recipient['user_id']) && $recipient['message_privacy'] == 1) {
                if (lui_IsFollowing($wo['user']['user_id'], $recipient['user_id']) === false) {
                    $can_replay = false;
                }
            ...
            if ($wo['config']['user_lastseen'] == 1 && $recipient['showlastseen'] != 0) {
                $data['lastseen'] = lui_UserStatus($recipient['user_id'], $recipient['lastseen']);
            }
            ...
            $attachments      = lui_GetLastAttachments($user_id);
            $attachments_html = '';
            ...
            $data['messages_count']   = lui_CountMessages(array(
                'new' => false,
                'user_id' => $user_id
            ));
    ...
    if ($s == 'get_group_messages' && isset($_GET['group_id']) && is_numeric($_GET['group_id']) && $_GET['group_id'] > 0 && lui_CheckMainSession($hash_id)) {
        $html     = '';
        ...
        if (lui_IsGChatOwner($group_id)) {
            $onclick = "Wo_DeleteGroupChat";
        }
        @lui_UpdateGChatLastSeen($group_id);
        foreach ($messages as $wo['message']) {
            $html .= lui_LoadPage('messages/group-text-list');
        }
        $data = array(
            'status' => 200,
            'html' => $html,
    ...
    if ($s == 'get_page_messages' && !empty($_GET['page_user_id']) && !empty($_GET['page_id']) && lui_CheckMainSession($hash_id)) {
        $html                                       = '';
        ...
        foreach ($messages as $wo['message']) {
            $html .= lui_LoadPage('messages/page-chat-list');
        }
        $data = array(
            'status' => 200,
            'html' => $html,
    ...
    if ($s == 'send_message') {
        if ($wo['config']['who_upload'] == 'pro' && $wo['user']['is_pro'] == 0 && !lui_IsAdmin() && (!empty($_FILES['sendMessageFile']) || !empty($_POST['message-record']))) {
            ...
            if ($wo['user']['message_privacy'] != 2) {
                $reply_id = 0;
                $story_id = 0;
                if (isset($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0 && lui_CheckMainSession($hash_id) === true) {
                    $html          = '';
                    $media         = '';
                    $mediaFilename = '';
                    $mediaName     = '';
                    ...
                    $sticker = '';
                    if (isset($_POST['chatSticker']) && lui_IsUrl($_POST['chatSticker']) && strpos($_POST['chatSticker'], '.gif') !== false && !$mediaFilename && !$mediaName) {
                        ...
                        if (!empty($matches) && !empty($matches[2]) && !empty($matches[2][0]) && !empty($matches[3]) && !empty($matches[3][0]) && !empty($matches[4]) && !empty($matches[4][0])) {
                            ...
                            if (strpos($headers['Content-Type'], 'image/') !== false) {
                                $sticker = (isset($_POST['chatSticker']) && lui_IsUrl($_POST['chatSticker'])) ? $_POST['chatSticker'] : '';
                            } else {
                                $invalid_file         = 2;
                                $_POST['chatSticker'] = '';
                            }
                    ...
                    if ($messages > 0) {
                        ...
                        foreach ($messages as $wo['message']) {
                            $wo['message']['color'] = lui_GetChatColor($wo['user']['user_id'], $_POST['user_id']);
                            $html .= lui_LoadPage('messages/messages-text-list');
                        }
                        $data                   = array(
                            'status' => 200,
                        ...
                        if ($wo['config']['emailNotification'] == 1) {
                            ...
                            if (!in_array(false, $send_notif)) {
                                ...
                                $wo['emailNotification']['msg_text'] = lui_Secure($_POST['textSendMessage'],1);
                ...
                } else if (isset($_POST['group_id']) && is_numeric($_POST['group_id']) && $_POST['group_id'] > 0 && lui_CheckMainSession($hash_id) === true) {
                    $html          = '';
                    $media         = '';
                    $mediaFilename = '';
                    $mediaName     = '';
                    ...
                    $message_id = lui_RegisterGroupMessage(array(
                        'from_id' => lui_Secure($wo['user']['user_id'],0),
                        'group_id' => lui_Secure($_POST['group_id'],0),
                        'text' => lui_Secure($_POST['textSendMessage'],1),
                        'media' => lui_Secure($mediaFilename,0),
                        'mediaFileName' => lui_Secure($mediaName,0),
                        'time' => time(),
                    ...
                    if ($message_id > 0) {
                        @lui_UpdateGChat(lui_Secure($_POST['group_id']), array(
                            "time" => time()
                        ));
                        $message = lui_GetGroupMessages(array(
                            'id' => $message_id,
                ...
                } else if (isset($_POST['page_id']) && is_numeric($_POST['page_id']) && lui_CheckMainSession($hash_id) === true) {
                    $page_data    = lui_PageData($_POST['page_id']);
                    $invalid_file = 1;
                    if (!empty($page_data)) {
                        $html          = '';
                        $media         = '';
                        ...
                        if ($page_data['user_id'] == $wo['user']['user_id']) {
                            if ($page_data['user_id'] == $_POST['to_id']) {
                                $to_id = lui_Secure($_POST['from_id']);
                            } else {
                                $to_id = lui_Secure($_POST['to_id']);
                            }
                        }
                        if (!empty($_POST['reply_id']) && is_numeric($_POST['reply_id']) && $_POST['reply_id'] > 0) {
                            $reply_id = lui_Secure($_POST['reply_id']);
                        ...
                        $last_id = lui_RegisterPageMessage(array(
                            'from_id' => lui_Secure($wo['user']['user_id'],0),
                            'page_id' => lui_Secure($_POST['page_id'],0),
                            'to_id' => $to_id,
                            'text' => lui_Secure($_POST['textSendMessage'],1),
                            'media' => lui_Secure($mediaFilename,0),
                            'mediaFileName' => lui_Secure($mediaName,0),
    ...
    if ($s == 'load_previous_messages') {
        ...
        if (!empty($_GET['user_id']) && is_numeric($_GET['user_id']) && $_GET['user_id'] > 0 && !empty($_GET['before_message_id'])) {
            ...
            $before_message_id = lui_Secure($_GET['before_message_id']);
            $messages          = lui_GetMessages(array(
                'user_id' => $user_id,
            ...
            $messages          = lui_GetPageMessages(array(
                ...
                'to_id' => !empty($_GET['from_id']) ? lui_Secure($_GET['from_id']) : 0
            ));
            if ($messages > 0) {
                foreach ($messages as $wo['message']) {
                    $html .= lui_LoadPage('messages/group-text-list');
    ...
    if ($s == 'update_recipients') {
        $html  = '';
        $users = lui_GetMessagesUsers($wo['user']['user_id']);
        $array = array();
        if (!empty($users)) {
        ...
        if (count($array) > 0) {
            foreach ($array as $wo['recipient']) {
                if (!empty($wo['recipient']['message']['page_id'])) {
                    ...
                    $html .= lui_LoadPage('messages/messages-page-list');
                } else {
                    ...
                    $html .= lui_LoadPage('messages/messages-recipients-list');
                }
            }
            $data = array(
                'status' => 200,
    ...
    if ($s == 'get_new_messages') {
        ...
        $reactions                   = array();
        if (isset($_GET['user_id']) && is_numeric($_GET['user_id']) && $_GET['user_id'] > 0 && lui_CheckMainSession($hash_id) === true) {
            $user_id = lui_Secure($_GET['user_id']);
            if (!empty($user_id)) {
                $user_id  = $_GET['user_id'];
                $messages = lui_GetMessages(array(
                ...
                if (!empty($user_id) && $wo['config']['message_typing'] == 1) {
                    $isTyping = lui_IsTyping($user_id);
                    if ($isTyping === true) {
                        $img               = lui_UserData($user_id);
                        $data['is_typing'] = 200;
                        $data['img']       = $img['avatar'];
        ...
        } else if (isset($_GET['group_id']) && is_numeric($_GET['group_id']) && $_GET['group_id'] > 0 && lui_CheckMainSession($hash_id) === true) {
            $group_id = lui_Secure($_GET['group_id']);
            if (!empty($group_id)) {
                $group_id = $group_id;
                $messages = lui_GetGroupMessages(array(
        ...
        } else if (!empty($_GET['from_id']) && !empty($_GET['page_id']) && lui_CheckMainSession($hash_id) === true) {
            $page_id  = lui_Secure($_GET['page_id']);
            $page     = lui_PageData($page_id);
        ...
        if (!empty($user_id)) {
            $data['color'] = lui_GetChatColor($wo['user']['user_id'], $user_id);
        }
        if (!empty($reactions)) {
    ...
    if ($s == 'delete_message') {
        if (isset($_GET['message_id']) && lui_CheckMainSession($hash_id) === true) {
            $message_id = lui_Secure($_GET['message_id']);
            $message    = $db->where('id', $message_id)->getOne(T_MESSAGES);
            if (!empty($message_id) || is_numeric($message_id) || $message_id > 0) {
                if (lui_DeleteMessage($message_id) === true) {
                    $data['status'] = 200;
                    if (!empty($message)) {
    ...
    if ($s == 'delete_conversation') {
        if (isset($_GET['user_id']) && lui_CheckMainSession($hash_id) === true) {
            $user_id = lui_Secure($_GET['user_id']);
            if (!empty($user_id) || is_numeric($user_id) || $user_id > 0) {
                if (lui_DeleteConversation($user_id) === true) {
                    $data = array(
                        'status' => 200,
    ...
    if ($s == 'clear_group_chat') {
        if (isset($_GET['id']) && lui_CheckMainSession($hash_id) === true) {
        ...
        if (!empty($_GET['message_id']) && is_numeric($_GET['message_id']) && $_GET['message_id'] > 0 && !empty($_GET['reaction']) && in_array($_GET['reaction'], $reactions_types)) {
            ...
            if (!empty($message)) {
                ...
                $db->insert(T_REACTIONS, array(
                    ...
                    'reaction' => lui_Secure($_GET['reaction'])
                ));
                $data = array(
                    'status' => 200,
                    'reactions' => lui_GetPostReactions($message_id, 'message'),
