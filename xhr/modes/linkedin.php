...
if ($f == 'experience') {
    if ($s == 'add') {
    	...
    	if (empty($data['message'])) {
            if (!empty($_POST['experience_end'])) {
                $experience_end = lui_Secure($_POST['experience_end']);
            }
            if (!empty($_POST['headline'])) {
                $headline = lui_Secure($_POST['headline']);
            }
            if (!empty($_POST['link'])) {
                $link = lui_Secure($_POST['link']);
            }
    		...
    		$insert_data = array('title' => lui_Secure($_POST['title']),
                                 'company_name' => lui_Secure($_POST['company_name']),
                                 'location' => lui_Secure($_POST['location']),
                                 'experience_start' => lui_Secure($_POST['experience_start']),
                                 'experience_end' => $experience_end,
                                 'industry' => lui_Secure($_POST['industry']),
                                 'description' => lui_Secure($_POST['description'],0,true,1),
                                 'image' => $image,
                                 'link' => $link,
                                 'headline' => $headline,
                                 'time' => time(),
                                 'user_id' => $wo['user']['id'],
                                 'employment_type' => lui_Secure($_POST['employment_type']));
    ...
    if ($s == 'delete') {
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            $experience = $db->where('id',lui_Secure($_POST['id']))->getOne(T_USER_EXPERIENCE);
            if (!empty($experience) && ($experience->user_id == $wo['user']['user_id'] || lui_IsAdmin())) {
                if (!empty($experience->image)) {
                    @unlink($experience->image);
                    lui_DeleteFromToS3($experience->image);
                }
    ...
    if ($s == 'edit') {
        ...
        if (empty($data['message'])) {
            ...
            if (!empty($experience) && ($experience->user_id == $wo['user']['user_id'] || lui_IsAdmin())) {
                ...
                $update_data = array('title' => lui_Secure($_POST['title']),
                                     'company_name' => lui_Secure($_POST['company_name']),
                                     'location' => lui_Secure($_POST['location']),
                                     'experience_start' => lui_Secure($_POST['experience_start']),
                                     'experience_end' => $experience_end,
...
if ($f == 'certification') {
    if ($s == 'add') {
        ...
        if (empty($data['message'])) {
            ...
            if (!empty($_POST['certification_end'])) {
                $certification_end = lui_Secure($_POST['certification_end']);
            }
            if (!empty($_POST['credential_id'])) {
                $credential_id = lui_Secure($_POST['credential_id']);
            }
            if (!empty($_POST['credential_url'])) {
                $credential_url = lui_Secure($_POST['credential_url']);
            }
            if (!empty($_FILES["pdf"])) {
                $fileInfo = array(
            ...
            $insert_data = array('name' => lui_Secure($_POST['name']),
                                 'issuing_organization' => lui_Secure($_POST['issuing_organization']),
                                 'credential_id' => $credential_id,
                                 'credential_url' => $credential_url,
                                 'certification_start' => lui_Secure($_POST['certification_start']),
    ...
    if ($s == 'delete') {
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            $certification = $db->where('id',lui_Secure($_POST['id']))->getOne(T_USER_CERTIFICATION);
            if (!empty($certification) && ($certification->user_id == $wo['user']['user_id'] || lui_IsAdmin())) {
    ...
    if ($s == 'edit') {
        ...
        if (empty($data['message'])) {
            ...
            if (!empty($certification) && ($certification->user_id == $wo['user']['user_id'] || lui_IsAdmin())) {
                ...
                $update_data = array('name' => lui_Secure($_POST['name']),
                                     'issuing_organization' => lui_Secure($_POST['issuing_organization']),
                                     'credential_id' => $credential_id,
                                     'credential_url' => $credential_url,
                                     'certification_start' => lui_Secure($_POST['certification_start']),
...
if ($f == 'project') {
    if ($s == 'add') {
        ...
        if (empty($data['message'])) {
            if (!empty($_POST['project_end'])) {
                $project_end = lui_Secure($_POST['project_end']);
            }
            if (!empty($_POST['associated_with'])) {
                $associated_with = lui_Secure($_POST['associated_with']);
            }
            if (!empty($_POST['description'])) {
                $description = lui_Secure($_POST['description'],0,true,1);
            }
            if (!empty($_POST['project_url'])) {
                $project_url = lui_Secure($_POST['project_url']);
            }
            $insert_data = array('name' => lui_Secure($_POST['name']),
                                 'description' => $description,
                                 'associated_with' => $associated_with,
                                 'project_url' => $project_url,
                                 'project_start' => lui_Secure($_POST['project_start']),
    ...
    if ($s == 'delete') {
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            $project = $db->where('id',lui_Secure($_POST['id']))->getOne(T_USER_PROJECTS);
            if (!empty($project) && ($project->user_id == $wo['user']['user_id'] || lui_IsAdmin())) {
...
if ($f == 'open_to') {
    if ($s == 'find_job') {
        ...
        if (empty($data['message'])) {
            ...
            $insert_data = array('user_id' => $wo['user']['id'],
                                 'job_title' => lui_Secure($_POST['job_title']),
                                 'job_location' => lui_Secure($_POST['job_location']),
                                 'workplaces' => lui_Secure(implode(",",$_POST['workplaces'])),
                                 'job_type' => lui_Secure(implode(",",$_POST['job_type'])),
                                 'type' => 'find_job',
                                 'time' => time());
    ...
    if ($s == 'delete_job') {
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            $db->where('user_id',lui_Secure($_POST['id']))->where('type','find_job')->delete(T_USER_OPEN_TO);
            $data = array('status' => 200);
        }
        else{
    ...
    if ($s == 'edit_job') {
        ...
        if (empty($data['message'])) {
            ...
            $job = $db->where('id',lui_Secure($_POST['id']))->where('type','find_job')->getOne(T_USER_OPEN_TO);
      if (!empty($job) && ($job->user_id == $wo['user']['user_id'] || lui_IsAdmin())) {
                $insert_data = array('job_title' => lui_Secure($_POST['job_title']),
                                   'job_location' => lui_Secure($_POST['job_location']),
                                   'workplaces' => lui_Secure(implode(",",$_POST['workplaces'])),
                                   'job_type' => lui_Secure(implode(",",$_POST['job_type'])));
    ...
    if ($s == 'providing_services') {
      ...
      $_POST['services'] = strip_tags($_POST['services']);
            $insert_data = array('user_id' => $wo['user']['id'],
                               'services' => lui_Secure($_POST['services']),
                               'job_location' => lui_Secure($_POST['job_location']),
                               'description' => lui_Secure($_POST['description'],0,true,1),
                               'time' => time(),
                               'type' => 'service');
    ...
    if ($s == 'delete_service') {
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            $db->where('user_id',lui_Secure($_POST['id']))->where('type','service')->delete(T_USER_OPEN_TO);
            $data = array('status' => 200);
        }
        else{
            $data['message'] = $wo["lang"]['you_not_owner'];
    ...
    if ($s == 'edit_providing_services') {
        ...
        if (empty($data['message'])) {
            $service = $db->where('id',lui_Secure($_POST['id']))->where('type','service')->getOne(T_USER_OPEN_TO);
      if (!empty($service) && ($service->user_id == $wo['user']['user_id'] || lui_IsAdmin())) {
        $_POST['services'] = preg_replace('/on[^<>=]+=[^<>]*/m', '', $_POST['services']);
          $_POST['services'] = strip_tags($_POST['services']);
                $insert_data = array('services' => lui_Secure($_POST['services']),
                                   'job_location' => lui_Secure($_POST['job_location']),
                                   'description' => lui_Secure($_POST['description'],0,true,1));
...
if ($f == 'search_linkedin') {
    ...
    foreach ($info as $key => $wo['result']) {
        if ($wo['result']['sort_type'] == 'user') {
            $html .= lui_LoadPage('search/linkedin_user_list');
        }
        if ($wo['result']['sort_type'] == 'page') {
            $html .= lui_LoadPage('search/linkedin_page_list');
        }
        if ($wo['result']['sort_type'] == 'group') {
            $html .= lui_LoadPage('search/linkedin_group_list');
        }
        if ($wo['result']['sort_type'] == 'service') {
            $html .= lui_LoadPage('search/linkedin_service_list');
        }
    }
    $data = array(
        'status' => 200,
...
if ($f == "get_more_open_posts") {
    ...
    if (isset($_GET['after_last_id']) && is_numeric($_GET['after_last_id'])) {
        $posts = lui_GetOpenToWorkPosts(10,$_GET['after_last_id']);
        if (!empty($posts)) {
