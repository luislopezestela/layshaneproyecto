...
if ($f == 'notifications') {
    if ($s == 'get-users') {
        ...
        $html  = '';
        $users = lui_GetUsersByName($_POST['name']);
        if ($users && count($users) > 0) {
            foreach ($users as $wo['notificated-user']) {
                $html .= lui_LoadAdminPage('mass-notifications/list');
            }
            $data['status'] = 200;
            $data['html']   = $html;
        }
    ...
    if ($s == 'send') {
        ...
        if (!$error) {
            if (empty($_POST['notifc-users'])) {
                $users = lui_GetUserIds();
            } elseif ($_POST['notifc-users'] && strlen($_POST['notifc-users']) > 0) {
                $users = explode(',', $_POST['notifc-users']);
            }
            $url               = lui_Secure($_POST['url']);
            $message           = lui_Secure($_POST['description']);
            $registration_data = array(
                'full_link' => $url,
                'text' => $message,
                'recipients' => $users
            );
            if (lui_RegisterAdminNotification($registration_data)) {
                $data = array(
                    'status' => 200,
