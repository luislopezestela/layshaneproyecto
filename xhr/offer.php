...
if ($f == 'offer' && $wo['config']['offer_system'] == 1) {
            ...
            if (!empty($page_data) && lui_IsPageOnwer($page_data->page_id)) {
                ...
                if ($_POST['discount_type'] == 'discount_percent') {
                    if (empty($_POST['discount_percent']) || !is_numeric($_POST['discount_percent']) || $_POST['discount_percent'] < 1 || $_POST['discount_percent'] > 99) {
                        ...
                        $discount_percent = lui_Secure($_POST['discount_percent']);
                        $discount_amount  = 0;
                        $buy              = 0;
                        $get              = 0;
                        $spend            = 0;
                    ...
                    if (empty($_POST['discount_amount']) || !is_numeric($_POST['discount_amount']) || $_POST['discount_amount'] < 1) {
                        ...
                        $discount_amount  = lui_Secure($_POST['discount_amount']);
                        $discount_percent = 0;
                        $buy              = 0;
                        $get              = 0;
                        $spend            = 0;
                    ...
                    if (empty($_POST['discount_percent']) || !is_numeric($_POST['discount_percent']) || $_POST['discount_percent'] < 1 || $_POST['discount_percent'] > 99 || empty($_POST['buy']) || !is_numeric($_POST['buy']) || $_POST['buy'] < 1 || empty($_POST['get']) || !is_numeric($_POST['get']) || $_POST['get'] < 1) {
                        ...
                        $buy              = lui_Secure($_POST['buy']);
                        $get              = lui_Secure($_POST['get']);
                        $discount_amount  = 0;
                        $discount_percent = lui_Secure($_POST['discount_percent']);
                        $spend            = 0;
                        $amount_off       = 0;
                    ...
                    if (empty($_POST['spend']) || !is_numeric($_POST['spend']) || $_POST['spend'] < 1 || empty($_POST['amount_off']) || !is_numeric($_POST['amount_off']) || $_POST['amount_off'] < 1) {
                        ...
                        $discount_amount  = 0;
                        $discount_percent = 0;
                        $spend            = lui_Secure($_POST['spend']);
                        $amount_off       = lui_Secure($_POST['amount_off']);
                    }
                }
                ...
                if (empty($data['error'])) {
                    $offer_id    = $db->insert(T_OFFER, array(
                        ...
                        'description' => lui_Secure($_POST['description'],1),
                        'expire_date' => lui_Secure($_POST['expire_date']),
                        'expire_time' => lui_Secure($_POST['expire_time']),
                        'discounted_items' => lui_Secure($_POST['discounted_items']),
                    ...
                    // $description = mb_substr(lui_Secure($_POST['description']), 0, 175, "UTF-8") . "...";
                    $description = lui_Secure($_POST['description'],1);
    ...
    if ($s == 'delete_offer' && !empty($_GET['offer_id']) && is_numeric($_GET['offer_id']) && $_GET['offer_id'] > 0) {
        $offer_id = lui_Secure($_GET['offer_id']);
        $offer    = $db->where('id', $offer_id)->getOne(T_OFFER);
        if (!empty($offer) && ($offer->user_id == $wo['user']['id'] || lui_IsModerator() || lui_IsAdmin())) {
            @unlink($offer->image);
    ...
    if ($s == 'load_more' && !empty($_POST['last_id']) && is_numeric($_POST['last_id']) && $_POST['last_id'] > 0) {
        $last_id = lui_Secure($_POST['last_id']);
        $offer   = $db->where('id', $last_id)->getOne(T_OFFER);
        $html    = '';
        if (!empty($offer)) {
            $offers = lui_GetAllOffers(array(
                'after_id' => $last_id,
                'limit' => 15
            ));
            foreach ($offers as $key => $wo['offer']) {
                $html .= lui_LoadPage('offers/offers');
            }
        }
        $data['status'] = 200;
        $data['html']   = $html;
    ...
    if ($s == 'get_offer' && !empty($_POST['offer_id']) && is_numeric($_POST['offer_id']) && $_POST['offer_id'] > 0) {
        $wo['offer']    = lui_GetOfferById($_POST['offer_id']);
        $html           = '';
        $data['status'] = 400;
        if (!empty($wo['offer'])) {
            $wo['offer']['description'] = lui_EditMarkup($wo['offer']['description'], true, true, true);
            $wo['offer']['description'] = str_replace('<br>', "\n", $wo['offer']['description']);
            $html                       = lui_LoadPage('modals/edit_offer');
            $data['status']             = 200;
            $data['html']               = $html;
        }
    }
    if ($s == 'edit_offer' && !empty($_POST['offer_id']) && is_numeric($_POST['offer_id']) && $_POST['offer_id'] > 0) {
        ...
        $offer    = $db->where('id', $offer_id)->getOne(T_OFFER);
        if (!empty($offer) && (lui_IsPageOnwer($offer->page_id) || $offer->user_id == $wo['user']['id'] || lui_IsModerator() || lui_IsAdmin())) {
            if (!empty($_POST['discount_type']) && in_array($_POST['discount_type'], $discount_type) && !empty($_POST['page_id'])) {
                ...
                if (!empty($page_data) && lui_IsPageOnwer($page_data->page_id)) {
                    ...
                    if (empty($data['error'])) {
                        $description    = mb_substr(lui_Secure($_POST['description'],1), 0, 175, "UTF-8") . "...";
