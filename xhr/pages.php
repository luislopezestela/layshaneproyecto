...
if ($f == 'pages') {
    if ($s == 'create_page') {
        if (!empty($_POST['page_name']) && ($_POST['page_name'] == 'layshane_star' || $_POST['page_name'] == 'sunshine' || $_POST['page_name'] == $wo['config']['theme'])) {
            $_POST['page_name'] = "";
        }
        if (empty($_POST['page_name']) || empty($_POST['page_title']) || empty(lui_Secure($_POST['page_title'])) || lui_CheckSession($hash_id) === false) {
            $errors[] = $error_icon . $wo['lang']['please_check_details'];
        } else {
            $is_exist = lui_IsNameExist($_POST['page_name'], 0);
        ...
        if (empty($errors)) {
            ...
            $re_page_data = array(
                ...
                'page_title' => lui_Secure($_POST['page_title'],1),
                'page_description' => lui_Secure($_POST['page_description'],1),
                'page_category' => lui_Secure($_POST['page_category']),
            ...
            if (!empty($fields)) {
                foreach ($fields as $key => $field) {
                    if ($field['required'] == 'on' && empty($_POST['fid_' . $field['id']])) {
                        ...
                        $re_page_data['fid_' . $field['id']] = lui_Secure($_POST['fid_' . $field['id']]);
                    }
                }
            }
            $register_page = lui_RegisterPage($re_page_data);
            if ($register_page) {
                $data = array(
                    'status' => 200,
                    'location' => lui_SeoLink('index.php?link1=timeline&u=' . lui_Secure($_POST['page_name']))
                );
            }
        }
        header("Content-type: application/json");
    ...
    if ($s == 'update_information_setting') {
        if (!empty($_POST['page_id']) && is_numeric($_POST['page_id']) && $_POST['page_id'] > 0 && lui_CheckSession($hash_id) === true) {
            $PageData = lui_PageData($_POST['page_id']);
            if (!empty($_POST['website'])) {
                if (!filter_var($_POST['website'], FILTER_VALIDATE_URL)) {
                    $errors[] = $error_icon . $wo['lang']['website_invalid_characters'];
                }
            }
            if ($PageData['user_id'] == $wo['user']['id'] || lui_IsCanPageUpdate($_POST['page_id'], 'info')) {
                if (empty($errors)) {
                    $Update_data = array(
    ...
    if ($s == 'update_sociallink_setting') {
        if (!empty($_POST['page_id']) && is_numeric($_POST['page_id']) && $_POST['page_id'] > 0 && lui_CheckSession($hash_id) === true) {
            $PageData = lui_PageData($_POST['page_id']);
            if ($PageData['user_id'] == $wo['user']['id'] || lui_IsCanPageUpdate($_POST['page_id'], 'social')) {
                if (empty($errors)) {
                    $Update_data = array(
    ...
    if ($s == 'update_images_setting') {
        if (isset($_POST['page_id']) && is_numeric($_POST['page_id']) && $_POST['page_id'] > 0 && lui_CheckSession($hash_id) === true) {
            ...
            if (!empty($Userdata['page_id'])) {
                if ($Userdata['user_id'] == $wo['user']['id'] || lui_IsCanPageUpdate($_POST['page_id'], 'avatar')) {
                    if (isset($_FILES['avatar']['name'])) {
                        if (lui_UploadImage($_FILES["avatar"]["tmp_name"], $_FILES['avatar']['name'], 'avatar', $_FILES['avatar']['type'], $_POST['page_id'], 'page') === true) {
                            $page_data = lui_PageData($_POST['page_id']);
                        }
                    }
                    if (isset($_FILES['cover']['name'])) {
                        if (lui_UploadImage($_FILES["cover"]["tmp_name"], $_FILES['cover']['name'], 'cover', $_FILES['cover']['type'], $_POST['page_id'], 'page') === true) {
                            $page_data = lui_PageData($_POST['page_id']);
                        }
                    }
                    if (empty($errors)) {
    ...
    if ($s == 'update_general_settings') {
        ...
        if (!empty($_POST['page_id']) && is_numeric($_POST['page_id']) && $_POST['page_id'] > 0 && lui_CheckSession($hash_id) === true) {
            $PageData = lui_PageData($_POST['page_id']);
            if (empty($_POST['page_name']) OR empty($_POST['page_category']) OR empty($_POST['page_title']) OR empty(lui_Secure($_POST['page_title']))) {
                $errors[] = $error_icon . $wo['lang']['please_check_details'];
            } else {
                ...
                if ($PageData['user_id'] == $wo['user']['id'] || lui_IsCanPageUpdate($_POST['page_id'], 'general')) {
                    if (empty($errors)) {
                        $sub_category = '';
                        if (!empty($_POST['page_sub_category']) && !empty($wo['page_sub_categories'][$_POST['page_category']])) {
                            foreach ($wo['page_sub_categories'][$_POST['page_category']] as $key => $value) {
                        ...
                        if (!empty($_POST['users_post'])) {
                            if (in_array($_POST['users_post'], $array)) {
                                $Update_data['users_post'] = lui_Secure($_POST['users_post']);
                            }
                        }
                        $fields = lui_GetCustomFields('page');
                        if (!empty($fields)) {
                            foreach ($fields as $key => $field) {
                                if ($field['required'] == 'on' && empty($_POST['fid_' . $field['id']])) {
                                    ...
                                    $Update_data['fid_' . $field['id']] = lui_Secure($_POST['fid_' . $field['id']]);
                                }
                            }
                        }
                        if (lui_UpdatePageData($_POST['page_id'], $Update_data)) {
    ...
    if ($s == 'delete_page') {
        if (!empty($_POST['page_id']) && is_numeric($_POST['page_id']) && $_POST['page_id'] > 0 && lui_CheckSession($hash_id) === true) {
            if (!lui_HashPassword($_POST['password'], $wo['user']['password']) && !lui_CheckPageAdminPassword($_POST['password'], $_POST['page_id'])) {
                $errors[] = $error_icon . $wo['lang']['current_password_mismatch'];
            }
            if (empty($errors)) {
                $page_data = lui_PageData($_POST['page_id']);
                if ($page_data['user_id'] == $wo['user']['id'] || lui_IsCanPageUpdate($_POST['page_id'], 'delete_page')) {
                    if (lui_DeletePage($_POST['page_id']) === true) {
                        $data = array(
                            'status' => 200,
                            'message' => $success_icon . $wo['lang']['page_deleted'],
                            'location' => lui_SeoLink('index.php?link1=pages')
                        );
                    }
                }
            }
    ...
    if ($s == 'add_admin') {
        ...
        if (isset($_GET['page_id']) && isset($_GET['user_id'])) {
            ...
            if ($page_data['user_id'] == $wo['user']['id'] || lui_IsCanPageUpdate($_GET['page_id'], 'admins')) {
                $page = lui_Secure($_GET['page_id']);
                $user = lui_Secure($_GET['user_id']);
                $code = lui_AddPageAdmin($user, $page);
                if ($code === 1) {
                    $data['status'] = 200;
                    $data['code']   = 1;
                } else if ($code === 0) {
    ...
    if ($s == 'get_mbr' && isset($_GET['name']) && isset($_GET['page']) && is_numeric($_GET['page'])) {
        ...
        if (is_array($users) && count($users) > 0) {
            foreach ($users as $wo['member']) {
                ...
                $html .= lui_LoadPage('page-setting/admin-list');
            }
            $data['status'] = 200;
            $data['html']   = $html;
        }
    ...
    if ($s == 'get_more_likes') {
        ...
        if (isset($_GET['user_id']) && isset($_GET['after_last_id'])) {
            foreach (lui_GetLikes($_GET['user_id'], 'profile', 10, $_GET['after_last_id']) as $wo['PageList']) {
                $html .= lui_LoadPage('timeline/likes-list');
            }
        }
        $data = array(
            'status' => 200,
    ...
    if ($s == 'get_next_page') {
        ...
        foreach (lui_PageSug(1, $page_id) as $wo['PageList']) {
            $wo['PageList']['user_name'] = $wo['PageList']['name'];
            $html                        = lui_LoadPage('sidebar/sidebar-home-page-list');
        }
        $data = array(
            'status' => 200,
            'html' => $html
    ...
    if ($s == 'get_likes') {
        ...
        if (!empty($_GET['user_id'])) {
            foreach (lui_GetLikes($_GET['user_id'], 'sidebar', 12) as $wo['PageList']) {
                $wo['PageList']['user_name'] = @mb_substr($wo['PageList']['name'], 0, 10, "utf-8");
                $html .= lui_LoadPage('sidebar/sidebar-page-list');
            }
            $data = array(
                'status' => 200,
                'html' => $html
    ...
    if ($s == 'rate_page' && isset($_POST['page_id']) && is_numeric($_POST['page_id']) && $_POST['page_id'] > 0 && isset($_POST['val'])) {
        $val  = lui_Secure($_POST['val']);
        $id   = lui_Secure($_POST['page_id']);
        $text = lui_Secure($_POST['text'],1);
        $data = array(
            'status' => 304,
            'message' => $wo['lang']['page_rated']
        );
        if (lui_RatePage($id, $val, $text)) {
            $data['status'] = 200;
            $data['val']    = $val;
            unset($data['message']);
        }
    ...
    if ($s == 'load_reviews' && isset($_GET['page']) && isset($_GET['after_id'])) {
        $page_id = lui_Secure($_GET['page']);
        $id      = lui_Secure($_GET['after_id']);
        $data    = array(
            'status' => 404
        );
        $reviews = lui_GetPageReviews($page_id, $id);
        $html    = '';
        if (count($reviews) > 0) {
            foreach ($reviews as $wo['review']) {
                $html .= lui_LoadPage('page/review-list');
            }
            $data['status'] = 200;
            $data['html']   = $html;
        }
    ...
    if ($s == 'privileges') {
        if (!empty($_POST['page_id']) && is_numeric($_POST['page_id']) && $_POST['page_id'] > 0 && !empty($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0) {
            $page_data = lui_PageData($_POST['page_id']);
            if ($page_data['user_id'] == $wo['user']['id'] || lui_IsCanPageUpdate($_POST['page_id'], 'admins')) {
                $update_array = array(
                    'general' => 0,
                    'info' => 0,
                    'social' => 0,
                ...
                if (lui_UpdatePageAdminData($_POST['page_id'], $update_array, $_POST['user_id'])) {
                    $data = array(
                        'status' => 200,
                        'message' => $success_icon . $wo['lang']['setting_updated']
                    );
