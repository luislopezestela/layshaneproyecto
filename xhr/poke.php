<?php 
if ($f == 'poke') {
    if (!empty($_GET['received_user_id']) && !empty($_GET['send_user_id']) && lui_CheckMainSession($hash_id) === true) {
        $received_user_id = lui_Secure($_GET['received_user_id']);
        $send_user_id     = lui_Secure($_GET['send_user_id']);
        if (isset($_GET['poke_id']) && !empty($_GET['poke_id'])) {
            $poke_id  = lui_Secure($_GET['poke_id']);
