...
if ($f == 'posts') {
    if ($s == 'fetch_url') {
        if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $_POST["url"], $match)) {
            $youtube_video = lui_Secure($match[1]);
            ...
            $tweet_url   = lui_Secure($_POST['url']);
            ...
            $tiktok_url  = lui_Secure($_POST['url']);
            ...
            $wo['wo_ad_id'] = '';
            $wo['rvad_con'] = '';
            $description    = lui_LoadPage('players/video');
            $output         = array(
                'title' => $title,
                'images' => array(
                    $thumbnail
    ...
    if ($s == 'search_for_posts') {
        ...
        if (!empty($_GET['search_query'])) {
            $search_data = lui_SearchForPosts($_GET['id'], $_GET['search_query'], 20, $_GET['type']);
            if (count($search_data) == 0) {
                $html = lui_LoadPage('story/filter-no-stories-found');
            } else {
    ...
    if ($s == 'get_new_hashtag_posts') {
        ...
        if (!empty($_GET['before_post_id']) && !empty($_GET['hashtagName'])) {
            $posts = lui_GetHashtagPosts($_GET['hashtagName'], 0, 20, $_GET['before_post_id']);
            foreach ($posts as $wo['story']) {
                if (!empty($_GET['api'])) {
                    echo lui_LoadPage('story/api-posts');
                } else {
    ...
    if ($s == 'insert_new_post') {
        ...
        if (lui_CheckSession($hash_id) === false) {
            return false;
            die();
        }
        if ($wo['config']['who_upload'] == 'pro' && $wo['user']['is_pro'] == 0 && !lui_IsAdmin() && (!empty($_FILES['postFile']) || !empty($_FILES['postVideo']) || !empty($_FILES['postMusic']) || !empty($_FILES['postPhotos']) || !empty($_POST['postRecord']))) {
        ...
        if (isset($_POST['recipient_id']) && !empty($_POST['recipient_id'])) {
            $recipient_id = lui_Secure($_POST['recipient_id']);
        } else if (isset($_POST['event_id']) && !empty($_POST['event_id'])) {
            $event_id = lui_Secure($_POST['event_id']);
            ...
            $group_id = lui_Secure($_POST['group_id']);
        ...
        if (isset($_FILES['postFile']['name'])) {
            if ($_FILES['postFile']['size'] > $wo['config']['maxUpload']) {
                $errors = str_replace('{file_size}', lui_SizeUnits($wo['config']['maxUpload']), $wo['lang']['file_too_big']);
            } else if (lui_IsFileAllowed($_FILES['postFile']['name']) == false) {
        ...
        if (isset($_FILES['postVideo']['name']) && empty($mediaFilename)) {
            ...
            if ($fileType === 'video' && lui_IsFfmpegFileAllowed($_FILES['postVideo']['name']) && !lui_IsVideoNotAllowedMime($_FILES["postVideo"]["type"])) {
                $not_video = false;
            }
            if ($_FILES['postVideo']['size'] > $wo['config']['maxUpload']) {
                $errors = str_replace('{file_size}', lui_SizeUnits($wo['config']['maxUpload']), $wo['lang']['file_too_big']);
            } else if (lui_IsFileAllowed($_FILES['postVideo']['name']) == false && $wo['config']['ffmpeg_system'] != 'on') {
                $errors = $wo['lang']['file_not_supported'];
        ...
        if (isset($_FILES['postMusic']['name']) && empty($mediaFilename)) {
            if ($_FILES['postMusic']['size'] > $wo['config']['maxUpload']) {
                $errors = str_replace('{file_size}', lui_SizeUnits($wo['config']['maxUpload']), $wo['lang']['file_too_big']);
            } else if (lui_IsFileAllowed($_FILES['postMusic']['name']) == false) {
                $errors = $wo['lang']['file_not_supported'];
            } else {
                $fileInfo = array(
        ...
        if (isset($_FILES['postPhotos']['name']) && empty($mediaFilename) && empty($_POST['album_name'])) {
            if (count($_FILES['postPhotos']['name']) == 1) {
                if ($_FILES['postPhotos']['size'][0] > $wo['config']['maxUpload']) {
                    $errors = str_replace('{file_size}', lui_SizeUnits($wo['config']['maxUpload']), $wo['lang']['file_too_big']);
                } else if (lui_IsFileAllowed($_FILES['postPhotos']['name'][0]) == false) {
                    $errors = $wo['lang']['file_not_supported'];
                } else {
                    $fileInfo = array(
                    ...
                    if (!empty($media)) {
                        $image_file = lui_GetMedia($media['filename']);
                        $upload     = true;
                        if ($wo['config']['adult_images'] == 1 && !detect_safe_search($image_file) && $wo['config']['adult_images_action'] == 1) {
                            ...
                            lui_DeleteFromToS3($media['filename']);
                        }
                        $mediaFilename = $media['filename'];
                        $mediaName     = $media['name'];
                    }
        ...
        if (!empty($_POST['url_link']) && !empty($_POST['url_title']) && filter_var($_POST['url_link'], FILTER_VALIDATE_URL)) {
            ...
            if (!empty($_POST['url_image'])) {
                $import_url_image = @lui_ImportImageFromUrl($_POST['url_image']);
            }
            if (preg_match("/(http|https):\/\/twitter\.com\/[a-zA-Z0-9_]+\/status\/[0-9]+/", $_POST['url_link'])) {
                $url_content      = '';
                $import_url_image = '';
                $url_title        = '';
                $tweet_url        = lui_Secure($_POST['url_link']);
                $tweeturl         = 'https://publish.twitter.com/oembed?url=' . $tweet_url;
                $api_request      = @file_get_contents($tweeturl);
                if (!empty($api_request)) {
                    $json_decode = json_decode($api_request);
                ...
                $tiktok_url       = lui_Secure($_POST['url_link']);
                $tiktok_url       = 'https://www.tiktok.com/oembed?url=' . $tiktok_url;
                $api_request      = @file_get_contents($tiktok_url);
                $url_content      = '';
                $import_url_image = '';
        ...
        if (empty($errors) && $invalid_file == false) {
            ...
            $post_data                = array(
                ...
                'postText' => lui_Secure($post_text,1),
                'recipient_id' => lui_Secure($recipient_id),
                'postRecord' => lui_Secure($_POST['postRecord']),
                'postFile' => lui_Secure($mediaFilename, 0),
                'postFileName' => lui_Secure($mediaName,1),
                'postMap' => lui_Secure($post_map),
                'postPrivacy' => lui_Secure($post_privacy),
                'postLinkTitle' => lui_Secure($url_title,1),
                'postLinkContent' => lui_Secure($url_content),
                'postLink' => lui_Secure($url_link),
                'postLinkImage' => lui_Secure($import_url_image, 0),
                'album_name' => lui_Secure($album_name,1),
                'multi_image' => lui_Secure($multi),
                'postFeeling' => lui_Secure($feeling),
                'postListening' => lui_Secure($listening),
                'postPlaying' => lui_Secure($playing),
                'postWatching' => lui_Secure($watching),
                'postTraveling' => lui_Secure($traveling),
                'postFileThumb' => lui_Secure($video_thumb),
                'time' => time(),
                'blur' => $blur,
                'multi_image_post' => 0,
                'active' => $post_active
            ...
            if (isset($_POST['postSticker']) && lui_IsUrl($_POST['postSticker']) && empty($_FILES) && empty($_POST['postRecord'])) {
                ...
                if (!empty($matches[0][0]) && lui_IsUrl($matches[0][0])) {
                    $post_data['postPhoto'] = @lui_ImportImageFromUrl($matches[0][0]);
                }
            }
            if (!empty($is_option)) {
                $post_data['poll_id'] = 1;
            ...
            if (!empty($_POST['post_color']) && !empty($post_text) && empty($_POST['postRecord']) && empty($mediaFilename) && empty($mediaName) && empty($post_map) && empty($url_title) && empty($url_content) && empty($url_link) && empty($import_url_image) && empty($album_name) && empty($multi) && empty($video_thumb) && empty($post_data['postPhoto'])) {
                $post_data['color_id'] = lui_Secure($_POST['post_color']);
            }
        ...
        if (!empty($id)) {
            lui_CleanCache();
            lui_UpdateUserDetails($wo['user'], true, false, false, true);
            if ($is_option == true) {
                foreach ($_POST['answer'] as $key => $value) {
                    $add_opition = lui_AddOption($id, $value);
                }
            }
            if (isset($_FILES['postPhotos']['name'])) {
                if (count($_FILES['postPhotos']['name']) > 0) {
                    for ($i = 0; $i < count($_FILES['postPhotos']['name']); $i++) {
                        ...
                        $image_file = lui_GetMedia($file['filename'] ?? '');
                        if ($wo['config']['adult_images'] == 1 && !detect_safe_search($image_file) && $wo['config']['adult_images_action'] == 1) {
                            $blur = 1;
                        } elseif ($wo['config']['adult_images'] == 1 && detect_safe_search($image_file) == false && $wo['config']['adult_images_action'] == 0) {
                            $errors = $wo['lang']['adult_image_file'];
                            ...
                            lui_DeleteFromToS3($file['filename']);
                        }
                        if (!empty($file)) {
                            $media_album                   = lui_RegisterAlbumMedia($id, $file['filename']);
                            ...
                            $new_id                        = lui_RegisterPost($post_data);
                            $media_album                   = lui_RegisterAlbumMedia($new_id, $file['filename'], $id);
                        }
                    }
                }
            }
            if ($wo['config']['post_approval'] == 1 && !lui_IsAdmin()) {
                ...
                if ($wo['config']['notify_new_post'] == 1) {
                    if (empty($wo['story']['page_id']) && empty($wo['story']['group_id']) && empty($wo['story']['event_id']) && $wo['story']['postPrivacy'] < 3) {
                        $post_id = $wo['story']['id'];
                        $users   = lui_GetFollowNotifyUsers($wo['user']['user_id']);
                        if (!empty($users)) {
    ...
    if ($s == 'send_notify' && lui_CheckMainSession($hash_id) === true) {
        if (!empty($_POST['post_id']) && is_numeric($_POST['post_id']) && $_POST['post_id'] > 0) {
            ...
            $wo['story'] = $db->where('id', lui_Secure($_POST['post_id']))->where('user_id', $wo['user']['id'])->getOne(T_POSTS);
    ...
    if ($s == 'delete_post' && lui_CheckMainSession($hash_id) === true) {
        if (!empty($_GET['post_id'])) {
            $wo['story'] = $db->where('id', lui_Secure($_GET['post_id']))->ArrayBuilder()->getOne(T_POSTS);
            $deleted = false;
            if (!empty($wo['story']['blog_id']) && $wo['story']['blog_id'] > 0) {
                if (lui_DeleteMyBlog($wo['story']['blog_id'])) {
                    $deleted = true;
                }
            }else if (lui_DeletePost($_GET['post_id']) === true) {
                $deleted = true;
            }
            if ($deleted == true) {
                if (!empty($wo['story'])) {
    ...
    if ($s == 'report_comment' && lui_CheckMainSession($hash_id) === true) {
        if (!empty($_GET['comment_id'])) {
            $post_data = array(
                'comment_id' => $_GET['comment_id']
            );
            if (lui_ReportPost($post_data) == 'unreport') {
                $data = array(
                    'status' => 300,
    ...
    if ($s == 'load_more_posts') {
        ...
        if (!empty($_GET['filter_by_more']) && $_GET['filter_by_more'] == 'story' && isset($_GET['story_id']) && isset($_GET['user_id'])) {
            $args           = array();
            $args['offset'] = lui_Secure($_GET['story_id']);
            if ($_GET['user_id'] > 0) {
                $args['user'] = lui_Secure($_GET['user_id']);
            }
            foreach (lui_GetStroies($args) as $wo['story']) {
                echo lui_LoadPage('status/content');
            }
        } else if (!empty($_GET['filter_by_more']) && $_GET['filter_by_more'] == 'most_liked') {
            $most_liked_posts = lui_GetPosts(array(
            ...
            foreach ($most_liked_posts as $wo['story']) {
                if ($is_api == true) {
                    ...
                    echo sanitize_output(lui_LoadPage('story/content'));
                }
            }
            ...
            $get_posts = lui_GetPosts($postsData);
            $is_api    = false;
            ...
            if (!empty($_GET['posts_count']) && !empty($get_posts) && $is_api == false) {
                if ($_GET['posts_count'] > 9 && $_GET['posts_count'] < 15) {
                    echo lui_GetAd('post_first', false);
                } else if ($_GET['posts_count'] > 20 && $_GET['posts_count'] < 28) {
                    echo lui_GetAd('post_second', false);
                } else if ($_GET['posts_count'] > 29) {
                    echo lui_GetAd('post_third', false);
                }
            }
    ...
    if ($s == 'edit_post') {
        ...
        if (!empty($_POST['post_id']) && is_numeric($_POST['post_id']) && (!empty($_POST['text']) || !empty($_FILES['images']))) {
            $post_id    = lui_Secure($_POST['post_id']);
            $post       = $db->where('id', $post_id)->getOne(T_POSTS);
            $updatePost = '';
            if (!empty($post) && lui_IsPostOnwer($post_id, $wo['user']['id'])) {
                $wo['no_mention'] = array();
                $mention_regex    = '/@\[([0-9]+)\]/i';
                ...
                $updatePost = lui_UpdatePost(array(
                ...
                if (!empty($_POST['all_images']) && $_POST['all_images'] == 1) {
                    ...
                    if ($image_count_ < 1 && !empty($post->postFile) && strpos($post->postFile, '_image') !== false) {
                        ...
                        lui_DeleteFromToS3($post->postFile);
                        $db->where('id', $post_id)->update(T_POSTS, array(
                            'multi_image' => '0',
                            'postFile' => '',
                            'multi_image_post' => 0
                                ...
                                if (!empty($file) && $invalid_file == 0) {
                                    if (count($_FILES['images']['name']) == 1 && !empty($_POST['all_images']) && $_POST['all_images'] == 1) {
                                        ...
                                        if (!empty($post->postFile) && $image_count < 1) {
                                            ...
                                            $post_data['time']             = time();
                                            $new_id                        = lui_RegisterPost($post_data);
                                            lui_RegisterAlbumMedia($post_id, $post->postFile);
                                            $db->where('id', $post_id)->update(T_POSTS, array(
                                                'multi_image' => '1',
                                                'postFile' => '',
                                                'multi_image_post' => 0
                                        ...
                                        $post_data['time']             = time();
                                        $new_id                        = lui_RegisterPost($post_data);
                                        $media_album                   = lui_RegisterAlbumMedia($post_id, $file['filename']);
                                        $media_album                   = lui_RegisterAlbumMedia($new_id, $file['filename'], $post_id);
                                    }
                                }
                            }
                        }
        ...
        if (!empty($_POST['post_id'])) {
            ...
            if (!empty($post) && lui_IsPostOnwer($post_id, $wo['user']['id'])) {
                if (!empty($_POST['image_id']) && is_numeric($_POST['image_id'])) {
                    $image_id = lui_Secure($_POST['image_id']);
                    if ($image_count > 1) {
                        ...
                        while ($fetched_data_s = mysqli_fetch_assoc($query_two_2)) {
                            ...
                            if (!empty($image_post)) {
                                lui_DeletePost($image_post->post_id);
                            }
                            $explode2 = @end(explode('.', $fetched_data_s['image']));
                            $explode3 = @explode('.', $fetched_data_s['image']);
                            $media_2  = $explode3[0] . '_small.' . $explode2;
    ...
    if ($s == "update_post_privacy") {
        if (!empty($_GET['post_id']) && isset($_GET['privacy_type']) && lui_CheckMainSession($hash_id) === true) {
            $updatePost = lui_UpdatePostPrivacy(array(
                'post_id' => lui_Secure($_GET['post_id']),
                'privacy_type' => lui_Secure($_GET['privacy_type'])
            ));
            if (isset($updatePost)) {
                $data = array(
                    'status' => 200,
    ...
    if ($s == 'register_like') {
        if (!empty($_GET['post_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddLikes($_GET['post_id']) == 'unliked') {
                $data = array(
                    'status' => 300,
                    'likes' => lui_CountLikes($_GET['post_id']),
                    'wonders' => lui_CountWonders($_GET['post_id']),
    ...
    if ($s == 'delete_reaction') {
        if (!empty($_GET['post_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_DeleteReactions($_GET['post_id'])) {
                $data = array(
                    'status' => 200,
                    'like_lang' => $wo['lang']['like'],
                    'reactions' => lui_GetPostReactions($_GET['post_id'])
                );
            }
        }
        header("Content-type: application/json");
    ...
    if ($s == 'delete_comment_reaction') {
        if (!empty($_GET['comment_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_DeleteCommentReactions($_GET['comment_id'])) {
                $data = array(
                    'status' => 200,
                    'reactions' => lui_GetPostReactions($_GET['comment_id'], "comment")
                );
            }
        }
        header("Content-type: application/json");
    ...
    if ($s == 'delete_replay_reaction') {
        if (!empty($_GET['replay_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_DeleteReplayReactions($_GET['replay_id'])) {
                $data = array(
                    'status' => 200,
                    'reactions' => lui_GetPostReactions($_GET['replay_id'], "replay")
                );
            }
        }
        header("Content-type: application/json");
    ...
    if ($s == 'register_reaction') {
        if (!empty($_GET['post_id']) && !empty($_GET['reaction']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddReactions($_GET['post_id'], $_GET['reaction']) == 'reacted') {
                $data = array(
                    'status' => 200,
                    'reactions' => lui_GetPostReactions($_GET['post_id']),
                    'like_lang' => $wo['lang']['liked']
    ...
    if ($s == 'register_comment_reaction') {
        if (!empty($_GET['comment_id']) && !empty($_GET['reaction']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddCommentReactions($_GET['comment_id'], $_GET['reaction']) == 'reacted') {
                $data = array(
                    'status' => 200,
                    'reactions' => lui_GetPostReactions($_GET['comment_id'], "comment"),
                    'like_lang' => $wo['lang']['liked']
    ...
    if ($s == 'register_replay_reaction') {
        if (!empty($_GET['reply_id']) && !empty($_GET['reaction']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddReplayReactions($_GET['user_id'], $_GET['reply_id'], $_GET['reaction']) == 'reacted') {
                $data = array(
                    'status' => 200,
                    'reactions' => lui_GetPostReactions($_GET['reply_id'], "replay"),
                    'like_lang' => $wo['lang']['liked']
                );
                if (lui_CanSenEmails()) {
                    $data['can_send'] = 1;
    ...
    if ($s == 'register_wonder') {
        if (!empty($_GET['post_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddWonders($_GET['post_id']) == 'unwonder') {
                $data                = array(
                    'status' => 300,
    ...
    if ($s == 'register_share') {
        if (!empty($_GET['post_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddShare($_GET['post_id']) == 'unshare') {
                $data = array(
                    'status' => 300,
                    'shares' => lui_CountShares($_GET['post_id'])
                );
            } else {
                $data = array(
                    'status' => 200,
    ...
    if ($s == 'register_comment') {
        if (!empty($_POST['post_id']) && isset($_POST['text'])) {
            ...
            if (!empty($_POST['gif_url'])) {
                $comment_image = lui_ImportImageFromUrl($_POST['gif_url']);
            }
            ...
            $C_Data = array(
                ...
                'page_id' => lui_Secure($page_id),
                'post_id' => lui_Secure($_POST['post_id']),
                'text' => lui_Secure($_POST['text'],1),
                'c_file' => lui_Secure($comment_image),
                'time' => time()
            );
            ...
            $R_Comment     = lui_RegisterPostComment($C_Data);
            $wo['comment'] = lui_GetPostComment($R_Comment);
            $wo['story']   = lui_PostData($_POST['post_id']);
            if (!empty($wo['comment'])) {
                $html          = lui_LoadPage('comment/content');
                $data          = array(
                    'status' => 200,
                    'html' => $html,
                    'comments_num' => lui_CountPostComment($_POST['post_id']),
                    'mention' => array()
                );
                $mention_regex = '/@([A-Za-z0-9_]+)/i';
    ...
    if ($s == 'register_reply') {
        if (!empty($_POST['comment_id']) && isset($_POST['text']) && lui_CheckMainSession($hash_id) === true) {
            $html    = '';
            $page_id = '';
            if (!empty($_POST['page_id'])) {
                $page_id = $_POST['page_id'];
            ...
            $C_Data      = array(
                'user_id' => lui_Secure($wo['user']['user_id']),
                'page_id' => lui_Secure($page_id),
                'comment_id' => lui_Secure($_POST['comment_id']),
                'text' => lui_Secure($_POST['text'],1),
                'c_file' => lui_Secure($comment_image),
                'time' => time()
            );
            $R_Comment   = lui_RegisterCommentReply($C_Data);
            $wo['reply'] = lui_GetCommentReply($R_Comment);
            if (!empty($wo['reply'])) {
                $html          = lui_LoadPage('comment/replies-content');
                $data          = array(
                    'status' => 200,
                    'html' => $html,
                    'replies_num' => lui_CountCommentReplies($_POST['comment_id']),
                    'mention' => array()
                );
                $mention_regex = '/@([A-Za-z0-9_]+)/i';
                preg_match_all($mention_regex, $_POST['text'], $matches);
    ...
    if ($s == 'update-reply') {
        if (!empty($_POST['id']) && !empty($_POST['text']) && lui_CheckMainSession($hash_id) === true) {
            ...
            if (lui_UpdateCommentReply($id, $update_datau)) {
                $reply = lui_GetCommentReply($id);
                $data  = array(
                    'status' => 200,
                    'text' => $reply['text'],
    ...
    if ($s == 'delete_comment') {
        if (!empty($_GET['comment_id']) && lui_CheckMainSession($hash_id) === true) {
            ...
            $DeleteComment = lui_DeletePostComment($_GET['comment_id']);
    ...
    if ($s == 'delete_comment_reply') {
        if (!empty($_GET['reply_id']) && lui_CheckMainSession($hash_id) === true) {
            $DeleteComment = lui_DeletePostReplyComment($_GET['reply_id']);
            if ($DeleteComment === true) {
                $data = array(
                    'status' => 200
                );
    ...
    if ($s == 'load_more_post_comments') {
        if (!empty($_GET['post_id'])) {
            ...
            if (!empty($_GET['comment_id']) && is_numeric($_GET['comment_id']) && $_GET['comment_id'] > 0) {
                $offset = lui_Secure($_GET['comment_id']);
            }
            $comments = lui_GetPostComments($_GET['post_id'], 50, $offset);
    ...
    if ($s == 'load_more_comments') {
        if (!empty($_GET['post_id'])) {
            $html        = '';
            $wo['story'] = lui_PostData($_GET['post_id']);
            foreach (lui_GetPostComments($_GET['post_id'], lui_CountPostComment($_GET['post_id'])) as $wo['comment']) {
                $html .= lui_LoadPage('comment/content');
            }
            $data = array(
                'status' => 200,
    ...
    if ($s == 'load_more_comments_sort') {
        if (!empty($_GET['post_id'])) {
            ...
            $wo['story'] = lui_PostData($_GET['post_id']);
            if (!empty($wo['story'])) {
                foreach (lui_GetPostCommentsSort($_GET['post_id'], lui_CountPostComment($_GET['post_id']), $_GET['type']) as $wo['comment']) {
                    $html .= lui_LoadPage('comment/content');
                }
            }
            $data = array(
    ...
    if ($s == 'load_more_replies') {
        if (!empty($_GET['comment_id'])) {
            $html = '';
            foreach (lui_GetCommentReplies($_GET['comment_id'], lui_CountCommentReplies($_GET['comment_id'])) as $wo['reply']) {
                $html .= lui_LoadPage('comment/replies-content');
            }
            $data = array(
                'status' => 200,
    ...
    if ($s == 'edit_comment') {
        if (!empty($_POST['comment_id']) && !empty($_POST['text']) && lui_CheckMainSession($hash_id) === true) {
            $updateComment = lui_UpdateComment(array(
    ...
    if ($s == 'register_comment_like') {
        if (!empty($_POST['comment_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddCommentLikes($_POST['comment_id'], $_POST['comment_text']) == 'unliked') {
                $data = array(
                    'status' => 300,
                    'likes' => lui_CountCommentLikes($_POST['comment_id'])
                );
            } else {
                $data = array(
                    'status' => 200,
            ...
            if ($wo['config']['second_post_button'] == 'dislike') {
                $data['dislike']   = 1;
                $data['wonders_c'] = lui_CountCommentWonders($_POST['comment_id']);
            }
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'register_comment_wonder') {
        if (!empty($_POST['comment_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddCommentWonders($_POST['comment_id'], $_POST['comment_text']) == 'unwonder') {
                $data = array(
                    'status' => 300,
                    'icon' => $wo['second_post_button_icon'],
                    'wonders' => lui_CountCommentWonders($_POST['comment_id'])
                );
            } else {
                $data = array(
                    'status' => 200,
            ...
            if ($wo['config']['second_post_button'] == 'dislike') {
                $data['dislike'] = 1;
                $data['likes_c'] = lui_CountCommentLikes($_POST['comment_id']);
            }
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'register_comment_reply_like') {
        if (!empty($_POST['reply_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddCommentReplyLikes($_POST['reply_id'], $_POST['comment_text']) == 'unliked') {
                $data = array(
                    'status' => 300,
                    'likes' => lui_CountCommentReplyLikes($_POST['reply_id'])
                );
            } else {
                $data = array(
                    'status' => 200,
            ...
            if ($wo['config']['second_post_button'] == 'dislike') {
                $data['dislike']   = 1;
                $data['wonders_r'] = lui_CountCommentReplyWonders($_POST['reply_id']);
            }
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'register_comment_reply_wonder') {
        if (!empty($_POST['reply_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_AddCommentReplyWonders($_POST['reply_id'], $_POST['comment_text']) == 'unwonder') {
                $data = array(
                    'status' => 300,
                    'icon' => $wo['second_post_button_icon'],
                    'wonders' => lui_CountCommentReplyWonders($_POST['reply_id'])
                );
            } else {
                $data = array(
                    'status' => 200,
            ...
            if ($wo['config']['second_post_button'] == 'dislike') {
                $data['dislike'] = 1;
                $data['likes_r'] = lui_CountCommentReplyLikes($_POST['reply_id']);
            }
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'save_post') {
        if (!empty($_GET['post_id']) && lui_CheckMainSession($hash_id) === true) {
            ...
            if (lui_SavePosts($post_data) == 'unsaved') {
                $data = array(
                    'status' => 300,
    ...
    if ($s == 'disable_comment') {
        if (!empty($_GET['post_id']) && lui_CheckMainSession($hash_id) === true) {
            ...
            if (lui_IsPostOnwer($_GET['post_id'], $wo['user']['user_id'])) {
                if ($type == 0) {
                    $db->where('id', lui_Secure($_GET['post_id']))->update(T_POSTS, array(
                        'comments_status' => 1
                    ));
                    $data = array(
                        'status' => 300,
    ...
    if ($s == 'pin_post') {
        if (!empty($_GET['post_id']) && lui_CheckMainSession($hash_id) === true) {
            ...
            if (lui_PinPost($_GET['post_id'], $type, $id) == 'unpin') {
                $data = array(
                    'status' => 300,
    ...
    if ($s == 'boost_post') {
        if (!empty($_GET['post_id']) && $wo['config']['pro'] == 1 && lui_CheckMainSession($hash_id) === true) {
            if (lui_BoostPost($_GET['post_id']) == 'unboosted') {
                $data = array(
                    'status' => 300,
    ...
    if ($s == 'mark_as_sold_post') {
        if (!empty($_GET['post_id']) && !empty($_GET['product_id']) && lui_CheckMainSession($hash_id) === true) {
            if (lui_MarkPostAsSold($_GET['post_id'], $_GET['product_id'])) {
                $data = array(
                    'status' => 200,
                    'text' => $wo['lang']['sold']
                );
    ...
    if ($s == 'get_post_reacted') {
        if (!empty($_GET['post_id']) && !empty($_GET['col'])) {
            ...
            if (!empty($_GET['type']) && !empty($wo['reactions_types'][$_GET['type']]) && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
                $reactedUsers = lui_GetPostReactionUsers($_GET['post_id'], $_GET['type'], 20, lui_Secure($_GET['offset']), lui_Secure($_GET['col']));
            } else {
                $reactedUsers = lui_GetPostReactionUsers($_GET['post_id'], $_GET['type'], 20, 0, lui_Secure($_GET['col']));
            }
            $post_info = array();
            if ($_GET['col'] == 'post') {
                $post_info = $db->where('id', lui_Secure($_GET['post_id']))->getOne(T_POSTS);
            } elseif ($_GET['col'] == 'comment') {
                $comment = $db->where('id', lui_Secure($_GET['post_id']))->getOne(T_COMMENTS);
                if (!empty($comment->post_id)) {
                    $post_info = $db->where('id', $comment->post_id)->getOne(T_POSTS);
                }
            } elseif ($_GET['col'] == 'replay') {
                $comment = $db->where('id', lui_Secure($_GET['post_id']))->getOne(T_COMMENTS_REPLIES);
            ...
            if (count($reactedUsers) > 0) {
                foreach ($reactedUsers as $wo['WondredLikedusers']) {
                    ...
                    if (!empty($post_info) && !empty($post_info->page_id)) {
                        $wo['WondredLikedusers']['page_info'] = lui_PageData($post_info->page_id);
                    }
                    if (!empty($wo['WondredLikedusers']['page_info']) && !empty($post_info) && $post_info->page_id > 0 && $wo['WondredLikedusers']['page_info']['user_id'] == $wo['WondredLikedusers']['user_id']) {
                        $data['html'] .= lui_LoadPage('story/page-post-likes');
                    } else {
                        ...
                        $data['html'] .= lui_LoadPage('story/post-likes-wonders');
                    }
                }
            } else {
        ...
        if (!empty($_GET['post_id']) && !empty($_GET['table'])) {
            ...
            if ($_GET['table'] == 'post') {
                if (!empty($_GET['type']) && $_GET['type'] == 'like' && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
                    $likedUsers = lui_GetPostLikes($_GET['post_id'], 20, lui_Secure($_GET['offset']));
                } else {
                    $likedUsers = lui_GetPostLikes($_GET['post_id']);
                }
            } elseif ($_GET['table'] == 'comment') {
                if (!empty($_GET['type']) && $_GET['type'] == 'like' && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
                    $likedUsers = lui_GetPostCommentLikes($_GET['post_id'], 20, lui_Secure($_GET['offset']));
                } else {
                    $likedUsers = lui_GetPostCommentLikes($_GET['post_id']);
                }
                $data['title'] = TextForMode('users_liked_comment');
            } elseif ($_GET['table'] == 'reply') {
                if (!empty($_GET['type']) && $_GET['type'] == 'like' && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
                    $likedUsers = lui_GetPostCommentReplyLikes($_GET['post_id'], 20, lui_Secure($_GET['offset']));
                } else {
                    $likedUsers = lui_GetPostCommentReplyLikes($_GET['post_id']);
                }
    ...
    if ($s == 'get_post_shared') {
        if (!empty($_GET['post_id'])) {
            ...
            if (!empty($_GET['type']) && $_GET['type'] == 'share' && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
                $sharedUsers = lui_GetPostShared($_GET['post_id'], 20, lui_Secure($_GET['offset']));
            } else {
                $sharedUsers = lui_GetPostShared($_GET['post_id']);
            }
        ...
        if (!empty($_GET['post_id']) && !empty($_GET['table'])) {
            ...
            if ($_GET['table'] == 'post') {
                if (!empty($_GET['type']) && $_GET['type'] == 'wonder' && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
                    $WonderedUsers = lui_GetPostWonders($_GET['post_id'], 20, lui_Secure($_GET['offset']));
                } else {
                    $WonderedUsers = lui_GetPostWonders($_GET['post_id']);
                }
            } elseif ($_GET['table'] == 'comment') {
                if (!empty($_GET['type']) && $_GET['type'] == 'wonder' && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
                    $WonderedUsers = lui_GetPostCommentWonders($_GET['post_id'], 20, lui_Secure($_GET['offset']));
                } else {
                    $WonderedUsers = lui_GetPostCommentWonders($_GET['post_id']);
                }
                $data['title'] = $wo['lang']['users_wondered_comment'];
            } elseif ($_GET['table'] == 'reply') {
                if (!empty($_GET['type']) && $_GET['type'] == 'wonder' && !empty($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
                    $WonderedUsers = lui_GetPostCommentReplyWonders($_GET['post_id'], 20, lui_Secure($_GET['offset']));
                } else {
                    $WonderedUsers = lui_GetPostCommentReplyWonders($_GET['post_id']);
                }
                $data['title'] = $wo['lang']['users_wondered_comment'];
            }
    ...
    if ($s == 'filter_posts') {
        if (!empty($_GET['filter_by']) && isset($_GET['id'])) {
            $html    = '';
            $options = array(
                'filter_by' => lui_Secure($_GET['filter_by']),
                'placement' => 'multi_image_post'
            );
            if (!empty($_GET['type'])) {
                if ($_GET['type'] == 'page') {
    ...
    if ($s == 'add-video-view' && isset($_GET['post_id']) && is_numeric($_GET['post_id'])) {
        ...
        $post_views = lui_AddPostVideoView($post_id);
    ...
    if ($s == 'hide_post' && isset($_GET['post']) && is_numeric($_GET['post'])) {
        ...
        $post_id = lui_Secure($_GET['post']);
        if (lui_HidePost($post_id)) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'share-post' && isset($_GET['id']) && is_numeric($_GET['id']) && isset($_GET['usr']) && is_numeric($_GET['usr'])) {
        ...
        $post_id = lui_Secure($_GET['id']);
        $owner   = lui_Secure($_GET['usr']);
        if (lui_SharePost($post_id, $owner)) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
