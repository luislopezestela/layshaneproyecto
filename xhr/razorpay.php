...
if ($f == 'razorpay') {
    ...
    if ($s == 'upgrade') {
    	if (!empty($_POST['type']) && in_array($_POST['type'], $types) && !empty($_POST['payment_id']) && !empty($_POST['order_id']) && !empty($_POST['merchant_amount']) && !empty($_POST['currency'])) {
    		...
    		$payment_id = lui_Secure($_POST['payment_id']);
			...
			$json = lui_CheckRazorpayPayment($payment_id,$check);
		    if (!empty($json) && empty($json->error_code)) {
			    ...
			    if ($stop == 0) {
			        ...
			        if (!in_array($pro_type, $pro_types_array)) {
			            $data['url'] = lui_SeoLink('index.php?link1=oops');
    ...
    if ($s == 'wallet') {
    	if (!empty($_POST['type']) && $_POST['type'] == 'wallet' && !empty($_POST['payment_id']) && !empty($_POST['order_id']) && !empty($_POST['merchant_amount']) && !empty($_POST['currency'])) {
    		...
    		$payment_id = lui_Secure($_POST['payment_id']);
    		$price    = lui_Secure($_POST['merchant_amount']);
    		$currency_code = "INR";
		    $check = array(
			    'amount' => $price,
			    'currency' => $currency_code,
			...
			if (!empty($json) && empty($json->error_code)) {
				$price = $price / 100;
				if (lui_ReplenishingUserBalance($price)) {
	                ...
	                $data['url'] = lui_SeoLink('index.php?link1=wallet');
	            } else {
	            	$data['url'] = lui_SeoLink('index.php?link1=wallet');
	            }
			}
