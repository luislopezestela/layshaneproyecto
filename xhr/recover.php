...
if ($f == 'recover') {
    if (empty($_POST['recoveremail'])) {
        ...
        } else if (lui_EmailExists($_POST['recoveremail']) === false) {
    ...
    if (empty($errors)) {
        $user_recover_data         = lui_UserData(lui_UserIdFromEmail($_POST['recoveremail']));
        $subject                   = $config['siteName'] . ' ' . $wo['lang']['password_rest_request'];
        $code              = md5(rand(111, 999) . uniqid() . time());
        $user_recover_data['link'] = lui_Link('index.php?link1=reset-password&code=' . $user_recover_data['user_id'] . '_' . $code);
        $time = time() + (60 * 60 * 12);
        ...
        $body                      = lui_LoadPage('emails/recover');
