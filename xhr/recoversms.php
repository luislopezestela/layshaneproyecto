...
if ($f == 'recoversms') {
    if (empty($_POST['recoverphone'])) {
        ...
        if (!in_array(true, lui_IsPhoneExist($_POST['recoverphone']))) {
    ...
    if (empty($errors)) {
        ...
        $user_id           = lui_UserIdFromPhoneNumber($_POST['recoverphone']);
        ...
        if ($query) {
            if (lui_SendSMSMessage($_POST['recoverphone'], $message) === true) {
                $data = array(
                    'status' => 200,
                    'message' => $success_icon . $wo['lang']['recoversms_sent'],
                    'location' => lui_SeoLink('index.php?link1=confirm-sms-password?code=' . $code)
                );
            } else {
