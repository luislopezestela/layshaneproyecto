...
if ($f == 'register') {
    ...
    if (empty($_POST['email']) || empty($_POST['username']) || empty($_POST['password']) || empty($_POST['confirm_password']) || empty($_POST['gender'])) {
        ...
        $is_exist = lui_IsNameExist($_POST['username'], 0);
        ...
        if (lui_IsBanned($_POST['username'])) {
            $errors = $error_icon . $wo['lang']['username_is_banned'];
        }
        if (lui_IsBanned($_POST['email'])) {
            $errors = $error_icon . $wo['lang']['email_is_banned'];
        }
        if (preg_match_all('~@(.*?)(.*)~', $_POST['email'], $matches) && !empty($matches[2]) && !empty($matches[2][0]) && lui_IsBanned($matches[2][0])) {
            $errors = $error_icon . $wo['lang']['email_provider_banned'];
        }
        if (lui_CheckIfUserCanRegister($wo['config']['user_limit']) === false) {
        ...
        if (!empty($_POST['phone_num'])) {
            if (!preg_match('/^\+?\d+$/', $_POST['phone_num'])) {
                ...
                if (lui_PhoneExists($_POST['phone_num']) === true) {
                    $errors = $error_icon . $wo['lang']['phone_already_used'];
                }
            }
        }
        if (lui_EmailExists($_POST['email']) === true) {
    ...
    if (empty($errors)) {
        ...
        $re_data  = array(
            'email' => lui_Secure($_POST['email'], 0),
            'username' => lui_Secure($_POST['username'], 0),
            'password' => $_POST['password'],
            'email_code' => lui_Secure($code, 0),
            'src' => 'site',
            'gender' => lui_Secure($gender),
            'lastseen' => time(),
            'active' => lui_Secure($activate),
            'birthday' => '0000-00-00'
        );
        if ($wo['config']['website_mode'] == 'linkedin' && !empty($_POST['currently_working']) && in_array($_POST['currently_working'], array(
            ...
            $re_data['currently_working'] = lui_Secure($_POST['currently_working'], 0);
        }
        if ($wo['config']['auto_username'] == 1) {
            if (!empty($_POST['first_name'])) {
                $re_data['first_name'] = lui_Secure($_POST['first_name'],1);
            }
            if (!empty($_POST['last_name'])) {
                $re_data['last_name'] = lui_Secure($_POST['last_name'],1);
            }
        }
        if ($gender == 'female') {
        ...
        if (!empty($_SESSION['ref']) && $wo['config']['affiliate_type'] == 0) {
            ...
            $ref_user_id = lui_UserIdFromUsername($_SESSION['ref']);
            if (!empty($ref_user_id) && is_numeric($ref_user_id)) {
                $re_data['ref_user_id'] = lui_Secure($ref_user_id);
            }
        }
        if (!empty($_POST['phone_num'])) {
            $re_data['phone_number'] = lui_Secure($_POST['phone_num']);
        }
        $in_code = (isset($_POST['invited'])) ? lui_Secure($_POST['invited']) : false;
        if (empty($_POST['phone_num'])) {
            $register = lui_RegisterUser($re_data, $in_code);
        } else {
            if ($activate == 1) {
                $register = lui_RegisterUser($re_data, $in_code);
            } else {
        ...
        if ($register === true) {
            $r_id = lui_UserIdFromUsername($_POST['username']);
            ...
            if ($activate == 1 || ($wo['config']['sms_or_email'] == 'mail' && $activate != 1)) {
                ...
                if (!empty($wo['config']['auto_friend_users'])) {
                    $autoFollow = lui_AutoFollow(lui_UserIdFromUsername($_POST['username']));
                }
                if (!empty($wo['config']['auto_page_like'])) {
                    lui_AutoPageLike(lui_UserIdFromUsername($_POST['username']));
                }
                if (!empty($wo['config']['auto_group_join'])) {
                    lui_AutoGroupJoin(lui_UserIdFromUsername($_POST['username']));
                }
            }
            if ($activate == 1) {
                $data  = array(
                ...
                $login = lui_Login($_POST['username'], $_POST['password']);
                if ($login === true) {
                    $session             = lui_CreateLoginSession(lui_UserIdFromUsername($_POST['username']));
                    $_SESSION['user_id'] = $session;
                    setcookie("user_id", $session, time() + (10 * 365 * 24 * 60 * 60));
                }
                $data['location'] = lui_SeoLink('index.php?link1=start-up');
                if ($wo['config']['membership_system'] == 1) {
                    $data['location'] = lui_SeoLink('index.php?link1=go-pro');
                }
            } else if ($wo['config']['sms_or_email'] == 'mail') {
                $wo['user']        = $_POST;
                $wo['code']        = $code;
                $body              = lui_LoadPage('emails/activate');
                ...
                $random_activation = lui_Secure(rand(11111, 99999));
                $message           = "Your confirmation code is: {$random_activation}";
                if (lui_SendSMSMessage($_POST['phone_num'], $message) === true) {
                    $register = lui_RegisterUser($re_data, $in_code);
                    if ($wo['config']['auto_username'] == 1) {
                        $r_id              = lui_UserIdFromUsername($_POST['username']);
                        $_POST['username'] = $_POST['username'] . "_" . $r_id;
                    ...
                    $user_id = lui_UserIdFromUsername($_POST['username']);
                    $query   = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `sms_code` = '{$random_activation}' WHERE `user_id` = {$user_id}");
                    $data    = array(
                        'status' => 300,
                        'location' => lui_SeoLink('index.php?link1=confirm-sms?code=' . $code)
                    );
                } else {
        ...
        if (!empty($field_data)) {
            $user_id = lui_UserIdFromUsername($_POST['username']);
            $insert  = lui_UpdateUserCustomData($user_id, $field_data, false);
        }
    }
    header("Content-type: application/json");
    if (isset($errors)) {
