...
if ($f == 'reports') {
    if ($s == 'report_user' && isset($_POST['user']) && is_numeric($_POST['user'])) {
        $user = lui_Secure($_POST['user']);
        $text = (!empty($_POST['text']) ? lui_Secure($_POST['text'],1) : '');
        $reason = (!empty($_POST['reason']) && in_array($_POST['reason'], $wo['config']['report_reasons']) ? lui_Secure($_POST['reason'],0) : '');
        $code = lui_ReportUser($user, $text,$reason);
        $data = array(
            'status' => 304
        );
        if ($code == 0) {
    ...
    if ($s == 'report_page' && isset($_POST['page']) && is_numeric($_POST['page']) && isset($_POST['text'])) {
        $page = lui_Secure($_POST['page']);
        $text = lui_Secure($_POST['text']);
        $code = lui_ReportPage($page, $text);
        $data = array(
            'status' => 304
        );
        if ($code == 0) {
    ...
    if ($s == 'report_group' && isset($_POST['group']) && is_numeric($_POST['group']) && isset($_POST['text'])) {
        $group = lui_Secure($_POST['group']);
        $text  = lui_Secure($_POST['text']);
        $code  = lui_ReportGroup($group, $text);
        $data  = array(
            'status' => 304
        );
        if ($code == 0) {
