    ...
    if (lui_CheckSession($hash_id) === true) {
        ...
        if (empty($errors)) {
            if (empty($_POST['amount'])) {
                ...
                if (lui_IsUserPaymentRequested($wo['user']['user_id']) === true) {
                    ...
                    $errors[] = $error_icon . $wo['lang']['invalid_amount_value_your'] . '' . lui_GetCurrency($wo['config']['ads_currency']) . $wo['user']['balance'];
                } else if ($wo['config']['m_withdrawal'] > $_POST['amount']) {
                    $errors[] = $error_icon . $wo['lang']['invalid_amount_value_withdrawal'] . ' ' . lui_GetCurrency($wo['config']['ads_currency']) . $wo['config']['m_withdrawal'];
                }
                if (empty($errors)) {
                    $insert_array = array('type' => lui_Secure($_POST['withdraw_method']));

                    if (!empty($_POST['paypal_email']) && $_POST['withdraw_method'] == 'paypal') {
                        $userU = lui_UpdateUserData($wo['user']['user_id'], array(
                            'paypal_email' => $_POST['paypal_email']
                        ));
                        $insert_array['transfer_info']       = lui_Secure($_POST['paypal_email']);
                    }
                        ...
                        $insert_array['iban']       = lui_Secure($_POST['iban']);
                        $insert_array['country']    = lui_Secure($_POST['country']);
                        $insert_array['full_name']  = lui_Secure($_POST['full_name']);
                        $insert_array['swift_code'] = lui_Secure($_POST['swift_code']);
                        $insert_array['address']    = lui_Secure($_POST['address']);
                        $userU                      = lui_UpdateUserData($wo['user']['user_id'], array(
                            'paypal_email' => ''
                        ));
                    }
                    else{
                        $insert_array['transfer_info']       = lui_Secure($_POST['transfer_to']);
                        $userU                      = lui_UpdateUserData($wo['user']['user_id'], array(
                            'paypal_email' => ''
                        ));
                    }
                    $insert_payment = lui_RequestNewPayment($wo['user']['user_id'], $_POST['amount'], $insert_array);
                    if ($insert_payment) {
                        $data           = array(
                            'status' => 200,
