...
if ($f == 'reset_password') {
    if (isset($_POST['id'])) {
        ...
        if (lui_isValidPasswordResetToken($_POST['id']) === false && lui_isValidPasswordResetToken2($_POST['id']) === false) {
        ...
        } else if (lui_TwoFactor($user_id[0], 'id') === false) {
            $_SESSION['code_id'] = $user_id[0];
            $password = $_POST['password'];
            if (lui_ResetPassword($user_id[0], $password) === true) {
                $data               = array(
                    'status' => 600,
        ...
        if (empty($errors) && empty($phone)) {
            $password = $_POST['password'];
            if (lui_ResetPassword($user_id[0], $password) === true) {
                $_SESSION['user_id'] = lui_CreateLoginSession($user_id[0]);
            }
            $data = array(
                'status' => 200,
