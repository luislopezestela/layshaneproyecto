...
if ($f == 'resned_ac_email') {
    if (isset($_SESSION['code_id'])) {
        ...
        $user    = lui_UserData($_SESSION['code_id']);
        ...
        if (!empty($_POST['email'])) {
            $user2 = $db->where('email',lui_Secure($_POST['email']))->getOne(T_USERS);
            if (empty($user2)  || (!empty($user2) && $user2->user_id != $user['user_id'])) {
                $errors[] = $error_icon . $wo['lang']['failed_to_send_code_fill'];
            }
            if (lui_EmailExists($_POST['email']) === true && $user['email'] != $_POST['email']) {
            ...
            $user2 = $db->where('phone_number',lui_Secure($_POST['phone_number']))->getOne(T_USERS);
            if (empty($user2)  || (!empty($user2) && $user2->user_id != $user['user_id'])) {
                $errors[] = $error_icon . $wo['lang']['failed_to_send_code_fill'];
            }
            ...
            if (lui_PhoneExists($_POST['phone_number']) === true) {
        ...
        if (empty($errors)) {
            if ($email == 1 && $phone == 0) {
                ...
                //$query                  = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `email` = '" . lui_Secure($_POST['email']) . "', `email_code` = '$code' WHERE `user_id` = {$user_id}");
                ...
                $user_id           = $_SESSION['code_id'];
                $phone_num         = lui_Secure($_POST['phone_number']);
                ...
                if ($query) {
                    if (lui_SendSMSMessage($_POST['phone_number'], $message) === true) {
                        $data = array(
                            'status' => 600,
