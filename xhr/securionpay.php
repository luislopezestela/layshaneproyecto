...
if ($f == "securionpay") {
	if ($s == 'create') {
		if (!empty($_POST['amount']) && is_numeric($_POST['amount']) && $_POST['amount'] > 0) {
			require_once 'luisincludes/librerias/securionpay/vendor/autoload.php';
			$price = lui_Secure($_POST['amount']);
	                ...
	                if ($value['id'] == $_POST['charge']['id']) {
	                    if (!empty($value['metadata']) && !empty($value['metadata']['user_key']) && !empty($value['amount'])) {
	                        if ($wo['user']['user_id'] == $value['metadata']['user_key']) {
	                        	$amount = intval(lui_Secure($value['amount'])) / 100;
	                        	if (lui_ReplenishingUserBalance($amount)) {
					                ...
					                $url = lui_SeoLink('index.php?link1=wallet');
