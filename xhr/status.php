...
if ($f == 'status') {
    if ($s == 'new' && $wo['config']['can_use_story']) {
        ...
        if (!isset($_FILES['statusMedia'])) {
            ...
            if (!lui_IsValidMimeType($_FILES['statusMedia']['type'])) {
                $error = true;
            }
        }
        if (!$error) {
            ...
            if (isset($_POST['title']) && strlen($_POST['title']) >= 2) {
                $registration_data['title'] = lui_Secure($_POST['title']);
            }
            if (isset($_POST['description']) && strlen($_POST['description']) >= 10) {
                $registration_data['description'] = lui_Secure($_POST['description']);
            }
            if (count($registration_data) > 0) {
                if (!empty($_FILES["statusMedia"])) {

                    $files   = lui_MultipleArrayFiles($_FILES["statusMedia"]);
                    $sources = array();
                    $thumb   = '';
                    foreach ($files as $fileInfo) {
                        $registration_data['posted']  += 1;
                        $registration_data['expire']  = time() + (60 * 60 * 24);
                        $last_id = lui_InsertUserStory($registration_data);
                        ...
                        if ($fileInfo['size'] > 0) {
                            ...
                            if (empty($_FILES["cover"]) && $wo['config']['ffmpeg_system'] == 'on') {
                                ...
                                $image_thumb      = $dir . '/' . lui_GenerateKey() . '_' . date('d') . '_' . md5(time()) . "_image.jpeg";
                                ...
                                if (file_exists($image_thumb) && !empty(@getimagesize($image_thumb))) {
                                    $crop_image                   = lui_Resize_Crop_Image(400, 400, $image_thumb, $image_thumb, 60);
                            ...
                            if (empty($thumb)) {
                                if (in_array(strtolower(pathinfo($media['filename'], PATHINFO_EXTENSION)), array(
                                    ...
                                    $fileget           = file_get_contents(lui_GetMedia($thumb), false, stream_context_create($arrContextOptions));
                                    if (!empty($fileget)) {
                                        $importImage = @file_put_contents($thumb, $fileget);
                                    }
                                    $crop_image                   = lui_Resize_Crop_Image(400, 400, $thumb, $last_file, 60);
                                    $wo['config']['amazone_s3']   = $amazone_s3;
                                    $wo['config']['wasabi_storage']   = $wasabi_storage;
                                    $wo['config']['backblaze_storage']   = $backblaze_storage;
                                    $wo['config']['ftp_upload']   = $ftp_upload;
                                    ...
                                    $upload_s3                    = lui_UploadToS3($media['filename']);
                                    $thumb                        = $last_file;
                                }
                            }
                        }
                    ...
                    if (count($sources) > 0) {
                        foreach ($sources as $registration_data) {
                            lui_InsertUserStoryMedia($registration_data);
                        }
    ...
    if ($s == 'p' && isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
        ...
        $html    = '';
        $stories = lui_GetStroies(array(
            'user' => $id
        ));
        if (count($stories) > 0) {
            foreach ($stories as $wo['story']) {
                $html .= lui_LoadPage('status/content');
            }
            $data = array(
                "status" => 200,
                "html" => $html
            ...
            $html = lui_LoadPage('status/no-stories');
            $data = array(
                "status" => 404,
                "html" => $html
            );
    ...
    if ($s == 'lm' && isset($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] > 0) {
        ...
        $html    = '';
        $stories = lui_GetAllStories($_GET['offset']);
        if ($stories && count($stories) > 0) {
            foreach ($stories as $wo['status']) {
                $html .= lui_LoadPage('admin/status/status-list');
            }
            $data = array(
                'status' => 200,
                'html' => $html
        ...
        if (!empty($_POST['ids'])) {
            foreach ($_POST['ids'] as $key => $value) {
                if (is_numeric($value) && $value > 0) {
                    lui_DeleteStatus(lui_Secure($value));
                }
            }
            $data = array(
                'status' => 200
    ...
    if ($s == 'register_reaction') {
        ...
        if (!empty($_GET['story_id']) && is_numeric($_GET['story_id']) && $_GET['story_id'] > 0 && !empty($_GET['reaction']) && in_array($_GET['reaction'], $reactions_types)) {
            ...
            if (!empty($story)) {
                ...
                $text                    = 'story';
                $type2                   = lui_Secure($_GET['reaction']);
                ...
                $data = array(
                    'status' => 200,
                    'reactions' => lui_GetPostReactions($story_id, 'story'),
