...
if ($f == 'update_data') {
    if (lui_CheckMainSession($hash_id) === true) {
        ...
        $data['pop']           = 0;
        $data['status']        = 200;
        $data['notifications'] = lui_CountNotifications(array(
            'unread' => true
        ));
        $data['html']          = '';
        ...
        $data['messages'] = lui_CountMessages(array(
            'new' => true
        ), 'interval');
        $chat_groups      = lui_CheckLastGroupUnread();
        $data['messages'] = $data['messages'] + count($chat_groups);
        $data['calls']    = 0;
        $data['is_call']  = 0;
        $check_calles     = lui_CheckFroInCalls();
        if ($check_calles !== false && is_array($check_calles)) {
            $wo['incall']                 = $check_calles;
            $wo['incall']['in_call_user'] = lui_UserData($check_calles['from_id']);
            $data['calls']                = 200;
            $data['is_call']              = 1;
            $data['call_id']              = $wo['incall']['id'];
            $data['calls_html']           = lui_LoadPage('modals/in_call');
        }
        $data['audio_calls']   = 0;
        $data['is_audio_call'] = 0;
        $check_calles          = lui_CheckFroInCalls('audio');
        if ($check_calles !== false && is_array($check_calles)) {
            $wo['incall']                 = $check_calles;
            $wo['incall']['in_call_user'] = lui_UserData($check_calles['from_id']);
            ...
            $data['call_id']              = $wo['incall']['id'];
            $data['audio_calls_html']     = lui_LoadPage('modals/in_audio_call');
        }
        $data['followRequests']      = lui_CountFollowRequests();
        $data['followRequests']      = $data['followRequests'] + lui_CountGroupChatRequests();
