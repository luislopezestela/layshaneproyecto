    ...
    if (isset($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0 && lui_CheckSession($hash_id) === true) {
        ...
        if (!empty($Userdata['user_id'])) {
            ...
            if (isset($_FILES['background_image']['name'])) {
                if (lui_UploadImage($_FILES["background_image"]["tmp_name"], $_FILES['background_image']['name'], 'background_image', $_FILES['background_image']['type'], $_POST['user_id']) === true) {
                    $background_image_status = 1;
                    if (!empty($mediaFilename)) {
                        lui_DeleteFromToS3($Userdata['background_image']);
                    }
                }
            }
