...
if ($f == "update_general_settings") {
    if (isset($_POST) && lui_CheckSession($hash_id) === true) {
        if (empty($_POST['username']) OR empty($_POST['email'])) {
            ...
            if (!empty($Userdata['user_id'])) {
                if ($_POST['email'] != $Userdata['email']) {
                    if (lui_EmailExists($_POST['email'])) {
                ...
                if ($_POST['phone_number'] != $Userdata['phone_number']) {
                    $is_exist = lui_IsPhoneExist($_POST['phone_number']);
                    if (in_array(true, $is_exist)) {
                ...
                if (isset($_POST['wallet']) && (lui_IsAdmin() || lui_IsModerator())) {
                    if (is_numeric($_POST['wallet'])) {
                        $wallet = $_POST['wallet'];
                    }
                }
                $type = $Userdata['admin'];
                if (!empty($_POST['type']) && lui_IsAdmin()) {
                    if ($_POST['type'] == 'admin') {
                        $type = 1;
                ...
                if (!empty($_POST['pro_type']) && (lui_IsAdmin() || lui_IsModerator())) {
                    if ($_POST['pro_type'] == 'free') {
                        $member_type = 0;
                        $member_pro  = 0;
                        $down        = lui_DownUpgradeUser($Userdata['user_id']);
                    } else if (in_array($_POST['pro_type'], array_keys($wo["pro_packages"]))) {
                        $member_type = lui_Secure($_POST['pro_type']);
                        $member_pro  = 1;
                        $time        = time();
                    }
                }
                ...
                if (empty($errors)) {
                        ...
                        if ($_POST['email'] != $wo['user']['email'] && $wo['config']['sms_or_email'] == 'mail' && $wo['config']['emailValidation'] == 1) {
                            ...
                            if ($send) {
                                $update_code    = $db->where('user_id', $wo['user']['user_id'])->update(T_USERS, array(
                                    ...
                                    'new_email' => lui_Secure($_POST['email'], 0)
                                ));
                                $save           = false;
                                $data['type']   = 'email';
                                $data['status'] = 200;
                            ...
                            if (empty($matches[1][0]) && empty($matches[0][0])) {
                                ...
                                $send = lui_SendSMSMessage($_POST['phone_number'], $message);
                                if ($send) {
                                    $update_code    = $db->where('user_id', $wo['user']['user_id'])->update(T_USERS, array(
                                        'email_code' => $hash_code,
                                        'new_phone' => lui_Secure($_POST['phone_number'])
                                    ));
                                    $save           = false;
                                    $data['type']   = 'phone';
                                    $data['status'] = 200;
                    ...
                    if ($save == true) {
                        ...
                        if (!empty($_POST['weather_unit']) && in_array($_POST['weather_unit'], array(
                            ...
                            $Update_data['weather_unit'] = lui_Secure($_POST['weather_unit']);
                        }
                        ...
                        if (!empty($_POST['phone_number'])) {
                            $Update_data['phone_number'] = lui_Secure($_POST['phone_number']);
                        }
                        if (lui_UpdateUserData($_POST['user_id'], $Update_data, $unverify)) {
                            $field_data = array();
                            if (!empty($_POST['custom_fields'])) {
                                $fields = lui_GetProfileFields('general');
                            ...
                            if (empty($errors)) {
                                $data = array(
                                    ...
                                    'username' => lui_SeoLink('index.php?link1=timeline&u=' . lui_Secure($_POST['username'])),
                                    'username_or' => lui_Secure($_POST['username'])
                                );
                            }
                        }
                    }
