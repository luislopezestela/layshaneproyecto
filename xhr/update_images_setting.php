    ...
    if (isset($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0 && lui_CheckSession($hash_id) === true) {
        ...
        if (!empty($Userdata['user_id'])) {
            if (isset($_FILES['avatar']['name'])) {
                if (lui_UploadImage($_FILES["avatar"]["tmp_name"], $_FILES['avatar']['name'], 'avatar', $_FILES['avatar']['type'], $_POST['user_id']) === true) {
                    $Userdata = lui_UserData($_POST['user_id']);
                }
            }
            if (isset($_FILES['cover']['name'])) {
                if (lui_UploadImage($_FILES["cover"]["tmp_name"], $_FILES['cover']['name'], 'cover', $_FILES['cover']['type'], $_POST['user_id']) === true) {
                    $Userdata = lui_UserData($_POST['user_id']);
                }
            }
            if (empty($errors)) {
                ...
                if (lui_UpdateUserData($_POST['user_id'], $Update_data)) {
                    $userdata2 = lui_UserData($_POST['user_id']);
                    $data      = array(
                        'status' => 200,
