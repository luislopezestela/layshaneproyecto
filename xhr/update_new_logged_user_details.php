...
if ($f == 'update_new_logged_user_details') {
    if (empty($_POST['new_password']) || empty($_POST['username']) || empty($_POST['repeat_new_password']) || lui_CheckSession($hash_id) === false) {
        ...
        if (lui_UserExists($_POST['username']) === true) {
        ...
        if (empty($errors)) {
            ...
            if (lui_UpdateUserData($_POST['user_id'], $Update_data)) {
                $get_user = lui_UserData($_POST['user_id']);
                $data     = array(
                    'status' => 200,
