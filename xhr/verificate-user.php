...
if ($f == 'verificate-user') {
    ...
    if (!$error) {
        $registration_data = array(
            'user_id' => $wo['user']['id'],
            'message' => lui_Secure($_POST['text']),
            'user_name' => lui_Secure($_POST['name']),
            'passport' => '',
            'photo' => '',
            'type' => 'User',
            'seen' => 0
        );
        $last_id           = lui_SendVerificationRequest($registration_data);
        if ($last_id && is_numeric($last_id)) {
            ...
            if (lui_UpdateVerificationRequest($last_id, $update_data)) {
                $data['status']  = 200;
