...
if ($f == 'view_story_by_id') {
    ...
    if (!empty($_POST['story_id']) && is_numeric($_POST['story_id']) && $_POST['story_id'] > 0 && !empty($_POST['type']) && in_array($_POST['type'], $types)) {
        $data['story_id'] = 0;
        $main_story       = $db->where('id', lui_Secure($_POST['story_id']))->getOne(T_USER_STORY);
        if (!empty($main_story)) {
            if ($_POST['type'] == 'previous') {
                $story = $db->where('id', lui_Secure($_POST['story_id']), '<')->where('user_id', $main_story->user_id)->orderBy('id', "DESC")->getOne(T_USER_STORY);
                if (!empty($story) && !empty($story->id)) {
                    $data['story_id'] = $story->id;
                }
                if (empty($story) && !empty($_POST['story_type']) && $_POST['story_type'] == 'friends') {
                    $all_stories   = lui_GetAllStatus();
                    $next_story_id = 0;
                    $n_ids         = array();
                ...
                $story = $db->where('id', lui_Secure($_POST['story_id']), '>')->where('user_id', $main_story->user_id)->orderBy('id', "ASC")->getOne(T_USER_STORY);
                if (!empty($story) && !empty($story->id)) {
                    $data['story_id'] = $story->id;
                }
                if (empty($story) && !empty($_POST['story_type']) && $_POST['story_type'] == 'friends') {
                ...
                $story = $db->where('id', lui_Secure($_POST['story_id']))->getOne(T_USER_STORY);
                if (!empty($story) && !empty($story->id)) {
                    $data['story_id'] = $story->id;
                }
            }
