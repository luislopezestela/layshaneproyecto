...
if ($f == 'wallet') {
    ...
    if ($s == 'replenish-user-account') {
        ...
        if (empty($error)) {
            $data = lui_ReplenishWallet($_GET['amount']);
    ...
    if ($s == 'get-paid') {
        if (isset($_GET['success']) && $_GET['success'] == 1 && isset($_GET['token']) && !empty($_GET['token'])) {
            include_once "luisincludes/includes/paypal_config.php";
            $token = lui_Secure($_GET['token']);
            $ch = curl_init();
            ...
            if (!empty($result)) {
                ...
                if (!empty($result->status) && $result->status == 'COMPLETED') {
                    if (lui_ReplenishingUserBalance($_GET['amount'])) {
                        $_GET['amount']                 = lui_Secure($_GET['amount']);
    ...
    if ($s == 'send' && $wo['loggedin'] === true) {
        ...
        if (empty($user_id) || empty($amount) || empty($userdata) || empty(floatval($wallet)) || $amount < 0) {
            ...
            $currency        = lui_GetCurrency($wo['config']['ads_currency']);
        ...
        if (!empty($_GET['type']) && in_array($_GET['type'], array(
            ...
            if ($_GET['type'] == 'pro') {
                ...
                if (!empty($_GET['pro_type']) && in_array($_GET['pro_type'], array_keys($wo["pro_packages"]))) {
                    $_GET['pro_type'] = lui_Secure($_GET['pro_type']);
                ...
                if (!empty($_GET['price']) && is_numeric($_GET['price']) && $_GET['price'] > 0) {
                    if (!empty($_GET['fund_id']) && is_numeric($_GET['fund_id']) && $_GET['fund_id'] > 0) {
                        $fund_id = lui_Secure($_GET['fund_id']);
                        $price   = lui_Secure($_GET['price']);
            ...
            if (empty($data['message'])) {
                if ($_GET['type'] == 'pro') {
                    $is_pro = 0;
                    $stop   = 0;
                    // $user   = lui_UserData($wo['user']['user_id']);
                    // if ($user['is_pro'] == 1) {
                    //     $stop = 1;
                    //     if ($user['pro_type'] == 1) {
                    ...
                    if ($stop == 0) {
                        ...
                        if ($is_pro == 1) {
                            ...
                            $mysqli             = lui_UpdateUserData($wo['user']['user_id'], $update_array);
                            ...
                            $create_payment     = lui_CreatePayment($pro_type);
                            if ($mysqli) {
                                if ((!empty($_SESSION['ref']) || !empty($wo['user']['ref_user_id'])) && $wo['config']['affiliate_type'] == 1 && $wo['user']['referrer'] == 0) {
                                    ...
                                    if ($wo['config']['amount_percent_ref'] > 0) {
                                        if (!empty($ref_user_id) && is_numeric($ref_user_id)) {
                                            $update_user = lui_UpdateUserData($wo['user']['user_id'], array(
                                ...
                                $data['status'] = 200;
                                $data['url']    = lui_SeoLink('index.php?link1=upgraded');
                            }
                        } else {
                            $data['message'] = $error_icon . $wo['lang']['something_wrong'];
                        }
